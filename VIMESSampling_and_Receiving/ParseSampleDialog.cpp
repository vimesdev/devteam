...
void CParseSampleDialog::OnParseSelect(){
	...
	UpdateData(true);
	int nLength;
	int n = 0, nIndex = 1, i = 0;
	int nProtocol;
	CString tmpStr, szRefName, szLength, szIndex;
	CString szSQL;
	CRecord rs(&pMF->m_db);
	szSQL.Format(_T("SELECT * FROM hms_instrument_protocol WHERE hip_id='%s' "), m_szProtocolKey);
	rs.ExecSQL(szSQL);
	if(rs.IsEOF())
		return;
	
	CString szSOF;
	rs.GetValue(_T("hip_sof"), szSOF);
	rs.GetValue(_T("hip_type"), nProtocol);


	szSOF.Replace(_T("<LF>"), _T("\n"));
	m_wndList.BeginLoad();
	if(nProtocol == 1){
		...
		while(*pstr != NULL){
			if(_tcsncmp(pstr, _T("H|"), 2) == 0){
				i=0;
				tmpStr.Empty();
				while(*pstr != NULL && i < szSOF.GetLength()){
					
					if(szSOF[i] == *pstr)
					{
						i++;
						szRefName.Empty();
						m_wndList.AddItems(_T(""), szRefName, tmpStr, NULL);
						if(szSOF[i] == _T('<')){
							int j = i;
							while(i < szSOF.GetLength() && szSOF[i] != _T('>'))	i++;
							i++;
							szRefName = szSOF.Mid(j, i-j);							
						}
						tmpStr.Empty();
					}
					else
...
long CParseSampleDialog::OnProtocolLoadData(){
	...
	if(m_wndProtocol.IsSearchKey() && !m_szProtocolKey.IsEmpty()){
		szWhere.Format(_T(" WHERE hip_id='%s' "), m_szProtocolKey);
	};
	m_wndProtocol.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT hip_id, hip_name FROM hms_instrument_protocol %s ORDER BY hip_id"), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndProtocol.AddItems(
			rs.GetValue(_T("hip_id")), 
			rs.GetValue(_T("hip_name")), NULL);
		rs.MoveNext();
	}
	return nCount;
