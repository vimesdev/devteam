...
//	m_wndPerformDepartment.SetCheckValue(true);
	...
	m_wndTestList.InsertColumn(0, _T("ID"), CFMT_TEXT, 90);
	m_wndTestList.InsertColumn(1, _T("Name"), CFMT_TEXT, 220);
	//m_wndTestList.InsertColumn(2, _T("Unit"), CFMT_TEXT, 50);
	m_wndTestList.SetCheckBox(true);

	m_wndSelectList.InsertColumn(0, _T("ID"), CFMT_TEXT, 90);
	m_wndSelectList.InsertColumn(1, _T("Name"), CFMT_TEXT, 220);
...
long CHMSParaclinicalDialog::OnTestListLoadData(){
	...
	{
		szWhere.Format(_T(" AND lower(limsid_name) like(chr(37)||lower('%s')||chr(37)) "), m_szSearch);
	}
	szExisting=_T("'X'");
	for (int i =0; i < m_arItems.GetCount(); i++){
		szExisting.AppendFormat(_T(",'%s'"), m_arItems[i]);
	}
	szSQL.Format(_T("SELECT limsid_code as code, limsid_name as name, limsid_unit as unit ") \
		_T("FROM lims_instrument LEFT JOIN lims_instrument_data ON(limsi_id=limsid_instid) ") \
		_T("WHERE limsi_groupid='%s' AND limsid_active='Y' AND limsid_code not in(%s) %s ORDER BY limsid_index, limsid_name "), m_szGroup, szExisting, szWhere);
...
long CHMSParaclinicalDialog::OnSelectListLoadData(){
	...
	szSQL.Format(_T("SELECT * FROM lims_instrument LEFT JOIN lims_instrument_data ON(limsi_id=limsid_instid) ") \
		_T(" WHERE limsid_active='Y' AND limsid_code in(%s) ORDER BY limsi_groupid, limsid_index "), szExisting);
	rs.ExecSQL(szSQL);
//_fmsg(_T("%s"), szSQL);
	m_wndSelectList.BeginLoad();
	while(!rs.IsEOF()){
		szGroup = rs.GetValue(_T("limsi_groupid"));
		if(szGroup != szOldGroup){
			szSQL.Format(_T("SELECT * FROM hms_feegroup WHERE hfg_id='%s' "), szGroup);
			rs2.ExecSQL(szSQL);
			if(!rs2.IsEOF())
...
void CHMSParaclinicalDialog::OnApplySelect(){
	...
	for (int i =0; i < m_wndSelectList.GetItemCount(); i++){
		...
		if(!szGroup.IsEmpty() && szGroup != szOldGroup){
			...
			*/
			szSQL.Format(_T("SELECT lims_order_add(%ld, '%s', %d, '%s', '%s', '%s', '%s','%s') "),
				pMF->m_nDocumentNo, pMF->m_szDeptPatient, pMF->m_nRoomID, m_szOrderDate, pMF->m_szDoctor, szGroup, szStatus, pMF->GetCurrentUser());
			rs.ExecSQL(szSQL);
_fmsg(_T("%s"), szSQL);
		...
		{
			szID = m_wndSelectList.GetItemText(i, 0);
			szSQL.Format(_T("INSERT INTO lims_order_line(limsol_oid, limsol_instcode, limsol_itemid) VALUES(%ld, '', '%s') "), nOrderID, szID);
_fmsg(_T("%s"), szSQL);
			pMF->ExecSQL(szSQL);
		}
	}
...
int CHMSParaclinicalDialog::OnHMSParaclinicalDialogListLoadData(){
	...
	TCHAR chOld=_T(' '), ch;
	//1: Nhom can lam sang
	szSQL.Format(_T("SELECT * FROM hms_feegroup WHERE substr(hfg_id, 1, 2) IN('B1') AND hfg_active='Y' AND hfg_level=2 ORDER BY hfg_id "));
	
	rs.ExecSQL(szSQL);
	m_arGroup.RemoveAll();
	while(!rs.IsEOF()){
		rs.GetValue(_T("hfg_id"), szCode);
		m_wndParaclinicalList.AddItem(rs.GetValue(_T("hfg_name")));
