...
void CLIMSMergeResultDialog::OnInitializeComponents(){
	...
	m_wndPatientList.InsertColumn(1, _T("SID"), CFMT_NUMBER, 125);
	m_wndPatientList.InsertColumn(2, _T("PID"), CFMT_NUMBER, 125);
	m_wndPatientList.InsertColumn(3, _T("Patient Name"), CFMT_TEXT, 300);
	m_wndPatientList.InsertColumn(4, _T("Age"), CFMT_TEXT, 100);
	m_wndPatientList.InsertColumn(5, _T("Perform Date"), CFMT_DATETIME, 200);
	m_wndPatientList.InsertColumn(6, _T("Oid"), CFMT_NUMBER, 0);
	m_wndPatientList.InsertColumn(7, _T("Instid"), CFMT_NUMBER, 0);


	m_wndResList.InsertColumn(0, _T("Index"), CFMT_NUMBER, 50);
	m_wndResList.InsertColumn(1, _T("Test Name"), CFMT_TEXT, 250);
	m_wndResList.InsertColumn(2, _T("Unit"), CFMT_TEXT, 100);
	m_wndResList.InsertColumn(3, _T("Result"), CFMT_TEXT, 100);
	m_wndResList.InsertColumn(4, _T("Male Index"), CFMT_TEXT, 100);
	m_wndResList.InsertColumn(5, _T("Female Index"), CFMT_TEXT, 100);
	m_wndResList.InsertColumn(6, _T("Comment"), CFMT_TEXT, 200);
...
int CLIMSMergeResultDialog::SetMode(int nMode){
 		...
 		case VM_NONE: 
 			EnableControls(TRUE); 
 			EnableButtons(TRUE, 0, 2, 2, -1); 
 			SetDefaultValues(); 
 			break; 
 		}; 
//		m_wndSID.EnableWindow(true);
//		m_wndInstrument.EnableWindow(TRUE);
...
long CLIMSMergeResultDialog::OnInstrumentLoadData(){
	...
	if(m_wndInstrument.IsSearchKey() && !m_szInstrumentKey.IsEmpty()){
		 szWhere.Format(_T(" and lims_id='%s' "), m_szInstrumentKey);
	};
	m_wndInstrument.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT limsi_id as id, limsi_name as name FROM lims_instrument ") \
		_T("WHERE limsi_groupid='%s' %s ORDER BY id "), m_szTestGroup, szWhere);
...
void CLIMSMergeResultDialog::OnPatientNameChange(){
	UpdateData(true);
	m_wndPatientList.Search(3, m_szPatientName);
} 
...
void CLIMSMergeResultDialog::OnPatientListSelectChange(int nOldItem, int nNewItem){
	...
	m_szName = m_wndPatientList.GetItemText(nNewItem, 3);
	OnResListLoadData();
...
long CLIMSMergeResultDialog::OnPatientListLoadData(){
	...
	if(!m_szSID.IsEmpty())
		szWhere.Format(_T(" and limso_barcode = '%ld' "), ToInt(m_szSID));
	
	szWhere.AppendFormat(_T(" and limso_instid=%d "), str2int(m_szInstrumentKey));
...
_T(" WHERE limsol_oid=%ld and limsol_instid=%d "), m_nOID, m_nInstID);
_fmsg(_T("%s"), szSQL);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
...
void CLIMSMergeResultDialog::OnApplySelect(){
	...
	CString szMessage, szLabel;
	TranslateString(_T("Are you sure you want to map ([%s] - %s) -> ([%ld] - %s) "), szLabel);
	szMessage.Format(szLabel, m_szSID, m_szPatientName, pMF->m_wndPatientProfile.m_nDocumentNo, pMF->m_wndPatientProfile.m_szPatientName);
	int ret = ShowMessageBox(szMessage, MB_YESNOCANCEL|MB_DEFBUTTON2);
	if(ret != IDYES)
		return;
...
_tprintf(_T("\r\n%s"), szSQL);
	...
	szSQL.Format(_T("UPDATE hms_testorder SET hpc_status='T', hpc_instid=%d, hpc_practitioner='%s', hpc_performdate=systimestamp ") \
