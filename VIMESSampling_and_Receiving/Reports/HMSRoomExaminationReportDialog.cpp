...
//_msg(_T("%s"), szSQL);
	...
	while(!rs.IsEOF())
	{
		...
		if(szNewLine != szOldLine && !szNewLine.IsEmpty()){
			...
			{	nRow++;
				...
				tmpStr.Format(_T("%d"), grpQty);
				xls.SetCellText(3, nRow, tmpStr, FMT_INTEGER,true,10,0);
				tmpStr.Format(_T("%.2f"), grpCost);
				xls.SetCellText(5, nRow, tmpStr, FMT_DECIMAL,true,10,0);				
				ttlCost += grpCost;		
				ttQty   += grpQty;
				grpCost = 0;
				grpQty	=0;
		...
		xls.SetCellText(4, nRow, tmpStr, FMT_DECIMAL);
		
		rs.GetValue(_T("sumservprice"), cost);
		grpCost += cost;
		tmpStr.Format(_T("%.2f"), cost);
		xls.SetCellText(5, nRow, tmpStr, FMT_DECIMAL);
	...
	if(ttlCost > 0){
		...
		tmpStr.Format(_T("%d"),ttQty);
		xls.SetCellText(3, nRow, tmpStr, FMT_INTEGER,true,10,0);
		tmpStr.Format(_T("%.2f"), ttlCost);
		xls.SetCellText(5, nRow, tmpStr, FMT_DECIMAL, true);
	}
