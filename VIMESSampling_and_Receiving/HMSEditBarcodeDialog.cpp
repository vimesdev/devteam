...
void CHMSEditBarcodeDialog::OnInitializeComponents(){
	...
	m_wndCurrentBarcode.SetReadOnly(TRUE);
	m_wndNewBarcode.SetLimitText(10);
...
int CHMSEditBarcodeDialog::OnNewBarcodeCheckValue(){
	...
	szSQL.Format(_T("SELECT count(*) FROM hms_testorder WHERE hpc_sid=%ld and hpc_docno<>%ld and trunc(hpc_orderdate) >= sysdate-1 "),
		m_nNewBarcode, m_nDocumentNo);
	rs.ExecSQL(szSQL);
	if(rs.GetIntValue() > 0)
	{
		m_wndNewBarcode.SetToolTipMessage(_T("This barcode has been using by other patient"));
		return -1;
	}
	return 0;
} 
...
int CHMSEditBarcodeDialog::OnSaveHMSEditBarcodeDialog(){
	...
	szSQL.Format(_T("LIMS_EDITBARCODE(%ld, %ld, %ld) "), m_nDocumentNo, m_nOrderID, m_nNewBarcode);
