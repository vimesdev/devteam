...
void CWVCellDataItem::DrawItem(CPaintDC *pDC, CRect *pCellArea, BOOL bMView/*= FALSE*/)
{
	COleDateTime dtS, dtE;
	int nItemHeight = GetSystemMetrics(SM_CXVSCROLL);
	CString strBuf, szTemp;
	...
	{
		if (!bMView)
			strBuf.Format(_T("%s - %s  %s"), dtS.Format(_T("%H%M")), dtE.Format(_T("%H%M")), GetItemText());
		else
			strBuf.Format(_T("%s  %s"), dtS.Format(_T("%H%M")), GetItemText());
	}
	else
	{
		if (!bMView)
	...
	if (!bMView)
		pDC->DrawText(strBuf, &rText, DT_WORDBREAK | DT_CALCRECT);
	else
	...
	if (!bMView)
		pDC->DrawText(strBuf, &rText, DT_END_ELLIPSIS | DT_WORDBREAK | DT_VCENTER);
	else
