...
static void _OnSearchDepartmentSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CHMSPatientRecordInfo* )pWnd)->OnSearchDepartmentSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnSearchDepartmentSelendokFnc(CWnd *pWnd){
	((CHMSPatientRecordInfo*)pWnd)->OnSearchDepartmentSelendok();
}
/*static void _OnSearchDepartmentSetfocusFnc(CWnd *pWnd){
	((CHMSPatientRecordInfo*)pWnd)->OnSearchDepartmentKillfocus();
}*/
/*
static void _OnSearchDepartmentKillfocusFnc(CWnd *pWnd){
	((CHMSPatientRecordInfo*)pWnd)->OnSearchDepartmentKillfocus();
}
*/
static long _OnSearchDepartmentLoadDataFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo *)pWnd)->OnSearchDepartmentLoadData();
}
...
static int _OnBorrowDocumnetFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnBorrowDocument();
}
static int _OnReturnDocumnetFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnReturnDocument();
}
static int _OnEditRecordBorrowFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnEditRecordBorrow();
} 
static int _OnDeleteRecordBorrowFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnDeleteRecordBorrow();
}
static int _OnSaveRecordBorrowFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnSaveRecordBorrow();
}
static int _OnCancelRecordBorrowFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnCancelRecordBorrow();
}
static int _OnPrintFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnPrint();
}
static int _OnReloadDataFnc(CWnd *pWnd){
	return ((CHMSPatientRecordInfo*)pWnd)->OnReloadData();
}
static int _OnImageQuantityInputFnc(CWnd* pWnd)
{
	((CHMSPatientRecordInfo*) pWnd)->OnImageQuantityInput();
	return 0;
}
...
void CHMSPatientRecordInfo::OnCreateComponents()
{
	...
	m_wndSearchDepartmentLabel.Create(this, _T("Department"), 495, 30, 565, 55);
	m_wndSearchDepartment.Create(this,570, 30, 730, 55);
	m_wndNameLabel.Create(this, _T("Name"), 735, 30, 775, 55);
	m_wndName.Create(this,780, 30, 930, 55);
	...
	m_wndTreatmentTypeLabel.Create(this, _T("Type"), 425, 250, 485, 275);
	m_wndTreatmentType.Create(this, 490, 250, 705, 275); 
	...
	m_wndStoredDateLabel.Create(this, _T("Stored Date"), 270, 520, 345, 545);
...
void CHMSPatientRecordInfo::OnInitializeComponents()
{
	...
	m_wndAdmitDepartment.SetLimitText(81);
	...
	m_wndEndDepartment.SetLimitText(81);
	...
	m_wndRecordStatus.LimitText(81);
	...
	m_wndBorrower.SetLimitText(65);
	m_wndBorrower.SetInitCap(true);
	...
	m_wndBorrowStatus.LimitText(81);
	...
	m_wndReturnStatus.LimitText(81);
	m_wndReturner.SetLimitText(65);
	m_wndReturner.SetInitCap(true);
	...
	m_wndNotes.SetLimitText(254);
	//m_wndNotes.SetCheckValue(true);

	//m_wndSearchPatient.ModifyStyle(WS_TABSTOP, 0);
	...
	m_wndRecordList.InsertColumn(8, _T("Diagnostic"), CFMT_TEXT, 200);
	...
	m_wndBorrowList.InsertColumn(0, _T("ID"), CFMT_TEXT, 40);
	m_wndBorrowList.InsertColumn(1, _T("Borrow Date"), CFMT_DATE, 90);
	m_wndBorrowList.InsertColumn(2, _T("Return Date"), CFMT_DATE, 90);
	...
	m_wndBorrowList.InsertColumn(6, _T("Department"), CFMT_TEXT, 100);
	...
	m_wndSearchDepartment.InsertColumn(0, _T("ID"), CFMT_TEXT, 70);
	m_wndSearchDepartment.InsertColumn(1, _T("Name"), CFMT_TEXT, 250);



	m_wndBorrowStatus.InsertColumn(0, _T("ID"), CFMT_TEXT, 50);
	m_wndBorrowStatus.InsertColumn(1, _T("Description"), CFMT_TEXT, 150);
	...
	m_hms_record_borrowTbl.SetTableName(_T("hms_record_borrow"));
	m_hms_record_borrowTbl.AddField(_T("hrb_refidx"), dfLong); 
	m_hms_record_borrowTbl.AddField(_T("hrb_idx"), dfLong); 
	m_hms_record_borrowTbl.AddField(_T("hrb_withdrawdate"), dfDateTime);
	m_hms_record_borrowTbl.AddField(_T("hrb_numberdate"), dfLong);
	m_hms_record_borrowTbl.AddField(_T("hrb_returndate"), dfDateTime); 
	m_hms_record_borrowTbl.AddField(_T("hrb_brstatus"), dfText, 1); 
	m_hms_record_borrowTbl.AddField(_T("hrb_borrower"), dfText, 65); 
	m_hms_record_borrowTbl.AddField(_T("hrb_deptid"), dfText, 7); 
	m_hms_record_borrowTbl.AddField(_T("hrb_membercode"), dfText, 13); 
	m_hms_record_borrowTbl.AddField(_T("hrb_reason"), dfText, 254); 
	m_hms_record_borrowTbl.AddField(_T("hrb_handover"), dfText, 65); 
	m_hms_record_borrowTbl.AddField(_T("hrb_returner"), dfText, 65); 
	m_hms_record_borrowTbl.AddField(_T("hrb_receiver"), dfText, 65); 
	m_hms_record_borrowTbl.AddField(_T("hrb_rcvstatus"), dfText, 1); 
	m_hms_record_borrowTbl.AddField(_T("hrb_note"), dfText, 254); 
	m_hms_record_borrowTbl.AddField(_T("hrb_information"), dfText, 254); 
...
void CHMSPatientRecordInfo::OnSetWindowEvents()
{
	...
	m_wndRecordList.SetWindowText(_T("Record List"));
	m_wndRecordList.AddEvent(1, _T("Delete"), _OnDeleteHMSPatientRecordInfoFnc, 0, 0, 0);
	...
	m_wndSearchDepartment.SetEvent(WE_SELENDOK, _OnSearchDepartmentSelendokFnc);
	//m_wndSearchDepartment.SetEvent(WE_SETFOCUS, _OnSearchDepartmentSetfocusFnc);
	//m_wndSearchDepartment.SetEvent(WE_KILLFOCUS, _OnSearchDepartmentKillfocusFnc);
	m_wndSearchDepartment.SetEvent(WE_SELCHANGE, _OnSearchDepartmentSelectChangeFnc);
	m_wndSearchDepartment.SetEvent(WE_LOADDATA, _OnSearchDepartmentLoadDataFnc);
	//m_wndSearchDepartment.SetEvent(WE_ADDNEW, _OnSearchDepartmentAddNewFnc);
	...
	m_wndBorrowList.AddEvent(1, _T("Borrow Document      F5"), _OnBorrowDocumnetFnc, 0, VK_F5, 0);
	m_wndBorrowList.AddEvent(2, _T("Return Document      F6"), _OnReturnDocumnetFnc, 0, VK_F6, 0);
	m_wndBorrowList.AddEvent(3, _T("Edit                 F7"), _OnEditRecordBorrowFnc, 0, VK_F7, 0);
	m_wndBorrowList.AddEvent(4, _T("Delete               F8"), _OnDeleteRecordBorrowFnc, 0, VK_F8, 0);
	m_wndBorrowList.AddEvent(5, _T("Save                 F9"), _OnSaveRecordBorrowFnc, 0, VK_F9, 0);
	m_wndBorrowList.AddEvent(0,_T("-"));
	m_wndBorrowList.AddEvent(6, _T("Cancel               F10"), _OnCancelRecordBorrowFnc, 0, VK_F10, 0);
	...
	AddEvent(1, _T("Add	Ctrl+A"), 0, 0, 'A', VK_CONTROL);
	...
	AddEvent(5, _T("Print	Ctrl+P"), _OnPrintFnc, 0, 'P', VK_CONTROL);
	AddEvent(0, _T("-"));
	AddEvent(6, _T("Cancel	Ctrl+T"), _OnCancelHMSPatientRecordInfoFnc, 0, 'T', VK_CONTROL);
	AddEvent(0, _T("-"));
	AddEvent(7, _T("Image Quantity Input"), _OnImageQuantityInputFnc);
	//SetMenuState(5, FALSE);
	...
	SetWindowFont(&m_wndArchivalNo, GetFaceName(), GetFaceSize() + 3, TRUE);
	SetWindowFont(&m_wndTreatmentType, GetFaceName(), GetFaceSize() + 3, TRUE);
	...
	m_wndFullName.SetTextColor(RGB( 0, 0, 255));
	SetWindowFont(&m_wndFullName, GetFaceName(), GetFaceSize()+2, true);
	
	/*if(pMF->m_szFontPatientR.GetLength() > 5){
		SetWindowFont(&m_wndFullName, pMF->m_szFontPatientR, GetFaceSize()+2, true);
		SetWindowFont(&m_wndRecordList, pMF->m_szFontPatientR, GetFaceSize(), false);
	}*/
...
void CHMSPatientRecordInfo::OnDoDataExchange(CDataExchange* pDX)
{
	...
	DDX_TextEx(pDX, m_wndSearchDepartment.GetDlgCtrlID(), m_szSearchDepartmentKey);
	...
	DDX_Text(pDX, m_wndTreatmentType.GetDlgCtrlID(), m_szTreatmentType);
...
void CHMSPatientRecordInfo::GetDataToScreen()
{
	...
	else
	{
		...
		if (!rs.IsEOF())
		{
			...
			rs.GetValue(_T("PatientName"), m_szFullName);
			...
			OnBorrowListLoadData();
			if (m_wndBorrowList.GetItemCount() > 0)
			{
				m_wndBorrowList.SetCurSel(0);
				m_nbID = ToLong(m_wndBorrowList.GetItemText(0, 0));
				m_nIndex = 0;
			}
			else
			{
			...
			m_szTreatmentType.Format(_T("Bệnh án điều trị nội trú"));
...
int CHMSPatientRecordInfo::SetMode(int nMode)
{
  		...
  		switch(nMode)
		{
 		...
 		case VM_EDIT:
			...
			m_wndArchivalNo.EnableWindow(true);
		...
		m_wndSearchDepartment.EnableWindow(TRUE);
 		UpdateData(FALSE); 
 		return nOldMode; 
}
...
int CHMSPatientRecordInfo::OnNameCheckValue()
{
	UpdateData(TRUE);
	OnRecordListLoadData();
	//m_wndRecordList.Search(2, m_szName, SEARCH_NAME);
	return 1;
} 
void CHMSPatientRecordInfo::OnSearchSelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szDocNo;
	m_wndDocumentNo.GetWindowText(szDocNo);
	if (szDocNo.IsEmpty())
...
void CHMSPatientRecordInfo::OnRecordListSelectChange(int nOldItem, int nNewItem)
{
	...
	if (nSel < 0) return;
	m_nDocNo = ToLong(m_wndRecordList.GetItemText(nSel, 0));
	//m_szClass = m_wndRecordList.GetItemText(nSel, 11);
	GetDataToScreen();	
} 
...
long CHMSPatientRecordInfo::OnRecordListLoadData()
{
	...
	if(!m_szSearchDepartmentKey.IsEmpty()){
		m_szSearch.AppendFormat(_T(" AND hcr_dischargedept ='%s' "), m_szSearchDepartmentKey);
	}
	...
	if (m_wndRecordList.GetItemCount() > 0)
		m_wndRecordList.SetCurSel(0);
...
int CHMSPatientRecordInfo::OnDocNoCheckValue()
{
	...
	if (rs.GetRecordCount() > 0)
	{
		//if (rs.GetIntValue() > 0)
		//{
		//m_wndDocNo.SetToolTipMessage(_T("This value is already exist in the record"));
		//return -1;
		m_szClass = rs.GetValue(_T("hpr_class"));
		GetDataToScreen();
		OnBorrowListLoadData();
		//m_wndDocNo.EnableWindow(FALSE);
		m_wndSearchPatient.SetFocus();
		//}
	}
	else
	{
...
bool CHMSPatientRecordInfo::LoadDataToScreen()
{
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, tmpStr;
...
void CHMSPatientRecordInfo::OnSearchPatientSelect()
{
	...
	UpdateData(true);
	GetDataToScreen();
	//CHMSSearchPatient dlg(pMF);
	//if (dlg.DoModal() == IDOK)
	//{
	//	m_nDocNo = dlg.m_nDoNo;
	//	//CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	//	CRecord rs(&pMF->m_db);
	//	CString szSQL;
	//	szSQL.Format(_T("SELECT * FROM hms_patient_record WHERE hpr_docno=%ld"), m_nDocNo);
	...
	//	else
	//	{
	//		if (dlg.m_szSuggestion.Trim() == _T("D"))
	//			m_szClass = _T("I");
	//		else
	//			m_szClass = _T("E");
	//		LoadDataToScreen();			
...
{
	UpdateData(TRUE);
	//MessageBox(m_szAdmissionNo);
	m_szAdmitNo = m_szAdmissionNo;
	m_bCheck = true;
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("SELECT Count(*) FROM hms_clinical_record WHERE hcr_recordno='%s'"), m_szAdmitNo);
	rs.ExecSQL(szSQL);
	if (rs.GetIntValue() > 0)
	{
		m_wndAdmissionNo.SetToolTipMessage(_T("This value is already exist in the database"));
		return -1;
	}
	return 0;
} 
...
int CHMSPatientRecordInfo::OnArchivalNoCheckValue()
{
	...
	szSQL.Format(_T("SELECT Count(*) FROM hms_patient_record WHERE hpr_docrec='%s'"), m_szArchivalNo);
	szSQL1.Format(_T("SELECT sc_id FROM sys_company"));
	rs.ExecSQL(szSQL);
	rs1.ExecSQL(szSQL1);
	if (rs.GetIntValue() > 0 && rs1.GetIntValue() == 12096 )
	{
		m_wndArchivalNo.SetToolTipMessage(_T("This value is already exist in the database"));
		return -1;
	}
	return 0;
} 
...
{
	UpdateData(TRUE);	
	if (CompareDateTime(m_szStoredDate, m_szDischargeDate) < 0)
	{
		m_wndStoredDate.SetToolTipMessage(_T("This value must be greater or equal to Discharge Date"));
		return -1;
	}
	return 0;
} 
...
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	//CHMSPatientRecordReportDialog dlg(this);
	//dlg.DoModal();
} 
...
void CHMSPatientRecordInfo::OnBorrowListSelectChange(int nOldItem, int nNewItem)
{
	...
	m_nIndex = nNewItem;
	m_nbID = ToLong(m_wndBorrowList.GetItemText(nNewItem, 0));
	GetBorrowInfor();
} 
...
long CHMSPatientRecordInfo::OnBorrowListLoadData()
{
	...
	m_wndBorrowList.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT hrb_idx AS Indexs, ")\
		         _T("hrb_withdrawdate AS BorrowDate, ")\
				 _T("hrb_returndate AS ReturnDate, ")\
				 _T("hrb_numberdate AS NumberofDate, ")\
				 _T("(SELECT ss_desc FROM sys_sel WHERE ss_code=hrb_brstatus AND ss_id='hms_patientrecord_brstatus' AND rownum <=1) AS BorrowStatus, ")\
				 _T("hrb_borrower AS Borrower, ")\
				 _T("(SELECT sd_name FROM sys_dept WHERE sd_id=hrb_deptid AND rownum <=1) AS Department, ")\
				 _T("hrb_reason AS Reason, ")\
				 _T("hrb_handover AS Deliverer, ")\
				 _T("hrb_returner AS Returner, ")\
				 _T("hrb_receiver AS Receiver, ")\
				 _T("(SELECT ss_desc FROM sys_sel WHERE ss_code=hrb_rcvstatus AND ss_id='hms_patientrecord_rcvstatus' AND rownum <=1) AS ReturnStatus, ")\
				 _T("hrb_note AS Note ")\
				 _T("FROM hms_record_borrow ")\
				 _T("WHERE hrb_refidx=%ld ORDER BY 1"), m_nIdx);
	nCount = rs.ExecSQL(szSQL);
	CDate date;
	while(!rs.IsEOF()){ 
		m_wndBorrowList.AddItems(
			rs.GetValue(_T("Indexs")), 
...
{
	UpdateData(TRUE);
	if (CompareDate(m_szBorrowDate, m_szStoredDate) < 0)
	{
		m_wndBorrowDate.SetToolTipMessage(_T("This value must be greater than or equal to Stored Date"));
		return -1;
	}
	return 0;
} 
...
void CHMSPatientRecordInfo::OnSearchDepartmentSelectChange(int nOldItemSel, int nNewItemSel){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
void CHMSPatientRecordInfo::OnSearchDepartmentSelendok(){
	 
}
/*void CHMSPatientRecordInfo::OnSearchDepartmentSetfocus(){
	
}*/
/*void CHMSPatientRecordInfo::OnSearchDepartmentKillfocus(){
	
}*/
long CHMSPatientRecordInfo::OnSearchDepartmentLoadData()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
	szWhere.Empty();
	if(m_wndSearchDepartment.IsSearchKey() && !m_szSearchDepartmentKey.IsEmpty())
	{
		szWhere.Format(_T(" WHERE sd_id='%s' "), m_szSearchDepartmentKey);
	}
	m_wndSearchDepartment.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT sd_id AS ID, sd_name AS Name FROM sys_dept %s ORDER BY sd_id"), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndSearchDepartment.AddItems(
			rs.GetValue(_T("ID")), 
			rs.GetValue(_T("Name")), NULL);
		rs.MoveNext();
	}
...
/*void CHMSPatientRecordInfo::OnSearchDepartmentAddNew(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
long CHMSPatientRecordInfo::OnBorrowStatusLoadData()
{
	...
	{
		szWhere.Format(_T(" AND ss_code='%s' "), m_szBorrowStatusKey);
	}
	m_wndBorrowStatus.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT ss_code AS id, ss_desc AS description FROM sys_sel ")\
		         _T("WHERE ss_id='hms_patientrecord_brstatus' %s ORDER BY ss_code"), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
...
{
	UpdateData(TRUE);
	if (CompareDate(m_szReturnDate, m_szBorrowDate) < 0)
	{
		m_wndBorrowDate.SetToolTipMessage(_T("This value must be greater than or equal to Borrow Date"));
		return -1;
	}
	return 0;
} 
...
long CHMSPatientRecordInfo::OnReturnStatusLoadData()
{
	...
	{
		szWhere.Format(_T(" AND ss_code='%s' "), m_szReturnStatusKey);
	}
	m_wndReturnStatus.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT ss_code AS id, ss_desc AS description FROM sys_sel ")\
		         _T("WHERE ss_id='hms_patientrecord_rcvstatus' %s ORDER BY ss_code"), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
...
int CHMSPatientRecordInfo::OnDeleteHMSPatientRecordInfo()
{
	...
	CRecord rs(&pMF->m_db);
 	CString szSQL;
	szSQL.Format(_T("SELECT Count(*) FROM hms_record_borrow WHERE hrb_refidx=%ld"), m_nIdx);
	rs.ExecSQL(szSQL);
	if (rs.GetIntValue() > 0)
	{
		ShowMessageBox(_T("This record has diary information. You can not delete it"), MB_OK | MB_ICONERROR);
		return -1;
	}
 	if(ShowMessage(1, MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO) 
 		return -1; 
	szSQL.Format(_T("DELETE FROM hms_patient_record WHERE hpr_docno=%ld AND hpr_docrec='%s'"), m_nDocNo, m_szArchivalNo); 
	_fmsg(_T("%s"), szSQL);
...
{	
	...
	{ 
 	...
 	{ 
 		...
 		//SetMode(VM_VIEW);
		//if (!m_szAdmitNo.IsEmpty() && m_bCheck)
		//{
		//	CRecord rs(&pMF->m_db);
		//	szSQL.Format(_T("UPDATE hms_clinical_record SET hcr_recordno='%s' WHERE hcr_docno=%ld AND hcr_patientno=%ld"),
		//		         m_szAdmitNo, m_nDocNo, ToLong(m_szPatientNo));
		//	//MessageBox(szSQL);
		//	rs.ExecSQL(szSQL);
		//	m_bCheck = false;
		//}
		//m_szRecNo = m_szArchivalNo;
		//m_nDoNo = m_nDocNo;
		GetDataToScreen();
		if (bAdd)
		{
			m_wndDocNo.SetFocus();
			m_wndDocNo.SetSel(m_wndDocNo.GetWindowTextLength(), m_wndDocNo.GetWindowTextLength());
		}
		OnRecordListLoadData();
		m_wndDocNo.SetFocus();
		m_wndDocNo.SetSel(0, 8);
 	} 
 	else 
 	{ 
 	} 
...
{
 	if(m_wndRecordList.GetCurSel() >= 0) 
 	{ 
		m_nDocNo = ToLong(m_wndRecordList.GetItemText(m_wndRecordList.GetCurSel(), 0));
		GetDataToScreen();
		OnBorrowListLoadData();
 	} 
 	else 
...
void CHMSPatientRecordInfo::SetValidControl()
{
	m_wndDocNo.EnableWindow(FALSE);
	m_wndArchivalNo.EnableWindow(TRUE);
	m_wndStoredDate.EnableWindow(TRUE);	
	//m_wndRecordStatus.EnableWindow(TRUE);
	m_wndRoomID.EnableWindow(TRUE);
	m_wndCabinet.EnableWindow(TRUE);
	m_wndDrawer.EnableWindow(TRUE);
	m_wndLocation.EnableWindow(TRUE);
	m_wndShortcut.EnableWindow(TRUE);
	m_wndNote.EnableWindow(TRUE);
}
int CHMSPatientRecordInfo::OnBorrowDocument()
{
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	if (!GetMode() == VM_VIEW)
		return -1;
	if (m_szRecordStatusKey.Trim().MakeUpper() != _T("A"))
		return -1;

	if(m_nOutPatient) SetBRMode(VM_BORROWDOC);
	return 0;
}
int CHMSPatientRecordInfo::OnReturnDocument()
{
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	if (!GetMode() == VM_VIEW)
		return -1;
	int nSel = m_wndBorrowList.GetItemCount() - 1;
	//if (nSel < 0) return -1;
	if (m_szRecordStatusKey.Trim().MakeUpper() != _T("B"))
		return -1;
	CString szReturnDate = m_wndBorrowList.GetItemText(nSel, 2);
	if (!szReturnDate.IsEmpty()) 
		return -1;
	SetBRMode(VM_RETURNDOC);
	return 0;
}
int CHMSPatientRecordInfo::OnEditRecordBorrow()
{
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	if (!GetMode() == VM_VIEW)
		return -1;
	int nSel = m_wndBorrowList.GetCurSel();
	if (nSel < 0) return -1;
	CString szReturnDate = m_wndBorrowList.GetItemText(nSel, 2);
	if (szReturnDate.IsEmpty())
		SetBRMode(VM_BORROWEDIT);
	else
		SetBRMode(VM_BREDIT);
	return 0;
}
int CHMSPatientRecordInfo::OnDeleteRecordBorrow()
{
	if(GetBRMode() != VM_BRVIEW) 
 		return -1;
	int nSel = m_wndBorrowList.GetCurSel();
	if (nSel < 0) return -1;
 	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
 	...
 	if(ShowMessage(1, MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO) 
 		return -1; 
	CString m_szID = m_wndBorrowList.GetItemText(nSel, 0);
	szSQL.Format(_T("DELETE FROM hms_record_borrow WHERE hrb_idx=%ld"), ToLong(m_szID)); 
 	int ret = pMF->ExecSQL(szSQL); 
 	if(ret >= 0)
	{ 
		CString szReturnDate = m_wndBorrowList.GetItemText(nSel, 2);
		...
		{
			CRecord rs(&pMF->m_db);
			szSQL.Format(_T("UPDATE hms_patient_record SET hpr_status='A' WHERE hpr_idx=%ld"), m_nIdx);
			rs.ExecSQL(szSQL);
			GetDataToScreen();
		}
		OnBorrowListLoadData();
		if (m_wndBorrowList.GetItemCount() > 0)
		{
			SetItemSel(&m_wndBorrowList, m_szID, m_nIndex);
		}
		else
		{
			SetBRMode(VM_BRNONE);
...
int CHMSPatientRecordInfo::OnSaveRecordBorrow()
{
	//UpdateData(TRUE);
	//MessageBox(m_szDepartmentKey);
	if(GetBRMode() != VM_BORROWDOC && GetBRMode() != VM_RETURNDOC && 
	   GetBRMode() != VM_BREDIT && GetBRMode() != VM_BORROWEDIT)
	{
 		return -1; 
	}
 	if(!IsValidateData())
	...
	//MessageBox(m_szDepartmentKey);
 	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
 	CString szSQL;
	m_hms_record_borrowTbl.SetValue(_T("hrb_refidx"), m_nIdx); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_withdrawdate"), m_szBorrowDate);
	m_hms_record_borrowTbl.SetValue(_T("hrb_numberdate"), m_nNumberOfDate);
	m_hms_record_borrowTbl.SetValue(_T("hrb_returndate"), m_szReturnDate); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_brstatus"), m_szBorrowStatusKey); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_borrower"), m_szBorrower); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_deptid"), m_szDepartmentKey);  
	m_hms_record_borrowTbl.SetValue(_T("hrb_reason"), m_szReason); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_handover"), m_szDeliverer); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_returner"), m_szReturner); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_receiver"), m_szReceiver); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_rcvstatus"), m_szReturnStatusKey); 
	m_hms_record_borrowTbl.SetValue(_T("hrb_note"), m_szNotes); 
	if(GetBRMode() == VM_BORROWDOC)
	{ 
		szSQL = m_hms_record_borrowTbl.GetInsertSQL(_T("hrb_idx, hrb_membercode, hrb_information, hrb_returndate"));
 	} 
	else if(GetBRMode() == VM_RETURNDOC || GetBRMode() == VM_BREDIT || GetBRMode() == VM_BORROWEDIT)
	{ 
		int nSel = m_wndBorrowList.GetCurSel();
		if (nSel < 0) return -1;
		long nIdx = ToLong(m_wndBorrowList.GetItemText(nSel, 0));
 		CString szWhere; 
		szWhere.Format(_T(" WHERE hrb_idx=%ld "), nIdx);
		CDate date;
		date.ParseDate(m_szReturnDate);
		if (date.GetYear() != 1752)
			szSQL = m_hms_record_borrowTbl.GetUpdateSQL(_T("hrb_idx, hrb_refidx, hrb_membercode, hrb_information"));
		else
			szSQL = m_hms_record_borrowTbl.GetUpdateSQL(_T("hrb_idx, hrb_refidx, hrb_membercode, hrb_information, hrb_returndate"));
 		szSQL += szWhere; 
 	}
	//MessageBox(szSQL);
    _fmsg(_T("%s"), szSQL); 
 	...
 	{ 
		...
		if (GetBRMode() == VM_BORROWDOC)
		{
			szSQL.Format(_T("UPDATE hms_patient_record SET hpr_status='B' WHERE hpr_idx=%ld"), m_nIdx);
			rs.ExecSQL(szSQL);
			GetDataToScreen();
		}
		else if (GetBRMode() == VM_RETURNDOC)
		{
			szSQL.Format(_T("UPDATE hms_patient_record SET hpr_status='A' WHERE hpr_idx=%ld"), m_nIdx);
			rs.ExecSQL(szSQL);
			GetDataToScreen();
		...
		OnBorrowListLoadData();
		szSQL.Format(_T("SELECT max(hrb_idx) AS Index FROM hms_record_borrow"));
		rs.ExecSQL(szSQL);
		m_nbID = ToLong(rs.GetValue(_T("Index")));
		CString szID;
		szID.Format(_T("%ld"), m_nbID);
		SetItemSel(&m_wndBorrowList, szID, m_nIndex);
 	} 
 	else 
 	{ 
 	} 
...
int CHMSPatientRecordInfo::OnCancelRecordBorrow()
{
	if (m_wndBorrowList.GetCurSel() >= 0)
	{
		m_nbID = ToLong(m_wndBorrowList.GetItemText(m_wndBorrowList.GetCurSel(), 0));
		GetBorrowInfor();
	}
 	else 
 	{  
...
int CHMSPatientRecordInfo::OnPrint()
{
	OnPrintSelect();
	return 0;
}
void CHMSPatientRecordInfo::SetBorrowControl(bool bState1, bool bState2)
{
	m_wndBorrowDate.EnableWindow(bState1);
	m_wndNumberOfDate.EnableWindow(bState1);
	m_wndBorrower.EnableWindow(bState1);
	m_wndDepartment.EnableWindow(bState1);
	m_wndReason.EnableWindow(bState1);
	m_wndBorrowStatus.EnableWindow(bState1);
	m_wndDeliverer.EnableWindow(bState1);
	if (bState1 && !bState2)
	{
		m_wndBorrowDate.SetCheckValue(bState1);
		m_wndNumberOfDate.SetCheckValue(bState1);
		m_wndBorrower.SetCheckValue(bState1);
		m_wndDepartment.SetCheckValue(bState2);
		m_wndReason.SetCheckValue(bState2);
		m_wndBorrowStatus.SetCheckValue(bState1);
		m_wndDeliverer.SetCheckValue(bState1);
	}
	else if (bState1 && bState2)
	{
		m_wndBorrowDate.SetCheckValue(bState1);
		m_wndNumberOfDate.SetCheckValue(bState1);
		m_wndBorrower.SetCheckValue(bState1);
	...
	m_wndReturnDate.EnableWindow(bState2);
	m_wndReturnStatus.EnableWindow(bState2);
	m_wndReturner.EnableWindow(bState2);
	m_wndReceiver.EnableWindow(bState2);
	m_wndNotes.EnableWindow(bState2);
	if (bState2 && !bState1)
	{
		m_wndReturnDate.SetCheckValue(bState2);
		m_wndReturnStatus.SetCheckValue(bState2);
		m_wndReturner.SetCheckValue(bState2);
		m_wndReceiver.SetCheckValue(bState2);
		m_wndNotes.SetCheckValue(bState1);
	}
	else if (bState1 && bState2)
	{
		m_wndReturnDate.SetCheckValue(bState2);
		...
		m_wndReturner.SetCheckValue(bState2);
		m_wndReceiver.SetCheckValue(bState2);
		m_wndNotes.SetCheckValue(false);
	}
}
int CHMSPatientRecordInfo::OnReloadData()
{
	OnRecordListLoadData();
	return 0;
}
void CHMSPatientRecordInfo::GetBorrowInfor()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("SELECT hrb_withdrawdate AS BorrowDate, ")\
				 _T("hrb_returndate AS ReturnDate, ")\
				 _T("hrb_numberdate AS NumberofDate, ")\
				 _T("hrb_brstatus AS BorrowStatus, ")\
				 _T("hrb_borrower AS Borrower, ")\
				 _T("hrb_deptid AS Department, ")\
				 _T("hrb_reason AS Reason, ")\
				 _T("hrb_handover AS Deliverer, ")\
				 _T("hrb_returner AS Returner, ")\
				 _T("hrb_receiver AS Receiver, ")\
				 _T("hrb_rcvstatus AS ReturnStatus, ")\
				 _T("hrb_note AS Note ")\
				 _T("FROM hms_record_borrow ")\
				 _T("WHERE hrb_idx=%ld"), m_nbID);
	rs.ExecSQL(szSQL);
	if(!rs.IsEOF())
	{  
		rs.GetValue(_T("BorrowDate"), m_szBorrowDate); 
		rs.GetValue(_T("ReturnDate"), m_szReturnDate); 
		rs.GetValue(_T("NumberofDate"), m_nNumberOfDate); 
		rs.GetValue(_T("BorrowStatus"), m_szBorrowStatusKey); 
		rs.GetValue(_T("Borrower"), m_szBorrower); 
		rs.GetValue(_T("Department"), m_szDepartmentKey); 
		rs.GetValue(_T("Reason"), m_szReason);
		rs.GetValue(_T("Deliverer"), m_szDeliverer); 
		rs.GetValue(_T("Returner"), m_szReturner); 
		rs.GetValue(_T("Receiver"), m_szReceiver); 
		rs.GetValue(_T("ReturnStatus"), m_szReturnStatusKey); 
...
void CHMSPatientRecordInfo::SetBRMode(int nMode)
{
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	switch (nMode)
	{
		case VM_BORROWDOC:
			...
			m_szReceiver.Empty();
			m_szNotes.Empty();
			SetBorrowControl(true, false);
			m_szBorrowDate = pMF->GetSysDate();
			m_wndBorrowDate.SetFocus();
			m_nState = VM_BORROWDOC;
			break;
		case VM_BORROWEDIT:
			SetBorrowControl(true, false);
			m_wndBorrowDate.SetFocus();
			m_nState = VM_BORROWEDIT;
			break;
		case VM_RETURNDOC:
			SetBorrowControl(false, true);
			m_wndReturnDate.SetFocus();
			m_nState = VM_RETURNDOC;
			break;
		case VM_BREDIT:
			SetBorrowControl(true, true);
			m_wndBorrowDate.SetFocus();
			m_nState = VM_BREDIT;
			break;
		case VM_BRVIEW:
			SetBorrowControl(false, false);
			m_nState = VM_BRVIEW;
			break;
		case VM_BRNONE:
			SetBorrowControl(false, false);
...
int CHMSPatientRecordInfo::GetBRMode()
{
	return m_nState;
}
void CHMSPatientRecordInfo::SetItemSel(CGuiListCtrl *pList, CString szID, int nIndex)
{
	if (GetBRMode() == VM_BORROWDOC)
	{
	...
	else if (GetBRMode() == VM_BREDIT || GetBRMode() == VM_BORROWEDIT || 
			 GetBRMode() == VM_RETURNDOC)
	{
...
void CHMSPatientRecordInfo::OnImageQuantityInput()
{
	CPRImageLineInput *pop = new CPRImageLineInput();
	pop->m_nDocNo = m_nDocNo;
	pop->DoModal();
}
