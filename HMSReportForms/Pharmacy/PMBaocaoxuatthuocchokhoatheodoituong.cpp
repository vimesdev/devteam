...
void CrptBaocaoxuatthuocchokhoatheodoituong::OnExportBaocaoxuathuocchokhoatheodoituong()
{
	...
	_T("	%s") \
	_T(" 	FROM mev ") \
	_T(" 	LEFT JOIN m_productitem_view35 ON(product_item_id=sitemid)") \
	_T(" 	WHERE expqty > 0 and deptid in(%s) and expdate BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') %s ") \
	_T(" 	) tbl") \
...
void CrptBaocaoxuatthuocchokhoatheodoituong::OnExportBaocaoxuathuocchokhoatheodoituong2()
{
	CMainFrame_E10 *pMF = (CMainFrame_E10 *) AfxGetMainWnd();
	UpdateData(true);
	BeginWaitCursor();	
...
CString CrptBaocaoxuatthuocchokhoatheodoituong::GetQueryStringByRow()
{
	CMainFrame_E10 *pMF = (CMainFrame_E10 *) AfxGetMainWnd();
	UpdateData(true);
	BeginWaitCursor();
