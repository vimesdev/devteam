#ifndef MAMaterialPayment_H
#define MAMaterialPayment_H
...
#include "DbField.h"
#include "Excel.h"
class CMAMaterialPayment : public CGuiView{
protected:
public:
	...
	void			OnExportSelect(); 
	CMAMaterialPayment(CWnd *pParent);
	~CMAMaterialPayment();
	...
	int OnAddMAMaterialPayment(); 
	int OnEditMAMaterialPayment(); 
	int OnDeleteMAMaterialPayment(); 
	int OnSaveMAMaterialPayment(); 
	int OnCancelMAMaterialPayment(); 
	int OnMAMaterialPaymentListLoadData();
	void WriteHeader(CExcel* pXls, CRecord* pRs);
	void WriteDetail(CExcel* pXls, CRecord* pRs);
};
#endif
