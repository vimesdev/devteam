...
static void _OnSaleOrderSelectFnc(CWnd *pWnd){
	((CPMSPrescriptionListReportDialog*)pWnd)->OnSaleOrderSelect();
}
...
//	m_wndToDate.SetCheckValue(true);
	...
	m_wndList.InsertColumn(2, _T("DeptID"), CFMT_TEXT, 90);
...
void CPMSPrescriptionListReportDialog::OnSaleOrderSelect(){
...
long CPMSPrescriptionListReportDialog::OnListLoadData(){
	...
	if (m_szCondition == _T("O"))	
		szSQL.Format(_T(" SELECT hrl_id as id, hrl_name as name, hrl_deptid as deptid FROM hms_roomlist left join sys_dept ON (hrl_deptid = sd_id) WHERE sd_type ='KB' and hrl_active = 'Y' ORDER BY hrl_deptid, hrl_id"));	
	if (m_szCondition == _T("I")) 
...
void CPMSPrescriptionListReportDialog::OnExportSelect(){
	...
	menu.AppendMenu(MF_BYPOSITION, 2, tmpStr);
	menu.AppendMenu(MF_SEPARATOR);
	TranslateString(_T("Export Detail By Drug Format"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 3, tmpStr);
	CRect rect;
...
void CPMSPrescriptionListReportDialog::OnPrintPreview()
{
	...
	if (m_szCondition == _T("A"))
		...
		if (m_szCondition == _T("O"))
		{
			...
			szDeptType2.Format(_T(" and su_deptid IN('C1.1', 'C1.2', 'C1.3', 'TYC') "));
			if (!szRoomC11.IsEmpty())
			{
		...
		else
		{
			...
			szDeptType2.Format(_T(" and su_deptid NOT IN('C1.1', 'C1.2', 'C1.3', 'TYC') "));
			if(!szListID.IsEmpty())
			{
		...
		szSQL.Format(_T(" SELECT") \
			...
			_T("   product_name ||'['||product_classname||']' AS pdname,") \
			...
			_T("   WHERE so_org_id                                     IN('GL','PM','TM', 'EM', 'SOM')") \
...
void CPMSPrescriptionListReportDialog::OnPrintSynthesis(){
	...
	while (!rs.IsEOF())
	{
		...
		rptDetail->SetValue(_T("8"), rs.GetValue(_T("tb_theo_bac_sy")));
		nTotalOrder++;
		rs.MoveNext();
	}
...
void CPMSPrescriptionListReportDialog::OnPrintDetailByDrug(){
	CMainFrame_E10 *pMF = (CMainFrame_E10 *) AfxGetMainWnd();
	UpdateData(true);
	CReport rpt;
	...
	_T("                   hpo_doctor, ") \
	_T("                   product_id, ") \
	_T("                   product_name ||'['||product_classname||']' as NAME, ") \
	_T("                   product_purchase_uomid, ") \
...
void CPMSPrescriptionListReportDialog::OnExportSynthesis(){
	...
	while (!rs.IsEOF())
	{
		...
		xls.SetCellText(7, nRow, rs.GetValue(_T("tb_theo_bac_sy")), FMT_NUMBER1);
		nRow++;
		nOrder++;
		rs.MoveNext();
	}
...
void CPMSPrescriptionListReportDialog::OnExportDetailByDrug(){
	CMainFrame_E10 *pMF = (CMainFrame_E10 *) AfxGetMainWnd();
	UpdateData(true);
	CString tmpStr, szDate, szSysDate, szSQL, szObjectID, szObjectName, szWhere, szWhere1, 
		szWhere2, tmpStr2,szListID, szRoomID, szRoomName;
...
CString CPMSPrescriptionListReportDialog::GetQueryString(){
	...
	szSQL.Format(_T(" SELECT (SELECT sd_name FROM sys_dept WHERE sd_id = deptid) AS deptid,") \
		...
		_T("   hd_patientno %s") \
		_T(" ORDER BY deptid,") \
		_T("   orderid"), m_szFromDate, m_szToDate, szWhere, szWhereRoomID, m_szFromDate, m_szToDate, szWhere2, szWhereRoomID2, szWhere3, szHavingClause);
	...
	szSQL.Format(
		...
		_T("        ROUND(( SUM(amount)") \
		_T("            over (") \
		_T("              PARTITION BY doctor) ) ") \
		_T("		/ ( Count(*)") \
		_T("            over (") \
		_T("              PARTITION BY doctor) ), 2) AS tb_theo_Bac_sy") \
		_T(" FROM   (%s) "), tmpStr);
...
CString CPMSPrescriptionListReportDialog::GetQueryString_dtl()
{
	CString tmpStr, szDate, szSysDate, szSQL, szObjectID, szObjectName, szWhere, szWhere1, 
		szWhere2, tmpStr2,szListID, szRoomID, szRoomName;
	...
	tmpStr2 = _T("' '");
	szWhere.Format(_T(" AND approved_date BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS')") \
				   _T(" AND NVL(order_type, 'X') <> 'C'")
	...
	else
	{
		if (m_szCondition == _T("O"))
		{
			szDeptType.Format(_T(" and sd_type ='KB' "));
			if (!szRoomC11.IsEmpty())				
				szWhereRoomID.AppendFormat(_T(" and ((room_id in (%s) and department_id= 'C1.1') "), szRoomC11);
			if (!szRoomC12.IsEmpty())
				if (!szWhereRoomID.IsEmpty())
					szWhereRoomID.AppendFormat(_T(" or (room_id in (%s) and department_id= 'C1.2') "), szRoomC12);
				else
					szWhereRoomID.AppendFormat(_T(" and ((room_id in (%s) and department_id= 'C1.2') "), szRoomC12);
			if (!szRoomC13.IsEmpty())
				if (!szWhereRoomID.IsEmpty())
					szWhereRoomID.AppendFormat(_T(" or (room_id in (%s) and department_id= 'C1.3')"), szRoomC13);
				else
					szWhereRoomID.AppendFormat(_T(" and ((room_id in (%s) and department_id= 'C1.3')"), szRoomC13);
			if (!szWhereRoomID.IsEmpty())
				szWhereRoomID.AppendFormat(_T(")"));
		}
		else
		{
			szDeptType.Format(_T(" and sd_type ='DT' "));
			if(!szListID.IsEmpty())
				szWhereRoomID.Format(_T(" and department_id in(%s) ") , szListID);
		}
	}
	if (!szDeptType.IsEmpty())
		szWhere.AppendFormat(_T("%s"), szDeptType);
	...
	if(!m_szDoctorKey.IsEmpty())
		szWhere.AppendFormat(_T(" and doctor_id ='%s' "), m_szDoctorKey);
	...
	if (!szStocks.IsEmpty())
		szWhere.AppendFormat(_T(" AND storage_id IN(%s)"), szStocks);

	if (m_nMinTotalAmt > 0)
		szWhere2.Format(_T(" AND taxamount >= %ld"), m_nMinTotalAmt);
	if (!m_szApprovedByKey.IsEmpty())
		szWhere.AppendFormat(_T(" AND approved_by = '%s'"), m_szApprovedByKey);
	//szPrice = _T("product_taxprice");
	//if (m_bServPrice)
	//szPrice = _T("hpol_unitprice");
	if (m_bOriental)
	...
	if (m_bSaleOrder)
	{
		szWhere.AppendFormat(_T(" AND doc_type = 'SOO'"));
	}
	szSQL.Format(_T(" SELECT Get_username(hpo_doctor) doctor,") \
	_T("        NAME,") \
	_T("        Get_uomname(product_purchase_uomid) unit,") \
	_T("        price,") \
	_T("        SUM(qty) qty,") \
	_T("        SUM(amount) amount") \
	_T(" FROM   (SELECT    order_id AS orderid,") \
	_T("                   qty_export * unit_price AS amount,") \
	_T("                   SUM(qty_export * unit_price)") \
	_T("                     OVER (") \
	_T("                       PARTITION BY (order_id)) AS taxamount,") \
	_T("                   doc_no AS docno,") \
	_T("                   Hms_getagebydoc(doc_no) AS age,") \
	_T("                   doctor_id AS hpo_doctor,") \
	_T("                   product_id,") \
	_T("                   product_name") \
	_T("                   ||'['") \
	...
	_T("         FROM      m_export_patient_view pv") \
	_T("         LEFT JOIN m_productitem_view iv ON ( iv.product_item_id = pv.product_item_id )") \
	_T("		 LEFT JOIN sys_dept ON (sd_id = department_id)") \
	_T("         WHERE     1 = 1 %s %s)") \
	_T(" WHERE  1 = 1 %s") \
	_T(" GROUP  BY hpo_doctor,NAME,price,product_purchase_uomid") \
	_T(" ORDER  BY hpo_doctor,NAME ")
	, szWhere, szWhereRoomID, szWhere2);
	return szSQL;
}
