...
static void _OnduySelectFnc(CWnd *pWnd){
	((CPMSStockCardReportDialog*)pWnd)->OnduySelect();
}
...
void CPMSStockCardReportDialog::OnCreateComponents(){
	...
	if(pMF->GetCurrentUser()== _T("duy"))
	{
		m_wndduy.Create(this, _T("duy"), 200, 555, 280, 580);
	}
}
...
void CPMSStockCardReportDialog::OnduySelect(){
	CMainFrame_E10 *pMF = (CMainFrame_E10*) AfxGetMainWnd();
...
void CPMSStockCardReportDialog::OnExport(bool bPreview){
	...
	BeginWaitCursor();
	CExcel xls;
	CRecord rs(&pMF->m_db), rsr(&pMF->m_db);
	CString szSQL, tmpStr, szWhere, szGroupBy, szCategory;
	int nSheet = 0, nID = 0, nIdx = 1, nRow = 0, nCol = 0;
	...
	for (int i = 0; i < m_wndList.GetItemCount(); i++){
		if (m_wndList.GetCheck(i)){
			...
			xls.SetCellText(0, 3, _T("THẺ KHO"), 4098, true);
			...
			//Qty onhand
			xls.SetCellText(0, 5, _T("Tồn đầu kỳ: "), FMT_TEXT);
			if (m_bByProductLine)
			...
			xls.SetCellText(2, 5, _T("Tên thuốc"), FMT_TEXT);
			tmpStr = m_wndList.GetItemText(i, 1);
			...
			xls.SetCellText(1, 6, _T("Chứng từ"), 4098, true);
			xls.SetCellText(1, 7, _T("Số hiệu"), 4098, true);
			xls.SetCellText(2, 7, _T("Ngày"), 4098, true);
			xls.SetCellText(3, 6, _T("Trích yếu"), 4098, true);
			xls.SetCellText(4, 6, _T("Ngày xuất nhập"), 4098, true);
			xls.SetCellText(5, 6, _T("Số lượng"), 4098, true);
			xls.SetCellText(5, 7, _T("Nhập"), 4098, true);
			xls.SetCellText(6, 7, _T("Xuất"), 4098, true);
			xls.SetCellText(7, 7, _T("Tồn"), 4098, true);
			xls.SetCellText(8, 6, _T("Ký nhận"), 4098, true);
			if (m_bByProductLine)
			...
			while (!rs.IsEOF())
			{
				...
				xls.SetCellText(8, nRow, rs.GetValue(_T("invoiceid")), FMT_NUMBER1);
				nRow++;
				rs.MoveNext();
			}
			xls.SetCellMergedColumns(0, nRow, 5);
			xls.SetCellText(0, nRow, _T("Tổng cộng: "), 4098, true);
			...
			nRow++;
			xls.SetCellMergedColumns(0, nRow, 5);
			xls.SetCellText(0, nRow, _T("Trong đó: "), 4098, true);
			xls.SetCellText(4, nRow+1, _T("Điều chuyển"), 4098, true);
			xls.SetCellText(4, nRow+2, _T("BN"), 4098, true);
			nRow++;
			...
			szSQL.Format(_T(" SELECT orderno AS so_hieu,") \
			_T("        To_char(orderdate, 'dd/mm/yyyy') AS ngay,") \
			_T("        '['||department_to_id ||']'||Get_doctype(doctype) AS trich_yeu,") \
			_T("        To_char(approveddate, 'dd/mm/yyyy') AS ngay_xuat_nhap,") \
			_T("        qtyorder AS so_luong,") \
			_T("		'['||refproduct_code||'] '||refproduct_name as refproduct_name") \
			_T(" FROM   m_transactionline_ref_view") \
			_T(" WHERE  storage_to_id = %ld %s") \
			_T(" ORDER BY approveddate "), str2long(m_szStockKey), szWhere);
			...
			tmpStr.Format(_T("Sheet %d-Hàng thay thế"), nSheet);
			xls.AddSheet(tmpStr);
			xls.SetWorksheet(nSheet);
			nRow = 0;
			xls.SetCellText(nCol+0, nRow, _T("STT"), 4098, true);
			xls.SetCellText(nCol+1, nRow, _T("Số hiệu"), 4098, true);
			xls.SetCellText(nCol+2, nRow, _T("Ngày"), 4098, true);
			xls.SetCellText(nCol+3, nRow, _T("Trích yếu"), 4098, true);
			xls.SetCellText(nCol+4, nRow, _T("Ngày xuất nhập"), 4098, true);
			xls.SetCellText(nCol+5, nRow, _T("Số lượng"), 4098, true);
			xls.SetCellText(nCol+6, nRow, _T("Thay thế bởi"), 4098, true);
			xls.SetColumnWidth(0, 8);
			xls.SetColumnWidth(1, 15);
			xls.SetColumnWidth(2, 15);
			xls.SetColumnWidth(4, 15);
			xls.SetColumnWidth(3, 30);
			xls.SetColumnWidth(6, 100);
			nRow++;
			nIdx = 1;
			nTmp = 0;
			while (!rsr.IsEOF())
			{
				...
				for (int j = 0; j < rsr.GetFieldCount(); j++)
				{
					if (rsr.GetFieldName(j).MakeUpper() == _T("SO_LUONG"))
					{
						nTmp += str2double(rsr.GetValue(j));
					}
					xls.SetCellText(nCol+j+1, nRow, rsr.GetValue(j), FMT_TEXT);
				}
				nRow++;
				rsr.MoveNext();
			}
			xls.SetCellText(nCol+5, nRow, double2str(nTmp), FMT_NUMBER1, true);
			nSheet++;
		}
	}
...
void CPMSStockCardReportDialog::OnExport4(bool bPreview){
	_debug(_T("%s"), CString(typeid(this).name()));
	CMainFrame_E10 *pMF = (CMainFrame_E10*) AfxGetMainWnd();
	UpdateData(true);
	BeginWaitCursor();
	...
	for (int i = 0; i < m_wndList.GetItemCount(); i++){
		if (m_wndList.GetCheck(i)){
			...
			while (!rs.IsEOF())
			{
				...
				rs.GetValue(_T("iotype"),szIotype);
				if (szCategory == _T("I"))
				{
					nOnhand += nTmp;
					nTotalImport += nTmp;
					...
					if(szIotype== _T("DRO") || szIotype== _T("CRO")    )
					{
						long nTransactionID = 0, nProductId =0;
						nTransactionID = str2long(rs.GetValue(_T("invoiceid")));
						nProductId= str2long(rs.GetValue(_T("itemid")));
						
							szSQL.Format(_T("	SELECT  ") \
										_T("		HMS_IPHARMAORDERLINE_R.HPOLR_QTYREVERSE as qty, ") \
										_T("		'['||mt_department_id||'] - '||hpolr_docno||'-'||GET_PATIENTNAME(hpolr_docno) as descr ") \
										_T("	FROM m_transaction ") \
										_T("	LEFT join HMS_IPHARMAORDERLINE_R on (HMS_IPHARMAORDERLINE_R.HPOLR_RETORDER_ID = mt_transaction_id ) ") \
										_T("	WHERE mt_doctype IN  ('DRO','CRO') ") \
										_T("		AND  mt_transaction_id=%ld and hpolr_product_id =%ld"),nTransactionID,nProductId);
						...
						while(!rs2.IsEOF())
						{
							nRow++;
							xls.SetCellText(1, nRow, rs.GetValue(_T("invoiceno")), 4098);
							tmpStr.Format(_T("%s - %s"),rs2.GetValue(_T("descr")), rs.GetValue(_T("descr")));
							xls.SetCellText(3, nRow, tmpStr, FMT_TEXT);
							rs2.GetValue(_T("qty"), nTmp);
							xls.SetCellText(5, nRow, double2str(nTmp), FMT_NUMBER1);
				...
				else
				{
					...
					if(szIotype== _T("DRO") || szIotype== _T("CRO") || szIotype== _T("DMO") )
					{
						long nTransactionID = 0, nProductId =0;
						nTransactionID = str2long(rs.GetValue(_T("invoiceid")));
						nProductId= str2long(rs.GetValue(_T("itemid")));
						...
						if(szIotype == _T("DMO"))
						{
							szSQL.Format(_T("select '['||hpo_docno||'] -'||get_patientname(hpo_docno) as descr, ") \
										_T("			hpol_qtyorder as qty ") \
										_T("		from hms_ipharmaorder   ") \
										_T("		left join hms_ipharmaorderline on (hpol_orderid = hpo_orderid) ") \
										_T("		where hpo_transaction_id = %ld and hpol_product_id = %ld "), nTransactionID,nProductId);
	...
	xls.SetActiveSheet(0);
	EndWaitCursor();
	xls.Save(_T("Exports\\The kho chi tiet.xls"));
} 
...
void CPMSStockCardReportDialog::OnPrint(bool bPreview)
{
	...
	{
		szReportTitle = _T("PMS_STOCKCARD");
	}
	else if (pMF->GetModuleID() == _T("MA") || pMF->GetModuleID() == _T("BB"))
	{
		szReportTitle = _T("PMS_STOCKCARD_MA");
	}
	else
	{
		szReportTitle.Format(_T("%s_STOCKCARD"), pMF->GetModuleID());
	}
	...
	for (int i =0; i < m_wndList.GetItemCount(); i++)
	{
		if(m_wndList.GetCheck(i)){			
			...
			szSQL.Format(_T(" SELECT impstockid,") \
						...
						_T(" WHERE qty > 0") \
						_T(" AND impstockid=%d") \
						_T(" AND iodate BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') AND to_timestamp('%s:59', 'YYYY-MM-DD HH24:MI:SS') ") \
						_T(" %s") \
						_T(" GROUP BY impstockid, expstockid, iodate, orderdate, invoiceno, ") \
...
void CPMSStockCardReportDialog::OnOrderPrint(bool bPreview)
{
	...
	CReport rpt;
	_msg(_T("Chào bạn!"));
	CString tmpStr, szDate, szSysDate, szSQL, szObjects, szWhere, tmpStr2, szItems, szReportTitle, szPath;

	if(pMF->GetModuleID() == _T("PM"))
	{
		szReportTitle = _T("PMS_DANHSACHTHUOCYEUCAUCHUADUYET");
	}
	else if (pMF->GetModuleID() == _T("MA") || pMF->GetModuleID() == _T("BB"))
	{
		szReportTitle = _T("MAS_DANHSACHVATTUYEUCAUCHUADUYET");
	}
	else
	{
		szReportTitle.Format(_T("%s_DANHSACHVATTUYEUCAUCHUADUYET"), pMF->GetModuleID());
	}
	szPath.Format(_T("Reports/HMS/%s.RPT"), szReportTitle);
	if(!rpt.Init(szPath, true))
		return;
...
void CPMSStockCardReportDialog::OnOrderPrint2(bool bPreview)
{
	CMainFrame_E10 *pMF = (CMainFrame_E10 *) AfxGetMainWnd();
	CReport rpt;
	CString tmpStr, szDate, szSysDate, szSQL, szObjects, szWhere, tmpStr2, szReportTitle, szPath;
	...
	{
		szReportTitle = _T("PMS_DANHSACHTHUOCYEUCAUCHUADUYET_MA");
	}
	else
	{
		szReportTitle.Format(_T("%s_DANHSACHTHUOCYEUCAUCHUADUYET"), pMF->GetModuleID());
	}
	szPath.Format(_T("Reports/HMS/%s.RPT"), szReportTitle);
	if(!rpt.Init(szPath, true))
		return;
