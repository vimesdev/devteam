...
void CBloodBankListService::OnExportSelect()
{
	...
	if (!szList.IsEmpty())
		szListKey.AppendFormat(_T(" AND product_id IN(%s)"), szList);szSQL.Format(_T(" SELECT ngay,khoa,tenbn,sohs,doituong,") \
			_T("        dv150,dv250,dv350,tienmau,tienxn,") \
			_T("        tongtien") \
			_T(" FROM(SELECT ngay,khoa,get_patientname(sohs) tenbn,sohs, get_objectname(doituong) doituong,") \
...
_msg(_T("%s, %s"), tmpStr2, dv2);
	...
	szSQL.Format(_T(" SELECT ngay,khoa,tenbn,doituong,") \
		...
		_T(" LEFT JOIN hms_ipharmaorder ON(hpo_orderid=hpc_bedid and hpo_docno=hpc_docno)") \
		_T(" LEFT JOIN hms_fee_list ON(hpcl_itemid=hfl_feeid)") \
		_T(" WHERE hpo_org_id='BB' and hpo_orderstatus='A' and hpc_object in(1, 3, 8, 13) and hpo_approvedate BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS')") \
		_T(" UNION ALL") \
		_T(" SELECT hpo_approvedate ngay,") \
		_T("        hpc_deptid khoa,") \
		_T("        hpcl_docno sohs,hpc_object doituong,") \
		...
		_T(" LEFT JOIN hms_ipharmaorder ON(hpo_orderid=hpc_bedid and hpo_docno=hpc_docno)") \
		_T(" LEFT JOIN hms_fee_list ON(hpcl_itemid=hfl_feeid)") \
		_T(" WHERE hpo_org_id='BB' and hpo_orderstatus='A' and hpc_object not in(7,1, 3, 8, 13) and hpo_approvedate BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS'))") \
		_T(" WHERE 1=1 %s %s") \
		_T(" GROUP BY ngay,khoa,sohs,doituong") \
		_T(" ORDER BY ngay)") \
