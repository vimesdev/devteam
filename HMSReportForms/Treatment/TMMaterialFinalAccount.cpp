...
static void _OnExportSelectFnc(CWnd *pWnd){
	...
	pVw->OnExportSelect_New();
}
static void _OnDrugSelectFnc(CWnd *pWnd){
	((CTMMaterialFinalAccount*)pWnd)->OnDrugSelect();
}
static void _OnMaterialSelectFnc(CWnd *pWnd){
	((CTMMaterialFinalAccount*)pWnd)->OnMaterialSelect();
}
static void _OnAddPXSelectFnc(CWnd *pWnd){
	((CTMMaterialFinalAccount*)pWnd)->OnAddPXSelect();
}
...
void CTMMaterialFinalAccount::OnCreateComponents(){
	...
	m_wndMaterial.Create(this, _T("Material"), 90, 95, 190, 120);
	m_wndExport.Create(this, _T("&Export"), 365, 95, 445, 120);
	m_wndAddPX.Create(this, _T("AddPX"), 195, 95, 325, 120);

	m_wndAll.Create(this, _T("All"), 5, 125, 85, 150);
	m_wndInPage.Create(this, _T("In Page"), 90, 125, 190, 150);
	m_wndOutPage.Create(this, _T("Out Page"), 195, 125, 325, 150);
...
long CTMMaterialFinalAccount::OnStorageLoadData(){
	...
	szSQL.Format(_T("SELECT msl_storage_id as id, msl_name as name FROM m_storagelist ") \
		_T("WHERE msl_isactive = 'Y' AND msl_type = 'C' AND msl_category IN ('A', 'I', 'S', 'P') AND msl_storage_id IN(%s) %s ORDER BY id "), szStorages, szWhere);
...
void CTMMaterialFinalAccount::OnAddPXSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();

}
void CTMMaterialFinalAccount::OnDrugSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	//if(m_bDrug)
	//{
...
void CTMMaterialFinalAccount::OnMaterialSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	//if(m_bMaterial)
	//{
...
void CTMMaterialFinalAccount::OnExportSelect(){
	...
	xls.SetCellText(0, 2, _T("BÁO CÁO QUYẾT TOÁN HÓA CHẤT, VẬT TƯ TIÊU HAO CHO XÉT NGHIỆM"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 12);
...
void CTMMaterialFinalAccount::OnExportSelect_New(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	BeginWaitCursor();
	CRecord rs(&pMF->m_db);
	...
	xls.SetCellText(0, 1, pMF->m_szHospitalName, FMT_TEXT , true);
		...
		xls.SetCellText(0, 2, _T("BÁO CÁO QUYẾT TOÁN HÓA CHẤT, VẬT TƯ TIÊU HAO CHO XÉT NGHIỆM"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 12);
		xls.SetCellText(0, 4, _T("Tên hóa chất"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(1, 4, _T("ÐVT"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(2, 4, _T("TỒN ĐẦU KỲ"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 11);
		xls.SetCellText(2, 5, _T("Các tủ trực"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(2, 6, _T("Quân"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(3, 6, _T("BHYT"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		...
		xls.SetCellText(6, 6, _T("T.Tiền"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(7, 4, _T("LĨNH TRONG THÁNG"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 11);
		xls.SetCellText(7, 5, _T("Bổ sung cho các tủ trực"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(7, 6, _T("Quân"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(8, 6, _T("BHYT"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(9, 6, _T("DV"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(10, 5, _T("Nhập"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		...
		xls.SetCellText(15, 5, _T("Xuất theo đối tượng"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(15, 6, _T("QUÂN"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(15, 7, _T("Trong gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(16, 7, _T("Ngoài gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(17, 7, _T("Khác"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(18, 6, _T("BHQN"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(18, 7, _T("Trong gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(19, 7, _T("Ngoài gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(20, 7, _T("Khác"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(21, 6, _T("BHYT"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(21, 7, _T("Trong gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(22, 7, _T("Ngoài gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(23, 7, _T("Khác"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(24, 6, _T("BẠN"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(24, 7, _T("Trong gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(25, 7, _T("Ngoài gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(26, 7, _T("Khác"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(27, 6, _T("C SÁCH"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(27, 7, _T("Trong gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(28, 7, _T("Ngoài gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(29, 7, _T("Khác"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(30, 6, _T("DỊCH VỤ"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(30, 7, _T("Trong gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(31, 7, _T("Ngoài gói"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(32, 7, _T("Khác"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(33, 5, _T("Tổng giá trị sử dụng"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(33, 6, _T("SL"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(34, 6, _T("T.Tiền"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(35, 4, _T("ÐIỀU CHỈNH TĂNG GIẢM"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(35, 5, _T("Tãng"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(35, 6, _T("Quân"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(36, 6, _T("BHYT"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(37, 6, _T("DV"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(38, 5, _T("Giảm"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(38, 6, _T("Quân"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(39, 6, _T("BHYT"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(40, 6, _T("DV"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(41, 4, _T("TỒN CUỐI KỲ"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 11);
		xls.SetCellText(41, 5, _T("Các tủ trực"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(41, 6, _T("Quân"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(42, 6, _T("BHYT"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(43, 6, _T("DV"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(44, 5, _T("Tổng cộng"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(44, 6, _T("SL"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(45, 6, _T("T.Tiền"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);
		xls.SetCellText(46, 6, _T("Đơn giá"), FMT_TEXT | FMT_CENTER | FMT_WRAPING, true, 10);

		

		xls.SetMerge(0, 0, 0, 2);
		xls.SetMerge(1, 1, 0, 2);
		xls.SetMerge(2, 2, 0, 31);
		xls.SetMerge(4, 7, 0, 0);
		xls.SetMerge(4, 7, 1, 1);
		xls.SetMerge(4, 4, 2, 6);
		xls.SetMerge(5, 5, 2, 4);
		...
		xls.SetMerge(6, 7, 13, 13);
		xls.SetMerge(6, 7, 14, 14);
		xls.SetMerge(4, 4, 15, 33);
		xls.SetMerge(5, 5, 15, 32);
		xls.SetMerge(6, 6, 15, 17);
		xls.SetMerge(6, 6, 18, 20);
		xls.SetMerge(6, 6, 21, 23);
		xls.SetMerge(6, 6, 24, 26);
		xls.SetMerge(6, 6, 27, 29);
		xls.SetMerge(6, 6, 30, 32);
		xls.SetMerge(5, 5, 33, 34);
		xls.SetMerge(6, 7, 33, 33);
		xls.SetMerge(6, 7, 34, 34);
		xls.SetMerge(4, 4, 35, 40);
		xls.SetMerge(5, 5, 35, 37);
		xls.SetMerge(6, 7, 35, 35);
		xls.SetMerge(6, 7, 36, 36);
		xls.SetMerge(6, 7, 37, 37);
		xls.SetMerge(5, 5, 38, 40);
		xls.SetMerge(6, 7, 38, 38);
		xls.SetMerge(6, 7, 39, 39);
		xls.SetMerge(6, 7, 40, 40);
		xls.SetMerge(4, 4, 41, 45);
		xls.SetMerge(5, 5, 41, 43);
		xls.SetMerge(6, 7, 41, 41);
		xls.SetMerge(6, 7, 42, 42);
		xls.SetMerge(6, 7, 43, 43);
		xls.SetMerge(5, 5, 44, 45);
		xls.SetMerge(6, 7, 44, 44);
		xls.SetMerge(6, 7, 45, 45);

	szSQL = GetQueryString_New();
	int nCount = rs.ExecSQL(szSQL);
	if (rs.IsEOF())
	{
		if (nCount < 0)
	...
	while (!rs.IsEOF())
	{
		...
		xls.SetCellText(15, nRow, rs.GetValue(_T("quan_inside")), FMT_NUMBER1);
		xls.SetCellText(16, nRow, rs.GetValue(_T("quan_outside")), FMT_NUMBER1);
		xls.SetCellText(17, nRow, rs.GetValue(_T("quan_k")), FMT_NUMBER1);

		xls.SetCellText(18, nRow, rs.GetValue(_T("bhqn_inside")), FMT_NUMBER1);
		xls.SetCellText(19, nRow, rs.GetValue(_T("bhqn_outside")), FMT_NUMBER1);
		xls.SetCellText(20, nRow, rs.GetValue(_T("bhqn_k")), FMT_NUMBER1);

		xls.SetCellText(21, nRow, rs.GetValue(_T("bh_inside")), FMT_NUMBER1);
		xls.SetCellText(22, nRow, rs.GetValue(_T("bh_outside")), FMT_NUMBER1);
		xls.SetCellText(23, nRow, rs.GetValue(_T("bh_k")), FMT_NUMBER1);

		xls.SetCellText(24, nRow, rs.GetValue(_T("ban_inside")), FMT_NUMBER1);
		xls.SetCellText(25, nRow, rs.GetValue(_T("ban_outside")), FMT_NUMBER1);
		xls.SetCellText(26, nRow, rs.GetValue(_T("ban_k")), FMT_NUMBER1);

		xls.SetCellText(27, nRow, rs.GetValue(_T("csach_inside")), FMT_NUMBER1);
		xls.SetCellText(28, nRow, rs.GetValue(_T("csach_outside")), FMT_NUMBER1);
		xls.SetCellText(29, nRow, rs.GetValue(_T("csach_k")), FMT_NUMBER1);

		xls.SetCellText(30, nRow, rs.GetValue(_T("dvyt_inside")), FMT_NUMBER1);
		xls.SetCellText(31, nRow, rs.GetValue(_T("dvyt_outside")), FMT_NUMBER1);
		xls.SetCellText(32, nRow, rs.GetValue(_T("dvyt_k")), FMT_NUMBER1);

		xls.SetCellText(33, nRow, rs.GetValue(_T("totalqty_export")), FMT_NUMBER1);
		rs.GetValue(_T("totalamount_export"), nTemp);
		nTotal[3] += nTemp;
		xls.SetCellText(34, nRow, double2str(nTemp), FMT_NUMBER1);
		xls.SetCellText(35, nRow, rs.GetValue(_T("solqty_adjustimport")), FMT_NUMBER1);
		xls.SetCellText(36, nRow, rs.GetValue(_T("insqty_adjustimport")), FMT_NUMBER1);
		xls.SetCellText(37, nRow, rs.GetValue(_T("serqty_adjustimport")), FMT_NUMBER1);
		xls.SetCellText(38, nRow, rs.GetValue(_T("solqty_adjustexport")), FMT_NUMBER1);
		xls.SetCellText(39, nRow, rs.GetValue(_T("insqty_adjustexport")), FMT_NUMBER1);
		xls.SetCellText(40, nRow, rs.GetValue(_T("serqty_adjustexport")), FMT_NUMBER1);
		xls.SetCellText(41, nRow, rs.GetValue(_T("solqty_lp")), FMT_NUMBER1);
		xls.SetCellText(42, nRow, rs.GetValue(_T("insqty_lp")), FMT_NUMBER1);
		xls.SetCellText(43, nRow, rs.GetValue(_T("serqty_lp")), FMT_NUMBER1);
		xls.SetCellText(44, nRow, rs.GetValue(_T("totalqty_lp")), FMT_NUMBER1);
		rs.GetValue(_T("totalamount_lp"), nTemp);
		nTotal[4] += nTemp;
		xls.SetCellText(45, nRow, double2str(nTemp), FMT_NUMBER1);

		xls.SetCellText(46, nRow, rs.GetValue(_T("price")), FMT_NUMBER1);
	...
	if(nTotal[1] + nTotal[2] + nTotal[3] + nTotal[4] > 0)
		{
			...
			xls.SetCellText(6, nRow, double2str(nTotal[1]), FMT_NUMBER1, true);
			xls.SetCellText(14, nRow, double2str(nTotal[2]), FMT_NUMBER1, true);
			xls.SetCellText(34, nRow, double2str(nTotal[3]), FMT_NUMBER1, true);
			xls.SetCellText(45, nRow, double2str(nTotal[4]), FMT_NUMBER1, true);
			nRow++;
		}
...
CString CTMMaterialFinalAccount::GetQueryString(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CString szSQL, szWhere, szFpWhere, szLpWhere, szWhere2, szStorages, szAddPx, szCategory;
	...
	CString szCase;
	if(m_nAll==0)
		szCase.Format(_T("hpol_qtyissue"));
	else if (m_nAll==1)
		szCase.Format(_T("HPOL_QTYINSIDE as hpol_qtyissue "));
	else
		szCase.Format(_T("(hpol_qtyissue- HPOL_QTYINSIDE) as hpol_qtyissue"));
	if (m_bAddPX)
	{
		szAddPx.Format(_T(" UNION ALL") \
			...
			_T(" AND mt_department_to_id = '%s' %s") \
			_T(" GROUP BY") \
			_T(" product_id,") \
			_T(" product_name,") \
			_T(" product_uomname"), m_szFromDate, m_szToDate, pMF->GetCurrentDepartmentID(), szWhere2);
	}
	...
	szSQL.Format(_T("SELECT product_id,") \
		...
		_T("       product_name,") \
		_T("       product_uomname") \
		_T("     ) tbl_lp USING (product_id, product_name, product_uomname)"), szFpWhere,szCase, pMF->GetCurrentDepartmentID(), szWhere, szAddPx, szLpWhere);	
		_msg(_T("%s"), szSQL);
		return szSQL;
}
CString CTMMaterialFinalAccount::GetQueryString_New(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CString szSQL, szWhere, szFpWhere, szLpWhere, szWhere2, szStorages, szAddPx, szCategory;
    ...
    if (szStorages == _T("40"))
    {
        szCategory.Format(_T(" AND msl_category IN ('I', 'P', 'A') "));
    }
    else
    {
        szCategory.Format(_T(" AND msl_category IN ('I', 'S', 'P') "));
    }
	if (m_bDrug)
	{
		szWhere.AppendFormat(_T(" AND product_org_id IN ('PM')"));
	...
	if (m_bAddPX)
	{
		szAddPx.Format(_T(" UNION ALL") \
			...
			_T("     SUM(mtl_qtyservice + mtl_qtyother)    AS serqty_import,") \
			_T("  0 as quan_inside,") \
			_T("   0 as quan_outside,") \
			_T("   SUM(mtl_qtysold + mtl_qtypolicy) AS quan_k,") \
			_T("  0 as bhqn_inside,") \
			_T("  0 as bhqn_outside,") \
			_T("  0 as bhqn_k,") \
			_T("  0 as bh_inside,") \
			_T("  0 as bh_outside,") \
			_T("   SUM(mtl_qtyservice + mtl_qtyother) as bh_k,") \
			_T("  0 as  ban_inside,") \
			_T("  0 as ban_outside,") \
			_T("  0 as ban_k,") \
			_T("  0 as csach_inside,") \
			_T("  0 as csach_outside,") \
			_T("  0 as csach_k,") \
			_T("  0 as dvyt_inside,") \
			_T("  0 as dvyt_outside,") \
			_T("  SUM(mtl_qtysoldins + mtl_qtyotherins) as dvyt_k,") \
			_T("     0 solpurchase_import,") \
			_T("     0 inspurchase_import,") \
			_T("     0 serpurchase_import,") \
			_T("     0 solqty_adjustimport,") \
			...
			_T("     0 serqty_adjustexport,") \
			_T("     SUM(mtl_qtydelivery)                                                                                                                                          AS totalqty_import,") \
			_T("     SUM(mtl_qtydelivery * mtl_saleprice) AS totalamount_import,") \
			_T("     SUM(mtl_qtydelivery)                                                                                                                                          AS totalqty_export,") \
			_T("     SUM(mtl_qtydelivery * mtl_saleprice) AS totalamount_export,") \
			_T("     0 totalqty_adjustimport,") \
			_T("     0 totalamount_adjustimport,") \
			_T("     0 totalqty_adjustexport,") \
			_T("     0 totalamount_adjustexport") \
			...
			_T(" product_id,") \
			_T(" product_name,") \
			_T(" product_uomname,mtl_saleprice"), m_szFromDate, m_szToDate, pMF->GetCurrentDepartmentID(), szWhere2);
	}
	...
	szSQL.Format(_T(" SELECT product_id,") \
				...
				_T("     END) serqty_fp,") \
				_T("     SUM(import_qty   - export_qty) totalqty_fp,") \
				_T("     SUM(( import_qty - export_qty ) * product_taxprice ) totalamount_fp") \
				_T("   FROM") \
				_T("     (SELECT storage_id,") \
				_T("       product_item_id sitem_id,") \
				_T("       qtyimport import_qty,") \
				...
				_T("   GROUP BY product_id,") \
				_T("     product_name,") \
				_T("     product_uomname, product_taxprice ORDER BY product_name,product_taxprice ") \
				_T("   ) tbl_fp") \
				_T(" FULL JOIN") \
				_T("   (SELECT product_id,") \
				_T("     product_name,") \
				...
				_T("     SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(1)") \
				_T("       AND deptid NOT IN ('K')") \
				_T("       THEN export_qty_inside") \
				_T("       ELSE 0") \
				_T("     END) AS quan_inside,  ") \
				_T("    SUM(CASE") \
				_T("       WHEN pobject   IN(1)") \
				_T("       AND deptid NOT IN ('K')") \
				_T("       THEN export_qty_outside") \
				_T("       ELSE 0") \
				_T("     END) AS quan_outside,") \
				_T("      SUM(  CASE") \
				_T("       WHEN pobject   IN(1)") \
				_T("       AND deptid ='K'") \
				_T("       THEN export_qty") \
				...
				_T("     SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(11, 13)") \
				_T("       AND deptid NOT IN('K')") \
				_T("       THEN export_qty_inside") \
				_T("       ELSE 0") \
				_T("     END) AS bhqn_inside,") \
				_T("    SUM(  CASE") \
				_T("       WHEN pobject   IN(11, 13)") \
				_T("       AND deptid NOT IN('K')") \
				_T("       THEN export_qty_outside") \
				_T("       ELSE 0") \
				_T("     END) AS bhqn_outside,") \
				_T(" SUM(     CASE") \
				_T("       WHEN pobject   IN(11, 13)") \
				_T("       AND deptid ='K'") \
				_T("       THEN export_qty") \
				...
				_T("         SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(10,2,6,9,4,12,5)") \
				_T("       AND deptid NOT IN('K')") \
				_T("       THEN export_qty_inside") \
				_T("       ELSE 0") \
				_T("     END) AS bh_inside,") \
				...
				_T("         SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(8)") \
				_T("       AND deptid NOT IN('K')") \
				_T("       THEN export_qty_inside") \
				_T("       ELSE 0") \
				_T("     END) AS ban_inside,") \
				_T("    SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(8)") \
				_T("       AND deptid NOT IN('K')") \
				...
				_T("     SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(3)") \
				_T("       AND deptid NOT IN('K')") \
				_T("       THEN export_qty_inside") \
				_T("       ELSE 0") \
				_T("     END) AS csach_inside,") \
				_T("    SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(3)") \
				_T("       AND deptid NOT IN('K')") \
				...
				_T("     SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(7)") \
				_T("       AND deptid NOT IN('K')") \
				_T("       THEN export_qty_inside") \
				_T("       ELSE 0") \
				_T("     END) AS dvyt_inside,") \
				_T("   SUM(") \
				_T("     CASE") \
				_T("       WHEN pobject   IN(7)") \
				_T("       AND deptid NOT IN('K')") \
				...
				_T("     END) serqty_adjustexport,") \
				_T("     SUM(import_qty  + purchase_qty) totalqty_import,") \
				_T("     SUM((import_qty + purchase_qty) * product_taxprice) totalamount_import,") \
				_T("     SUM(export_qty) totalqty_export,") \
				_T("     SUM(export_qty * product_taxprice) totalamount_export,") \
				_T("     SUM(adjust_qtyimport) totalqty_adjustimport,") \
				_T("     SUM(adjust_qtyimport * product_taxprice) totalamount_adjustimport,") \
				_T("     SUM(adjust_qtyexport) totalqty_adjustexport,") \
				_T("     SUM(adjust_qtyexport * product_taxprice) totalamount_adjustexport") \
				_T("  FROM") \
				_T("     (SELECT storage_id,") \
				_T("       product_item_id sitem_id,") \
				_T("       CASE") \
				...
				_T("       END purchase_qty,") \
				_T("       0 export_qty,") \
				_T("       0 as export_qty_inside,") \
				_T("       0 as export_qty_outside,") \
				_T("       CASE") \
				_T("         WHEN iotype = 'ADO'") \
				_T("         THEN qtyimport") \
				_T("         ELSE 0") \
				...
				_T("       hpol_qtyissue,") \
				_T("       HPOL_QTYINSIDE,") \
				_T("       hpol_qtyissue - HPOL_QTYINSIDE,") \
				_T("       0,") \
				_T("       0,") \
				_T("       hpol_unitprice,") \
				_T("       hpo_approvedate,") \
				...
				_T("     ON(hpolr_orderid = hpo_orderid)") \
				_T("     LEFT JOIN hms_pharmaorderline_view2") \
				_T("     ON(hpolr_orderid              = hpol_orderid") \
				_T("     AND hpolr_product_item_id     = hpol_product_item_id)") \
				_T("     WHERE hpo_ordertype           = 'C'") \
				_T("     AND NVL(hpol_processed, 'N') <> 'N'") \
				_T("     )") \
				_T("   LEFT JOIN m_productitem_view") \
				_T("   ON ( sitem_id = product_item_id )") \
				_T("   LEFT JOIN m_storagelist") \
				...
				_T("   GROUP BY product_id,") \
				_T("     product_name,") \
				_T("     product_uomname,product_taxprice %s") \
				_T("   ) tbl_io USING (product_id, product_name, product_uomname,price)") \
				_T(" FULL JOIN") \
				_T("   (SELECT product_id,") \
				_T("     product_name,") \
				_T("     product_uomname,") \
				...
				_T("     END) serqty_lp,") \
				_T("     SUM(import_qty   - export_qty) totalqty_lp,") \
				_T("     SUM(( import_qty - export_qty ) * product_taxprice) totalamount_lp") \
				_T("   FROM") \
				_T("     (SELECT storage_id,") \
				_T("       product_item_id sitem_id,") \
				_T("       qtyimport import_qty,") \
				...
				_T("   GROUP BY product_id,") \
				_T("     product_name,") \
				_T("     product_uomname,product_taxprice") \
				_T("   ) tbl_lp USING (product_id, product_name, product_uomname,price)") ,szCategory, szFpWhere, pMF->GetCurrentDepartmentID(), szCategory, szWhere, szAddPx, szCategory, szLpWhere);	
		_fmsg(_T("%s"), szSQL);
		return szSQL;
}