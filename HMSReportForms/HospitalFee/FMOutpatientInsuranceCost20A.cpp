...
static void _OnSoldierPatientSelectFnc(CWnd *pWnd){
	 ((CFMOutpatientInsuranceCost20A*)pWnd)->OnSoldierPatientSelect();
}
static void _OnNotinSoldierPatientSelectFnc(CWnd *pWnd){
	 ((CFMOutpatientInsuranceCost20A*)pWnd)->OnNotinSoldierPatientSelect();
}
 static long _OnphanloaibhLoadDataFnc(CWnd *pWnd)
{
	return ((CFMOutpatientInsuranceCost20A *)pWnd)->OnphanloaibhLoadData();
}
...
{
	/*m_wndReportCondition.Create(this, _T("Report Condition"), 5, 5, 480, 480);
	m_wndDeptInfo.Create(this, _T("Dept"), 10, 150, 475, 474);
	m_wndFromDateLabel.Create(this, _T("From Date"), 10, 30, 90, 55);
	m_wndFromDate.Create(this,95, 29, 235, 54); 
	m_wndToDateLabel.Create(this, _T("To Date"), 242, 30, 327, 55);
	m_wndToDate.Create(this,335, 30, 475, 55); 
	m_wndStockLabel.Create(this, _T("Stock"), 10, 60, 90, 85);
	m_wndStock.Create(this,95, 61, 475, 86); 
	m_wndTypeLabel.Create(this, _T("Type"), 10, 90, 90, 115);
	m_wndType.Create(this,95, 91, 475, 116); 
	m_wndGroupLabel.Create(this, _T("Group"), 10, 120, 90, 145);
	m_wndGroup.Create(this,95, 120, 475, 145); 
	m_wndDrugPTTT.Create(this, _T("Without Operation Material"), 6, 486, 191, 511);
	m_wndByDischargedDate.Create(this, _T("By Discharged Date"), 6, 516, 191, 541);
	m_wndOrderBy.Create(this, _T("Sort by Category"), 198, 486, 323, 511);
	m_wndPrintPreview.Create(this, _T("Print Pre&view"), 269, 546, 369, 571);
	m_wndExportXLS.Create(this, _T("&Export XLS"), 374, 546, 474, 571);
	m_wndList.Create(this,15, 175, 475, 474); 
	m_wndSoldierPatient.Create(this, _T("SoldierPatient"), 198, 516, 323, 541);
	m_wndNotinSoldierPatient.Create(this, _T("NotinSoldierPatient"), 329, 515, 475, 540);*/
	...
	m_wndSoldierPatient.Create(this, _T("SoldierPatient"), 198, 516, 323, 541);
	m_wndNotinSoldierPatient.Create(this, _T("NotinSoldierPatient"), 329, 515, 475, 540);
	m_wndFilterLabel.Create(this, _T("Bộ Lọc"), 6, 546, 86, 571);
	m_wndphanloaibh.Create(this,91, 546, 260, 571); 
}
...
long CFMOutpatientInsuranceCost20A::OnphanloaibhLoadData()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMOutpatientInsuranceCost20A::OnSoldierPatientSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();	
}
void CFMOutpatientInsuranceCost20A::OnNotinSoldierPatientSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();	
}
...
void CFMOutpatientInsuranceCost20A::OnExportSelect(){
	...
	TranslateString(_T("Export 20A Chuyển lên bảo hiểm"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 3, tmpStr);

	TranslateString(_T("Export 20A Format (update_24/08/2018) ***"), tmpStr);
...
void CFMOutpatientInsuranceCost20A::OnExport(){
	...
	xls.Save(_T("Exports\\SudungthuocngoaitruBHYT20a.xls"));
	EndWaitCursor();
...
void CFMOutpatientInsuranceCost20A::OnExport_1()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	CExcel xls;
	...
	xls.Save(_T("Exports\\SudungthuocBHYT20a_1_ngoaitru.xls"));
	EndWaitCursor();
}
CString CFMOutpatientInsuranceCost20A::GetQueryString2()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CString szSQL, szWhere, szOrderBy, szFeeCondition;
	CString szDept;
...
void CFMOutpatientInsuranceCost20A::OnExport_2()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	CExcel xls;
	...
	xls.Save(_T("Exports\\SudungthuocngoaitruBHYT20a_chuyenlencongbh.xls"));
	EndWaitCursor();
}
CString CFMOutpatientInsuranceCost20A::GetQueryString3()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CString szSQL, szWhere, szOrderBy, szFeeCondition;
	CString szDept;
	...
	if (m_bNotinSoldierPatient)
		szWhere.AppendFormat(_T(" AND substr(hd_cardno,1,2) NOT IN ('QN')"));
	szWhere.AppendFormat(_T(" AND nvl(htr_outpatient, 'X') = 'Y'"));
	szSQL.Format(_T(" SELECT patient_line, idx, typename, code, dosage, serial, drugname,ma_thuoc, mpei_hoat_chat,tenthuoc,mpei_duong_dung, mpei_ham_luong,mpei_so_dang_ky,mpei_don_vi_tinh, lvl, genericname, ") \
				_T("        groupname,  unit, countryname, price, ") \
				_T("        SUM(qty)    qty, ") \
				_T("        SUM(amount) amount ") \
				_T(" FROM   (SELECT case when hd_insline = 'Y' then cast ('III' as nvarchar2(3))") \
				...
				_T("     case when product_org_id IN ('PM') then mpei_ma_hoat_chat else mp_atccode end as ma_thuoc,") \
				_T("     mpei_hoat_chat,") \
				_T("     case when length(mpei_ten_thuoc)>0 then mpei_ten_thuoc else product_name end as tenthuoc,") \
				_T("     mpei_duong_dung,") \
				_T("     mpei_ham_luong,") \
				_T("     mpei_so_dang_ky,") \
				_T("     mpei_don_vi_tinh,") \
				...
				_T("		 LEFT JOIN hms_treatment_record ON (htr_docno = hcr_docno AND fi.hfe_treattime = htr_treattime)") \
				_T("         LEFT JOIN hms_pharmaorder_view ON (fe.hfe_orderid = hpo_orderid)") \
				_T("         LEFT JOIN m_productitem_view_bh ON( fe.hfe_itemid = product_item_id) ") \
				_T("		 LEFT JOIN m_product_extra_info") \
				...
				_T("               AND htr_status = 'T' ") \
				_T("               AND fe.hfe_category <> 'Y' ") \
				_T("               AND fe.hfe_type IN ('D', 'M') AND product_org_id IN ('PM','BB') ") \
				_T(" %s %s) ") \
...
void CFMOutpatientInsuranceCost20A::OnExport_3()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	CExcel xls;
...
CString CFMOutpatientInsuranceCost20A::GetQueryString4()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CString szSQL, szWhere, szOrderBy, szFeeCondition;
	CString szDept;
