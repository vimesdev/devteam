...
CString CFMGeneralInsuranceOutlayList::GetQueryString(){
	...
	szWhere.Format(_T(" AND rf.hfe_cash_id > 0 AND rf.hfe_status = 'P' AND rf.hfe_date BETWEEN to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') AND to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss')"), m_szFromDate, m_szToDate);
	if (!szDepts.IsEmpty())
	...
	szSQL.Format(_T(" SELECT hfe_date, ") \
				...
				_T("                                  AND i.hfe_objectid IN ( 1, 3, 8, 13 ) ) THEN f.hfe_patpaid ") \
				_T("                           ELSE 0 ") \
