#include "stdafx.h"
#include "FMProcessedPrescriptionListEX.h"
#include "HMSMainFrame.h"
#include "ReportDocument.h"
#include "Excel.h"
/*static void _OnFromDateChangeFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnFromDateChange();
} */
/*static void _OnFromDateSetfocusFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnFromDateSetfocus();} */ 
/*static void _OnFromDateKillfocusFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnFromDateKillfocus();
} */
static int _OnFromDateCheckValueFnc(CWnd *pWnd){
	return ((CFMProcessedPrescriptionListEX *)pWnd)->OnFromDateCheckValue();
} 
/*static void _OnToDateChangeFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnToDateChange();
} */
/*static void _OnToDateSetfocusFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnToDateSetfocus();} */ 
/*static void _OnToDateKillfocusFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnToDateKillfocus();
} */
static int _OnToDateCheckValueFnc(CWnd *pWnd){
	return ((CFMProcessedPrescriptionListEX *)pWnd)->OnToDateCheckValue();
} 
static void _OnClerkSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CFMProcessedPrescriptionListEX* )pWnd)->OnClerkSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnClerkSelendokFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnClerkSelendok();
}
/*static void _OnClerkSetfocusFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnClerkKillfocus();
}*/
/*static void _OnClerkKillfocusFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnClerkKillfocus();
}*/
static long _OnClerkLoadDataFnc(CWnd *pWnd){
	return ((CFMProcessedPrescriptionListEX *)pWnd)->OnClerkLoadData();
}
/*static void _OnClerkAddNewFnc(CWnd *pWnd){
	((CFMProcessedPrescriptionListEX *)pWnd)->OnClerkAddNew();
}*/
static void _OnDischargedDateSelectFnc(CWnd *pWnd){
	 ((CFMProcessedPrescriptionListEX*)pWnd)->OnDischargedDateSelect();
}
static void _OnPrintPreviewSelectFnc(CWnd *pWnd){
	CFMProcessedPrescriptionListEX *pVw = (CFMProcessedPrescriptionListEX *)pWnd;
...
CFMProcessedPrescriptionListEX::CFMProcessedPrescriptionListEX(CWnd *pParent){

	m_nDlgWidth = 1029;
	m_nDlgHeight = 773;
	SetDefaultValues();
}
CFMProcessedPrescriptionListEX::~CFMProcessedPrescriptionListEX(){
}
void CFMProcessedPrescriptionListEX::OnCreateComponents(){
...
void CFMProcessedPrescriptionListEX::OnInitializeComponents(){
...
void CFMProcessedPrescriptionListEX::OnSetWindowEvents(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMProcessedPrescriptionListEX::OnDoDataExchange(CDataExchange* pDX){
...
void CFMProcessedPrescriptionListEX::SetDefaultValues(){
...
int CFMProcessedPrescriptionListEX::SetMode(int nMode){
...
/*void CFMProcessedPrescriptionListEX::OnFromDateChange(){
	
} */
/*void CFMProcessedPrescriptionListEX::OnFromDateSetfocus(){
	
} */
/*void CFMProcessedPrescriptionListEX::OnFromDateKillfocus(){
	
} */
int CFMProcessedPrescriptionListEX::OnFromDateCheckValue(){
	return 0;
}
 
/*void CFMProcessedPrescriptionListEX::OnToDateChange(){
	
} */
/*void CFMProcessedPrescriptionListEX::OnToDateSetfocus(){
	
} */
/*void CFMProcessedPrescriptionListEX::OnToDateKillfocus(){
	
} */
int CFMProcessedPrescriptionListEX::OnToDateCheckValue(){
	return 0;
}
 
void CFMProcessedPrescriptionListEX::OnClerkSelectChange(int nOldItemSel, int nNewItemSel){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
}
 
void CFMProcessedPrescriptionListEX::OnClerkSelendok(){
	 
}

/*void CFMProcessedPrescriptionListEX::OnClerkSetfocus(){
	
}*/
/*void CFMProcessedPrescriptionListEX::OnClerkKillfocus(){
	
}*/
long CFMProcessedPrescriptionListEX::OnClerkLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
...
/*void CFMProcessedPrescriptionListEX::OnClerkAddNew(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
} */
void CFMProcessedPrescriptionListEX::OnDischargedDateSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
}
void CFMProcessedPrescriptionListEX::OnPrintPreviewSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	CReport rpt;
	...
	double nthuocdichvu_trongvien = 0, nthuocdichvu_yeucau = 0, nthuocbaohiem_trongvien = 0, nthuocbaohiem_yeucau = 0;

	int nIdx = 1;
	if (!rpt.Init(_T("Reports\\HMS\\PMS_DANHSACHDUYETDONTHUOC_TPCN.RPT")))
		return;
	...
	/*szSQL.Format(_T(" SELECT tit, appdate, ") \
				...
				_T("				   END          AS deptname, ") \
				_T("                   CASE ") \
				_T("                     WHEN product_org_id = 'PM' THEN HPOL_QTYORDER * mtl_saleprice ") \
				_T("                     ELSE 0 ") \
				_T("                   END                                      AS drugamt, ") \
				_T("                   CASE ") \
				_T("                     WHEN product_org_id = 'MA' THEN HPOL_QTYORDER * mtl_saleprice ") \
				_T("                     ELSE 0 ") \
				...
				_T("         WHERE  hpolr_qtyreverse > 0 AND mt_status = 'A' AND mt_storage_to_id IN (14, 15) %s ) ") \
				_T(" GROUP  BY tit, ") \
				_T("           appdate, ") \
				_T("           orderno, ") \
				_T("           deptname ") \
		...
		szSQL.Format(_T(" SELECT tit,") \
		_T("   appdate,") \
		_T("   orderno,") \
		_T("   deptname,") \
		_T("   SUM(thuocdichvu_trongvien) thuocdichvu_trongvien,") \
		_T("   SUM(thuocdichvu_yeucau) thuocdichvu_yeucau,") \
		_T("   SUM(thuocbaohiem_trongvien) thuocbaohiem_trongvien,") \
		_T("   SUM(thuocbaohiem_yeucau) thuocbaohiem_yeucau") \
		_T(" FROM") \
		_T("   (SELECT 'Drug Daily' AS tit,") \
		_T("     hpo_orderid orderid,") \
		_T("     TRUNC(mt_processeddate) appdate,") \
		_T("     mt_orderno orderno,") \
		_T("     CASE") \
		_T("       WHEN hpo_deptid = 'B5'") \
		_T("       THEN Get_departmentname(") \
		_T("         (SELECT ho_deptid FROM hms_operation WHERE ho_idx = hpo_reference_id") \
		_T("         ))") \
		_T("       ELSE Get_departmentname(hpo_deptid)") \
		_T("     END AS deptname,") \
		_T("     CASE") \
		_T("       WHEN product_org_id = 'PM' AND PRODUCT_PRODUCTTYPE='A1700' AND hpo_deptid <>'PTTYC' and HPO_OBJECT_ID=7 THEN HPOL_QTYORDER * mtl_saleprice") \
		_T("       ELSE 0") \
		_T("     END AS thuocdichvu_trongvien,") \
		_T("      CASE") \
		_T("       WHEN product_org_id = 'PM' AND PRODUCT_PRODUCTTYPE='A1700' AND hpo_deptid ='PTTYC' and HPO_OBJECT_ID=7 THEN HPOL_QTYORDER * mtl_saleprice") \
		_T("       ELSE 0") \
		_T("     END AS thuocdichvu_yeucau,") \
		_T("     CASE") \
		_T("       WHEN product_org_id = 'PM' AND PRODUCT_PRODUCTTYPE='A1700' AND hpo_deptid <>'PTTYC' and HPO_OBJECT_ID<>7 THEN HPOL_QTYORDER * mtl_saleprice") \
		_T("       ELSE 0") \
		_T("     END AS thuocbaohiem_trongvien,") \
		_T("      CASE") \
		_T("       WHEN product_org_id = 'PM' AND PRODUCT_PRODUCTTYPE='A1700' AND hpo_deptid ='PTTYC' and HPO_OBJECT_ID<>7 THEN HPOL_QTYORDER * mtl_saleprice") \
		_T("       ELSE 0") \
		_T("     END AS thuocbaohiem_yeucau") \
		_T("   FROM hms_ipharmaorder") \
		...
		_T("   AND hpo_storage_id                IN (14, 15)") \
		_T("   AND (COALESCE(hpo_reference_id, 0) = 0") \
		_T("   OR (COALESCE(hpo_reference_id, 0)  >0") \
		_T("   AND SUBSTR(mt_orderno,1,2)         ='DP'))") \
		_T("   AND hpo_processed                  = 'Y' %s") \
		_T("   UNION ALL") \
		_T("   SELECT 'Return Order' AS tit,") \
		_T("     mt_transaction_id orderid,") \
		_T("     TRUNC(mt_approveddate) appdate,") \
		_T("     mt_orderno orderno,") \
		_T("     CASE") \
		_T("       WHEN hpo_deptid = 'B5'") \
		...
		_T("     END AS deptname,") \
		_T("     CASE") \
		_T("       WHEN product_org_id = 'PM' AND PRODUCT_PRODUCTTYPE='A1700' AND hpo_deptid <>'PTTYC' and HPO_OBJECT_ID=7") \
		_T("       THEN hpolr_qtyreverse*HPOLr_UNITPRICE") \
		_T("        ELSE 0") \
		_T("     END AS thuocdichvu_trongvien,") \
		_T("    CASE") \
		_T("       WHEN product_org_id = 'PM' AND PRODUCT_PRODUCTTYPE='A1700' AND hpo_deptid ='PTTYC' and HPO_OBJECT_ID=7") \
		...
		_T("     END AS thuocbaohiem_yeucau") \
		_T("   FROM hms_ipharmaorderline_r") \
		_T("   LEFT JOIN VIMES.hms_ipharmaorder") \
		_T("   ON (hpo_orderid= hpolr_orderid)") \
		_T("   LEFT JOIN m_productitem_view") \
		_T("   ON ( hpolr_product_item_id = product_item_id )") \
		_T("   LEFT JOIN m_transaction") \
		_T("   ON (mt_transaction_id  = hpolr_retorder_id)") \
		_T("   WHERE hpolr_qtyreverse > 0") \
		_T("   AND mt_status          = 'A'") \
		_T("   AND mt_storage_to_id  IN (14, 15) %s") \
		_T("   )") \
		_T(" GROUP BY tit,") \
		_T("   appdate,") \
		_T("   orderno,") \
		_T("   deptname") \
		_T(" HAVING SUM(thuocdichvu_trongvien+thuocdichvu_yeucau+thuocbaohiem_trongvien+thuocbaohiem_yeucau)>0  ") \
		_T(" ORDER BY tit,") \
		_T("   appdate,") \
		_T("   orderno "), szWhere, szWhere1);
	...
	while (!rs.IsEOF())
	{
		...
		{
			if (nthuocdichvu_trongvien + nthuocdichvu_yeucau + nthuocbaohiem_trongvien + nthuocbaohiem_yeucau> 0)
			{
				rptDetail = rpt.AddDetail(rpt.GetGroupFooter(0));
				rptDetail->SetValue(_T("s7"), double2str(nthuocdichvu_trongvien));
				rptDetail->SetValue(_T("s8"), double2str(nthuocdichvu_yeucau));
				rptDetail->SetValue(_T("s9"), double2str(nthuocbaohiem_trongvien));
				rptDetail->SetValue(_T("s10"), double2str(nthuocbaohiem_yeucau));
			}
			...
			szOldTit = szNewTit;
			nIdx = 1;
			nthuocdichvu_trongvien = 0;
			nthuocdichvu_yeucau = 0;
			nthuocbaohiem_trongvien = 0;
			nthuocbaohiem_yeucau = 0;
		}
		...
		rs.GetValue(_T("thuocdichvu_trongvien"), tmpStr);
		nthuocdichvu_trongvien += str2double(tmpStr);
		rptDetail->SetValue(_T("7"), tmpStr);

		rs.GetValue(_T("thuocdichvu_yeucau"), tmpStr);
		nthuocdichvu_yeucau += str2double(tmpStr);
		rptDetail->SetValue(_T("8"), tmpStr);


		rs.GetValue(_T("thuocbaohiem_trongvien"), tmpStr);
		nthuocbaohiem_trongvien += str2double(tmpStr);
		rptDetail->SetValue(_T("9"), tmpStr);


		rs.GetValue(_T("thuocbaohiem_yeucau"), tmpStr);
		nthuocbaohiem_yeucau += str2double(tmpStr);
	...
	{
		rptDetail = rpt.AddDetail(rpt.GetGroupFooter(0));
		tmpStr.Format(_T("%f"), nthuocdichvu_trongvien);
		rptDetail->SetValue(_T("s7"), tmpStr);
		tmpStr.Format(_T("%f"), nthuocdichvu_yeucau);
		rptDetail->SetValue(_T("s8"), tmpStr);
		tmpStr.Format(_T("%f"), nthuocbaohiem_trongvien);
		rptDetail->SetValue(_T("s9"), tmpStr);
		tmpStr.Format(_T("%f"), nthuocbaohiem_yeucau);
...
void CFMProcessedPrescriptionListEX::OnExportSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
CString CFMProcessedPrescriptionListEX::GetQueryString(){
	CString szSQL;
	return szSQL;
}