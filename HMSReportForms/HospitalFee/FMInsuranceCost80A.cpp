...
{
	return ((CFMInsuranceCost80A *)pWnd)->OnphanloaibhLoadData();
}
...
long CFMInsuranceCost80A::OnphanloaibhLoadData()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMInsuranceCost80A::OnExportForInsSelect()
{
	...
	TranslateString(_T("Xuất ra Excel cho bảo hiểm mẫu 80A (cũ)"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 1, tmpStr);
	menu.AppendMenu(MF_SEPARATOR);

	TranslateString(_T("Xuất ra Excel cho bảo hiểm mẫu 80A(mới***)"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 2, tmpStr);

	TranslateString(_T("Xuất ra Excel biểu 80 lên cổng BH"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 3, tmpStr);

	TranslateString(_T("Xuất ra danh sách so sánh với 20A"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 4, tmpStr);

	TranslateString(_T("Xuất ra danh sách so sánh với 21A"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 5, tmpStr);

	TranslateString(_T("Xuất ra Excel cho bảo hiểm mẫu 80A(Update 23/6/2022 ***)"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 6, tmpStr);

	TranslateString(_T("Xuất ra Excel A11 ***)"), tmpStr);
	...
	switch (nPos)
	{
		...
		case 4:
			OnExportDetailPatient();
			break;
		case 5:
			OnExportDetailPatient21A();
			break;
		case 6:
			m_bIsInsPurpose = true;
			OnExportSelect_6();
...
void CFMInsuranceCost80A::OnExportDetailPatient()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
...
void CFMInsuranceCost80A::OnExportDetailPatient21A()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, tmpStr, szTemp, szWhere, szOldDay, szNewDay;
...
CString CFMInsuranceCost80A::GetQueryString_New(){
	CString szSQL;
	return szSQL;
}

void CFMInsuranceCost80A::GetDataFiltering(CString& szWhere, CString& szOrderBy, CString& szHavingClause, CString& szMaterialID){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
CString CFMInsuranceCost80A::GetMainQuery(){
	CString szSQL;

	return szSQL;
}

CString CFMInsuranceCost80A::GetQueryString_check()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szSubSQL, szWhere, szSubWhere, szOrderBy, szTable, szObjectCondition, szHavingClause;
...
CString CFMInsuranceCost80A::GetQueryString_check21A()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szSubSQL, szWhere, szSubWhere, szOrderBy, szTable, szObjectCondition, szHavingClause;
...
void CFMInsuranceCost80A::OnExportSelect_1()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString tmpStr, szTemp, szFromDateToDate, szDate, szMonth, szYear;
	...
	xls.SetCellText(68, nRow, _T("Đối tượng quân đội"), FMT_TEXT);
	...
	while (!rs.IsEOF()){
		...
		xls.SetCellText(nCol +68 , nRow, rs.GetValue(_T("doituong_qd")), FMT_TEXT);
...
CString CFMInsuranceCost80A::GetQueryString_1()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
...
// 	szMaterialID += _T("'A1500', 'A1600'");
    ...
    szSQL.Format(_T(" SELECT ") \
				...
				_T(" admitdate,dischargedate,totaltreat,hcr_mainicd,icd10,emergency,deptid,doituong_qd,invoice_date, invoice_date_detail,") \
				_T(" inscost,drugfee,drugfeek,bloodfee,testfee,pacsfee,normtechfee,hitechfee,materialfee,replace_materialfee,ref_materialfee,bedfee,transportfee,") \
				...
				_T("		  hcr_sumtreat, hp_provid, hp_distid, hp_villid, hc_regdate, hc_expdate, hcr_reldisease, hi_name, ") \
				_T("		  hd_emergency, deptid, hd_admitdept, invoice_date, hig_group, hd_army_objectid") \
				_T(" HAVING SUM(cost) > 0 AND SUM(inspaid-discount) >= 0 ") \
				_T(" %s") \
				_T(" ORDER BY ") \
...
void CFMInsuranceCost80A::OnExportSelect_2()
{
...
CString CFMInsuranceCost80A::GetQueryString_2()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szSubSQL, szWhere, szSubWhere, szOrderBy, szTable, szObjectCondition, szHavingClause;
...
void CFMInsuranceCost80A::OnExportSelect_6()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString tmpStr, szTemp, szFromDateToDate, szDate, szMonth, szYear;
	...
	xls.SetCellText(69, nRow, _T("Đối tượng quân đội"), FMT_TEXT);
	...
	while (!rs.IsEOF()){
		...
		xls.SetCellText(nCol +69 , nRow, rs.GetValue(_T("doituong_qd")), FMT_TEXT);
		//xls.SetCellText(nCol + 40, nRow, _T("\x30\x31"), FMT_TEXT);
		/*nIncrement = 0;
		if (m_bIsInsPurpose)
		{
...
CString CFMInsuranceCost80A::GetQueryString_6()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szSubSQL, szWhere, szWhereBQP, szSubWhere, szOrderBy, szTable, szObjectCondition, szHavingClause;
...
// 	szMaterialID += _T("'A1500', 'A1600'");
	...
	szSubWhere += szObjectCondition;
	//szWhere.AppendFormat(_T(" AND NVL(htr_outpatient, 'X') <> 'Y' AND fe.hfe_group <> 'D0000' AND fe.hfe_status <> 'C'"));
	//Thêm bộ lọc lấy bệnh nhân ghép gan
	if (m_szphanloaibhKey == _T("B3A"))
		{
			szWhere.AppendFormat(_T(" AND fe.hfe_itemid IN (select ss_code from sys_sel where ss_id='hms_pt_ghepgan')"));
		}
...
void CFMInsuranceCost80A::OnExportSelect_A11()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString tmpStr, szTemp, szFromDateToDate, szDate, szMonth, szYear;
...
CString CFMInsuranceCost80A::GetQueryString_A11()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szSubSQL, szWhere, szWhereBQP, szSubWhere, szOrderBy, szTable, szObjectCondition, szHavingClause;
