...
static void _OnByDateSelectFnc(CWnd *pWnd){
	return ((CFMUndischargedAdvancePatientList*) pWnd)->OnByDateSelect();
}
static void _OnByDeptSelectFnc(CWnd *pWnd){
	return ((CFMUndischargedAdvancePatientList*) pWnd)->OnByDeptSelect();
}
static void _OnInsuranceSelectFnc(CWnd *pWnd){
	return ((CFMUndischargedAdvancePatientList*) pWnd)->OnInsuranceSelect();
}
...
void CFMUndischargedAdvancePatientList::OnByDateSelect()
{
	m_szCondition = _T("hfe_date");
}
void CFMUndischargedAdvancePatientList::OnByDeptSelect()
{
	m_szCondition = _T("hfe_deptid");
}

void CFMUndischargedAdvancePatientList::OnInsuranceSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	if (!m_bInsurance)
...
void CFMUndischargedAdvancePatientList::OnPrintSelect(){
	...
	while (!rs.IsEOF())
	{
		if (!m_szCondition.IsEmpty())
			rs.GetValue(m_szCondition, szNewLine);
...
CString CFMUndischargedAdvancePatientList::GetQueryString(){
	...
	szWhere.AppendFormat(_T(" AND (hcr_status <> 'T' OR (hcr_status = 'T' AND hcr_dischargedate > to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss')))"), m_szToDate);
	szSQL.Format(_T(" SELECT   Get_patientname(hfe_docno) patient_name, ") \
				...
				_T("       AND hfe_status IN ('P', 'R') AND fac_posted = 'Y'") \
				_T(" ORDER     BY %s hfe_docno "), m_szFromDate, m_szToDate, szWhere, szOrderBy);
