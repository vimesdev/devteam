...
CString CFMTempSendPatientListByDay::GetQueryString(){
	...
	if (m_bSOD)
		//szWhere.AppendFormat(_T(" AND hfe_isreq = 'Y'"));
		szWhere.AppendFormat(_T(" AND hfe_deptid in ('TYC','PTTYC')"));
	else
		//szWhere.AppendFormat(_T(" AND hfe_objectid = 7 AND NVL(hfe_isreq, 'N') <> 'Y'"));
		szWhere.AppendFormat(_T(" AND hfe_objectid = 7 AND hfe_deptid not in ('TYC','PTTYC')"));
