...
void CFMDischargedUnpaidPatientListByDay::OnExportSelect()
{
	...
	xls.SetCellText(nCol + 12, nRow + 5, tmpStr, FMT_TEXT | FMT_CENTER | FMT_VCENTER | FMT_WRAPING, true, 11);
	...
	while (!rs.IsEOF())
	{
		...
		rs.GetValue(_T("otherpaid"), nCost);
		nGroupTotal[8] += nCost;
		tmpStr.Format(_T("%.2f"), nCost);
	...
	EndWaitCursor();
	xls.Save(_T("Exports\\DANHSACHBENHNHANRAVIENCHUATHANHTOAN_THEONGAY.xls"));
} 
...
CString CFMDischargedUnpaidPatientListByDay::GetQueryString(){
	...
	/*szSQL.Format(_T(" SELECT") \
				...
				_T(" FROM") \
				_T(" (") \
				_T(" SELECT Get_patientname(docno) AS pname,") \
		        _T("		  docno,") \
				_T("          deptid, ") \
				_T("          recordno, ") \
				_T("          ngayvao, ") \
				...
				_T("                   WHERE     1 = 1 AND dc.hd_object = '7'") \
				_T("                             AND hcr_status = 'T' AND d.hfe_status NOT IN ('C', 'R')   ") \
				_T("                             AND a.hfe_status = 'A' ") \
				_T(" UNION ALL") \
				_T("       SELECT hd_docno docno,") \
				_T("         hcr_dischargedept deptid,") \
				_T("         hcr_recordno recordno,") \
				_T("         0 refidx,") \
				_T("         0,") \
				...
				_T("       LEFT JOIN hms_doc dc") \
				_T("       ON (dc.hd_docno = i.hfe_docno)") \
				_T("       LEFT JOIN hms_clinical_record") \
				_T("       ON ( hcr_docno = dc.hd_docno )") \
				_T("       LEFT JOIN hms_fee_approve a") \
				_T("       ON ( a.hfe_docno      = dc.hd_docno )") \
				_T("       WHERE 1               = 1") \
	...
	szSQL.Format(_T(" SELECT tbl.*,") \
				_T(" CASE WHEN NVL(totaldeposit,0) + otherpaid < feeamt THEN feeamt-NVL(totaldeposit,0)-otherpaid ELSE 0 END as thu,") \
				_T(" CASE WHEN NVL(totaldeposit,0) + otherpaid > feeamt THEN NVL(totaldeposit,0)+ otherpaid-feeamt ELSE 0 END as chi") \
				_T(" FROM ") \
				_T(" (") \
				_T(" SELECT Get_patientname(HFE_DOCNO) AS pname,") \
				_T("   hfe_docno                       AS docno,") \
				_T("   hfe_deptid                      AS deptid,") \
				_T("   HCR_RECORDNO                    AS recordno,") \
				_T("   HFE_ADMITDATE ngayvao,") \
				_T("   HFE_DISCHARGEDATE ngayra,") \
				_T("   HFE_DEPOSIT AS deposit,") \
				_T("   (SELECT SUM(hfe_amount)") \
				_T("   FROM hms_fee_deposit") \
				_T("   WHERE hfe_docno  =hcr_docno") \
				_T("   AND hfe_patpaid <> hfe_amount") \
				_T("   AND hfe_patpaid  =0") \
				_T("   AND hfe_status   ='P'") \
				_T("   )                            AS totaldeposit,") \
				_T("   hfe_patpaid                  AS feeamt,") \
				_T("   NVL(hfe_otherpaid,0)         AS otherpaid,") \
				_T("   hfe_payment                  AS thuoi,") \
				_T("   hfe_refund                   AS chioi,") \
				_T("   GET_OBJECTNAME(hfe_objectid) AS doituong") \
				_T(" FROM hms_fee_invoice") \
				_T(" LEFT JOIN HMS_CLINICAL_RECORD") \
				_T(" ON (hfe_docno=hcr_docno)") \
				_T(" WHERE hcr_status='T' AND HFE_DISCHARGEDATE BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS')") \
				_T(" AND hfe_status                              ='A'") \
				_T(" AND (hfe_patpaid + hfe_payment + hfe_refund + hfe_amount )>0") \
				_T(" AND NVL(HCRF_PAYMENT, 'X') <> 'P'") \
				_T(" and (select count (*) from hms_fee where hfe_docno=hcr_docno AND hfe_status in ('O', 'A') and NVL(HFE_category, 'X')<>'Y')>0 ") \
				_T(" %s ") \
