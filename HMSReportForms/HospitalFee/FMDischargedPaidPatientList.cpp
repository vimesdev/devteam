...
CString CFMDischargedPaidPatientList::GetQueryString(){
	...
	/*szSQL.Format(_T("SELECT    Get_patientname(docno) pname, docno,") \
				...
				_T("           WHERE  ngaythu BETWEEN to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') AND to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') %s") \
				_T("           GROUP  BY docno, ") \
				_T("                     deptid, ") \
				_T("                     ngayvao, ") \
				_T("                     ngayra, ") \
	...
	szSQL.Format(_T("SELECT    Get_patientname(docno) pname, ") \
				...
				_T("           WHERE  ngayTU BETWEEN to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') AND to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') ") \
				_T("                   OR ngaythu BETWEEN to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') AND to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') %s") \
				_T("           GROUP  BY docno, ") \
				_T("                     deptid, ") \
				_T("                     ngayvao, ") \
				_T("                     ngayra, ") \
