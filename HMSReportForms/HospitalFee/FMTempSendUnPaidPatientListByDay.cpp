...
void CFMTempSendUnPaidPatientListByDay::OnCreateComponents(){
	...
	m_wndSOD.Create(this, _T("Service On Demand"), 5, 350, 150, 375);
...
void CFMTempSendUnPaidPatientListByDay::OnExportSelect(){
	...
	{
		nRow++;
		xls.SetCellText(nCol + 1, nRow, _T("C?ng toàn vi?n"), FMT_TEXT | FMT_WRAPING, true);
...
CString CFMTempSendUnPaidPatientListByDay::GetQueryString(){
	...
	if (m_bSOD)
		//szWhere.AppendFormat(_T(" AND d.hfe_isreq = 'Y' AND d.hfe_amount > d.hfe_patpaid AND (hd_status <> 'T' OR (hd_status = 'T' AND hd_enddate < to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss'))) AND (hdf_acceptedfee <> 'P' OR (hdf_acceptedfee = 'P' AND hdf_accepteddate > to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss')))"), m_szFromDate, m_szToDate);
		//szWhere.AppendFormat(_T(" AND d.hfe_isreq = 'Y' AND d.hfe_amount > d.hfe_patpaid AND (hdf_acceptedfee <> 'P' OR (hdf_acceptedfee = 'P' AND hdf_accepteddate > TO_TIMESTAMP('%s','YYYY-MM-DD HH24:MI:SS')))"), m_szFromDate, m_szToDate);
		//szWhere.AppendFormat(_T(" AND d.hfe_isreq = 'Y' "));
		  szWhere.AppendFormat(_T(" AND d.hfe_deptid in ('TYC','PTTYC')"));
	else
		szWhere.AppendFormat(_T(" AND d.hfe_objectid = 7 AND d.hfe_deptid not in ('TYC','PTTYC') AND (NVL(hcrf_acceptedfee, 'N') <> 'Y'))"));
	szSQL.Format(_T(" SELECT    Get_patientname(d.hfe_docno)     pname, ") \
				...
				_T(" LEFT JOIN hms_doc ON ( d.hfe_docno = hd_docno ) ") \
				_T(" LEFT JOIN fa_cash ON( d.hfe_cash_id = fac_cash_id)")
				_T(" LEFT JOIN hms_fee_invoice i ON (i.hfe_invoiceno = hcrf_invoicefee)") \
