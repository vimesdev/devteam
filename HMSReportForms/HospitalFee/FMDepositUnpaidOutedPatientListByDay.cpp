...
void CFMDepositUnpaidOutedPatientListByDay::OnSetWindowEvents()
{
	...
	m_szToDate = pMF->GetSysDate();
	m_szFromDate = _T("2016/01/01 00:00");
	//m_szFromDate += _T("00:00");
	m_szToDate += _T("23:59");
	m_wndFromDate.EnableWindow(false);
	//m_szToDate = pMF->GetSysDate();
	//m_szFromDate = _T("2015/01/01 00:00");
	//m_szToDate += _T("23:59");
	UpdateData(false);
...
void CFMDepositUnpaidOutedPatientListByDay::OnPrintSelect()
{
	...
	CRecord rs(&pMF->m_db);
	CString szSQL, tmpStr, szTemp, szSysDate, szOldLine, szNewLine, szObjectNames, szMoney;
	bool bCheckServ = false, bCheckIns = false;
	double nGroupTotal[13];
	double nTotal[13];
	double nCost;
	int nIndex = 1;
	CStringArray strArr;
	...
	ShowMessageBox(_T("Báo cáo sẽ lấy theo ngày ra viện (nội trú) hoặc ngày kết thúc (phòng khám), những bệnh nhân đã kết thúc chưa thanh toán (phòng khám & nội trú), chỉ tính đối tượng dịch vụ!"));
...
CString CFMDepositUnpaidOutedPatientListByDay::GetQueryString(){
	...
	if (!szDepts.IsEmpty())
	{
		szWhere.AppendFormat(_T(" and dischargedept in(%s) "), szDepts);
	}
	if (m_bSOD)
		szWhere.AppendFormat(_T(" AND d.hfe_deptid in ('TYC','PTTYC') AND (NVL(dathanhtoan, 'X') <> 'P') AND d.hfe_patpaid <> d.hfe_amount AND d.hfe_patpaid=0"));
	else
	szWhere.AppendFormat(_T(" AND d.hfe_deptid not in ('TYC','PTTYC') AND (NVL(dathanhtoan, 'N') <> 'P') AND d.hfe_patpaid <> d.hfe_amount AND d.hfe_patpaid=0"));
	szSQL.Format(_T("SELECT    Get_patientname(docno) pname, ") \
					...
					_T("                             dischargedept deptid, ") \
					_T("                             recordno recordno, ") \
					_T("                             d.hfe_refidx refidx, ") \
					_T("                             d.hfe_amount, ") \
					_T("                             ngayvao AS ngayvao, ") \
					_T("                             ngayra AS ngayra ") \
					_T("                   FROM      hms_fee_deposit d ") \
					_T("                   LEFT JOIN HMS_DOCSTATUS_VIEW ON ( docno = d.hfe_docno ) ") \
					_T("				   LEFT JOIN hms_fee_invoice i ON (i.hfe_invoiceno = hoadon)") \
					_T("                   WHERE     1 = 1 %s") \
					_T("                             AND DOCSTATUS = 'T'") \
					_T("                             AND d.hfe_status IN ('P')") \
					_T("                             AND d.hfe_objectid = '7' ") \
					_T("                  ) ") \
					_T("           WHERE  ngayra BETWEEN to_timestamp('%s','yyyy/mm/dd hh24:mi:ss') AND to_timestamp('%s','yyyy/mm/dd hh24:mi:ss') ") \
					_T("           GROUP  BY docno, ") \
					...
					_T("             docno "), szWhere, m_szFromDate, m_szToDate);
