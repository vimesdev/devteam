...
/*static void _OnYearChangeFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnYearChange();
} */
/*static void _OnYearSetfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnYearSetfocus();} */ 
/*static void _OnYearKillfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnYearKillfocus();
} */
static int _OnYearCheckValueFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong *)pWnd)->OnYearCheckValue();
} 
static void _OnReportPeriodSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CFMDanhmuchopdong* )pWnd)->OnReportPeriodSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnReportPeriodSelendokFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnReportPeriodSelendok();
}
/*static void _OnReportPeriodSetfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnReportPeriodKillfocus();
}*/
/*static void _OnReportPeriodKillfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnReportPeriodKillfocus();
}*/
static long _OnReportPeriodLoadDataFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong *)pWnd)->OnReportPeriodLoadData();
}
/*static void _OnReportPeriodAddNewFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnReportPeriodAddNew();
}*/
/*static void _OnFromDateChangeFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnFromDateChange();
} */
/*static void _OnFromDateSetfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnFromDateSetfocus();} */ 
/*static void _OnFromDateKillfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnFromDateKillfocus();
} */
static int _OnFromDateCheckValueFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong *)pWnd)->OnFromDateCheckValue();
} 
/*static void _OnToDateChangeFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnToDateChange();
} */
/*static void _OnToDateSetfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnToDateSetfocus();} */ 
/*static void _OnToDateKillfocusFnc(CWnd *pWnd){
	((CFMDanhmuchopdong *)pWnd)->OnToDateKillfocus();
} */
static int _OnToDateCheckValueFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong *)pWnd)->OnToDateCheckValue();
} 
static void _OnByDateSelectFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*) pWnd)->OnByDateSelect();
}
static void _OnByDeptSelectFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*) pWnd)->OnByDeptSelect();
}
static void _OnInsuranceSelectFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*) pWnd)->OnInsuranceSelect();
}
static void _OnPrintSelectFnc(CWnd *pWnd){
	CFMDanhmuchopdong *pVw = (CFMDanhmuchopdong *)pWnd;
	pVw->OnPrintSelect();
} 
...
static long _OnObjectListLoadDataFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*)pWnd)->OnObjectListLoadData();
} 
static void _OnObjectListDblClickFnc(CWnd *pWnd){
	((CFMDanhmuchopdong*)pWnd)->OnObjectListDblClick();
} 
static void _OnObjectListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CFMDanhmuchopdong*)pWnd)->OnObjectListSelectChange(nOldItem, nNewItem);
} 
static int _OnObjectListDeleteItemFnc(CWnd *pWnd){
	 return ((CFMDanhmuchopdong*)pWnd)->OnObjectListDeleteItem();
} 
static long _OnDeptListLoadDataFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*)pWnd)->OnDeptListLoadData();
} 
static void _OnDeptListDblClickFnc(CWnd *pWnd){
	((CFMDanhmuchopdong*)pWnd)->OnDeptListDblClick();
} 
static void _OnDeptListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CFMDanhmuchopdong*)pWnd)->OnDeptListSelectChange(nOldItem, nNewItem);
} 
static int _OnDeptListDeleteItemFnc(CWnd *pWnd){
	 return ((CFMDanhmuchopdong*)pWnd)->OnDeptListDeleteItem();
} 
static int _OnObjectListCheckAllFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*)pWnd)->OnObjectListCheckAll();
}
static int _OnObjectListUnCheckAllFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*)pWnd)->OnObjectListUnCheckAll();
}
static int _OnDeptListCheckAllFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*)pWnd)->OnDeptListCheckAll();
}
static int _OnDeptListUnCheckAllFnc(CWnd *pWnd){
	return ((CFMDanhmuchopdong*)pWnd)->OnDeptListUnCheckAll();
}
static void _OnThanhtoanhoadonSelectFnc(CWnd *pWnd){
	  ((CFMDanhmuchopdong*)pWnd)->OnThanhtoanhoadonSelect();
}
static void _OnThanhtoanhopdongSelectFnc(CWnd *pWnd)
{
	  ((CFMDanhmuchopdong*)pWnd)->OnThanhtoanhopdongSelect();
}
static void _OnTatcaSelectFnc(CWnd *pWnd){
	  ((CFMDanhmuchopdong*)pWnd)->OnTatcaSelect();
}
static int _OnListSearchItemFnc(CWnd *pWnd)
{
	 ((CFMDanhmuchopdong*)pWnd)->OnListSearchItem();
	 return 0;
} 
CFMDanhmuchopdong::CFMDanhmuchopdong(CWnd *pParent)
{
	m_nDlgWidth = 585;
	m_nDlgHeight = 585;
	SetDefaultValues();
}
CFMDanhmuchopdong::~CFMDanhmuchopdong()
{
}
void CFMDanhmuchopdong::OnCreateComponents()
{
	m_wndReportCondition.Create(this, _T("Report Condition"), 5, 5, 430, 483);
	...
	m_wndThanhtoanhoadon.Create(this, _T("Thanh toán hóa đơn"), 5, 489, 160, 514);
	m_wndThanhtoanhopdong.Create(this, _T("Thanh toán hợp đồng"), 5, 515, 160, 540);
	m_wndPrint.Create(this, _T("&Print"), 265, 489, 345, 514);
	m_wndPrint.EnableWindow(false);
	m_wndExport.Create(this, _T("&ExportXLS"), 350, 489, 430, 514);
	m_wndObjectList.Create(this,10, 89, 425, 250); 
	m_wndDeptList.Create(this,10, 254, 425, 479); 
	m_wndTatca.Create(this, _T("Tất cả"), 5, 542, 160, 567);
}
void CFMDanhmuchopdong::OnInitializeComponents(){
...
void CFMDanhmuchopdong::OnSetWindowEvents(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	...
	m_wndThanhtoanhoadon.SetEvent(WE_CLICK, _OnThanhtoanhoadonSelectFnc);
	m_wndThanhtoanhopdong.SetEvent(WE_CLICK, _OnThanhtoanhopdongSelectFnc);
	...
	m_wndDeptList.AddEvent(3, _T("Search"), _OnListSearchItemFnc);
...
void CFMDanhmuchopdong::OnDoDataExchange(CDataExchange* pDX){
	DDX_Text(pDX, m_wndYear.GetDlgCtrlID(), m_nYear);
	DDX_TextEx(pDX, m_wndReportPeriod.GetDlgCtrlID(), m_szReportPeriodKey);
	DDX_TextEx(pDX, m_wndFromDate.GetDlgCtrlID(), m_szFromDate);
	DDX_TextEx(pDX, m_wndToDate.GetDlgCtrlID(), m_szToDate);
	//DDX_Check(pDX, m_wndInsurance.GetDlgCtrlID(), m_bInsurance);
	DDX_Radio(pDX, m_wndThanhtoanhoadon.GetDlgCtrlID(), m_nThanhtoanhoadon);
	DDX_Radio(pDX, m_wndThanhtoanhopdong.GetDlgCtrlID(), m_nThanhtoanhopdong);
	DDX_Radio(pDX, m_wndTatca.GetDlgCtrlID(), m_nTatca);

}
void CFMDanhmuchopdong::SetDefaultValues(){
	...
	m_szCondition.Empty();
	m_bInsurance = FALSE;
	m_nThanhtoanhoadon=1;
	m_nThanhtoanhopdong=1;
	m_nTatca=0;

}

int CFMDanhmuchopdong::SetMode(int nMode){
...
/*void CFMDanhmuchopdong::OnYearChange(){
	
} */
/*void CFMDanhmuchopdong::OnYearSetfocus(){
	
} */
/*void CFMDanhmuchopdong::OnYearKillfocus(){
	
} */

int CFMDanhmuchopdong::OnYearCheckValue(){
	UpdateData(TRUE);
	if (m_nYear > 0)
	{
		CDateTime dt;
...
void CFMDanhmuchopdong::OnReportPeriodSelectChange(int nOldItemSel, int nNewItemSel){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
}
 
void CFMDanhmuchopdong::OnReportPeriodSelendok(){
	CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd();
	CString tmpStr;
	CDate dte, date;
...
/*void CFMDanhmuchopdong::OnReportPeriodSetfocus(){
	
}*/
/*void CFMDanhmuchopdong::OnReportPeriodKillfocus(){
	
}*/
long CFMDanhmuchopdong::OnReportPeriodLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
...
/*void CFMDanhmuchopdong::OnReportPeriodAddNew(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
} */
/*void CFMDanhmuchopdong::OnFromDateChange(){
	
} */
/*void CFMDanhmuchopdong::OnFromDateSetfocus(){
	
} */
/*void CFMDanhmuchopdong::OnFromDateKillfocus(){
	
} */
int CFMDanhmuchopdong::OnFromDateCheckValue(){
	return 0;
}
 
/*void CFMDanhmuchopdong::OnToDateChange(){
	
} */
/*void CFMDanhmuchopdong::OnToDateSetfocus(){
	
} */
/*void CFMDanhmuchopdong::OnToDateKillfocus(){
	
} */
int CFMDanhmuchopdong::OnToDateCheckValue(){
	return 0;
}
 
void CFMDanhmuchopdong::OnByDateSelect()
{
	m_szCondition = _T("hfe_date");
}
void CFMDanhmuchopdong::OnByDeptSelect()
{
	m_szCondition = _T("hfe_deptid");
}

void CFMDanhmuchopdong::OnInsuranceSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	if (!m_bInsurance)
...
void CFMDanhmuchopdong::OnPrintSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMDanhmuchopdong::OnPrintFormat1()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMDanhmuchopdong::OnPrintFormat2(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMDanhmuchopdong::OnExportSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);	
	...
	CExcel xls;
	if (!xls.Load(_T("Exports\\CONGNO\\DANHMUCHOPDONG.xls"))) AfxMessageBox(_T("Chưa có File trong thư mục Export\CONGNO"));
	//return;
	xls.SetWorksheet(0);
	...
	if (!szDepts.IsEmpty())
		{
		xls.SetCellText(0, 4, tmpStr, FMT_TEXT | FMT_CENTER, true, 10);
		}
	else
		xls.SetCellText(0, 4, _T("Tất cả các khoa"), FMT_TEXT | FMT_CENTER, true, 10);
	...
	while(!rs.IsEOF())
	{
		...
		rs.GetValue(_T("noidung"), tmpStr);
		xls.SetCellText(nCol+1, nRow, tmpStr, FMT_TEXT);

		rs.GetValue(_T("nhacungcap"), tmpStr);
		xls.SetCellText(nCol+2, nRow, tmpStr, FMT_TEXT);

		rs.GetValue(_T("masothue"), tmpStr);
		xls.SetCellText(nCol+3, nRow, tmpStr, FMT_TEXT);

		rs.GetValue(_T("hopdong"), tmpStr);
		xls.SetCellText(nCol+4, nRow, tmpStr, FMT_TEXT);

		rs.GetValue(_T("ngaykihopdong"), tmpStr);
		xls.SetCellText(nCol+5, nRow, tmpStr, FMT_TEXT);		

		rs.GetValue(_T("giatrihopdong"), nTemp);
		nGroupTotal[6] += nTemp;
		xls.SetCellText(nCol+6, nRow, double2str(nTemp), FMT_NUMBER2);

		rs.GetValue(_T("quyetdinhthau"), tmpStr);
		xls.SetCellText(nCol+7, nRow, tmpStr, FMT_TEXT);

		rs.GetValue(_T("goithau"), tmpStr);
		xls.SetCellText(nCol+8, nRow, tmpStr, FMT_TEXT);

		rs.GetValue(_T("tyle_ung"), tmpStr);
		xls.SetCellText(nCol+9, nRow, tmpStr, FMT_INTEGER);	
		
		rs.GetValue(_T("total_giatrida_tamung"), nTemp);
		nGroupTotal[10] += nTemp;
		xls.SetCellText(nCol+10, nRow, double2str(nTemp), FMT_NUMBER2);

		rs.GetValue(_T("total_giatrida_thuchien"), nTemp);
		nGroupTotal[11] += nTemp;
		xls.SetCellText(nCol+11, nRow, double2str(nTemp), FMT_NUMBER2);

		rs.GetValue(_T("total_giatrida_thanhtoan"), nTemp);
		nGroupTotal[12] += nTemp;
		xls.SetCellText(nCol+12, nRow, double2str(nTemp), FMT_NUMBER2);

		rs.GetValue(_T("total_connophaitra"), nTemp);
		nGroupTotal[13] += nTemp;
		xls.SetCellText(nCol+13, nRow, double2str(nTemp), FMT_NUMBER2);

		rs.GetValue(_T("total_dutamung"), nTemp);
		nGroupTotal[14] += nTemp;
		xls.SetCellText(nCol+14, nRow, double2str(nTemp), FMT_NUMBER2);	
	...
	EndWaitCursor();
	xls.Save(_T("Exports\\CONGNO\\DANHMUCHOPDONG2.xls"));
	
}
void CFMDanhmuchopdong::OnExportFormat1(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
void CFMDanhmuchopdong::OnExportFormat2(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	CString tmpStr, szTemp;
...
void CFMDanhmuchopdong::OnObjectListDblClick(){
	
}
 
void CFMDanhmuchopdong::OnObjectListSelectChange(int nOldItem, int nNewItem){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
}
 
int CFMDanhmuchopdong::OnObjectListDeleteItem(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	 return 0;
}
 
long CFMDanhmuchopdong::OnObjectListLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	CString szWhere;
...
void CFMDanhmuchopdong::OnDeptListDblClick(){
	
}
 
void CFMDanhmuchopdong::OnDeptListSelectChange(int nOldItem, int nNewItem){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
}
 
int CFMDanhmuchopdong::OnDeptListDeleteItem(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	 return 0;
}
 
long CFMDanhmuchopdong::OnDeptListLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
CString CFMDanhmuchopdong::GetQueryString()
{
	...
	if (!szDepts.IsEmpty())
	{
		szWhere.AppendFormat(_T(" and c_o.DEPARTMENT_ID in(%s) "), szDepts);
	}

	
	if (m_nThanhtoanhoadon == 0)
	{
		szWhere.AppendFormat(_T(" and c_o.ordertype = 'HD' "));
	}

	if (m_nThanhtoanhopdong == 0)
	{
		szWhere.AppendFormat(_T(" and c_o.ordertype = 'TU' "));
	}

	if (m_nTatca == 0)
	{
		szWhere.AppendFormat(_T(" and 0=0 "));
	}
		...
		_T(" goithau,") \
		_T(" tyle_ung,") \
		_T(" coalesce(giatrida_tamung,0) as total_giatrida_tamung,") \
		_T(" coalesce(giatrida_thuchien,0) as total_giatrida_thuchien,") \
		_T(" coalesce(giatrida_thanhtoan,0) as total_giatrida_thanhtoan,") \
		_T(" coalesce(giatrihopdong-giatrida_thanhtoan,0) as total_connophaitra,") \
		_T(" coalesce(giatrihopdong-giatrida_tamung,0) as total_dutamung") \
		_T(" FROM") \
		_T(" (") \
		_T(" SELECT adc_description as noidung,") \
		_T(" (select adp_name from AD_PARTNER where adp_partner_id=c_ol.partner_id) as nhacungcap,") \
		_T(" (select adp_taxcode from AD_PARTNER where adp_partner_id=c_ol.partner_id) as masothue,") \
		_T(" adc_contract_no as hopdong,") \
		_T(" TO_CHAR(adc_signeddate, 'DD/MM/YYYY') as ngaykihopdong,") \
		_T(" adc_amount as giatrihopdong,") \
		_T(" ADCP_QUYETDINHPHEDUYET as quyetdinhthau,") \
		_T(" ADCP_DESCRIPTION as goithau,") \
		_T(" adc_tyleung as tyle_ung,") \
		_T(" (select sum(amount) from C_ORDERLINE where contract_id=adc_contract_id AND ordertype='TU') as giatrida_tamung,") \
		_T(" (select sum(amount) from C_ORDERLINE where contract_id=adc_contract_id AND ordertype='HD') as giatrida_thuchien,") \
		_T(" (select sum(amount) from C_INVOICE where contract_id=adc_contract_id AND posted='Y') as giatrida_thanhtoan") \
		_T(" FROM") \
		_T(" C_ORDERLINE c_ol") \
		_T(" LEFT JOIN C_ORDER c_o ON (c_o.c_order_id=c_ol.ORDER_ID)") \
		_T(" LEFT JOIN C_INVOICE c_iv ON (c_o.c_order_id=c_iv.order_id) ") \
		_T(" LEFT JOIN ad_contract ON (c_ol.contract_id=adc_contract_id)") \
		_T(" LEFT JOIN ad_contract_package ON (adcp_contract_package_uid=adc_contract_package_id)") \
		_T(" WHERE 1=1 %s AND adc_signeddate BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') and TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') ") \
		_T(" )") \
		_T(" GROUP BY noidung,") \
		_T(" nhacungcap,") \
		_T(" masothue,") \
		...
		_T(" tyle_ung,") \
		_T(" giatrida_tamung,") \
		_T(" giatrida_thuchien,") \
		_T(" giatrida_thanhtoan") \
		_T(" order by ngaykihopdong"), szWhere, m_szFromDate, m_szToDate);

	//_msg(_T("%s"),szSQL);
	return szSQL;
}

CString CFMDanhmuchopdong::GetQueryString1()
{
...
int CFMDanhmuchopdong::OnDeptListCheckAll()
{
	for (int i = 0; i < m_wndDeptList.GetItemCount(); i++)
	{
		if (!m_wndDeptList.GetCheck(i))
...
int CFMDanhmuchopdong::OnDeptListUnCheckAll()
{
	for (int i = 0; i < m_wndDeptList.GetItemCount(); i++)
	{
		if (m_wndDeptList.GetCheck(i))
...
int CFMDanhmuchopdong::OnObjectListCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (!m_wndObjectList.GetCheck(i))
...
int CFMDanhmuchopdong::OnObjectListUnCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (m_wndObjectList.GetCheck(i))
...
void CFMDanhmuchopdong::OnThanhtoanhoadonSelect(){
	
}
void CFMDanhmuchopdong::OnThanhtoanhopdongSelect(){
	
}
void CFMDanhmuchopdong::OnTatcaSelect(){
	
}
int CFMDanhmuchopdong::OnListSearchItem()
{
