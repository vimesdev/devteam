...
{
	((CFMServiceRefundByDept*)pWnd)->OnListCheckAllDVAB();
	return 0;
}
static int _OnListUnCheckAllDVABFnc(CWnd *pWnd)
{
	((CFMServiceRefundByDept*)pWnd)->OnListUnCheckAllDVAB();
	return 0;
}

static int _OnListUnCheckAllDVCLCFnc(CWnd *pWnd)
{
	((CFMServiceRefundByDept*)pWnd)->OnListUnCheckAllDVCLC();
	return 0;
}
...
void CFMServiceRefundByDept::OnSetWindowEvents(){
	...
	m_wndList.AddEvent(3, _T("Chọn phiếu thu/chi AB"), _OnListCheckAllDVABFnc);
	m_wndList.AddEvent(4, _T("Bỏ Chọn phiếu thu/chi AB"), _OnListUnCheckAllDVABFnc);
	
	m_wndList.AddEvent(5, _T("Bỏ Chọn phiếu thu/chi CLC"), _OnListUnCheckAllDVCLCFnc);

	m_wndList.AddEvent(15, _T("Search"), _OnListSearchItemFnc);
...
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CString szSQL, szSQL1, szWhere, szWhere1, szReceiptStr;
	CString szSQL2,szSQL3,szSQL4,szSQL5,szSQL6,szSQL7,szSQL8,szSQL9,szSQL10,szSQL11,szSQL12,szSQL13;
	CString szSQL14,szSQL15,szSQL16,szSQL17,szSQL18,szSQL19,szSQL20,szSQL21,szSQL22,szSQL23,szSQL24,szSQL25,szSQL26,szSQL27,szSQL28,szSQL29,szSQL30;
	CRecord rs(&pMF->m_db);
	int nCount = 0;
	szReceiptStr = _T("-1");
	UpdateData(true);
	...
	{
	//1. Bắt đầu sửa ở đây ngày 14/09/2018
	szSQL1.Format(_T(" SELECT f.hfe_docno,f.hfe_type,r.hfe_class,r.hfe_invoiceno,") \
	...
	_T("       AND r.hfe_type      ='V'") \
	_T("       AND r.Hfe_Serialno  ='PC'"),szWhere); \
	...
	//4. Tra lai tien goi dich vu//
	_T("       SELECT f.hfe_docno,") \
	_T("         f.hfe_type,") \
	_T("         r.hfe_class,") \
	_T("         r.hfe_invoiceno,") \
	_T("         f.HFE_DEPTID khoa_thuchien,") \
	_T("         f.hfe_itemid,") \
	_T("         rl.hfe_patpaid tong_tien,") \
	_T("         f.hfe_category") \
	_T("       FROM hms_fee f") \
	_T("       LEFT JOIN hms_fee_list fl ON (hfe_itemid  =fl.hfl_feeid AND f.hfe_type IN ('X'))") \
	_T("       LEFT JOIN Hms_Fee_Refundline rl ON (f.hfe_docno =rl.hfe_docno AND F.Hfe_Fee_Id=Rl.Hfe_Fee_Refundline_Id)") \
	_T("       LEFT JOIN Hms_Fee_Refund r  ON (r.hfe_invoiceno  = rl.hfe_invoiceno AND r.hfe_docno      = rl.hfe_docno)") \
	_T("       WHERE 1             =1 %s") \
	_T("       AND f.hfe_status    IN ('P','R')") \
	_T("       AND f.hfe_type      IN ('X')") \
	//_T("       AND F.Hfe_Group     IN ('F0000') AND f.hfe_itemid in (select ss_code from sys_sel where ss_id='HMS_SERVICE_PACKAGE' )") \//
	_T("       AND F.Hfe_Group     IN ('F0000')") \
	_T("       AND f.hfe_category  IN ('N','BQP','XX')") \
	_T("       AND r.hfe_desc      <> 'Ho\xE0n tr\x1EA3 \x62\x1EC7nh nh\xE2n th\x61nh to\xE1n r\x61 vi\x1EC7n'"),szWhere); \

	//Doan nay tinh tien	
	szSQL16.Format(_T(" SELECT f.hfe_docno, f.hfe_type,") \
	_T("         r.hfe_invoiceno,") \
	_T("         CASE") \
	...
	_T("     ORDER BY khoa_thuchien") \
	_T("     ))") \
	_T("   ) tbl_cost ON ( tbl_qs.khoa_thuchien = tbl_cost.khoa_thuchien) WHERE tong_tien>0"),szSQL1, szSQL2,szSQL3,szSQL4,szSQL5,szSQL6,szSQL7,szSQL8,szSQL9,szSQL10,szSQL11,szSQL12,szSQL13,szSQL14,szSQL15, szWhere1,szSQL16,szSQL17,szSQL18,szSQL19,szSQL20,szSQL21,szSQL22,szSQL23,szSQL24,szSQL25,szSQL26,szSQL27,szSQL28,szSQL29,szSQL30, szWhere1); \
	}
	return szSQL;
}
int CFMServiceRefundByDept::OnListCheckAllDVAB()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd();
	CString szItemText;	
	for (int i=0; i<m_wndList.GetItemCount(); i++)	
...
int CFMServiceRefundByDept::OnListUnCheckAllDVAB()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd();
	CString szItemText;	
	for (int i=0; i<m_wndList.GetItemCount(); i++)	
...
int CFMServiceRefundByDept::OnListUnCheckAllDVCLC()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd();
	CString szItemText;	
	for (int i=0; i<m_wndList.GetItemCount(); i++)	
