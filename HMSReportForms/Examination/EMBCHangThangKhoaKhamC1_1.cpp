...
void CRptBCHangThangKhoaKhamC1_1::OnSetWindowEvents(){
	...
	m_szToDate += _T("23:59");
	SetMode(VM_EDIT);
	m_bZoneOrder = pMF->GetCurrentDepartmentID()==_T("TYC")?true:false;
}
...
void CRptBCHangThangKhoaKhamC1_1::OnExportSelect(){
	...
	tmpStr.Format(_T("BÁO CÁO TỔNG HỢP THÁNG PHÒNG KHÁM (%s)"), pMF->GetCurrentDepartmentID());
	...
	xls.SetCellText(nCol + 4, nRow + 5, _T("BH Quân"), FMT_TEXT | FMT_CENTER, true, 11);
	...
	xls.SetCellText(nCol + 11, nRow + 5, _T("BH Quân"), FMT_TEXT | FMT_CENTER, true, 11);
	xls.SetCellText(nCol + 12, nRow + 5, _T("BH Quân nhân"), FMT_TEXT | FMT_CENTER, true, 11);
	xls.SetCellText(nCol + 13, nRow + 5, _T("BHYT nợ thẻ"), FMT_TEXT | FMT_CENTER, true, 11);
	xls.SetCellText(nCol + 14, nRow + 5, _T("Tổng cộng"), FMT_TEXT | FMT_CENTER, true, 11);
	xls.SetCellText(nCol + 15, nRow + 5, _T("Khu"), FMT_TEXT | FMT_CENTER, true, 11);
	if (szDept != _T("C1.1"))
	{
...
CString CRptBCHangThangKhoaKhamC1_1::GetQueryString(){
	...
	if (m_bPatient){
		...
		szSubSQL.AppendFormat(_T(" SELECT 1 as title, hd_docno,") \
					...
					_T(" AND he_deptid = '%s' "), pMF->GetCurrentDepartmentID(), m_szFromDate, m_szToDate, pMF->GetCurrentDepartmentID());
	...
	if (m_bDrugOrder){
		...
		szSubSQL.AppendFormat(_T(" SELECT 3 as title, hd_docno, hpo_roomid as he_roomid, hpo_orderdate dte, null as vaokhoa, ") \
						...
						_T(" 		AND hpo_orderstatus IN ('A', 'S') AND hpo_ordertype <> 'C' AND hpo_deptid = '%s'") \
	...
	szSQL.Format(_T(" SELECT title, hrl_type as extype, hrl_id as roomid, hrl_name as roomname, hrl_zone, vaokhoa,") \
					...
					, szSubSQL, pMF->GetCurrentDepartmentID(), szWhere);
	if (m_bZoneOrder)
	{
