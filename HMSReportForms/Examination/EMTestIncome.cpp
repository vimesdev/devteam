...
CString CEMTestIncome::GetQueryString(){
	...
	szWhere.Format(_T(" AND hpc_status IN ('P', 'S', 'T')  AND hfe_class = 'E' ") \
				   _T(" AND hpc_orderdate BETWEEN TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS')"), m_szFromDate, m_szToDate);
	...
	szSQL.Format(_T(" SELECT hrl_name as nme,") \
				...
				_T("           hd_object") \
				_T(" ORDER  BY %s,") \
				_T("           Cast(hd_object AS INTEGER) "), szWhere, szWhere, pMF->GetCurrentDepartmentID(), szOrder);
_fmsg(_T("%s"), szSQL);
	return szSQL;
}
