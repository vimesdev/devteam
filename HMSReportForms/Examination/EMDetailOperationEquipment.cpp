#include "stdafx.h"
#include "EMDetailOperationEquipment.h"
...
void CrptDetailOperationEquipment::OnCreateComponents()
{
	...
	m_wndOutPatient.Create(this, _T("OutPatient"), 260, 120, 340, 145);
	m_wndInPatient.Create(this, _T("InPatient"), 345, 120, 425, 145);
}
...
void CrptDetailOperationEquipment::OnPrintSelect()
{
	...
	szSQL.Format(_T(" SELECT itemid, operatype, TRIM(operaname || ' - Số hồ sơ: ' || docno) AS operaname, sum(totalnumber) as totalnumber, price, sum(cost) as amount") \
		_T(" FROM ") \
		_T(" (") \
		_T("  SELECT ho_idx as orderid,") \
		_T(" 	ho_itemid as itemid,") \
		_T(" 	hfe_desc as operaname, ") \
		...
		_T("  LEFT JOIN hms_fee ON (ho_docno=hfe_docno AND ho_itemid=hfe_itemid) ") \
		_T("  WHERE hfe_type='O' %s ") \
		_T("  AND ho_orderdate BETWEEN to_timestamp('%s','yyyy/mm/dd hh24:mi:ss') AND to_timestamp('%s','yyyy/mm/dd hh24:mi:ss') AND ho_status <>'O' %s ") \
		_T(" )tblmain") \
		_T(" GROUP BY itemid, operatype, operaname, price, docno") \
	...
	szDate.Format(tmpStr, CDateTime::Convert(m_szFromDate, yyyymmdd | hhmmss, ddmmyyyy | hhmmss),CDateTime::Convert(m_szToDate, yyyymmdd | hhmmss, ddmmyyyy | hhmmss));
	...
	{	
		...
		szSQL.Format(_T(" SELECT mp_producttype AS type,") \
			_T("   product_typeid AS typename,") \
			_T("   id,") \
			_T("   name,") \
			_T("   unit,") \
			_T("   price,") \
			...
			_T("   AND ho_patientno=hd_patientno)") \
			_T("   LEFT JOIN hms_pharmaorder") \
			_T("   ON(hpo_reference_id=ho_idx") \
			_T("   AND ho_docno       =hpo_docno)") \
			...
			_T("   AND ho_status NOT IN('O')") \
			_T("   AND ho_itemid IN('%s')") \
			...
			_T("   name"), m_szFromDate, m_szToDate, szItemID, m_szFromDate, m_szToDate, szItemID);
		rsl.ExecSQL(szSQL);

		rsl.ExecSQL(szSQL);		
		TCHAR *lszChar[] = {_T("I"),_T("II"),_T("III"),_T("IV"),_T("V"),_T("VI"),_T("VII"),_T("VIII"),_T("IX"),_T("X")};
		...
		while(!rsl.IsEOF())
		{
			...
			if(!szNewLine.IsEmpty() && szNewLine != szOldLine )
			{
				if (nAmount > 0)
				{
					...
					tmpStr.Format(_T("Cộng(%s):"), lszChar[nChar]);
				...
				if (nChar > 1)
				{
					...
					tmpStr.Format(_T("Cộng(%s):"), lszChar[2]);
					rptDetail->SetValue(_T("t2"), tmpStr);
...
void CrptDetailOperationEquipment::OnExportSelect()
{
	...
	xls.SetCellText(0, 5, _T("TỔNG HỢP CHI TIẾT THUỐC - VTTH LÀM PT - TT"), FMT_TEXT, true, 12);
	TranslateString(_T("From Date"), tmpStr);
	szFromDateToDate.Format(_T("%s: %s"), tmpStr, CDateTime::Convert(m_szFromDate, yyyymmdd | hhmmss, ddmmyyyy | hhmmss));
	TranslateString(_T("To Date"),tmpStr);
	szFromDateToDate.AppendFormat(_T(" %s: %s"), tmpStr, CDateTime::Convert(m_szToDate, yyyymmdd | hhmmss, ddmmyyyy | hhmmss));	
	xls.SetCellText(0, 6, szFromDateToDate, FMT_TEXT, true, 12);	
	
	int nRow = 8;
	
	xls.SetCellText(0, nRow, _T("STT"), FMT_TEXT | FMT_CENTER, true);	
	xls.SetCellText(1, nRow, _T("Tên PT - TT"), FMT_TEXT | FMT_CENTER, true);
	xls.SetCellText(2, nRow, _T("Nhóm PT - TT"), FMT_TEXT | FMT_CENTER, true);
	xls.SetCellText(3, nRow, _T("Số lượng"), FMT_TEXT | FMT_CENTER, true);
	xls.SetCellText(4, nRow, _T("Đơn giá"), FMT_TEXT | FMT_CENTER, true);
	xls.SetCellText(5, nRow, _T("Tổng tiền"), FMT_TEXT | FMT_CENTER, true);
	...
	while(!rs.IsEOF())
	{
		...
		rs.GetValue(_T("totalnumber"), tmpStr);		
		xls.SetCellText(3, nRow, tmpStr, FMT_INTEGER, true);
		...
		{	
			...
			if (!szNewLine.IsEmpty() && szOldLine != szNewLine)
			{
				if(nAmount > 0)
				{
					...
					tmpStr.Format(_T("Cộng(%s):"), lszChar[nGroup]);
					xls.SetCellText(1, nRow, tmpStr, FMT_TEXT, true);
