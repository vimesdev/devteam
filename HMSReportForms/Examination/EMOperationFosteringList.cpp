...
CString CEMOperationFosteringList::GetQueryString(){
	...
	szSQL.Format(_T(" SELECT ho_docno doc_no,") \
				...
				_T("   case when ho_recordno IN  ('YAG.X', 'YA') then 'X' else '' end AS Type1,") \
				_T("   case when ho_recordno NOT IN  ('YAG.X', 'YA') then 'X' else '' end AS Type3,") \
				_T("   case when ho_recordno IN  ('YAG.X', 'YA') then 'Lo\x1EA1i 1' else 'Lo\x1EA1i 3' end AS itemtype,") \
				//_T("   case when ho_recordno IN  ('YAG.X', 'YA') then '85500' else '24000' end AS amount,") \//
				_T("   case when ho_recordno IN  ('YAG.X', 'YA') then '51300' else '14400' end AS amount,") \
				_T("   Get_username(ho_practitioner) AS thefirst,") \
				_T("   Get_username(ho_anesthetist)  AS thesecond,") \
				_T("   CASE WHEN ho_recordno IN ('YAG.X', 'YA') THEN Get_username(ho_assistant) ELSE NULL END AS assist") \
