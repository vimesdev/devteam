...
static long _OnGroupLoadDataFnc(CWnd *pWnd){
	return ((CEMExaminationListReport *)pWnd)->OnGroupLoadData();
}
static long _OnDoctorLoadDataFnc(CWnd *pWnd){
	return ((CEMExaminationListReport *)pWnd)->OnDoctorLoadData();
}
...
long CEMExaminationListReport::OnGroupLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
long CEMExaminationListReport::OnDoctorLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
void CEMExaminationListReport::OnExport1(){
	_debug(_T("%s"), CString(typeid(this).name()));
	CHMSMainFrame  *pMF = (CHMSMainFrame *) AfxGetMainWnd();
	UpdateData(true);
	CExcel xls;
	...
	xls.SetColumnWidth(13, 40);
	xls.SetColumnWidth(14, 20);
	xls.SetColumnWidth(15, 20);
	xls.SetColumnWidth(16, 40);
	//Header
	...
	while (!rs.IsEOF())
	{
		...
		//_msg(_T("%s"), tmpStr);
		{
		xls.SetCellText(nCol + 15, nRow,_T(" ") , FMT_TEXT);
		}
		else		
	...
	xls.Save(_T("Exports\\Danh Sach Benh Nhan Kham Benh3.xls"));


} 
void CEMExaminationListReport::OnExport2(){
	_debug(_T("%s"), CString(typeid(this).name()));
	CHMSMainFrame  *pMF = (CHMSMainFrame *) AfxGetMainWnd();
	UpdateData(true);
	CExcel xls;
	...
	xls.Save(_T("Exports\\Danh Sach Benh Nhan Kham Benh2.xls"));
...
CString CEMExaminationListReport::GetQueryString()
{
	 ...
	 for (int i=0 ; i<= m_wndRoom.GetCount(); i++)
	 {
		 if(m_wndRoom.GetCheck(i))
		 {
			 ...
			 if(!szRoomID.IsEmpty())
					...
					szRoomName += _T(", ");						
			szRoomName.AppendFormat(_T("%s"), m_wndRoom.GetCheck(i, 0));
		 }
	 }
	 if (!m_szRoomKey.IsEmpty())
	...
	szSQL.Format(_T("  SELECT  distinct hd_docno as docno,    ") \
				...
				_T("           hms_getaddress(nvl(hd_provid, hp_provid), nvl(hd_distid, hp_distid), nvl(hd_villid, hp_villid)) AS noicap, HD_RELDISEASE as benhkem ") \
				_T("   FROM hms_patient ") \
				_T("   LEFT JOIN hms_doc ON (hd_patientno = hp_patientno) ") \
...
CString CEMExaminationListReport::GetQueryString2()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
	CString szSQL, szWhere, szStatus;
	...
	szSQL.Format(_T("  SELECT  distinct hp_patientno as docno,    ") \
				...
				_T("           extract(year from hp_birthdate) as yob, get_objectname(hd_object) as obj,hd_object,    ") \
				_T("           get_syssel_desc('hms_rank', hp_rank) as rank, ") \
				_T("           hp_workplace as workplace,hd_diagnostic as diagno, ") \
				_T("           TRIM(hd_telephone || ' ' || hd_contacttel) AS telephone, HD_RELDISEASE as benhkem ") \
				_T("   FROM hms_patient ") \
				_T("   LEFT JOIN hms_doc ON (hd_patientno = hp_patientno) ") \
				_T("   LEFT JOIN hms_exam ON (hd_docno = he_docno)") \
				_T("  %s AND he_deptid = '%s' AND he_status IN ('T', 'P')") \
