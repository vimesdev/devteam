...
static void _OnContractSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CPMSSupplierPaymentList* )pWnd)->OnContractSelectChange(nOldItemSel, nNewItemSel);
} 
...
void CPMSSupplierPaymentList::OnCreateComponents(){
	m_wndGeneralDepartmentUsage.Create(this, _T("Import Sheet List"), 5, 5, 575, 210);
	...
	m_wndContractLabel.Create(this, _T("Contract"), 10, 180, 100, 205);
...
void CPMSSupplierPaymentList::OnContractSelectChange(int nOldItemSel, int nNewItemSel){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
