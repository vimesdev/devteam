...
void CHMSTreatmentTrackingDiseaseDialog::OnCreateComponents(){
	...
	CArray<CWnd*, CWnd*> arrHide;
	arrHide.Add(&m_wndksdptoksdt);
	arrHide.Add(&m_wndLyDo);
	arrHide.Add(&m_wndLyDoChuyen);
	arrHide.Add(&m_wndDietLabel);
	arrHide.Add(&m_wndDiet);
	arrHide.Add(&m_wndNurseAssistanceLabel);
	arrHide.Add(&m_wndNurseAssistance);
	arrHide.Add(&m_wndTemperatureSelectLabel);
	arrHide.Add(&m_wndTemperatureSelect);
	arrHide.Add(&m_wndInputPulseTemperature);
	arrHide.Add(&m_wndPulseLabel);
	arrHide.Add(&m_wndPulse);
	arrHide.Add(&m_wndTemperatureLabel);
	arrHide.Add(&m_wndTemperature);
	arrHide.Add(&m_wndBloodPressureLabel);
	arrHide.Add(&m_wndBloodPressure);
	arrHide.Add(&m_wndLabelSpec);
	arrHide.Add(&m_wndBloodPressureHight);
	arrHide.Add(&m_wndBreathingRateLabel);
	arrHide.Add(&m_wndBreathingRate);
	CWnd* pWnd = NULL;
	for (int i = 0; i < arrHide.GetCount(); i++)
	{
		pWnd = arrHide.GetAt(i);
		pWnd->ShowWindow(SW_HIDE);
	}
	m_wndSelectFromOrder.Create(this, _T(""), 0, 0, 0, 0);
	m_wndSelectFromOrder.ShowWindow(SW_HIDE);
	m_wndSelectFromOrder.EnableWindow(FALSE);
}
...
//	m_wndBloodPressure.SetMask(_T("###/###"));
	...
	m_hms_siexamTbl.AddField(_T("hsie_refidx"), dfLong);	
...
//	else
	...
	else if (pMF->m_UserInfo.su_groupid == _T("A") && pMF->CheckPermission(_T("admin.TM.100.1")))
	{
		fDate -= 10000;
		tDate += 10000;
	}
