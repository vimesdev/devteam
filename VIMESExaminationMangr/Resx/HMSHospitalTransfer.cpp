...
static void _OnDTthuDungSelectFnc(CWnd *pWnd){
	 ((CHMSHospitalTransfer*)pWnd)->OnDTthuDungSelect();
}
...
void CHMSHospitalTransfer::OnCreateComponents(){
	...
	m_wndDTthuDung.Create(this, _T("DT thu dung"), 10, 460, 273, 485);
...
void CHMSHospitalTransfer::OnSetWindowEvents(){
	...
	m_wndDTthuDung.SetEvent(WE_CLICK, _OnDTthuDungSelectFnc);
...
void CHMSHospitalTransfer::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Check(pDX, m_wndDTthuDung.GetDlgCtrlID(), m_bDTthuDung);
...
void CHMSHospitalTransfer::OnPrintSelect(){
	...
	void CHMSHospitalTransfer::OnDTthuDungSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
