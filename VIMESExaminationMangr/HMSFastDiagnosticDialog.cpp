...
void CHMSFastDiagnosticDialog::OnInitializeComponents(){
	...
	m_hms_otherdiagnosisTbl.AddField(_T("hod_conclusion"), dfText, 1024);	//Ket luan
...
long CHMSFastDiagnosticDialog::OnDiagnosticICDLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return pMF->LoadICD(&m_wndDiagnosticICD, m_szDiagnosticICDKey);
...
long CHMSFastDiagnosticDialog::OnDiagnosticICD2LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return pMF->LoadICD(&m_wndDiagnosticICD2, m_szDiagnosticICD2Key);
...
long CHMSFastDiagnosticDialog::OnDiagnosticICD3LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return pMF->LoadICD(&m_wndDiagnosticICD3, m_szDiagnosticICD3Key);
...
long CHMSFastDiagnosticDialog::OnDiagnosticICD4LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return pMF->LoadICD(&m_wndDiagnosticICD4, m_szDiagnosticICD4Key);
...
long CHMSFastDiagnosticDialog::OnDiagnosticICD5LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return pMF->LoadICD(&m_wndDiagnosticICD5, m_szDiagnosticICD5Key);
...
int CHMSFastDiagnosticDialog::OnSaveHMSFastDiagnosticDialog(){
 	...
 	if(GetMode() == VM_ADD){
 		...
 		szSQL = m_hms_otherdiagnosisTbl.GetUpdateSQL(_T("hod_patientno,hod_docno"));
 		szSQL += szWhere;
 	}
 _fmsg(_T("%s"), szSQL);
 	int ret = pMF->ExecSQL(szSQL);
...
void CHMSFastDiagnosticDialog::OnRecalConclusion(){
	...
	if(!m_szDiagnosticICDKey.IsEmpty())
		tmpStr.AppendFormat(_T("- %s\r\n"), m_wndDiagnosticICD.GetCurrent(1));
	
	if(!m_szDiagnosticICD2Key.IsEmpty())
		tmpStr.AppendFormat(_T("- %s\r\n"), m_wndDiagnosticICD2.GetCurrent(1));

	if(!m_szDiagnosticICD3Key.IsEmpty())
		tmpStr.AppendFormat(_T("- %s\r\n"), m_wndDiagnosticICD3.GetCurrent(1));

	if(!m_szDiagnosticICD4Key.IsEmpty())
		tmpStr.AppendFormat(_T("- %s\r\n"), m_wndDiagnosticICD4.GetCurrent(1));

	if(!m_szDiagnosticICD5Key.IsEmpty())
		tmpStr.AppendFormat(_T("- %s\r\n"), m_wndDiagnosticICD5.GetCurrent(1));
