...
void CWMDateCtrl::DrawWeekCellHeaders(CPaintDC *pDC)
{
	...
	CString strDay = dtDate.Format(_T("%A, %m/%d/%Y"));

	// Find Monday
	while (dtDate.GetDayOfWeek() != 2)
	{
		dtDate -= 1;
		strDay = dtDate.Format(_T("%A, %m/%d/%Y"));
	}
	...
	for (int x = 1; x <= 7; x++)
	{
		...
		if (pCellData->GetCellDate() == dtCurrentDate)
		{
			...
			CRect rText;
			rText.CopyRect(&rHdr);
			TranslateString(pCellData->GetCellDate().Format(_T("%A")), tmpStr);
			TranslateString(pCellData->GetCellDate().Format(_T("%B")), szMonth);
			szDay = pCellData->GetCellDate().Format(_T("%d"));
			strBuf.Format(_T("%s, %s - %s"), tmpStr, szDay, szMonth);
...
#ifdef DEBUG
	strBuf.Format(L"OnLButtonDown - Clicked point: %d, %d\n", point.x, point.y);
	TRACE(strBuf);
#endif
...
#ifdef DEBUG
	strBuf.Format(L"OnLButtonDblClk - Clicked point: %d, %d\n", point.x, point.y);
	TRACE(strBuf);
#endif
...
#ifdef DEBUG
	strBuf.Format(L"OnRButtonDown - Clicked point: %d, %d\n", point.x, point.y);
	TRACE(strBuf);
#endif
	// Clear Highlight from any of the CellDataItems
	while (pos != NULL)
...
#ifdef DEBUG
	strBuf.Format(L"OnRButtonUp - Clicked point: %d, %d\n", point.x, point.y);
	TRACE(strBuf);
#endif
...
long CWMDateCtrl::InsertItem(COleDateTime dtStart, COleDateTime dtEnd,
							 CString strLine /*=""*/, int nImage/*=-1*/, long nItem/*=0*/)
{
	long nItemID = -1;
...
void CWMDateCtrl::DrawMonthHeader(CPaintDC *pDC)
{
	...
	for (int x = 1; x <= 7; x++)
	{
		...
		TranslateString(dtDate.Format(_T("%A")), tmpStr);
		pDC->DrawText(tmpStr, &rHdrText, 
		...
		else
		{
			...
			CString tmpStr2;
			TranslateString(dtDate.Format(_T("%a")), tmpStr);
//			strText = dtDate.Format(_T("%a + /"));
			dtDate += 1;
			TranslateString(dtDate.Format(_T("%a")), tmpStr2);
		//	strText += dtDate.Format(_T("%a"));
			strText.Format(_T("%s + / %s"), tmpStr, tmpStr2);
...
void CWMDateCtrl::DrawMonthCellHeaders(CPaintDC *pDC)
{
	...
	for (nItem = 1; nItem <= WV_TOTALCELLS; nItem++)
	{
		...
		if (pCell->GetCellDate() == dtCurrentDate)
		{
			...
			{
				TranslateString(pCell->GetCellDate().Format(_T("%B")), tmpStr);
				strBuf.Format(_T("%s, %s"), tmpStr, pCell->GetCellDate().Format(_T("%d")));
			}
			else
				strBuf = pCell->GetCellDate().Format(_T("%d"));
		...
		{	
			...
			TranslateString(pCell->GetCellDate().Format(_T("%B")), tmpStr);
			strText.Format(_T("%s %d"), tmpStr, pCell->GetCellDate().GetDay());
		}
		else
		{
