	...
	CGuiDialog(pParent)
{
	...
	m_bPrinting = false;
	m_bGeneralExamHealth = true;
	m_szFileName = L"KB_KSKCB";
	m_szType = L"GENERAL_EXAM_DOC";
}
...
void CHMSGeneralRankExamine::OnCreateComponents(){
	...
	m_wndPrehistory.Create(this, _T(""), 0, 0, 0, 0);
	m_wndPrehistory.ShowWindow(SW_HIDE);
	m_wndPrehistory.EnableWindow(FALSE);
}
...
void CHMSGeneralRankExamine::OnSetWindowEvents(){
	...
	if (!m_bGeneralExamHealth)
	{
		m_szFileName = L"KSK_MS3_KSKDK";
	}
	m_szFilePath.Format(_T("Reports\\HMS\\%s.HTML"), m_szFileName);
	m_wndLayout.Load(m_szFilePath);
	GetDataToScreen();
...
_tprintf(_T("\r\nxxx"));
	...
	else if (m_szBoNhiem == _T("N"))
	{
		...
		szLTitle.Format(_T("Quy trình đi học(áp dụng với cán bộ cấp thượng đại tá)"));
...
_T(" hd_icd as icd10, hd_conclusion as conclusion, HP_WORKPLACE as noilamviec, hd_telephone as mobile ") \
_T(" FROM hms_patient") \
...
_T(" WHERE hd_docno  = %ld"), pMF->m_nDocumentNo);
	...
	if(!rs.IsEOF())
	{
		...
		m_wndLayout.SetValue(_T("NOI_LAM_VIEC"), tmpStr);
		
		rs.GetValue(_T("mobile"), tmpStr);
		m_wndLayout.SetValue(_T("MOBILE"), tmpStr);
...
_T(" AND he_receptidx=%ld "), pMF->m_nDocumentNo, pMF->m_nRefIndex);
	...
	szSQL.Format(_T("select * ") \
		_T(" from %s") \
		_T(" where sohoso = %ld"), _T("hms_html_view"), pMF->m_nDocumentNo);
	rs.ExecSQL(szSQL);
	if (!rs.IsEOF())
	{
		for (int i = 0; i < rs.GetFieldCount(); i++)
		{
			m_wndLayout.SetValue(rs.GetFieldName(i), rs.GetValue(i));
		}
	}
...
*/
	...
	szSQL.Format(_T("SELECT hde_name, to_char(hde_value) as hde_value, hde_doctor FROM hms_doc_emr WHERE hde_docno=%ld and hde_deptid='%s' and hde_type='%s'  "),
			pMF->m_nDocumentNo, pMF->m_szDept, m_szType);
	...
	szSQL.Format(_T("SELECT hde_name, to_char(hde_value) as hde_value, hde_doctor, ss_desc ") \
		...
		_T("WHERE hde_docno=%ld and hde_deptid='%s' and hde_type='%s'  and length(ss_code) > 0 ") \
		_T("ORDER BY ss_index "), pMF->m_nDocumentNo, pMF->m_szDept, m_szType);
	...
	while(!rs.IsEOF())
	{
		...
		if(m_bPrinting)
		{
			...
			os << L"<div style=\"width:100%;height:10px;\"></div>";
		}
...
void CHMSGeneralRankExamine::OnReloadInfoSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CRecord rsl(&pMF->m_db);
	CString szSQL;
	...
	if(!rs.IsEOF())
	{
		...
		{
			szSQL.Format(_T("SELECT hde_name, to_char(hde_value) as hde_value, hde_doctor FROM hms_doc_emr WHERE hde_docno=%ld and hde_deptid='%s' and hde_type='%s'  "),
			nMaxDocNo, pMF->m_szDept, m_szType);
...
void CHMSGeneralRankExamine::OnUpdateSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	SetMode(VM_EDIT);
} 
void CHMSGeneralRankExamine::OnSaveSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnSaveHMSGeneralRankExamine();
	
} 
void CHMSGeneralRankExamine::OnCancelSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnCancelHMSGeneralRankExamine();
} 


void CHMSGeneralRankExamine::OnPrintSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	...
	if (m_bGeneralExamHealth)
	{
		szID = L"phieukhamsuckhoe_captuong_108";
	}
	else
	{
		szID = L"phieukhamsuckhoe_ms3_108";
	}
	jv[L"id"] = szID;
	jv[L"docno"] = m_nDocumentNo;
	jv[L"type"] = m_szType;
	jv.ToString(szBody);
_tprintf(L"\nszBody: %s", szBody);
	if (rpt.Post(szBody))
	...
	CString szFilePath;
	szFilePath.Format(_T("Reports\\HMS\\%s_IN.HTML"), m_szFileName);
	m_wndLayout.Load(szFilePath);
	m_bPrinting = true;
	GetDataToScreen();
	m_bPrinting = false;
	CString szDoctor;
...
void CHMSGeneralRankExamine::OnENTExamSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	if(!pMF->CheckPermission(_T("EMR.04")))
	{
	...
	CHMSGeneralRankSpecialExamineDialog dlg(this, _T("TMH"));
	dlg.m_szDocType = m_szType;
	if(dlg.DoModal() == IDOK)
	{

	}
} 
void CHMSGeneralRankExamine::OnOMFExamSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	if(!pMF->CheckPermission(_T("EMR.05")))
	{
		ShowMessageBox(_T("Permission denied"));
	...
	CHMSGeneralRankSpecialExamineDialog dlg(this, _T("RHM"));
	dlg.m_szDocType = m_szType;
	if(dlg.DoModal() == IDOK)
	{
...
void CHMSGeneralRankExamine::OnEYEExamSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	if(!pMF->CheckPermission(_T("EMR.06")))
	{
		ShowMessageBox(_T("Permission denied"));
...
void CHMSGeneralRankExamine::OnSPECExamSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
int CHMSGeneralRankExamine::OnSaveHMSGeneralRankExamine(){
 	...
 	if(!IsValidateData())
 		return -1;
 	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
 	CString szSQL, jData, szWhere;


	szSQL.Format(_T("DELETE FROM hms_doc_emr ") \
		_T("WHERE hde_docno = %ld and hde_type='%s' and hde_name not in('TMH','RHM','MAT') "), 
		pMF->m_nDocumentNo, m_szType);
	//if (!m_bGeneralExamHealth)
	{
		szWhere.Format(
			_T(" and instr(hde_name, '+') <=0 ") \
			_T(" and hde_name NOT IN (SELECT ss_code FROM sys_sel ") \
			_T(" WHERE ss_id = 'HMS_GENERAL_EXAM_DOC')"));
	...
	//  _tprintf(_T("\r\n%s"), s.c_str());
 	...
 	{
		jv.ToString(jData);
		emrTbl.SetValue(_T("hde_name"), L"js_data");
		emrTbl.SetValue(_T("hde_value"), jData);
		szSQL.Format(_T("HMS_DOC_EMR_CREATE(%s) "), emrTbl.FormatSQL());
		pMF->ExecDML(szSQL);
 		SetMode(VM_VIEW);
 	}
...
BOOL CHMSGeneralRankExamine::PreTranslateMessage(MSG* pMsg)
{
