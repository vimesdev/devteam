...
static int _OnZoneCheckValueFnc(CWnd *pWnd){
	return ((CHMSClinicZoneDialog *)pWnd)->OnZoneCheckValue();
}
...
void CHMSClinicZoneDialog::OnInitializeComponents(){
	...
	m_wndZone.InsertColumn(1, _T("Alias"), CFMT_TEXT, 80);
	m_wndZone.InsertColumn(2, _T("Name"), CFMT_TEXT, 450);
...
int CHMSClinicZoneDialog::OnZoneCheckValue()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	UpdateData(TRUE);
	szSQL.Format(_T("SELECT hrl_id FROM hms_roomlist WHERe hrl_deptid='%s' and hrl_doctorid='%s' "), m_szZoneKey, pMF->GetCurrentUser());
	rs.ExecSQL(szSQL);
_tprintf(_T("\r\n%s"), szSQL);
	if(!rs.IsEOF())
	{
		rs.GetValue(_T("hrl_id"), m_szRoomKey);
	}
	else
	{
		if(m_szZoneKey == _T("C1.1"))
			m_szRoomKey = pMF->m_UserInfo.su_hms_xroom;
		else
			m_szRoomKey.Empty();
	}
	UpdateData(FALSE);
...
long CHMSClinicZoneDialog::OnZoneLoadData()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szFilter;
	szFilter.Format(_T("and sd_id in('C1.1','C1.3', 'TYC','AB', 'TTTVDT', 'TTHTSS', 'C11BCLC', 'KB_A16') ")); 
	return pMF->LoadDepartment(&m_wndZone, m_szZoneKey, szFilter);
/*
	CRecord rs(&pMF->m_db);
...
long CHMSClinicZoneDialog::OnRoomLoadData(){
	...
	szSQL.Format(_T("SELECT hrl_id as id, hrl_name as name ") \
		_T("FROM hms_roomlist WHERE hrl_deptid='%s'  %s ORDER BY id "), m_szZoneKey, szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndRoom.AddItems(
			rs.GetValue(_T("id")), 
...
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	//Comment đoạn này lại vì đã có phân quyền nhìn thấy phòng khám theo khu rồi

	UpdateData(true);
	if ((pMF->GetCurrentDepartmentID() == _T("C1.1") 
		|| pMF->GetCurrentDepartmentID() == _T("TYC") 
		|| pMF->GetCurrentDepartmentID() == _T("PTTYC")) 
		&& m_szRoomKey.IsEmpty() && !pMF->CheckPermission(_T("EM.100.99")))
	{
		ShowMessageBox(_T("Bạn phải chọn phòng làm việc!"),MB_ICONERROR | MB_OK);
		return;
	}
