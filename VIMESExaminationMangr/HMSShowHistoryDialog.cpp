...
static long _OnListLoadDataFnc(CWnd *pWnd){
	return ((CHMShowHistoryDialog*)pWnd)->OnListLoadData();
} 
static void _OnListDblClickFnc(CWnd *pWnd){
	((CHMShowHistoryDialog*)pWnd)->OnListDblClick();
} 
static void _OnListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CHMShowHistoryDialog*)pWnd)->OnListSelectChange(nOldItem, nNewItem);
} 
static int _OnListDeleteItemFnc(CWnd *pWnd){
	 return ((CHMShowHistoryDialog*)pWnd)->OnListDeleteItem();
} 
static void _OnViewSelectFnc(CWnd *pWnd){
	CHMShowHistoryDialog *pVw = (CHMShowHistoryDialog *)pWnd;
	pVw->OnViewSelect();
} 
...
static int _OnAddHMSDocumentPreviewDialogFnc(CWnd *pWnd){
	 return ((CHMShowHistoryDialog*)pWnd)->OnAddHMSDocumentPreviewDialog();
} 
static int _OnEditHMSDocumentPreviewDialogFnc(CWnd *pWnd){
	 return ((CHMShowHistoryDialog*)pWnd)->OnEditHMSDocumentPreviewDialog();
} 
static int _OnDeleteHMSDocumentPreviewDialogFnc(CWnd *pWnd){
	 return ((CHMShowHistoryDialog*)pWnd)->OnDeleteHMSDocumentPreviewDialog();
} 
static int _OnSaveHMSDocumentPreviewDialogFnc(CWnd *pWnd){
	 return ((CHMShowHistoryDialog*)pWnd)->OnSaveHMSDocumentPreviewDialog();
} 
static int _OnCancelHMSDocumentPreviewDialogFnc(CWnd *pWnd){
	 return ((CHMShowHistoryDialog*)pWnd)->OnCancelHMSDocumentPreviewDialog();
} 
static void _OnCloseSelectFnc(CWnd *pWnd){
	CHMShowHistoryDialog *pVw = (CHMShowHistoryDialog *)pWnd;
	pVw->OnCloseSelect();
} 
CHMShowHistoryDialog::CHMShowHistoryDialog(CWnd *pParent):
	CGuiDialog(pParent){
...
CHMShowHistoryDialog::~CHMShowHistoryDialog(){
}
void CHMShowHistoryDialog::OnCreateComponents()
{
	m_wndDocumentList.Create(this, _T("Document List"), 5, 5, 820, 520);
	m_wndList.Create(this,10, 30, 810, 515); 
	m_wndView.Create(this, _T("Chi tiết"), 595, 525, 700, 550);
	m_wndClose.Create(this, _T("Close"), 705, 525, 810, 550);

	m_wndViewSelection.Create(this, _T(""), 0, 0, 0, 0);
	m_wndViewSelection.ShowWindow(SW_HIDE);
	m_wndViewSelection.EnableWindow(FALSE);
	m_wndViewAll.Create(this, _T(""), 0, 0, 0, 0);
	m_wndViewAll.ShowWindow(SW_HIDE);
	m_wndViewAll.EnableWindow(FALSE);
}
void CHMShowHistoryDialog::OnInitializeComponents()
{
...
//	m_wndList.SetCheckBox(true);
	...
	m_wndList.InsertColumn(2, _T("Ngày khám"), CFMT_DATE, 80);	
	m_wndList.InsertColumn(3, _T("Bản thân"), CFMT_TEXT, 300);
	m_wndList.InsertColumn(4, _T("Gia đình"), CFMT_TEXT, 200);
	m_wndList.InsertColumn(5, _T("Dị ứng thuốc"), CFMT_TEXT, 80);

}
void CHMShowHistoryDialog::OnSetWindowEvents()
{
...
void CHMShowHistoryDialog::OnDoDataExchange(CDataExchange* pDX){

}
void CHMShowHistoryDialog::GetDataToScreen(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
void CHMShowHistoryDialog::GetScreenToData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();

}
void CHMShowHistoryDialog::SetDefaultValues()
{


}
int CHMShowHistoryDialog::SetMode(int nMode){
...
void CHMShowHistoryDialog::OnListDblClick(){
	OnViewSelect();
} 
void CHMShowHistoryDialog::OnListSelectChange(int nOldItem, int nNewItem){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	if (nNewItem < 0)
		return;
	m_nDocNo = ToLong(m_wndList.GetItemText(nNewItem, 1));
} 
int CHMShowHistoryDialog::OnListDeleteItem(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	 return 0;
}
void CHMShowHistoryDialog::OnCloseSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	OnCancel();	
} 
long CHMShowHistoryDialog::OnListLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, tmpStr;

	szSQL.Format(_T(" SELECT") \
	_T(" hdh_docno as docno,") \
...
void CHMShowHistoryDialog::OnViewSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();	
	OnPrintExaminationSheet(m_nDocNo);
} 
void CHMShowHistoryDialog::OnViewSelectionSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	//docno_list.clear();
...
void CHMShowHistoryDialog::OnViewAllSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	/*docno_list.clear();
	for (int i = 0;i<m_wndList.GetItemCount();i++)
	{
...
int CHMShowHistoryDialog::OnAddHMSDocumentPreviewDialog(){
...
int CHMShowHistoryDialog::OnEditHMSDocumentPreviewDialog(){
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd(); 
 	SetMode(VM_EDIT);
...
int CHMShowHistoryDialog::OnDeleteHMSDocumentPreviewDialog(){
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
 	CString szSQL; 
...
int CHMShowHistoryDialog::OnSaveHMSDocumentPreviewDialog(){
 	if(GetMode() != VM_ADD && GetMode() != VM_EDIT) 
 		return -1; 
 	if(!IsValidateData()) 
 		return -1; 
...
int CHMShowHistoryDialog::OnCancelHMSDocumentPreviewDialog()
{
 	if(GetMode() == VM_EDIT) 
 	{ 
 		SetMode(VM_VIEW); 
...
int CHMShowHistoryDialog::OnHMSDocumentPreviewDialogListLoadData()
{
	return 0;
}
void CHMShowHistoryDialog::OnPrintExaminationSheet(int nDocumentNo, bool bPrintPrescription, bool bPreview, bool bDirect, bool bRequest)
{	
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
...
//Lay thong tin benh nhan
	...
	szSQL.Format(_T("  select trim(hp_surname||' '||hp_midname||' '||hp_firstname) as PatientName,") \
				...
				_T("      WHERE hd_docno=%ld"), pMF->m_nTreatTime, nDocumentNo);
