...
void CPhanKhamBenh::OnCreateComponents(){
	m_wndtthanLabel.Create(this, _T("\x31. To\xE0n th\xE2n:"), 5, 5, 155, 30);
	m_wndtthan.Create(this,5, 31, 465, 131, 1, 0, 1); 
	m_wndmachLabel.Create(this, _T("M\x1EA1\x63h"), 485, 5, 585, 30);
	m_wndmach.Create(this,590, 5, 690, 30); 
	m_wndndoLabel.Create(this, _T("Nhi\x1EC7t \x111\x1ED9"), 720, 5, 820, 30);
	m_wndndo.Create(this,830, 5, 930, 30); 
	m_wndhapLabel.Create(this, _T("Huy\x1EBFt \xE1p"), 485, 35, 585, 60);
	m_wndhap.Create(this,590, 35, 637, 60); 
	m_wndhap2.Create(this,642, 35, 690, 60); 
	m_wndnthoLabel.Create(this, _T("Nh\x1ECBp th\x1EDF"), 720, 35, 820, 60);
	m_wndntho.Create(this,830, 35, 930, 60); 
	m_wndccaoLabel.Create(this, _T("\x43hi\x1EC1u \x63\x61o"), 485, 68, 585, 93);
	m_wndccao.Create(this,590, 68, 690, 93); 
	m_wndcnangLabel.Create(this, _T("\x43\xE2n n\x1EB7ng"), 720, 68, 820, 93);
	...
	m_wndcaccoquanLabel.Create(this, _T("\x32. \x43\xE1\x63 \x63\x1A1 qu\x61n:"), 5, 132, 155, 157);
	m_wndthoanLabel.Create(this, _T("+ Tu\x1EA7n ho\xE0n"), 5, 158, 155, 183);
	m_wndthoan.Create(this,5, 184, 465, 259, 1, 0, 1); 
	m_wndhhapLabel.Create(this, _T("+ H\xF4 h\x1EA5p"), 470, 158, 620, 183);
	m_wndhhap.Create(this,470, 184, 930, 259, 1, 0, 1); 
	m_wndthoaLabel.Create(this, _T("+ Ti\xEAu h\xF3\x61"), 5, 260, 155, 285);
	m_wndthoa.Create(this,5, 286, 465, 361, 1, 0, 1); 
	m_wndtnieuLabel.Create(this, _T("+ Ti\x1EBFt ni\x1EC7u sinh \x64\x1EE5\x63"), 470, 260, 620, 285);
	m_wndtnieu.Create(this,470, 286, 930, 361, 1, 0, 1); 
	m_wndttkinhLabel.Create(this, _T("+ T\xE2m - Th\x1EA7n kinh"), 5, 362, 155, 387);
	m_wndttkinh.Create(this,5, 388, 465, 463, 1, 0, 1); 
	m_wndcxkhopLabel.Create(this, _T("+ \x43\x1A1 \x78\x1B0\x1A1ng kh\x1EDBp"), 470, 362, 620, 387);
	m_wndcxkhop.Create(this,470, 388, 930, 463, 1, 0, 1); 
	m_wndckkhacLabel.Create(this, _T("+ \x43\xE1\x63 kho\x61 kh\xE1\x63"), 4, 464, 154, 489);
	m_wndckkhac.Create(this,5, 490, 465, 565, 1, 0, 1); 
	m_wndblkhacLabel.Create(this, _T("+ \x43\xE1\x63 \x62\x1EC7nh l\xFD kh\xE1\x63"), 470, 464, 620, 489);
	m_wndblkhac.Create(this,470, 490, 930, 565, 1, 0, 1); 
	m_wndxndalamLabel.Create(this, _T("\x33. \x43\xE1\x63 \x78\xE9t nghi\x1EC7m \x111\xE3 l\xE0m:"), 5, 566, 205, 591);
	m_wndxnghiem.Create(this,5, 592, 930, 692, 1, 0, 1); 

	m_wndhap2Label.Create(this, _T(""), 0, 0, 0, 0);
	m_wndhap2Label.ShowWindow(SW_HIDE);
	m_wndhap2Label.EnableWindow(FALSE);
}
...
void CPhanKhamBenh::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT COUNT(*) AS solan FROM hms_doc_emr WHERE hde_docno=%ld and hde_name='%s' and hde_type='BENH_AN_KB'"), m_nDocumentNo, m_szRecordNo);
	rs.ExecSQL(szSQL);
	...
	if(n > 0)
	{
		for (int i = 0; i < n; i++)
		{
			szSQL.Format(_T("SELECT to_char(hde_value) as hde_value FROM hms_doc_emr WHERE hde_docno=%ld and hde_name='%s' and hde_type='BENH_AN_KB' and hde_refidx = %d "), m_nDocumentNo, m_szRecordNo, i);
			rs.ExecSQL(szSQL);
...
int CPhanKhamBenh::OnSavePhanKhamBenh(){
	...
	//szSQL.Format(_T("DELETE FROM hms_doc_emr WHERE hde_docno=%ld and hde_name='%s' and hde_type='BENH_AN_KB'"), m_nDocumentNo, m_szRecordNo);
	//pMF->ExecSQL(szSQL);
	...
	{
		tmpStr = strList[i];
		szSQL.Format(_T("HMS_DOC_EMR_EX_CREATE('BENH_AN_KB', %ld, '%s', %d, %d, '%s', '%s', '%s') "),
