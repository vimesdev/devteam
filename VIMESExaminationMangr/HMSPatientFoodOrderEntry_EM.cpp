...
{
	return ((CHMSPatientFoodOrderEntry *)pWnd)->OnFromDateCheckValue();
}
static int _OnToDateCheckValueFnc(CWnd *pWnd)
{
	return ((CHMSPatientFoodOrderEntry *)pWnd)->OnToDateCheckValue();
}

CHMSPatientFoodOrderEntry::CHMSPatientFoodOrderEntry(CWnd *pParent, int nMode, int nType):
CGuiPopup(pParent)
{
	...
	m_pWndOrder = pParent;
	m_nType = nType;
	m_szSkipName2 = _T("'PK1','PK2','PK3','PK4','PK5','PK6','PK7','PK8','PK9','PK10', 'PK11', 'PK12'");
	m_bHaucan = FALSE;
...
{
	m_wndOrderDate.Create(this,379, 5, 474, 30); 
	m_wndMultipleDay.Create(this, _T("Multiple Day"), 6, 5, 116, 30);
	m_wndDays.Create(this,120, 5, 215, 30); 
	m_wndOrderDateLabel.Create(this, _T("Date"), 294, 5, 374, 30);
	m_wndFromDateLabel.Create(this, _T("From Date"), 5, 35, 115, 60);
	m_wndFromDate.Create(this,120, 35, 215, 60); 
	m_wndToDateLabel.Create(this, _T("To Date"), 294, 35, 374, 60);
	m_wndToDate.Create(this,379, 35, 474, 60); 
	m_wndPatientNameLabel.Create(this, _T("Patient Name"), 5, 65, 115, 90);
	m_wndPatientName.Create(this,120, 65, 375, 90); 
	m_wndDocumentNo.Create(this,380, 65, 475, 90); 
	m_wndNameLabel.Create(this, _T("\x102n s\xE1ng"), 5, 95, 115, 120);
	m_wndName.Create(this,120, 95, 375, 120); 
	m_wndQtyLabel.Create(this, _T("S\x1ED1 su\x1EA5t"), 380, 95, 435, 120);
	m_wndQty.Create(this,440, 95, 475, 120); 
	m_wndName1Label.Create(this, _T("\x102n tr\x1B0\x61"), 5, 125, 115, 150);
	m_wndName1.Create(this,120, 125, 375, 150); 
	m_wndQty1Label.Create(this, _T("S\x1ED1 su\x1EA5t"), 380, 125, 435, 150);
	m_wndQty1.Create(this,440, 125, 475, 150); 
	m_wndName2Label.Create(this, _T("\x102n t\x1ED1i"), 5, 155, 115, 180);
	m_wndName2.Create(this,120, 155, 375, 180); 
	m_wndQty2Label.Create(this, _T("S\x1ED1 su\x1EA5t"), 380, 155, 435, 180);
	m_wndQty2.Create(this,440, 155, 475, 180); 
	m_wndBreakfastRelativeLabel.Create(this, _T("\x102n s\xE1ng ng\x1B0\x1EDDi nh\xE0"), 5, 185, 115, 210);
	m_wndBreakfastRelative.Create(this,120, 185, 375, 210); 
	m_wndQty3Label.Create(this, _T("Qty3"), 380, 185, 435, 210);
	m_wndQty3.Create(this,440, 185, 475, 210); 
	m_wndNoteLabel.Create(this, _T("Note"), 5, 215, 115, 240);
	m_wndNote.Create(this,120, 215, 374, 240); 
	m_wndSave.Create(this, _T("&Save"), 379, 215, 434, 240);
	m_wndClose.Create(this, _T("X"), 439, 215, 474, 240);
...
void CHMSPatientFoodOrderEntry::OnInitializeComponents()
{
	...
	m_wndPatientName.InsertColumn(1, _T("Vào từ"), CFMT_TEXT, 80);
...
int CHMSPatientFoodOrderEntry::OnFromDateCheckValue(){
	return 0;
}
int CHMSPatientFoodOrderEntry::OnToDateCheckValue(){
	return 0;
}
...
//	m_szOrderDate = m_szCurrentOrderDate.Left(10);
	...
	if(m_nDocumentNo != str2long(m_szPatientNameKey))
	{
		...
		szSQL.Format(_T("SELECT ") \
				_T("HE_PATIENTNO as patientno, ") \
				_T("HE_DOCNO as docno, ") \
				_T("HE_DEPTID as deptid, ") \
				_T("0 as refidx, ") \
				_T("HE_DOCTOR as doctor, ") \
				_T("HE_ROOMID as roomid, ") \
				_T("0 as bedid ") \
			_T("FROM hms_exam ") \
			_T("LEFT JOIN hms_doc on(he_docno = hd_docno ) ") \
			_T("WHERE hd_docno=%ld AND  NVL(hd_status,'X') <> 'T' AND HE_DEPTID='%s' AND he_roomid=150"), m_nDocumentNo,  pMF->m_szDept);
	_fmsg(_T("%s"), szSQL);		
		rs.ExecSQL(szSQL);
		m_nRefIndex = 0;
		m_nRoomID = 0;
...
long CHMSPatientFoodOrderEntry::OnPatientNameLoadData()
{
	...
	CString szSQL, szWhere, szFromTime, szToTime;
	CStringArray arr_Docs;
	szFromTime = _T("00:00:00");
	szToTime = _T("23:59:00");
	...
	szSQL.Format(_T("    SELECT     ") \
				...
				_T("   WHERE 1=1 AND ((he_deptid='C1.1' AND he_roomid in (162, 150) OR (he_deptid='C1.1' AND hrl_group='LT')))") \
				_T("   AND he_status IN('O','P','T') AND NVL(hd_status,'X') <> 'T' ") \
				_T("   AND he_examdate BETWEEN TO_TIMESTAMP('%s %s', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('%s %s', 'YYYY-MM-DD HH24:MI:SS')") \
				_T("   ORDER BY TRUNC(he_examdate),") \
				_T("   he_roomid,") \
				_T("   he_receptno "), m_szFromDate, szFromTime, m_szToDate, szToTime);
...
long CHMSPatientFoodOrderEntry::OnNameLoadData(){
	...
	else
	{
		if(pMF->m_szDept == _T("A16")){		
			szWhere.AppendFormat(_T(" and hfl_feeid IN('FF000132', 'FF000189','FF000207','FF000220','FF000221','FF000222','FF000474', 'FF000475', 'FF0001205', 'FF0001206', 'FF0001207', 'FF0030008', 'FF0030009', 'FF0030010', 'FF00031439') "));
		}
		else
			szWhere.AppendFormat(_T(" and (hfl_feeid IN('FF000132','FF000220','FF000221','FF000222','FF000474', 'FF000475', 'FF0001035', 'FF0001036', 'FF0001205', 'FF0001206', 'FF0001207', 'FF0030008', 'FF0030009', 'FF0030010', 'FF00031439', 'FF00031552') OR hfl_name2='sang' )"));
...
long CHMSPatientFoodOrderEntry::OnName2LoadData(){
	...
	if(m_wndName2.IsSearchKey() && !m_szName2Key.IsEmpty())
	{
		...
		szWhere.AppendFormat(_T(" and hfl_feeid NOT IN('FF000132', 'FF000189','FF000207','FF000220','FF000221','FF000222','FF000474', 'FF000475', 'FF0001205', 'FF0001206', 'FF0001207', 'FF0030008', 'FF0030009', 'FF0030010') AND COALESCE(hfl_categoryid, 0) <= 0"));	
		szWhere.AppendFormat(_T(" AND hfl_name2 IN (%s)"), m_szSkipName2);
		szWhere.AppendFormat(_T(" and NVL(hfl_deptid,'XX') <> 'PTTYC' "));
...
_T(" 	hfl_typeid as typeid ") \
_T(" FROM hms_fee_list ") \
_T(" WHERE hfl_typeid = 'F' and hfl_groupid = 'FF000' and nvl(hfl_subgroup,'N') <> 'NN' AND NVL(hfl_index1, 'N') <> '212' and hfl_active='Y' %s ") \
...
_tprintf(_T("\r\n%ld"), m_nOID);
	...
	for (int i=0; i < m_nDays; i++)
	{
		...
		szSQL.Format(_T("HMS_FOODORDER_CREATE_EM(%s) "), m_hms_feefodorderTbl.FormatSQL());
