...
static int _OnUserSelectItemFnc(CWnd *pWnd){
	 return ((CHMSPrescriptionLockInfoDialog*)pWnd)->OnUserSelectItem();
} 

static int _OnUserDeselectItemFnc(CWnd *pWnd){
	 return ((CHMSPrescriptionLockInfoDialog*)pWnd)->OnUserDeselectItem();
} 
...
void CHMSPrescriptionLockInfoDialog::OnInitializeComponents(){
	...
	m_wndUser.InsertColumn(1, _T("User ID"), CFMT_TEXT, 100);
	m_wndUser.InsertColumn(2, _T("User Name"), CFMT_TEXT, 250);
	m_wndUser.SetCheckBox(TRUE);
...
void CHMSPrescriptionLockInfoDialog::OnSetWindowEvents(){
	...
	m_wndUser.AddEvent(1, _T("Select All"), _OnUserSelectItemFnc);
	m_wndUser.AddEvent(0, _T("-"));
	m_wndUser.AddEvent(2, _T("Deselect All"), _OnUserDeselectItemFnc);
...
long CHMSPrescriptionLockInfoDialog::OnDepartmentLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szWhere;
	szWhere.Format(_T(" and sd_id in('C1.1','TYC') "));
...
void CHMSPrescriptionLockInfoDialog::OnUserDblClick(){
	int nSel = m_wndUser.GetCurSel();
	if(nSel < 0)
		return;

	BOOL bCheck = m_wndUser.GetCheck(nSel);
	m_wndUser.SetCheck(nSel, !bCheck);
} 
...
long CHMSPrescriptionLockInfoDialog::OnUserLoadData(){
	...
	if(!m_szName.IsEmpty())
	{
		szWhere.Format(_T(" and lower(su_name) like(chr(37)||lower('%s')||chr(37)) "), m_szName);
	}
	szWhere.AppendFormat(_T(" and su_userid not in(select hpl_userid from hms_prescription_lock where hpl_storage_id=%d and hpl_product_id = %ld) "), m_nStorage_ID, m_nProduct_ID);
...
_T(" OR su_hms_xdept ='%s') %s") \
_T(" ORDER BY su_roomid"), m_szDepartmentKey, m_szDepartmentKey, szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndUser.AddItems(
			_T(""), 
...
int CHMSPrescriptionLockInfoDialog::OnUserSelectItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();

	for (int i =0; i < m_wndUser.GetItemCount(); i ++)
	{
		m_wndUser.SetCheck(i, TRUE);
	}
...
int CHMSPrescriptionLockInfoDialog::OnUserDeselectItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	for (int i =0; i < m_wndUser.GetItemCount(); i ++)
	{
		m_wndUser.SetCheck(i, FALSE);
	}
...
void CHMSPrescriptionLockInfoDialog::OnApplySelect(){
	...
	for (int i =0; i < m_wndUser.GetItemCount(); i++)
	{
		if(m_wndUser.GetCheck(i))
		{
			szUserID = m_wndUser.GetItemText(i, 1);
			szSQL.Format(_T(" INSERT INTO hms_prescription_lock values(%d, %ld, '%s') "),
				m_nStorage_ID, m_nProduct_ID, szUserID);
			pMF->ExecSQL(szSQL);
		}
	}
