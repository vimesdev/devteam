...
void CTMShowPatientEatInDayDialog::OnInitializeComponents(){
	...
	m_wndList.InsertColumn(4, _T("SheetNo"), CFMT_TEXT, 80);
...
long CTMShowPatientEatInDayDialog::OnListLoadData(){
	...
	szSQL.Format(_T(" select get_patientname(hfo_docno) as pname, ") \
				...
				_T("       hfo_orderstatus as orderstatus") \
				_T(" from hms_feefood ") \
				_T(" LEFT join VIMES.hms_foodordersheet on (hfos_orderid= hfo_reforder_id)") \
	...
	while(!rs.IsEOF()){ 
		...
		m_wndList.AddItems(
			...
			rs.GetValue(_T("orderstatus")),	NULL);
		rs.MoveNext();
	}
	m_wndList.EndLoad(); 
	return nCount;
