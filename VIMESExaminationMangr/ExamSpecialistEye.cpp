...
void CExamSpecialistEye::OnSetWindowEvents(){
	...
	szSQL.Format(_T("Select count(*) from hms_advancedexmeye where haee_docno = %ld and haee_depttype ='E' "), m_nDocNo);
...
void CExamSpecialistEye::GetDataToScreen()
{
	...
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("Select * from hms_advancedexmeye where haee_docno = %ld and haee_depttype ='E' "), m_nDocNo);
	rs.ExecSQL(szSQL);
	if(!rs.IsEOF())
	{
...
void CExamSpecialistEye::GetScreenToData(){
	...
	m_hms_advancedexmeyeTbl.SetTableName(_T("hms_advancedexmeye"));
	m_hms_advancedexmeyeTbl.SetValue(_T("haee_facid"), m_nFacid);
	...
	m_hms_advancedexmeyeTbl.SetValue(_T("haee_roomid"), 0);
	...
	m_hms_advancedexmeyeTbl.SetValue(_T("haee_depttype"), _T("E"));	
...
int CExamSpecialistEye::OnSaveExamSpecialistEye()
{
	...
	{ 
 		CString szWhere; 
 		szWhere.Format(_T(" WHERE haee_docno = %ld and haee_depttype ='E' "), m_nDocNo); 
 		szSQL = m_hms_advancedexmeyeTbl.GetUpdateSQL(_T("haee_docno, haee_depttype, haee_idx, haee_facid, ")\
			                                         _T("haee_roomid, haee_bedid, haee_receptid")); 
 		szSQL += szWhere; 
 	} 
