...
static void _OnSurgeryNameSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((rptOperationPatientListReportDialog* )pWnd)->OnSurgeryNameSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnSurgeryNameSelendokFnc(CWnd *pWnd){
	((rptOperationPatientListReportDialog *)pWnd)->OnSurgeryNameSelendok();
}
/*static void _OnSurgeryNameSetfocusFnc(CWnd *pWnd){
	((rptOperationPatientListReportDialog *)pWnd)->OnSurgeryNameKillfocus();
}*/
/*static void _OnSurgeryNameKillfocusFnc(CWnd *pWnd){
	((rptOperationPatientListReportDialog *)pWnd)->OnSurgeryNameKillfocus();
}*/
static long _OnSurgeryNameLoadDataFnc(CWnd *pWnd){
	return ((rptOperationPatientListReportDialog *)pWnd)->OnSurgeryNameLoadData();
}
/*static void _OnSurgeryNameAddNewFnc(CWnd *pWnd){
	((rptOperationPatientListReportDialog *)pWnd)->OnSurgeryNameAddNew();
}*/
...
void rptOperationPatientListReportDialog::OnCreateComponents(){
	...
	m_wndReportPeriodLabel.Create(this, _T("Report Period"), 200, 30, 290, 55);
	m_wndReportPeriod.Create(this,295, 30, 395, 55); 
	m_wndFromDateLabel.Create(this, _T("From Date"), 10, 60, 90, 85);
	m_wndFromDate.Create(this,95, 60, 195, 85); 
	m_wndToDateLabel.Create(this, _T("To Date"), 200, 60, 290, 85);
	m_wndToDate.Create(this,295, 60, 395, 85); 
	m_wndDeptLabel.Create(this, _T("Room"), 10, 90, 90, 115);
	m_wndDept.Create(this,95, 90, 395, 115); 
	m_wndDoctorLabel.Create(this, _T("Doctor"), 10, 120, 90, 145);
	m_wndDoctor.Create(this,95, 120, 395, 145); 
	m_wndSurgeryNameLabel.Create(this, _T("Operation Group"), 10, 150, 90, 175);
	m_wndSurgeryName.Create(this,95, 150, 395, 175); 
	m_wndPrintPreview.Create(this, _T("&Print"), 150, 185, 230, 210);
	m_wndPrint.Create(this, _T("&Export"), 235, 185, 315, 210);
...
//	m_wndDoctor.SetCheckValue(true);
	...
	m_wndSurgeryName.LimitText(81);
	m_wndSurgeryName.SetCheckBox(TRUE);
	...
	m_wndSurgeryName.InsertColumn(0, _T("ID"), CFMT_TEXT | CFMT_RIGHT, 70);
	m_wndSurgeryName.InsertColumn(1, _T("Name"), CFMT_TEXT, 150);
}
...
long rptOperationPatientListReportDialog::OnDoctorLoadData()
{
	...
	szSQL.Format(_T("SELECT su_userid AS id, su_name AS name FROM sys_user ")\
				 ...
				 _T("WHERE sd_type IN ('KB', 'DT') %s ORDER BY su_userid, su_name"),
				 szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF())
	{ 
...
long rptOperationPatientListReportDialog::OnDeptLoadData()
{
	...
	{
		szWhere.Format(_T(" and hrl_id =%d "), ToInt(m_szDeptKey));
	}
	m_wndDept.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("Select hrl_id as id, hrl_name as name from hms_roomlist where hrl_deptid ='KB' %s"), szWhere);	
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndDept.AddItems(
			rs.GetValue(_T("id")), 
...
void rptOperationPatientListReportDialog::OnSurgeryNameSelectChange(int nOldItemSel, int nNewItemSel){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
void rptOperationPatientListReportDialog::OnSurgeryNameSelendok(){
	 
}
/*void rptOperationPatientListReportDialog::OnSurgeryNameSetfocus(){
	
}*/
/*void rptOperationPatientListReportDialog::OnSurgeryNameKillfocus(){
	
}*/
long rptOperationPatientListReportDialog::OnSurgeryNameLoadData()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
	...
	{
		szWhere.Format(_T(" AND hfg_id='%s' "), m_szSurgeryNameKey);
	}
...
/*void rptOperationPatientListReportDialog::OnSurgeryNameAddNew(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
void rptOperationPatientListReportDialog::PrintOperationPatientListReport(CString szFromDate, CString szToDate, CString szDoctor, bool bPreview)
{
	...
	CString szDate, szSysDate, szOperation;
	...
	{
		szWhere.AppendFormat(_T(" and ho_roomid = '%s' "), m_szDeptKey);
	}
	if(!szDoctor.IsEmpty())
	{
		szWhere.AppendFormat(_T(" AND ho_doctor='%s' "), szDoctor);
	}
	for (int i = 0; i < m_wndSurgeryName.GetCount(); i++)
	{
		if (m_wndSurgeryName.GetCheck(i))
		{
			if (!szOperation.IsEmpty())
				szOperation += _T(", ");
			szOperation.AppendFormat(_T("'%s'"), m_wndSurgeryName.GetCheck(i, 0));
		}
	}
	if (!szOperation.IsEmpty())
		szWhere.AppendFormat(_T(" AND hfl_groupid IN (%s) "), szOperation);
	...
	szSQL.Format(_T(" SELECT trim(hp_surname||' '||hp_midname||' '||hp_firstname) as patientname,") \
		...
		_T(" WHERE (hfl_report <> 'N' or hfl_report is null) and ho_depttype ='E' and date(ho_orderdate) BETWEEN date('%s') AND date('%s') AND ho_status <>'O' %s ") \
