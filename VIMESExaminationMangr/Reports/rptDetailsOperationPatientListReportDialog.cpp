...
rptDetailsOperationPatientListReportDialog::rptDetailsOperationPatientListReportDialog(CWnd *pParent):
	...
	m_nDlgHeight = 160;
	SetDefaultValues();
	m_szTitle =_T("The table lists patient procedure - surgery");
	m_bPreview = true;
}
...
void rptDetailsOperationPatientListReportDialog::OnCreateComponents(){
	...
	m_wndDeptLabel.Create(this, _T("Room"), 10, 90, 110, 115);
...
long rptDetailsOperationPatientListReportDialog::OnDeptLoadData(){
	...
	if(m_wndDept.IsSearchKey() && !m_szDeptKey.IsEmpty()){
		szWhere.Format(_T(" and hrl_id =%s "), m_szDeptKey);
	};
	m_wndDept.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("Select hrl_id as id, hrl_name as roomname from hms_roomlist left join sys_dept on(sd_id = hrl_deptid) where sd_type ='KB' %s"), szWhere);	
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndDept.AddItems(
			rs.GetValue(_T("id")), 
...
void rptDetailsOperationPatientListReportDialog::PrintOperationPatientListReport(CString szFromDate, CString szToDate, CString szDoctor, bool bPreview){
	...
	szSQL.Format(_T(" SELECT trim(hp_surname||' '||hp_midname||' '||hp_firstname) as patientname,") \
		...
		_T(" WHERE (hfl_report <> 'N' or hfl_report is null) and ho_depttype ='E' AND date(ho_orderdate) BETWEEN date('%s') AND date('%s') AND ho_status not in('O') %s ") \
...
void rptDetailsOperationPatientListReportDialog::ExportOperationPatientListReport(CString szFromDate, CString szToDate, CString szDoctor){
	...
	if (!m_szDeptKey.IsEmpty())
	{
		szWhere.Format(_T(" and ho_roomid =%s "),m_szDeptKey);
	}
	if(!szDoctor.IsEmpty())
	{
...
//_fmsg(_T("%s"), szSQL);
	...
	xls.SetCellText(0, 5, _T("BANG KE THUOC-VTTH BENH NHAN LAM PT-TT"),FMT_TEXT,true,12);
