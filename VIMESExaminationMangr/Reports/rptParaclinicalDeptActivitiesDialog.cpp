...
long CrptParaclinicalDeptActivitiesDialog::OnDepartmentLoadData()
{
	...
	szSQL.Format(_T("SELECT Distinct pcmso_deptid AS ID, (SELECT sd_name FROM sys_dept WHERE sd_id=pcmso_deptid) AS Name ")\
				 _T("FROM pcms_order ORDER BY pcmso_deptid"), szWhere);
...
void CrptParaclinicalDeptActivitiesDialog::OnPrintSelect()
{
	...
	szSQL.Format(_T(" SELECT (select hfg_name from hms_feegroup where hfg_id=substr(pcmso_groupid, 1, 2) || '000') as grouptype,") \
				 ...
				 _T("         case when ho_type in('I','D','C') and substr(hd_cardno, 1, 2)<>'HN' and hp_ethnic<>1 and (date(hd_admitdate)-hp_birthdate) > 365*6 then 1 else 0 end as inservobj2,") \
				 _T("		  case when substr(hd_cardno, 1, 2)='HN' and hp_ethnic<>1 and (date(hd_admitdate)-hp_birthdate) > 365*6 then 1 else 0 end as poorobj2,")\
	...
	while (!rs.IsEOF())
	{
		...
		szTemp = rs.GetValue(_T("poorobj2"));
...
void CrptParaclinicalDeptActivitiesDialog::OnExportSelect()
{
	...
	xls.SetCellMergedColumns(nCol, nRow + 6, 10);
	...
	TranslateString(_T("Poor Card"), szTemp);
	xls.SetCellText(nCol + 7, nRow + 8, szTemp, FMT_TEXT | FMT_CENTER, true, 12);
	
	xls.SetCellMergedColumns(nCol + 8, nRow + 7, 2);
	TranslateString(_T("Total Children Patient < 6"), szTemp);
	xls.SetCellText(nCol + 8, nRow + 7, szTemp, FMT_TEXT | FMT_CENTER, true, 12);
	xls.SetCellText(nCol + 8, nRow + 8, _T("Kinh"), FMT_TEXT | FMT_CENTER, true, 12);
	TranslateString(_T("Compatriots of Ethnic"), szTemp);
	xls.SetCellText(nCol + 9, nRow + 8, szTemp, FMT_TEXT | FMT_CENTER, true, 12);
	...
	while (!rs.IsEOF())
	{
		...
		szTemp = rs.GetValue(_T("minorobj3"));
		xls.SetCellText(nCol + 9, nRow, szTemp, FMT_INTEGER);
		tmpStr = rs.GetValue(_T("grouptype")).Trim();
		nRow++;
		rs.MoveNext();
	}
