...
void CHMSSpecialExamDialog::OnInitializeComponents() {
	...
	m_wndRoom.InsertColumn(1, _T("Description"), CFMT_TEXT, 500);


	m_wndExamType.InsertColumn(0, _T("ID"), CFMT_TEXT, 50);
	m_wndExamType.InsertColumn(1, _T(""), CFMT_TEXT, 0);
	m_wndExamType.InsertColumn(2, _T("Description"), CFMT_TEXT, 450);
	m_wndExamType.InsertColumn(3, _T("Ðon giá"), CFMT_NUMBER, 120);
...
int CHMSSpecialExamDialog::SetMode(int nMode) {
	...
	if (pMF->GetCurrentDepartmentID() == _T("KB_A16"))
	{
		m_wndExamType.EnableWindow(true);
	}
	else
...
long CHMSSpecialExamDialog::OnDepartmentLoadData()
{
    ...
    if (pMF->m_szDept == _T("C1.1"))
    {
        szWhere.AppendFormat(_T(" and sd_id in('C1.1','C1.3','TYC','AB', ")
                             _T("'TTTVDT', 'QLKBUT', 'KB_A4') "));
    }

    if (pMF->m_szDept == _T("C1.2"))
    {
        szWhere.AppendFormat(
            _T(" and sd_id in('C1.1','C1.3','C1.2','TYC','AB', 'TTTVDT', ")
            _T("'QLKBUT', 'KB_A11', 'KB_A4') "));
    }

    if (pMF->m_szDept == _T("C1.3"))
    {
        szWhere.AppendFormat(
            _T(" and sd_id in('C1.1','C1.2','C1.3','TYC','AB', 'TTTVDT', ")
            _T("'QLKBUT', 'KB_A11', 'KB_A4') "));
    }

    if (pMF->m_szDept == _T("TYC"))
    {
        szWhere.AppendFormat(_T(" and sd_id in('TYC','C1.1','C1.3', 'TTTM', ")
                             _T("'TTTVDT', 'QLKBUT', 'KB_A4') "));
    }

    if (pMF->m_szDept == _T("AB"))
    {
        szWhere.AppendFormat(_T(" and sd_id in('AB', 'C1.1','C1.2', 'C1.3', ")
                             _T("'TTTVDT', 'QLKBUT', 'TYC', 'KB_A4') "));
    }

    szSQL.Format(_T("SELECT sd_id as id, sd_name as name ")
                 _T("FROM sys_dept WHERE sd_type='KB' and sd_isactive='Y' %s ")
                 _T("ORDER BY sd_id"),
                 szWhere);
    //_msg(_T("%s"), szSQL);
...
long CHMSSpecialExamDialog::OnRoomLoadData()
{
	...
	int nCount = 0;
	_tprintf(_T("\r\n%s: %s"), m_szDepartmentKey, pMF->m_szDept);

	if (m_szDepartmentKey == pMF->m_UserInfo.su_deptid && m_szDepartmentKey == pMF->m_szDept)
		szWhere.AppendFormat(_T(" AND hrl_id NOT IN(%d) "), pMF->m_nRoomID);

	if (m_szDepartmentKey == _T("TYC"))
	{
		szWhere.AppendFormat(_T(" and hrl_type = 1"));
	}

	if (m_szDepartmentKey == _T("C1.3") && pMF->m_szDept != _T("C1.3"))
	{
		szWhere.AppendFormat(_T(" and NVL(hrl_roomid_is_vip, 'N') <> 'Y' "));
	}

	szSQL.Format(_T("SELECT hrl_id as id, hrl_name as name ") \
		_T("FROM hms_roomlist WHERE hrl_deptid='%s' AND HRL_ACTIVE='Y' %s ORDER BY hrl_id"), m_szDepartmentKey, szWhere);
	_tprintf(_T("\r\n%s"), szSQL);
	nCount = rs.ExecSQL(szSQL);
	//_msg(_T("%s"), szSQL);
	while (!rs.IsEOF()) {
...
long CHMSSpecialExamDialog::OnExamTypeLoadData() {
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT hfl_idx as idx, hfl_feeid as feeid, hfl_name as name, HFL_SERVPRICE as giadv FROM hms_fee_list ") \
		_T("WHERE hfl_typeid='E' and nvl(HFL_ACTIVE, 'N') = 'Y' and (hfl_deptid IN('%s', 'GL','') ) %s ORDER BY hfl_feeid "), m_szDepartmentKey, szWhere);
...
int CHMSSpecialExamDialog::OnSaveHMSSpecialExamDialog() {
	...
	if (pMF->GetCurrentDepartmentID() == _T("KB_A16"))
	{
		szSQL.Format(_T("HMS_CREATE_SPECIALEXAM_V2(%ld, '%s', %d, '%s', %d, '%s','%s',%d,%d) "),
			pMF->m_nDocumentNo, m_szDepartmentKey, str2int(m_szRoomKey), szFeeID, str2int(m_szExamTypeKey), pMF->GetCurrentUser(), pMF->GetCurrentDepartmentID(), pMF->m_nRoomID, m_nReceptidx);
	}
	else
	{
		szSQL.Format(_T("HMS_CREATE_SPECIALEXAM(%ld, '%s', %d, '%s', %d, '%s','%s',%d,%d) "),
			pMF->m_nDocumentNo, m_szDepartmentKey, str2int(m_szRoomKey), szFeeID, str2int(m_szExamTypeKey), pMF->GetCurrentUser(), pMF->GetCurrentDepartmentID(), pMF->m_nRoomID, m_nReceptidx);
	}
