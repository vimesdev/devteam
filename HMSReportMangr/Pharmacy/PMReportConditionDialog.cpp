...
long CPMSReportConditionDialog::OnStockLoadData(){
	...
	//szWhere.AppendFormat(_T(" and msl_org_id in('GL','%s') "), GetModuleID());
	//szWhere.AppendFormat(_T(" %s "), szFilter);

	//szStoragePerm = m_szStoragePerm;
...
long CPMSReportConditionDialog::OnGroupLoadData(){
	...
	szWhere.Empty();
	CString tmpStr;
	if(m_wndGroup.IsSearchKey() && str2long(m_szGroupKey) > 0){
		szWhere.Format(_T(" and mpc_product_category_id=%ld "), str2long(m_szGroupKey));
	};
	...
	_T(" where mpc_isactive='Y' and mpc_level = 1 ") \
	_T(" %s "), szWhere);
...
void CPMSReportConditionDialog::OnPrintPreviewInsSelect(){
	...
	if(bHavePrice)
	{
		szSQL.Format(_T(" SELECT * FROM") \
					...
					_T("			  product_name, product_purchase_uomid, product_classname, product_taxprice ") \
					_T(" ) tbl ") \
...
void CPMSReportConditionDialog::OnPrintImportExportAndInstockForDetailMoneyReport(bool bPreview){
	...
	szSQL.Format(_T(" SELECT * FROM") \
				...
				_T("     WHERE sitemid > 0 %s") \
				_T("     GROUP BY product_rootid, product_rootname, product_groupid, product_groupname, product_code,") \
				_T("			  product_name, product_purchase_uomid, product_classname, product_taxprice, product_saleprice1 ") \
				_T(" ) tbl ") \
				_T(" WHERE length(id) > 0 and (impqty + expqty + instockqty > 0)  ") \
				_T(" %s"), m_szFromDate, m_szToDate, szStockSel, m_szFromDate, szStockSel, szWhere, szOrderby);
	_fmsg(_T("%s"), szSQL);
