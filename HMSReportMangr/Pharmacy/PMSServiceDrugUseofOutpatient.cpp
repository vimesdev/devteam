...
CString CPMSServiceDrugUseofOutpatient::GetQueryString(){
	...
	szSQL.Format(_T(" SELECT  (select distinct hrl_name FROM hms_roomlist WHERE hrl_id = hpo_roomid AND hrl_deptid = 'C1.1') as room,") \
				...
				_T(" )") \
				_T(" GROUP BY hpo_roomid, hpo_doctor") \
				_T(" ORDER BY hpo_roomid, doctor"), szWhere);
	return szSQL;
