...
static void _OnDepartmentSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CRptBCHangThangKhoaKhamC1_1* )pWnd)->OnDepartmentSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnDepartmentSelendokFnc(CWnd *pWnd){
	((CRptBCHangThangKhoaKhamC1_1 *)pWnd)->OnDepartmentSelendok();
}
/*static void _OnDepartmentSetfocusFnc(CWnd *pWnd){
	((CRptBCHangThangKhoaKhamC1_1 *)pWnd)->OnDepartmentKillfocus();
}*/
/*static void _OnDepartmentKillfocusFnc(CWnd *pWnd){
	((CRptBCHangThangKhoaKhamC1_1 *)pWnd)->OnDepartmentKillfocus();
}*/
static long _OnDepartmentLoadDataFnc(CWnd *pWnd){
	return ((CRptBCHangThangKhoaKhamC1_1 *)pWnd)->OnDepartmentLoadData();
}
/*static void _OnDepartmentAddNewFnc(CWnd *pWnd){
	((CRptBCHangThangKhoaKhamC1_1 *)pWnd)->OnDepartmentAddNew();
}*/
...
void CRptBCHangThangKhoaKhamC1_1::OnCreateComponents(){
	m_wndReportCondition.Create(this, _T("Report Condition"), 5, 5, 490, 181);
	...
	m_wndExamRoomLabel.Create(this, _T("Exam Room"), 10, 121, 90, 146);
	m_wndExamRoom.Create(this,95, 121, 485, 146); 
	m_wndPatient.Create(this, _T("Patient"), 95, 151, 185, 176);
	m_wndTurn.Create(this, _T("Turn"), 190, 151, 280, 176);
	m_wndDrugOrder.Create(this, _T("Drug Order"), 285, 151, 375, 176);
	m_wndGoDept.Create(this, _T("Go Dept"), 395, 151, 485, 176);
	m_wndPrintPreview.Create(this, _T("Print Pre&view"), 300, 186, 410, 211);
	m_wndExport.Create(this, _T("&Export"), 415, 186, 490, 211);
...
void CRptBCHangThangKhoaKhamC1_1::OnDepartmentSelectChange(int nOldItemSel, int nNewItemSel){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
void CRptBCHangThangKhoaKhamC1_1::OnDepartmentSelendok(){
	 
}
/*void CRptBCHangThangKhoaKhamC1_1::OnDepartmentSetfocus(){
	
}*/
/*void CRptBCHangThangKhoaKhamC1_1::OnDepartmentKillfocus(){
	
}*/
long CRptBCHangThangKhoaKhamC1_1::OnDepartmentLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
...
void CRptBCHangThangKhoaKhamC1_1::OnExportSelect(){
	...
	CString szDate;
	EndWaitCursor();
	xls.Save(_T("Exports\\BAO CAO HANG THANG KHOA KHAM.xls"));
} 
...
CString CRptBCHangThangKhoaKhamC1_1::GetQueryString(){
	...
	UpdateData(true);
	szWhere.Format(_T(" AND he_examdate BETWEEN cast_string2timestamp('%s') and cast_string2timestamp('%s') AND he_deptid = '%s'"), m_szFromDate, m_szToDate, m_szDepartmentKey);
	if (m_bPatient){
		if (!szSubSQL.IsEmpty())
	...
	if (m_bTurn){
		...
		szSubSQL.AppendFormat(_T(" SELECT 2 as title, hd_docno, he_roomid, ") \
					...
					_T("		WHERE he_status IN ('P', 'T') %s"), szWhere);
	}
	...
	if (m_bDrugOrder){
		...
		szSubSQL.AppendFormat(_T(" SELECT 3 as title, hd_docno, hpo_roomid as he_roomid, ") \
						...
						_T("		AND hpo_deptid = '%s' %s"), m_szFromDate, m_szToDate, m_szDepartmentKey, szWhere);
	}
#ifdef COP
	_msg(_T("%s"), szSubSQL);
#endif
	if (m_bGoDept){
		...
		szSubSQL.AppendFormat(_T(" SELECT 4 as title, hd_docno, he_roomid,") \
					...
					_T("    and hcr_numinward > 0 AND hd_enddept = '%s'"), m_szFromDate, m_szToDate, m_szDepartmentKey);
	}
	szSQL.Format(_T(" SELECT title, hrl_type as extype, hrl_id as roomid, hrl_name as roomname,") \
					...
					_T(" 	ORDER BY title, cast(extype as integer), hrl_id"), szSubSQL, m_szDepartmentKey);_fmsg(_T("%s"), szSQL);
	return szSQL;
}