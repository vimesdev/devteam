...
long rptOperationPatientListReportDialog::OnSurgeryNameLoadData()
{
	...
	szSQL.Format(_T("SELECT hfg_id AS id, hfg_name AS name ")\
		         _T("FROM hms_feegroup WHERE substr(hfg_id, 1, 2) IN ('B4', 'B5') %s ORDER BY hfg_id"), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
...
void rptOperationPatientListReportDialog::ExportOperationPatientListReport(CString szFromDate, CString szToDate, CString szDoctor)
{
	...
	CRecord rs(&pMF->m_db);
	CString szSQL, tmpStr, szFromDateLabel, szToDateLabel, szWhere;
	CString szDate, szSysDate, szFromDateToDate, szOperation;
	szSysDate = pMF->GetSysDate(); 
	CReport rpt;
	szOperation.Empty();
...
//_fmsg(_T("%s"), szSQL);
	...
	xls.SetCellMergedColumns(0,5,9);	
