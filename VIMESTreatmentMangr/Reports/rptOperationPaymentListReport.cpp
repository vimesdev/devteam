...
long CrptOperationPaymentListReport::OnDepartmentLoadData()
{
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT sd_id AS id, sd_name as name FROM sys_dept WHERE sd_type IN ('KB', 'DT') %s ORDER BY sd_id"),
				 szWhere);
...
long CrptOperationPaymentListReport::OnOperationNameLoadData()
{
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT hfg_id AS id, hfg_name AS name FROM hms_feegroup ")\
...
void CrptOperationPaymentListReport::OnPrintSelect()
{
	...
	if (!szDeptID.IsEmpty())
	{
		szWhere.AppendFormat(_T(" AND ho_pdeptid IN (%s) "), szDeptID);
	}
	if (!szDoctorID.IsEmpty())
	{
	...
	szSQL.Format(_T(" SELECT hfe_group as group,") \
				...
				_T(" 	hfl_name as name,") \
				_T(" 	(select distinct hfg_name from hms_feegroup where hfg_id=hfe_group) as operatype,") \
				...
				_T(" 	WHERE (hfl_report <> 'N' or hfl_report Is null) and ho_status ='T' ") \
