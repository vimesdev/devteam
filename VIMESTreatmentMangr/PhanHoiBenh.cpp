...
CPhanHoiBenh::CPhanHoiBenh(){
	m_szType = _T("BENH_AN_HB");
}

CPhanHoiBenh::~CPhanHoiBenh(){
}

void CPhanHoiBenh::Init(int nMode, long nDocNo, CString szRecordNo, int nHtrIdx, int nTreatTime)
{
	CGuiView::SetMode(nMode);
	m_nDocumentNo = nDocNo;
	m_szRecordNo = szRecordNo;
...
void CPhanHoiBenh::OnCreateComponents(){
	m_wndvaolucLabel.Create(this, _T("Bệnh án làm vào lúc"), 5, 5, 165, 30);
	m_wndvaoluc.Create(this,175, 5, 325, 30); 
	m_wndvaongayLabel.Create(this, _T("Vào ngày thứ"), 590, 5, 740, 30);
	m_wndvaongay.Create(this,745, 5, 825, 30); 
	m_wndvaongay2Label.Create(this, _T("của bệnh"), 830, 5, 930, 30);
	m_wndlidoLabel.Create(this, _T("1. Lý do vào viện:"), 5, 35, 155, 60);
	m_wndlido.Create(this,5, 65, 930, 165, 1, 0, 1); 
	m_wndbenhsuLabel.Create(this, _T("2. Bệnh sử:"), 5, 170, 155, 195);
	m_wndbenhsu.Create(this,5, 200, 930, 350, 1, 0, 1); 
	m_wndtiensuLabel.Create(this, _T("3. Tiền sử:"), 5, 355, 155, 380);
	m_wndbtLabel.Create(this, _T("+ Bản thân:"), 5, 385, 155, 410);
	m_wndbthan.Create(this,5, 415, 465, 515, 1, 0, 1); 
	m_wndgdLabel.Create(this, _T("+ Gia đình:"), 470, 385, 620, 410);
...
void CPhanHoiBenh::GetDataToScreen(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szType, tmpStr, szUID, szData, szBanthan, szDiungthuoc, szDiungthuocchitiet, szStr;
	int n = 0;
	CString szFilter;
	...
	else
	{
		...
		szSQL.Format(_T("SELECT max(hde_treattime) as ntreat FROM hms_doc_emr ") \
			...
			_T(" AND hde_type='BENH_AN_HB' AND hde_treattime <= %d "), m_nDocumentNo, m_szRecordNo, m_nTreatTime);
		
		rs.ExecSQL(szSQL);
		rs.GetValue(_T("ntreat"), nMaxTreattime);
		szSQL.Format(_T("SELECT COUNT(*) AS solan FROM hms_doc_emr ") \
			_T(" WHERE hde_docno=%ld and hde_name in ('%s', 'BANGT') ") \
			_T(" AND hde_type='BENH_AN_HB' AND hde_treattime = %d "), m_nDocumentNo, m_szRecordNo, nMaxTreattime);
		rs.ExecSQL(szSQL);
		...
		if(n > 0)
		{
			for (int i = 0; i < n; i++)
			{
				szSQL.Format(_T("SELECT to_char(hde_value) as hde_value FROM hms_doc_emr WHERE hde_docno=%ld and hde_name in ('%s', 'BANGT') ") \
					_T(" and hde_type='BENH_AN_HB' and hde_refidx = %d AND hde_treattime = %d "), m_nDocumentNo, m_szRecordNo, i, nMaxTreattime);
				rs.ExecSQL(szSQL);
	...
	//Thêm đoạn xử lý bệnh nhân điều trị nội trú khoa cấp cứu
	//Bước 1: Check xem đã tạo dữ liệu EMR của khoa cấp cứu chưa	
	if ((pMF->UserC13ExamTab() == _T("Y") && (pMF->m_szDept == _T("C1.3") || pMF->m_szDept == _T("C1.3NT") || pMF->GetCurrentUser() == _T("lttmaivimes"))) )
	{
	szBanthan.Empty();
	szDiungthuoc.Empty();
	szDiungthuocchitiet.Empty();

	szSQL.Format(_T("hms_check_is_create_hoibenh(%ld) "), pMF->m_nDocumentNo);
		...
		if(!rs.IsEOF())
		{	
			rs.GetValue(_T("LYDOVAOVIEN"), m_szlido); 
			rs.GetValue(_T("BENHSU"), m_szbenhsu);
			rs.GetValue(_T("TIENSU_GIADINH"), m_szgdinh);
			rs.GetValue(_T("TIENSU_BANTHAN"), szBanthan);
			rs.GetValue(_T("TIENSU_DIUNGTHUOC"), szDiungthuoc);
			rs.GetValue(_T("TIENSU_DIUNGTHUOC_CHITIET"), szDiungthuocchitiet);

			if (!szBanthan.IsEmpty())
			{
				
				szStr.AppendFormat(_T("%s. "), szBanthan);
			}

			if (!szDiungthuoc.IsEmpty())
			{
				
				szStr.AppendFormat(_T("%s. "), szDiungthuoc);
			}
			if (!szDiungthuocchitiet.IsEmpty())
			{
				
				szStr.AppendFormat(_T("%s"), szDiungthuocchitiet);
			}
	...
	//Bước 2: Nếu chưa tạo thì đẩy giá trị default vào
}
...
int CPhanHoiBenh::OnSavePhanHoiBenh(){
	...
	szSQL.Format(_T("DELETE FROM hms_doc_emr WHERE hde_docno=%ld and hde_name='%s' and hde_type='BENH_AN_HB' AND hde_htridx = %d "), m_nDocumentNo, m_szRecordNo, m_nHtrIdx);
	pMF->ExecSQL(szSQL);
	...
	for (int i =0; i < strList.GetCount();i++)
	{
		...
		szSQL.Format(_T("HMS_DOC_EMR_CREATE('BENH_AN_HB', %ld, '%s', %d, %d, '%s', '%s', '%s', %d, %d) "),
...
void CPhanHoiBenh::Refresh(int nTreatTime)
{
	m_nTreatTime = nTreatTime;
	GetDataToScreen();
	SetMode(VM_VIEW);
