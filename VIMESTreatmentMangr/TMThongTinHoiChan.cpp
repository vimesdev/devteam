...
void CTMThongTinHoiChan::OnPrint(){
	...
	rptHeader->SetValue(_T("diagnostic"), m_szChanDoan);
...
void CTMThongTinHoiChan::OnPrint2(){
	...
	if (nCount <= 0)
	{
		szSQL.Format(_T("SELECT hde_value, get_patientname(hde_docno) as pname") \
			_T(" FROM hms_doc_emr where hde_docno = %ld and hde_type = 'HOI_CHAN' and hde_deptid = '%s'") \
			, m_nDocumentNo, m_szDeptID);
