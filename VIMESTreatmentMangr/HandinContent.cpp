...
static void _OnCCandfreefoodSelectFnc(CWnd *pWnd){
	 ((CHandinContent*)pWnd)->OnCCandfreefoodSelect();
}
static void _OnHTandfreefoodfeeSelectFnc(CWnd *pWnd)
{
	 ((CHandinContent*)pWnd)->OnHTandfreefoodfeeSelect();
}

static void _OnSolderFeeFoodFriceSelectFnc(CWnd *pWnd)
{
	 ((CHandinContent*)pWnd)->OnSolderFeeFoodFriceSelect();
}
static void _OnGeneralFeeFoodFriceSelectFnc(CWnd *pWnd)
{
    ((CHandinContent *)pWnd)->OnGeneralFeeFoodFriceSelect();
}
...
{
    /*m_wndHandinContent.Create(this, _T("Kiểm soát chế độ chính sách"), 0,
                                   3, 560, 403);
	...
	m_wndOtherNote.Create(this,5, 316, 550, 366, 1, 0, 1); 
	m_wndAdd.Create(this, _T("&Add"), 216, 406, 296, 431);
    m_wndEdit.Create(this, _T("&Edit"), 300, 406, 380, 431);
    m_wndSave.Create(this, _T("&Save"), 385, 406, 465, 431);
    m_wndCancel.Create(this, _T("&Cancel"), 470, 406, 550, 431);
	m_wndCCandfreefood.Create(this, _T("Bệnh nhân có thẻ CC1 nhưng không được miễn tiền ăn"), 5, 221, 545, 246);
	m_wndHTandfreefoodfee.Create(this, _T("Bệnh nhân có thẻ HT1/MS1 và được miễn tiền ăn"), 5, 254, 545, 279);
	m_wndSolderFeeFoodFrice.Create(this, _T("Tính phí tiền ăn như BN quân(thẻ HT1 hoặc đối tượng đặc biệt)"), 5, 285, 545, 310);
    m_wndGeneralFeeFoodFrice.Create(this, _T("Tính phí như BN cấp tướng hưu (thẻ HT)"), 5, 315, 545, 340);
    m_wndOtherNote.Create(this, 5, 347, 550, 397);*/
    m_wndHandinContent.Create(this, _T("Kiểm soát chế độ chính sách"), 0, 3,
                              560, 435);
    m_wndDocumentNoLabel.Create(this, _T("Document No"), 5, 27, 85, 52);
    m_wndDocumentNo.Create(this, 90, 27, 210, 52);
    m_wndOutDateLabel.Create(this, _T("Out Date"), 340, 27, 420, 52);
    ...
    m_wndSeverseDisease.Create(this, _T("Severse Disease"), 5, 180, 210, 205);
    m_wndZviettelCo.Create(this, _T("Zviettel Co"), 340, 180, 545, 205);
    m_wndOtherNote.Create(this, 5, 347, 550, 397, 1, 0, 1);
    m_wndAdd.Create(this, _T("&Add"), 216, 406, 296, 431);
    m_wndEdit.Create(this, _T("&Edit"), 300, 406, 380, 431);
    m_wndSave.Create(this, _T("&Save"), 385, 406, 465, 431);
    m_wndCancel.Create(this, _T("&Cancel"), 470, 406, 550, 431);
    m_wndCCandfreefood.Create(
        this, _T("Bệnh nhân có thẻ CC1 nhưng không được miễn tiền ăn"), 5, 221,
        545, 246);
    m_wndHTandfreefoodfee.Create(
        this, _T("Bệnh nhân có thẻ HT1/MS1 và được miễn tiền ăn"), 5, 254, 545,
        279);
    m_wndSolderFeeFoodFrice.Create(
        this,
        _T("Tính phí tiền ăn như BN quân(thẻ HT1 hoặc đối tượng đặc biệt)"), 5,
        285, 545, 310);
    m_wndGeneralFeeFoodFrice.Create(
        this, _T("Tính phí như BN cấp tướng hưu (thẻ HT)"), 5, 315, 545, 340);
	SetWindowFont(&m_wndDocumentNo, GetFaceName(), GetFaceSize()+5, true);	
...
void CHandinContent::OnInitializeComponents(){
	...
	m_hms_clinical_recordTbl.AddField(_T("HCR_CCFREEFOOD"), dfText, 1);
	m_hms_clinical_recordTbl.AddField(_T("HCR_HTFREEFOOD"), dfText, 1);
	m_hms_clinical_recordTbl.AddField(_T("HCR_SOLDER_FEEFOOD_PRICE"), dfText, 1);
    m_hms_clinical_recordTbl.AddField(_T("HCR_GENERAL_FEEFOOD_PRICE"), dfText, 1);
...
void CHandinContent::OnSetWindowEvents(){
	...
	m_wndCCandfreefood.SetEvent(WE_CLICK, _OnCCandfreefoodSelectFnc);
	m_wndHTandfreefoodfee.SetEvent(WE_CLICK, _OnHTandfreefoodfeeSelectFnc);
	m_wndSolderFeeFoodFrice.SetEvent(WE_CLICK, _OnSolderFeeFoodFriceSelectFnc);
    ...
    m_wndGeneralFeeFoodFrice.SetEvent(WE_CLICK, _OnGeneralFeeFoodFriceSelectFnc);
	//SetMode(VM_NONE);
	GetDataToScreen();
...
void CHandinContent::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Check(pDX, m_wndCCandfreefood.GetDlgCtrlID(), m_bCCandfreefood);
	DDX_Check(pDX, m_wndHTandfreefoodfee.GetDlgCtrlID(), m_bHTandfreefoodfee);
	DDX_Check(pDX, m_wndSolderFeeFoodFrice.GetDlgCtrlID(), m_bSolderFeeFoodFrice);
    DDX_Check(pDX, m_wndGeneralFeeFoodFrice.GetDlgCtrlID(), m_bGeneralFeeFoodFrice);
...
void CHandinContent::GetDataToScreen()
{
	...
	szSQL.Format(_T("SELECT * FROM hms_handin_content LEFT JOIN hms_doc ON(hd_docno = hhc_docno) left join HMS_CLINICAL_RECORD ON (hd_docno=hcr_docno) WHERE hhc_docno = '%s' "), m_szDocumentNo);
	rs.ExecSQL(szSQL);
	//_msg(_T("%s"), m_szDocumentNo);
	if(!rs.IsEOF())
	{
		m_szDocumentNo,
		...
		rs.GetValue(_T("HCR_CCFREEFOOD"), tmpStr);
		m_bCCandfreefood = false;
		if(tmpStr == _T("Y"))
			m_bCCandfreefood = true;

		rs.GetValue(_T("HCR_HTFREEFOOD"), tmpStr);
		m_bHTandfreefoodfee = false;
		if(tmpStr == _T("Y"))
			m_bHTandfreefoodfee = true;

		rs.GetValue(_T("HCR_SOLDER_FEEFOOD_PRICE"), tmpStr);
		m_bSolderFeeFoodFrice = false;
		if(tmpStr == _T("Y"))
			m_bSolderFeeFoodFrice = true;

		rs.GetValue(_T("HCR_GENERAL_FEEFOOD_PRICE"), tmpStr);
        m_bGeneralFeeFoodFrice = false;
        if (tmpStr == _T("Y"))
            m_bGeneralFeeFoodFrice = true;
...
void CHandinContent::GetScreenToData(){
	...
	tmpStr = _T("N");
	if(m_bCCandfreefood) tmpStr = _T("Y");
	m_hms_clinical_recordTbl.SetValue(_T("HCR_CCFREEFOOD"), tmpStr);

	tmpStr = _T("N");
	if(m_bHTandfreefoodfee) tmpStr = _T("Y");
	m_hms_clinical_recordTbl.SetValue(_T("HCR_HTFREEFOOD"), tmpStr);


	tmpStr = _T("N");
	if(m_bSolderFeeFoodFrice) tmpStr = _T("Y");
	m_hms_clinical_recordTbl.SetValue(_T("HCR_SOLDER_FEEFOOD_PRICE"), tmpStr);


	tmpStr = _T("N");
    if (m_bGeneralFeeFoodFrice)
        tmpStr = _T("Y");
    m_hms_clinical_recordTbl.SetValue(_T("HCR_GENERAL_FEEFOOD_PRICE"), tmpStr);
...
int CHandinContent::OnOutDateCheckValue(){
	...
	void CHandinContent::OnCCandfreefoodSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CHandinContent::OnHTandfreefoodfeeSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();

}

void CHandinContent::OnSolderFeeFoodFriceSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
}

void CHandinContent::OnGeneralFeeFoodFriceSelect()
{
    CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd();
}
...
int CHandinContent::OnSaveHandinContent(){
	...
	if(GetMode() == VM_ADD)
	{
		...
		szWhere.Format(_T(" WHERE hcr_docno='%s'"), m_szDocumentNo);
