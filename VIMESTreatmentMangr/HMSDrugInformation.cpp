...
static void _OnNotApprovedSelectFnc(CWnd *pWnd)
{
    ((CHMSDrugInformation *)pWnd)->OnNotApprovedSelect();
}
...
//	m_wndPrescription.Create(this, _T("&Prescription"), 515, 550, 610, 575);
    m_wndNotApproved.Create(this, _T("Chưa cấp phát"), 381, 550, 561, 575);
...
//	m_wndToDate.SetCheckValue(true);
	...
	m_wndList.InsertColumn(13, _T("Phiếu trả lại"), CFMT_TEXT, 100);
	m_wndList.InsertColumn(14, _T("Đối tượng"), CFMT_TEXT, 100);
	m_wndList.InsertColumn(15, _T("Mã hàng"), CFMT_TEXT, 60);
	m_wndList.InsertColumn(16, _T("Mã ATC_CODE"), CFMT_TEXT, 100);
    m_wndList.InsertColumn(17, _T("Product Item ID"), CFMT_TEXT, 50);
...
void CHMSDrugInformation::OnSetWindowEvents(){
	...
	m_wndList.AddEvent(0, _T("-"), NULL);
	m_wndList.AddEvent(7, _T("Tạo phiếu lĩnh máu & chế phẩm máu"), _OnCreateBloodOrderFnc);
	m_wndList.AddEvent(0, _T("-"), NULL);
...
//	m_wndDrugLog.SetEvent(WE_CLICK, _OnDrugLogSelectFnc);
    ...
    m_wndNotApproved.SetEvent(WE_CLICK, _OnNotApprovedSelectFnc);
...
//	pMF->ExecDML(szSQL);
	...
	if (m_bNotApproved)
    {
        szFilter.AppendFormat(_T(" and hms_get_approve_status(hpol_docno, hpol_orderlineid, hpol_product_item_id) <=0 "));
    }
	...
	szSQL.Format(_T(" SELECT ") \
				...
				_T("   trim(product_atccode) as atc_code, ") \
				_T("   1 as is_approved, hpol_product_item_id as product_item_id ") \
				...
				_T("   product_name,") \
				_T("   product_uomname,") \
				_T("   hpol_unitprice, hpo_docno, HPO_OBJECT_ID, product_atccode, ")
				_T("   hpol_docno, hpol_orderlineid, hpol_product_item_id ") \
				_T(" ORDER BY deptid,") \
				_T("   issuedate,") \
				_T(" hpo_storage_id, ") \
				_T("   drugname,") \
...
//_msg(_T("%s"), szSQL);
	...
	{ 
        ...
        CString szIssueQty, szOrderQty, szIsApprove;
		rs.GetValue(_T("issueqty"), szIssueQty);
		rs.GetValue(_T("orderqty"), szOrderQty);
        rs.GetValue(_T("is_approved"), szIsApprove);
        ...
        {
            m_wndList.SetSubItemBkColor(nItem, 3, RGB(255, 255, 153));
            m_wndList.SetSubItemTextColor(nItem, 3, RGB(0, 0, 0));
        }
		rs.MoveNext();
	}
	...
	szSQL.Format(_T(" SELECT ") \
				...
				_T("   HMS_GETOBJECTNAME(HPO_OBJECT_ID) as objectname, ") \
				_T("   trim(product_atccode) as atc_code, ") \
				_T("   hms_get_approve_status(hpol_docno, hpol_orderlineid, hpol_product_item_id) as is_approve, hpol_product_item_id as product_item_id ") \
				...
				_T("   product_name,") \
				_T("   product_uomname,") \
				_T("   hpol_unitprice,hpol_product_id, hpo_docno, HPO_OBJECT_ID, product_atccode, hpol_docno, hpol_orderlineid, hpol_product_item_id ") \
				_T(" ORDER BY deptid,") \
				_T("   issuedate,") \
				_T(" hpo_storage_id, ") \
				_T("   drugname,") \
...
_tprintf(_T("\n2\n"));
	...
	while(!rs.IsEOF()){ 
		...
		rs.GetValue(_T("issueqty"), szIssueQty);
		rs.GetValue(_T("orderqty"), szOrderQty);
        rs.GetValue(_T("is_approve"), szIsApprove);
		if(str2float(szIssueQty) <=0)
		{
			szIssueQty = _T("0");
		}
        ...
        if (szIsApprove == _T("0"))
		{
            m_wndList.SetSubItemBkColor(nItem, 3, RGB(255, 255, 153), false);            
            m_wndList.SetSubItemTextColor(nItem, 3, RGB(0, 0, 0), false);
		}
...
void CHMSDrugInformation::OnConfirmIssueDrug(){
	...
	dlg.m_nOrderQuantity = str2double(m_wndList.GetItemText(nSel, 5));
	dlg.m_nIssueQuantity = str2double(m_wndList.GetItemText(nSel, 6));
	dlg.m_nUnitPrice = str2double(m_wndList.GetItemText(nSel, 7));
    dlg.m_nProductItemID = str2double(m_wndList.GetItemText(nSel, 17));
	_tprintf(_T("\nunit:%f|text:%s"), dlg.m_nUnitPrice, m_wndList.GetItemText(nSel, 7));
	if(dlg.DoModal() == IDOK){
//		OnDrugListLoadData();
	//	OnListLoadData();
...
void CHMSDrugInformation::OnNotApprovedSelect()
{
    CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
    OnListLoadData();
}