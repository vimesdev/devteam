...
int CHMSAdmission::OnSaveHMSAdmission(){
	...
	if(res == -2)
	{
		ShowMessageBox(_T("Đang tồn tại hồ sơ điều trị tại khoa khác, không cho phép chuyển khoa"));
		return -1;
	}
	if(res == -3)
	{
		ShowMessageBox(_T("Bệnh nhân chưa được tiếp nhận vào khoa, không cho phép chuyển khoa"));
		return -1;
	}
	if(res == -4)
	{
		ShowMessageBox(_T("Bệnh nhân đã thanh toán, không cho phép chuyển khoa"));
		return -1;
	}
	if(res == -1)
	{
		ShowMessageBox(_T("Không cho phép chuyển khoa"));
		return -1;
	}
