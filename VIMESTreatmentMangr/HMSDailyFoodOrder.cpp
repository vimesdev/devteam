...
static void _OnBaoSuaSelectFnc(CWnd *pWnd){
	 ((CHMSDailyFoodOrder*)pWnd)->OnBaoSuaSelect();
}

static void _OnHaucannnSelectFnc(CWnd *pWnd){
	 ((CHMSDailyFoodOrder*)pWnd)->OnHaucannnSelect();
}

static void _OnHaucanSelectFnc(CWnd *pWnd){
	 ((CHMSDailyFoodOrder*)pWnd)->OnHaucanSelect();
}

static void _OnBaochinhSelectFnc(CWnd *pWnd){
	 ((CHMSDailyFoodOrder*)pWnd)->OnBaochinhSelect();
}
...
{	
	...
	m_wndGroupListRation.Create(this, _T("Food Type List"), 5, 185, 500, 606);
	m_wndOrderList.Create(this,15, 210, 495, 601);
	...
	m_wndOrderNoLabel.Create(this, _T("Số phiếu"), 8, 30, 98, 55);
	m_wndOrderNo.Create(this,106, 30, 237, 55); 
	m_wndFind.Create(this, _T("&Find"), 830, 210, 900, 235);
	m_wndOrderDateLabel.Create(this, _T("Order Date"), 8, 59, 98, 84);
	m_wndOrderDate.Create(this,106, 59, 237, 84); 
	m_wndIssueDateLabel.Create(this, _T("Issue Date"), 242, 59, 356, 84);
	m_wndIssueDate.Create(this,361, 59, 485, 84); 
	m_wndStatusLabel.Create(this, _T("Status"), 242, 30, 356, 55);
	m_wndStatus.Create(this,361, 30, 485, 55); 
	m_wndNoteLabel.Create(this, _T("Note"), 8, 90, 98, 115);
	m_wndNote.Create(this,106, 90, 485, 115); 
	m_wndHaucan.Create(this, _T("BA hậu cần"), 106, 127, 206, 152);
	m_wndPrimary.Create(this, _T("Primary"), 8, 126, 98, 151);
	m_wndBaoSua.Create(this, _T("Báo Sữa"), 342, 127, 435, 152);
	m_wndHaucannn.Create(this, _T("BA hậu cần NN"), 212, 127, 336, 152);

	m_wndAdd.Create(this, _T("&Add Sheet"), 15, 155, 105, 180);
	m_wndEdit.Create(this, _T("&Edit"), 110, 155, 200, 180);
	m_wndDelete.Create(this, _T("&Delete"), 205, 155, 295, 180);
	m_wndSave.Create(this, _T("&Save"), 300, 155, 390, 180);
	m_wndCancel.Create(this, _T("&Cancel"), 395, 155, 485, 180);
...
//	m_wndOrderNo.ModifyStyle(0, ES_NUMBER);
	...
	m_feefoodTbl.AddField(_T("HFOS_DEPTTYPE"), dfText, 5);
	m_feefoodTbl.AddField(_T("HFOS_MILK"), dfText, 4);
	m_feefoodTbl.AddField(_T("HFOS_DEPTTYPE_NN"), dfText, 5);
...
void CHMSDailyFoodOrder::OnSetWindowEvents(){
	...
	*/
	...
	m_wndHaucan.SetEvent(WE_CLICK, _OnHaucanSelectFnc);
	m_wndHaucannn.SetEvent(WE_CLICK, _OnHaucannnSelectFnc);
	m_wndPrimary.SetEvent(WE_CLICK, _OnBaochinhSelectFnc);
...
void CHMSDailyFoodOrder::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Check(pDX, m_wndHaucannn.GetDlgCtrlID(), m_bHaucannn);
}
void CHMSDailyFoodOrder::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT  ") \
		...
		_T("HFOS_ISPRIMARY as isprimary, HFOS_DEPTTYPE, HFOS_MILK,  HFOS_DEPTTYPE_NN ") \
	...
	if(!rs.IsEOF())
	{
		...
		rs.GetValue(_T("hfos_depttype_nn"), tmpStr);
		m_bHaucannn = FALSE;
...
void CHMSDailyFoodOrder::GetScreenToData(){
	...
	m_feefoodTbl.SetValue(_T("HFOS_DEPTTYPE_NN"), m_bHaucannn?_T("HCNN"):_T(""));	
...
void CHMSDailyFoodOrder::OnOrderListDblClick(){
	...
	void CHMSDailyFoodOrder::OnBaoSuaSelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
long CHMSDailyFoodOrder::OnPatientListLoadData()
{
	...
	szSQL.Format(_T(" SELECT distinct hfo_docno as docno, hfo_orderid as orderid,") \
		...
		_T(" 	hfo_roomid||'.'||hfo_bedid as bedid, hp_rank,") \
...
int CHMSDailyFoodOrder::OnRationListAddEntry()
{
	...
	else
	{
		...
		newPopup->m_szBatchNo = m_szBatchNo;
		newPopup->m_bHaucan = m_bHaucan;
		newPopup->m_bHaucannn = m_bHaucannn;
		//newPopup->ShowPopup(&m_wndRationList);
		newPopup->DoModal();
	}
	return 0;
...
void CHMSDailyFoodOrder::OnPrintSelect(){
	...
	while(!rs.IsEOF())
	{
		...
		//Báo ăn hậu cần người nhà in thêm mẫu này
		
		if(m_bHaucannn)
		{
			OnPrintDelivery_v2(m_nOID, szOrderDate, bPreview);			
		}
...
void CHMSDailyFoodOrder::GetOrderStatus()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
...
void CHMSDailyFoodOrder::OnBaochinhSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(TRUE);
...
void CHMSDailyFoodOrder::OnHaucanSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(TRUE);
...
void CHMSDailyFoodOrder::OnHaucannnSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(TRUE);
...
void CHMSDailyFoodOrder::OnPrintDelivery_v2(long nTransactionID, LPCTSTR lpszOrderDate, bool bPreview)
{
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	CReport rpt;	
	CString tmpStr, szSQL, szWhere;
	...
	//szWhere.Format(_T(" and nvl(HFOS_DEPTTYPE_NN,'N') = 'Y' "));	
...
void CHMSDailyFoodOrder::OnResizeLayout() {
	...
	AddResize(&m_wndRationInformation, 100, 100);
	AddResize(&m_wndRationList, 100, 100);*/
}