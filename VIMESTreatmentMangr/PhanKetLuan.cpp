...
void CPhanKetLuan::GetDataToScreen()
{
	...
	szSQL.Format(_T("hms_check_is_create_ketluan(%ld) "), pMF->m_nDocumentNo);
		...
		if(!rs.IsEOF())
		{	
			rs.GetValue(_T("BENHSU"), m_szbenhan); 
			rs.GetValue(_T("chandoansobo"), m_szcdsb);
			rs.GetValue(_T("chandoanphanbiet"), m_szcdpb);
			rs.GetValue(_T("chandoanxacdinh"), m_szcdxd);
			rs.GetValue(_T("kehoachxn"), m_szkhxn);
			rs.GetValue(_T("kehoachdieutri"), m_szkhdt);
			rs.GetValue(_T("tienluong"), m_sztluong);
		}
		}
		else
		{
