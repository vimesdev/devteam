...
static int _OnFromDate1CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnFromDate1CheckValue();
} 
static int _OnToDate1CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnToDate1CheckValue();
}


static int _OnFromDate2CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnFromDate2CheckValue();
} 
static int _OnToDate2CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnToDate2CheckValue();
}


static int _OnFromDate3CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnFromDate3CheckValue();
} 
static int _OnToDate3CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnToDate3CheckValue();
}

static int _OnFromDate4CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnFromDate4CheckValue();
}

static int _OnToDate4CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnToDate4CheckValue();
}

static int _OnFromDate5CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnFromDate5CheckValue();
} 

static int _OnToDate5CheckValueFnc(CWnd *pWnd)
{
	return ((CHMSDynBedLevelDialog *)pWnd)->OnToDate5CheckValue();
}
...
void CHMSDynBedLevelDialog::OnCreateComponents()
{
	...
	m_wndLevel1Label.Create(this, _T("Level 1"), 10, 30, 66, 55);
	m_wndLevel1.Create(this,67, 30, 360, 55); 
	m_wndFromDate1Label.Create(this, _T("From Date1"), 363, 30, 443, 55);
	m_wndBedInformation.Create(this, _T("Bed Information"), 5, 5, 998, 180);
	m_wndFromDate1.Create(this,444, 30, 564, 55); 
	m_wndToDate1Label.Create(this, _T("To Date1"), 568, 30, 648, 55);
	m_wndToDate1.Create(this,654, 30, 774, 55); 
	m_wndDays1Label.Create(this, _T("Days"), 778, 30, 843, 55);
	m_wndDays1.Create(this,844, 30, 879, 55); 
	m_wndPayrate1Label.Create(this, _T("Payrate"), 884, 30, 943, 55);
	m_wndPayrate1.Create(this,944, 30, 999, 55); 
	m_wndLevel2Label.Create(this, _T("Level 2"), 10, 60, 66, 85);
	m_wndLevel2.Create(this,67, 60, 360, 85); 
	m_wndFromDate2Label.Create(this, _T("From Date2"), 363, 60, 443, 85);
	m_wndFromDate2.Create(this,444, 60, 564, 85); 
	m_wndToDate2Label.Create(this, _T("To Date2"), 568, 60, 648, 85);
	m_wndToDate2.Create(this,654, 60, 774, 85); 
	m_wndDays2Label.Create(this, _T("Days"), 778, 60, 843, 85);
	m_wndDays2.Create(this,844, 60, 879, 85); 
	m_wndPayrate2Label.Create(this, _T("Payrate"), 884, 60, 943, 85);
	m_wndPayrate2.Create(this,944, 60, 999, 85); 
	m_wndLevel3Label.Create(this, _T("Level 3"), 10, 90, 66, 115);
	m_wndLevel3.Create(this,67, 90, 360, 115); 
	m_wndFromDate3Label.Create(this, _T("From Date3"), 363, 90, 443, 115);
	m_wndFromDate3.Create(this,444, 90, 564, 115); 
	m_wndToDate3Label.Create(this, _T("To Date3"), 568, 90, 648, 115);
	m_wndToDate3.Create(this,654, 90, 774, 115); 
	m_wndDays3Label.Create(this, _T("Days"), 778, 90, 843, 115);
	m_wndDays3.Create(this,844, 90, 879, 115); 
	m_wndPayrate3Label.Create(this, _T("Payrate"), 884, 90, 943, 115);
	m_wndPayrate3.Create(this,944, 90, 999, 115); 
	m_wndLevel4Label.Create(this, _T("Level 4"), 10, 120, 66, 145);
	m_wndLevel4.Create(this,67, 120, 360, 145); 
	m_wndFromDate4Label.Create(this, _T("From Date4"), 363, 120, 443, 145);
	m_wndFromDate4.Create(this,444, 120, 564, 145); 
	m_wndToDate4Label.Create(this, _T("To Date4"), 568, 120, 648, 145);
	m_wndToDate4.Create(this,654, 120, 774, 145); 
	m_wndDays4Label.Create(this, _T("Days"), 778, 120, 843, 145);
	m_wndDays4.Create(this,844, 120, 879, 145); 
	m_wndPayrate4Label.Create(this, _T("Payrate"), 884, 120, 943, 145);
	m_wndPayrate4.Create(this,944, 120, 999, 145); 
	m_wndLevel5Label.Create(this, _T("Level 5"), 10, 150, 66, 175);
	m_wndLevel5.Create(this,67, 150, 360, 175); 
	m_wndFromDate5Label.Create(this, _T("From Date5"), 363, 150, 443, 175);
	m_wndFromDate5.Create(this,444, 150, 564, 175); 
	m_wndToDate5Label.Create(this, _T("To Date5"), 568, 150, 648, 175);
	m_wndToDate5.Create(this,654, 150, 774, 175); 
	m_wndDays5Label.Create(this, _T("Days"), 778, 150, 843, 175);
	m_wndDays5.Create(this,844, 150, 879, 175); 
	m_wndPayrate5Label.Create(this, _T("Payrate"), 884, 150, 943, 175);
	m_wndPayrate5.Create(this,944, 150, 999, 175); 
	m_wndApply.Create(this, _T("&Apply"), 833, 186, 913, 211);
	m_wndClose.Create(this, _T("&Close"), 918, 186, 998, 211);
...
void CHMSDynBedLevelDialog::OnInitializeComponents(){
	...
	//m_wndFromDate1.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndFromDate1.SetCheckValue(true);
	//m_wndToDate1.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndToDate1.SetCheckValue(true);


	//m_wndFromDate2.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndFromDate2.SetCheckValue(true);
	//m_wndToDate2.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndToDate2.SetCheckValue(true);


	//m_wndFromDate3.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndFromDate3.SetCheckValue(true);
	//m_wndToDate3.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndToDate3.SetCheckValue(true);

	//m_wndFromDate4.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndFromDate4.SetCheckValue(true);
	//m_wndToDate4.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndToDate4.SetCheckValue(true);

	//m_wndFromDate5.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndFromDate5.SetCheckValue(true);
	//m_wndToDate5.SetMax(CDate(pMF->GetSysDateTime()));
	m_wndToDate5.SetCheckValue(true);

	m_wndLevel1.InsertColumn(0, _T("ID"), CFMT_NUMBER, 50);
	m_wndLevel1.InsertColumn(1, _T("Description"), CFMT_TEXT, 450);
	...
	m_wndLevel2.InsertColumn(1, _T("Description"), CFMT_TEXT, 450);
	...
	m_wndLevel3.InsertColumn(1, _T("Description"), CFMT_TEXT, 450);
	...
	m_wndLevel4.InsertColumn(1, _T("Description"), CFMT_TEXT, 450);
	...
	m_wndLevel5.InsertColumn(1, _T("Description"), CFMT_TEXT, 450);
	...
	m_hms_bed_itemsTbl.AddField(_T("hbi_fromdate"), dfDateTime); 
	m_hms_bed_itemsTbl.AddField(_T("hbi_todate"), dfDateTime);
	m_hms_bed_itemsTbl.AddField(_T("hbi_treattime"), dfInteger);
...
void CHMSDynBedLevelDialog::OnSetWindowEvents()
{
	...
	m_wndFromDate1.SetEvent(WE_CHECKVALUE, _OnFromDate1CheckValueFnc);	
	m_wndToDate1.SetEvent(WE_CHECKVALUE, _OnToDate1CheckValueFnc);
	
	m_wndFromDate2.SetEvent(WE_CHECKVALUE, _OnFromDate2CheckValueFnc);	
	m_wndToDate2.SetEvent(WE_CHECKVALUE, _OnToDate2CheckValueFnc);

	m_wndFromDate3.SetEvent(WE_CHECKVALUE, _OnFromDate3CheckValueFnc);	
	m_wndToDate3.SetEvent(WE_CHECKVALUE, _OnToDate3CheckValueFnc);

	m_wndFromDate4.SetEvent(WE_CHECKVALUE, _OnFromDate4CheckValueFnc);	
	m_wndToDate4.SetEvent(WE_CHECKVALUE, _OnToDate4CheckValueFnc);

	m_wndFromDate5.SetEvent(WE_CHECKVALUE, _OnFromDate5CheckValueFnc);	
	m_wndToDate5.SetEvent(WE_CHECKVALUE, _OnToDate5CheckValueFnc);
	
	/*m_szFromDate1 = m_szToDate1 = pMF->GetSysDateTime();
	


	m_szFromDate2 = m_szToDate2 = pMF->GetSysDateTime();
	


	m_szFromDate3 = m_szToDate3 = pMF->GetSysDateTime();
	
	m_szFromDate4 = m_szToDate4 = pMF->GetSysDateTime();
	

	m_szFromDate5 = m_szToDate5 = pMF->GetSysDateTime();*/
...
void CHMSDynBedLevelDialog::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_TextEx(pDX, m_wndFromDate1.GetDlgCtrlID(), m_szFromDate1);
	DDX_TextEx(pDX, m_wndToDate1.GetDlgCtrlID(), m_szToDate1);

	DDX_TextEx(pDX, m_wndFromDate2.GetDlgCtrlID(), m_szFromDate2);
	DDX_TextEx(pDX, m_wndToDate2.GetDlgCtrlID(), m_szToDate2);

	DDX_TextEx(pDX, m_wndFromDate3.GetDlgCtrlID(), m_szFromDate3);
	DDX_TextEx(pDX, m_wndToDate3.GetDlgCtrlID(), m_szToDate3);

	DDX_TextEx(pDX, m_wndFromDate4.GetDlgCtrlID(), m_szFromDate4);
	DDX_TextEx(pDX, m_wndToDate4.GetDlgCtrlID(), m_szToDate4);
	
	DDX_TextEx(pDX, m_wndFromDate5.GetDlgCtrlID(), m_szFromDate5);
	DDX_TextEx(pDX, m_wndToDate5.GetDlgCtrlID(), m_szToDate5);
...
void CHMSDynBedLevelDialog::GetScreenToData(){
	...
	m_hms_bed_itemsTbl.SetValue(_T("hbi_treattime"), pMF->GetTreatTime());
...
int CHMSDynBedLevelDialog::SetMode(int nMode){
 		...
 		case VM_ADD: 
			...
			m_wndFromDate1.EnableWindow(true);
			/*m_wndFromDate2.EnableWindow(true);
			m_wndFromDate3.EnableWindow(true);
			m_wndFromDate4.EnableWindow(true);
			m_wndFromDate5.EnableWindow(true);*/
			
			m_wndToDate1.EnableWindow(true);
			/*m_wndToDate2.EnableWindow(true);
			m_wndToDate3.EnableWindow(true);
			m_wndToDate4.EnableWindow(true);
			m_wndToDate5.EnableWindow(true);*/
...
int CHMSDynBedLevelDialog::OnFromDate1CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);	
...
int CHMSDynBedLevelDialog::OnToDate1CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szFromDate1, m_szToDate1);
	int ret = str2int(pMF->ExecDML(szSQL));
	if(ret == -1)
	{
		m_wndToDate1.SetToolTipMessage(_T("Thời gian kết thúc phải lớn hơn thời gian bắt đầu"));
		return -1;
	}
	if(m_nDays1 <= 0)
	{
		szSQL.Format(_T("SELECT to_date('%s','YYYY-MM-DD HH24:MI:SS')-to_date('%s', 'YYYY-MM-DD HH24:MI:SS') as qty FROM DUAL "),
			m_szToDate1, m_szFromDate1);
		rs.ExecSQL(szSQL);
		m_nDays1 = (float) rs.GetIntValue();
		UpdateData(FALSE);
	}
	return 0;
}

int CHMSDynBedLevelDialog::OnFromDate2CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szToDate1, m_szFromDate2);
	int ret = str2int(pMF->ExecDML(szSQL));

	//_msg(_T("%s"), szSQL);
	if(ret == -1)
	{
		m_wndToDate1.SetToolTipMessage(_T("Thời gian bắt đầu mức 2 phải lớn hơn thời gian kết thúc mức 1"));
		return -1;
	}
	UpdateData(FALSE);
	return 0;
...
int CHMSDynBedLevelDialog::OnToDate2CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szFromDate2, m_szToDate2);
	int ret = str2int(pMF->ExecDML(szSQL));
	if(ret == -1)
	{
		m_wndToDate2.SetToolTipMessage(_T("Thời gian kết thúc phải lớn hơn thời gian bắt đầu"));
		return -1;
	}
	if(m_nDays2 <= 0)
	{
		szSQL.Format(_T("SELECT to_date('%s','YYYY-MM-DD HH24:MI:SS')-to_date('%s', 'YYYY-MM-DD HH24:MI:SS') as qty FROM DUAL "),
			m_szToDate2, m_szFromDate2);
		rs.ExecSQL(szSQL);
		m_nDays2 = (float) rs.GetIntValue();
		UpdateData(FALSE);
	}
	return 0;
}

int CHMSDynBedLevelDialog::OnFromDate3CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szToDate2, m_szFromDate3);
	int ret = str2int(pMF->ExecDML(szSQL));

	//_msg(_T("%s"), szSQL);
	if(ret == -1)
	{
		m_wndToDate1.SetToolTipMessage(_T("Thời gian bắt đầu mức 3 phải lớn hơn thời gian kết thúc mức 2"));
		return -1;
	}
	UpdateData(FALSE);
	return 0;
...
int CHMSDynBedLevelDialog::OnToDate3CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szFromDate3, m_szToDate3);
	int ret = str2int(pMF->ExecDML(szSQL));
	if(ret == -1)
	{
		m_wndToDate3.SetToolTipMessage(_T("Thời gian kết thúc phải lớn hơn thời gian bắt đầu"));
		return -1;
	}
	if(m_nDays3 <= 0)
	{
		szSQL.Format(_T("SELECT to_date('%s','YYYY-MM-DD HH24:MI:SS')-to_date('%s', 'YYYY-MM-DD HH24:MI:SS') as qty FROM DUAL "),
			m_szToDate3, m_szFromDate3);
		rs.ExecSQL(szSQL);
		m_nDays3 = (float) rs.GetIntValue();
		UpdateData(FALSE);
	}
	return 0;
}

int CHMSDynBedLevelDialog::OnFromDate4CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szToDate3, m_szFromDate4);
	int ret = str2int(pMF->ExecDML(szSQL));

	//_msg(_T("%s"), szSQL);
	if(ret == -1)
	{
		m_wndToDate1.SetToolTipMessage(_T("Thời gian bắt đầu mức 4 phải lớn hơn thời gian kết thúc mức 3"));
		return -1;
	}
	UpdateData(FALSE);
	return 0;
...
int CHMSDynBedLevelDialog::OnToDate4CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szFromDate4, m_szToDate4);
	int ret = str2int(pMF->ExecDML(szSQL));
	if(ret == -1)
	{
		m_wndToDate4.SetToolTipMessage(_T("Thời gian kết thúc phải lớn hơn thời gian bắt đầu"));
		return -1;
	}
	if(m_nDays4 <= 0)
	{
		szSQL.Format(_T("SELECT to_date('%s','YYYY-MM-DD HH24:MI:SS')-to_date('%s', 'YYYY-MM-DD HH24:MI:SS') as qty FROM DUAL "),
			m_szToDate4, m_szFromDate4);
		rs.ExecSQL(szSQL);
		m_nDays4 = (float) rs.GetIntValue();
		UpdateData(FALSE);
	}
	return 0;
}

int CHMSDynBedLevelDialog::OnFromDate5CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szToDate4, m_szFromDate5);
	int ret = str2int(pMF->ExecDML(szSQL));

	//_msg(_T("%s"), szSQL);
	if(ret == -1)
	{
		m_wndToDate1.SetToolTipMessage(_T("Thời gian bắt đầu mức 5 phải lớn hơn thời gian kết thúc mức 4"));
		return -1;
	}
	UpdateData(FALSE);
	return 0;
...
int CHMSDynBedLevelDialog::OnToDate5CheckValue()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("HMS_CHECKDATE('%s','%s') "), m_szFromDate5, m_szToDate5);
	int ret = str2int(pMF->ExecDML(szSQL));
	if(ret == -1)
	{
		m_wndToDate5.SetToolTipMessage(_T("Thời gian kết thúc phải lớn hơn thời gian bắt đầu"));
		return -1;
	}
	if(m_nDays5 <= 0)
	{
		szSQL.Format(_T("SELECT to_date('%s','YYYY-MM-DD HH24:MI:SS')-to_date('%s', 'YYYY-MM-DD HH24:MI:SS') as qty FROM DUAL "),
			m_szToDate5, m_szFromDate5);
		rs.ExecSQL(szSQL);
		m_nDays5 = (float) rs.GetIntValue();
		UpdateData(FALSE);
	}
	return 0;
}
...
long CHMSDynBedLevelDialog::OnLevel1LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return LoadDynBedList(&m_wndLevel1, m_szLevel1Key);
...
int CHMSDynBedLevelDialog::OnDays1CheckValue(){
	...
	else
	{
		...
		m_wndFromDate2.EnableWindow(false);
		m_wndToDate2.EnableWindow(false);
...
long CHMSDynBedLevelDialog::OnLevel2LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return LoadDynBedList(&m_wndLevel2, m_szLevel2Key);
...
int CHMSDynBedLevelDialog::OnDays2CheckValue(){
	...
	else
	{
		...
		m_wndPayrate3.EnableWindow(true);
		m_wndFromDate3.EnableWindow(false);
		m_wndToDate3.EnableWindow(false);
...
long CHMSDynBedLevelDialog::OnLevel3LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return LoadDynBedList(&m_wndLevel3, m_szLevel3Key);
...
int CHMSDynBedLevelDialog::OnDays3CheckValue(){
	...
	else
	{
		...
		m_wndFromDate4.EnableWindow(false);
		m_wndToDate4.EnableWindow(false);
		m_wndPayrate4.EnableWindow(true);
		m_nDays4 = m_nDays5 = 0;
		m_wndLevel4.SetCurSel(-1);
		m_wndLevel5.SetCurSel(-1);
...
long CHMSDynBedLevelDialog::OnLevel4LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return LoadDynBedList(&m_wndLevel4, m_szLevel4Key);
...
int CHMSDynBedLevelDialog::OnDays4CheckValue(){
	...
	else
	{
		...
		m_wndPayrate5.EnableWindow(true);
		m_wndFromDate5.EnableWindow(false);
		m_wndToDate5.EnableWindow(false);
		m_nDays5 = 0;
		m_wndLevel5.SetCurSel(-1);
...
long CHMSDynBedLevelDialog::OnLevel5LoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return LoadDynBedList(&m_wndLevel5, m_szLevel5Key);
...
void CHMSDynBedLevelDialog::OnApplySelect(){
	...
	if(GetMode() != VM_ADD && GetMode() != VM_EDIT) 
 	...
 	if(!IsValidateData()) 
 		return; 
	if (CalcDaysBetween(m_szFromDate1, m_szAdmitDate) < 0)
	{
		ShowToolTip(&m_wndFromDate1, _T("Ngày vào không hợp lệ!"));
		return;
	}
	if (CalcDaysBetween(m_szToDate1, m_szOutDate) > 0 || CalcDaysBetween(m_szToDate2, m_szOutDate) > 0
		||CalcDaysBetween(m_szToDate3, m_szOutDate) > 0||CalcDaysBetween(m_szToDate4, m_szOutDate) > 0
		||CalcDaysBetween(m_szToDate5, m_szOutDate) > 0)
	{
		ShowMessageBox(_T("Ngày ra không hợp lệ!"));
		return;
	}
	...
	if(m_nDays1 > 0){
		...
		m_hms_bed_itemsTbl.SetValue(_T("hbi_fromdate"), m_szFromDate1);
		m_hms_bed_itemsTbl.SetValue(_T("hbi_todate"), m_szToDate1);
	...
	if(m_nDays2 > 0){
		...
		m_hms_bed_itemsTbl.SetValue(_T("hbi_reqpaid"), m_bRequestOption?_T("Y"):_T("N"));
		m_hms_bed_itemsTbl.SetValue(_T("hbi_fromdate"), m_szFromDate2);
		m_hms_bed_itemsTbl.SetValue(_T("hbi_todate"), m_szToDate2);
		szSQL = m_hms_bed_itemsTbl.GetInsertSQL();
		pMF->ExecSQL(szSQL);
		dte += m_nDays2;
	}	
	if(m_nDays3 > 0){
		...
		m_hms_bed_itemsTbl.SetValue(_T("hbi_reqpaid"), m_bRequestOption?_T("Y"):_T("N"));
		m_hms_bed_itemsTbl.SetValue(_T("hbi_fromdate"), m_szFromDate3);
		m_hms_bed_itemsTbl.SetValue(_T("hbi_todate"), m_szToDate3);
		szSQL = m_hms_bed_itemsTbl.GetInsertSQL();
		pMF->ExecSQL(szSQL);
		dte += m_nDays3;
	}	
	...
	if(m_nDays4 > 0){
		...
		m_hms_bed_itemsTbl.SetValue(_T("hbi_reqpaid"), m_bRequestOption?_T("Y"):_T("N"));
		m_hms_bed_itemsTbl.SetValue(_T("hbi_fromdate"), m_szFromDate4);
		m_hms_bed_itemsTbl.SetValue(_T("hbi_todate"), m_szToDate4);
		szSQL = m_hms_bed_itemsTbl.GetInsertSQL();
		pMF->ExecSQL(szSQL);
		dte += m_nDays4;
	}	
	...
	if(m_nDays5 > 0){
		...
		m_hms_bed_itemsTbl.SetValue(_T("hbi_reqpaid"), m_bRequestOption?_T("Y"):_T("N"));
		m_hms_bed_itemsTbl.SetValue(_T("hbi_fromdate"), m_szFromDate5);
		m_hms_bed_itemsTbl.SetValue(_T("hbi_todate"), m_szToDate5);
		szSQL = m_hms_bed_itemsTbl.GetInsertSQL();
		pMF->ExecSQL(szSQL);
		dte += m_nDays5;
	}	
...
int CHMSDynBedLevelDialog::CalcDaysBetween(CString szFrom, CString szTo)
{
	CDate dtFrom, dtTo;
	dtFrom.ParseDate(szFrom);
	dtTo.ParseDate(szTo);
	int nCount = dtFrom.Compare(dtTo);
	return nCount;
}
...
long CHMSDynBedLevelDialog::LoadDynBedList(CGuiComboBox* pCombo, CString szKey, CString szFilter)
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
	if(pCombo->IsSearchKey() && ToInt(szKey) > 0){
		szWhere.Format(_T(" and hdbl_idx=%d "), ToInt(szKey));
	};
	if (!szFilter.IsEmpty())
	{
	...
	if (pMF->GetObjectType() == _T("I"))
	{
		...
		_T(" WHERE  ho_docno = %ld AND ho_normal IN ( '1', '2', '3', 'S' ) "),
		pMF->m_nDocumentNo);

		rs.ExecSQL(szSQL);
		if (rs.GetIntValue() == 0)
		{
			szWhere.AppendFormat(_T(" and nvl(hdbl_type, 'X') <> 'S'"));
		}
	}
	pCombo->DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT * ") \
				_T(" FROM hms_dynbedlist ") \
				_T(" WHERE hdbl_deptid='%s' and hdbl_active='Y'  %s ") \
				_T(" ORDER BY hdbl_idx "), pMF->m_szDept, szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		pCombo->AddItems(
			rs.GetValue(_T("hdbl_idx")), 
