...
void CPhanKhamBenh::OnSetWindowEvents(){
	...
	SetScrollSizes(MM_TEXT, CSize(960, 700));
...
void CPhanKhamBenh::GetDataToScreen()
{
	...
	// 		rs.MoveNext();
	_tprintf(_T("\nGetTreatTime: %d"), pMF->GetTreatTime());
	if (m_nTreatTime == pMF->GetTreatTime())
	{
			JSONVALUE jData;
			CString szTemp;
			szSQL.Format(_T("SELECT * FROM hms_doc_emr_view WHERE hde_docno=%ld and hde_type='PATIENT_CARE' and hde_treattime = %d order by hde_refidx desc"), m_nDocumentNo, m_nTreatTime);
			rs.ExecSQL(szSQL);
			rs.GetValue(_T("hde_value"), tmpStr);
			jData.Clear();
			jData.Parse(tmpStr);
	...
	//Link bệnh án khoa C1.3 điều trị nội trú	
	if ((pMF->UserC13ExamTab() == _T("Y") && (pMF->m_szDept == _T("C1.3") || pMF->m_szDept == _T("C1.3NT") || pMF->GetCurrentUser() == _T("lttmaivimes"))) )
	{
	szSQL.Format(_T("hms_check_is_create_khambenh(%ld) "), pMF->m_nDocumentNo);
		...
		if(!rs.IsEOF())
		{	
			rs.GetValue(_T("toanthan"), m_sztthan); 
			rs.GetValue(_T("tuanhoan"), m_szthoan);
			rs.GetValue(_T("hohap"), m_szhhap);
			rs.GetValue(_T("tieuhoa"), m_szthoa);
			rs.GetValue(_T("tietnieu"), m_sztnieu);
			rs.GetValue(_T("thankinh"), m_szttkinh);
			rs.GetValue(_T("khop"), m_szcxkhop);

			rs.GetValue(_T("coquankhac"), m_szckkhac);
			rs.GetValue(_T("NHIPTIM"), m_nmach);
			rs.GetValue(_T("nhietdo"), m_nndo);

			rs.GetValue(_T("huyetap_max"), m_nhap);
			rs.GetValue(_T("huyetap_min"), m_nhap2);

			rs.GetValue(_T("nhiptho"), m_nntho);
			rs.GetValue(_T("chieucao"), m_nccao);
			rs.GetValue(_T("cannang"), m_ncnang);
			rs.GetValue(_T("dinhduong"), m_szdinhduong);
