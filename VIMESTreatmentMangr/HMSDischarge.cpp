...
//#include "HMSUnTerminatedExtendDialog.h"
...
static void _OnDischargeStatusSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CHMSDischarge* )pWnd)->OnDischargeStatusSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnDischargeStatusSelendokFnc(CWnd *pWnd){
	((CHMSDischarge *)pWnd)->OnDischargeStatusSelendok();
}
/*static void _OnDischargeStatusSetfocusFnc(CWnd *pWnd){
	((CHMSDischarge *)pWnd)->OnDischargeStatusKillfocus();
}*/
/*static void _OnDischargeStatusKillfocusFnc(CWnd *pWnd){
	((CHMSDischarge *)pWnd)->OnDischargeStatusKillfocus();
}*/
static long _OnDischargeStatusLoadDataFnc(CWnd *pWnd){
	return ((CHMSDischarge *)pWnd)->OnDischargeStatusLoadData();
}
/*static void _OnDischargeStatusAddNewFnc(CWnd *pWnd){
	((CHMSDischarge *)pWnd)->OnDischargeStatusAddNew();
}*/
...
void CHMSDischarge::OnCreateComponents(){
    ...
    m_wndClosingDoctorLabel.Create(this, _T("Closing Doctor"), 265, 30, 350,
                                   55);
    ...
    m_wndDischargeStatusLabel.Create(this, _T("Tình trạng rv"), 265, 60, 350,
                                     85);
    m_wndDischargeStatus.Create(this, 355, 60, 595, 85);
    m_wndICD10Label.Create(this, _T("ICD10"), 10, 90, 130, 115);
    m_wndICD10.Create(this, 135, 90, 595, 115);
    m_wndMainDiseaseLabel.Create(this, _T("Main Disease"), 10, 120, 130, 170);
    m_wndMainDisease.Create(this, 135, 120, 595, 170, TRUE, FALSE, TRUE);
    m_wndRelationDiseaseLabel.Create(this, _T("Relation Disease"), 10, 175, 130,
                                     225);
    m_wndRelationDisease.Create(this, 135, 175, 595, 225, TRUE, FALSE, TRUE);
    m_wndTreatMethodLabel.Create(this, _T("Treat Method"), 10, 230, 130, 305);
    m_wndTreatMethod.Create(this, 135, 230, 595, 305, TRUE, FALSE, TRUE);
    m_wndCommentLabel.Create(this, _T("Note"), 10, 310, 130, 385);
    m_wndComment.Create(this, 135, 310, 595, 385, TRUE, FALSE, TRUE);
    m_wndAdmissionStatusLabel.Create(this, _T("Admission Status"), 10, 390, 130,
                                     440);
    m_wndAdmissionStatus.Create(this, 135, 390, 595, 440, TRUE, FALSE, TRUE);
    m_wndYKienDenGhiLabel.Create(this, _T("Y kien den ghi"), 10, 445, 130, 495);
    m_wndYKienDenGhi.Create(this, 135, 445, 595, 495, TRUE, FALSE, TRUE);
    m_wndConclusionLabel.Create(this, _T("Conclusion /Advice"), 10, 500, 130,
                                550);
    m_wndConclusion.Create(this, 135, 500, 595, 550, TRUE, FALSE, TRUE);
    m_wndUpdate.Create(this, _T("&Update"), 245, 560, 330, 585);
    m_wndSave.Create(this, _T("&Save"), 335, 560, 420, 585);
    m_wndCancel.Create(this, _T("&Cancel"), 425, 560, 510, 585);
    m_wndPrint.Create(this, _T("&Print"), 515, 560, 600, 585);
}
...
//	m_wndEndDate.SetMax(CDateTime(pMF->GetSysDateTime()));
	...
	m_wndDischargeStatus.SetCheckValue(true);
...
//	m_wndConclusion.SetCheckValue(true);
	...
	m_wndResult.InsertColumn(1, _T("Description"), CFMT_TEXT, 150);
		...
		m_wndDischargeStatus.InsertColumn(0, _T("ID"), CFMT_NUMBER, 100);
	m_wndDischargeStatus.InsertColumn(1, _T("Description"), CFMT_TEXT, 150);
	...
	m_hms_clinicalTbl.AddField(_T("hcr_ttrang_rv"), dfInteger);
    m_hms_clinicalTbl.AddField(_T("hcr_comment"), dfText, 1024);
...
void CHMSDischarge::OnSetWindowEvents(){
	...
	m_wndDischargeStatus.SetEvent(WE_SELENDOK, _OnDischargeStatusSelendokFnc);
	//m_wndDischargeStatus.SetEvent(WE_SETFOCUS, _OnDischargeStatusSetfocusFnc);
	//m_wndDischargeStatus.SetEvent(WE_KILLFOCUS, _OnDischargeStatusKillfocusFnc);
	m_wndDischargeStatus.SetEvent(WE_SELCHANGE, _OnDischargeStatusSelectChangeFnc);
	m_wndDischargeStatus.SetEvent(WE_LOADDATA, _OnDischargeStatusLoadDataFnc);
	//m_wndDischargeStatus.SetEvent(WE_ADDNEW, _OnDischargeStatusAddNewFnc);
...
void CHMSDischarge::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_TextEx(pDX, m_wndDischargeStatus.GetDlgCtrlID(), m_szDischargeStatusKey);
...
void CHMSDischarge::GetDataToScreen(){
	...
	if(!rs.IsEOF())
	{
		...
		rs.GetValue(_T("hcr_ttrang_rv"),m_szDischargeStatusKey);
		...
		m_szICD10Key.Trim();
        rs.GetValue(L"hcr_comment", m_szComment);
...
int CHMSDischarge::SetMode(int nMode){
		...
		if(m_szDischarge == _T("Y"))
		{
				...
				else
			{
				...
				m_wndDischargeStatus.EnableWindow(TRUE);
			}
		}
...
void CHMSDischarge::OnDischargeStatusSelectChange(int nOldItemSel, int nNewItemSel){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
void CHMSDischarge::OnDischargeStatusSelendok(){
	 
}
/*void CHMSDischarge::OnDischargeStatusSetfocus(){
	
}*/
/*void CHMSDischarge::OnDischargeStatusKillfocus(){
	
}*/
long CHMSDischarge::OnDischargeStatusLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szWhere;
	szWhere.Format(_T(" and ss_code <>'2' "));
	return pMF->LoadSelectionList(&m_wndDischargeStatus, _T("HMS_TINH_TRANG_RV"), m_szDischargeStatusKey,szWhere);

	return 0;
}
/*void CHMSDischarge::OnDischargeStatusAddNew(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
void CHMSDischarge::OnPrintSelect(){
	...
	CString tmpStr;
	TranslateString(_T("In phiếu ra viện"), tmpStr);
	...
	nPos = menu.TrackPopupMenu(TPM_NONOTIFY|TPM_RETURNCMD|TPM_RIGHTALIGN|TPM_TOPALIGN, rect.right, rect.top-20*4, this);
	...
	case 2:
		...
		break;
	case 3:
		pMF->PrintDischargeSheetOut(pMF->m_nDocumentNo, pMF->m_nRefIndex);
		break;
	};
...
int CHMSDischarge::OnEditHMSDischarge()
{
   ...
   //16/07/2019 hayhv: them chuc nang kiem tra khoa cuoi cung moi duoc ket thuc ra vien
	...
	//Bổ sung phần check đối với những bệnh nhân có lượt điều trị > 1 nhưng lọt các trường hợp:
	//1. Ngày y lệnh < ngày vào viện
	//2. Ngày y lệnh > ngày ra viện
	//3. Ngày thanh toán < ngày ra viện (Đối với bệnh nhân yêu cầu check trong hàm HMS_FEE_INVOICE_PAYMENT_TYC)
	//ShowMessageBox(_T("Chạy vào đây không?"));
	/*
		szSQL.Format(_T("HMS_CLINICAL_CHECK_EXTEND(%ld, %d) "), pMF->m_nDocumentNo, pMF->GetTreatTime());
		int nCount = str2int(pMF->ExecDML(szSQL));
		if( nCount > 0 )
		{
			CHMSUnTerminatedExtendDialog termdlg(this);
			termdlg.m_nDocumentNo = pMF->m_nDocumentNo;
			if(termdlg.DoModal() == IDOK)
			{
...
//Cap nhat lai ho so dieu tri cua khoa
	...
	*/
	...
	parser.PushRowData(&m_hms_clinicalTbl);
	...
	m_hms_clinicalTbl.SetValue(_T("hcr_ttrang_rv"), ToInt(m_szDischargeStatusKey));
    m_hms_clinicalTbl.SetValue(_T("hcr_comment"),
                               m_szComment);
	parser.PushRowData(&m_hms_clinicalTbl, true);
	...
	if (m_szDischarge == _T("Y"))
	{
		...
		_T(" WHERE hcr_docno=%ld "),
			...
			pMF->m_nRefIndex,
			m_szRelationDisease,
			str2int(m_szDischargeStatusKey),
			pMF->m_nDocumentNo);
*/
	}
	else
 ...
 _fmsg(_T("%s"), szSQL);
	...
	int ret = str2int(pMF->ExecDML(szSQL));
 	...
 	{ 
		...
		{
			CString szMsg;
			szMsg.Format(_T("Cập nhật không thành công! Kiểm tra lại thông tin ra vào viện."));
		...
		pMF->ExecSQL(szSQL);
 		SetMode(VM_VIEW);
		CString szEvent, szDescr, szRowChange = parser.GetRowChange();
