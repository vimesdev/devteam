...
static void _OnOrderListDblClickFnc(CWnd *pWnd){
	((CHMSDailyOrderView*)pWnd)->OnOrderListDblClick();
} 
static void _OnOrderListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CHMSDailyOrderView*)pWnd)->OnOrderListSelectChange(nOldItem, nNewItem);
} 
static int _OnPreviewOrderDeviveryDetailFnc(CWnd *pWnd){
	return ((CHMSDailyOrderView*)pWnd)->OnPreviewDeliveryDetail();
}
static int _OnOrderListDeleteItemFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnOrderListDeleteItem();
} 

static int _OnOrderListPrintDeliveryDrugFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnOrderListPrintDeliveryDrug();
}
static int	_OnExportOrderDeliveryDrugFnc(CWnd *pWnd){
	return	((CHMSDailyOrderView*)pWnd)->OnExportOrderDeliveryDrug();
}
static int _OnOrderListPrintSheetDateFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnOrderListPrintSheetDate();
}
static int _OnOrderListPrintSheetpublicityFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnOrderListPrintSheetPublicity();
}

static long _OnPatientListLoadDataFnc(CWnd *pWnd){
	return ((CHMSDailyOrderView*)pWnd)->OnPatientListLoadData();
} 
static void _OnPatientListDblClickFnc(CWnd *pWnd){
	((CHMSDailyOrderView*)pWnd)->OnPatientListDblClick();
} 
static void _OnPatientListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CHMSDailyOrderView*)pWnd)->OnPatientListSelectChange(nOldItem, nNewItem);
} 

static int _OnPatientListIssueAllFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnPatientListIssueAll();
} 

static int _OnPatientListIssueFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnPatientListIssue();
} 

static int _OnPatientListRollbackFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnPatientListRollback();
} 

static int _OnPatientListDeleteItemFnc(CWnd *pWnd){
	 return ((CHMSDailyOrderView*)pWnd)->OnPatientListDeleteItem();
} 
...
/*static void _OnPatientNameSearchChangeFnc(CWnd *pWnd){
	((CHMSDailyOrderView *)pWnd)->OnPatientNameSearchChange();
} */
/*static void _OnPatientNameSearchSetfocusFnc(CWnd *pWnd){
	((CHMSDailyOrderView *)pWnd)->OnPatientNameSearchSetfocus();} */ 
/*static void _OnPatientNameSearchKillfocusFnc(CWnd *pWnd){
	((CHMSDailyOrderView *)pWnd)->OnPatientNameSearchKillfocus();
} */

static int _OnPatientNameSearchCheckValueFnc(CWnd *pWnd){
	return ((CHMSDailyOrderView *)pWnd)->OnPatientNameSearchCheckValue();
}
...
_msg(_T("%s"), szSQL);
*/
	szSQL.Format(_T(" SELECT mt_storage_id AS storage_id,") \
_T("   product_id,") \
...
void CHMSDailyOrderView::OnPatientListDblClick(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnPatientListIssueAll();
} 
void CHMSDailyOrderView::OnPatientListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
int CHMSDailyOrderView::OnPatientListIssueAll(){
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();	
...
int CHMSDailyOrderView::OnPatientListIssue(){
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	CString szSQL;
	CRecord rs(&pMF->m_db);
...
int CHMSDailyOrderView::OnPatientListRollback(){
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	CString szSQL;
	if(m_szStatus != _T("A"))
		return -1;
...
int CHMSDailyOrderView::OnPatientListDeleteItem(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	CRecord rs(&pMF->m_db);
...
long CHMSDailyOrderView::OnPatientListLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
...
long CHMSDailyOrderView::OnDrugListLoadData(){
	...
	szSQL.Format(_T(" SELECT hpol_orderid, ") \
				...
				m_nDocumentNo,  m_nBatchID, m_szPatientOrderDate,m_szOrderStatus);
_fmsg(_T("%s"), szSQL);
	nCount = rs.ExecSQL(szSQL);
	int nItem = 0;
...
int CHMSDailyOrderView::OnPatientNameSearchCheckValue(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnPatientListLoadData();
	return 0;
}
...
int CHMSDailyOrderView::OnExportOrderDeliveryDrug(){
	...
	if(dlg.DoModal() == IDOK){
		szSQL.Format(_T(" SELECT distinct trunc_date(hpo_orderdate) as orderdate ") \
			_T(" FROM hms_ipharmaorder ") \
			_T(" WHERE 	hpo_transaction_id=%ld ") \
			_T(" ORDER BY trunc_date(hpo_orderdate) DESC "), m_nBatchID);
			rs.ExecSQL(szSQL);
			
			while(!rs.IsEOF()){
				rs.GetValue(_T("orderdate"), tmpStr);
				if(dlg.m_nPortraid)
					OnExportDelivery_Lanscape(m_nBatchID, tmpStr);
				else
					OnExportDelivery_Portraid(m_nBatchID, tmpStr);
