...
static void _OnYesSelectFnc(CWnd *pWnd){
	  ((CHMSTreatmentCareInputDialog*)pWnd)->OnYesSelect();
}
static void _OnNoSelectFnc(CWnd *pWnd){
	  ((CHMSTreatmentCareInputDialog*)pWnd)->OnNoSelect();
}

static int _OnDaitienCheckValueFnc(CWnd *pWnd){
	return ((CHMSTreatmentCareInputDialog *)pWnd)->OnDaitienCheckValue();
}

static int _OnTieutienCheckValueFnc(CWnd *pWnd)
{
	return ((CHMSTreatmentCareInputDialog *)pWnd)->OnTieutienCheckValue();
} 
...
void CHMSTreatmentCareInputDialog::OnCreateComponents()
{
	...
	m_wndDauhieunhiemkhuanLabel.Create(this, _T("Dấu hiệu nhiễm khuẩn Bệnh Viện"), 3, 416, 207, 441);
	m_wndDauhieunhiemkhuan.Create(this,5, 446, 700, 494, 1, 0, 1);

	m_wndDaitienLabel.Create(this, _T("Đại tiện"), 5, 561, 125, 586);
	m_wndDaitien.Create(this,130, 561, 240, 586); 
	m_wndTieutienLabel.Create(this, _T("Tiểu tiện"), 245, 561, 349, 586);
	m_wndTieutien.Create(this,354, 561, 464, 586); 

	m_wnddanhmucnhiemkhuan108.Create(this, _T("Danh mục dấu hiệu nhiễm khuẩn bệnh viện"), 342, 416, 700, 441);
	m_wndYes.Create(this, _T("Có"), 208, 416, 260, 441);
	m_wndNo.Create(this, _T("Không"), 262, 416, 341, 441);
	m_wndSave.Create(this, _T("&Save"), 535, 561, 615, 586);
	m_wndClose.Create(this, _T("&Close"), 620, 561, 700, 586);

	//m_wndInputPulseTemperature.Create(this, _T(""), 0, 0, 0, 0);
	//m_wndInputPulseTemperature.ShowWindow(SW_HIDE);
	//m_wndInputPulseTemperature.EnableWindow(FALSE);
}
...
// 	m_wndSP02.SetCheckValue(false);
	...
	m_wndDauhieunhiemkhuan.SetCheckValue(true);
//	m_wndBloodPressure.SetMask(_T("###/###"));
	...
	m_hms_careinfoTbl.AddField(_T("hci_daitien"), dfText, 21);
	m_hms_careinfoTbl.AddField(_T("hci_tieutien"), dfText, 21);
	...
	m_wndDaitien.SetLimitText(21);
	//m_wndDaitien.SetCheckValue(true);
	m_wndTieutien.SetLimitText(21);
	//m_wndTieutien.SetCheckValue(true);
...
void CHMSTreatmentCareInputDialog::OnSetWindowEvents(){
	...
	m_wndDaitien.SetEvent(WE_CHECKVALUE, _OnDaitienCheckValueFnc);
	m_wndTieutien.SetEvent(WE_CHECKVALUE, _OnTieutienCheckValueFnc);
...
void CHMSTreatmentCareInputDialog::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Text(pDX, m_wndDaitien.GetDlgCtrlID(), m_szDaitien);
	DDX_Text(pDX, m_wndTieutien.GetDlgCtrlID(), m_szTieutien);
...
void CHMSTreatmentCareInputDialog::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT hci_createdby, hci_date, hci_disease, hci_care, hci_pulse, hci_temperature, hci_bloodpressure, ") \
		...
		_T(" REGEXP_SUBSTR(hci_reference,'[^|]+',1,2) AS pttt, HCI_DAUHIEUNHIEMKHUAN as dauhieunhiemkhuan, hci_daitien, hci_tieutien FROM HMS_CAREINFO where hci_docno = %ld and hci_idx = %d ")
	...
	if(!rs.IsEOF()){		
		...
		rs.GetValue(_T("hci_daitien"), m_szDaitien);
		rs.GetValue(_T("hci_tieutien"), m_szTieutien);
		_debug(_T("%s"), szHpt);
		if(szHpt == _T("6H"))
			m_nNormal = 1;
...
void CHMSTreatmentCareInputDialog::GetScreenToData(){
	...
	szSQL.Format(_T("SELECT hsie_idx as treat_idx") \
				...
				_T(" and trunc_date(hsie_date) = trunc_date(TO_DATE('%s', 'YYYY-MM-DD HH24:MI:SS')) ") \
				_T(" order by hsie_date desc"), pMF->m_nDocumentNo, pMF->GetSysDateTime());
	rs.ExecSQL(szSQL);
	//_msg(_T("%s"), szSQL);
	...
	m_hms_careinfoTbl.SetValue(_T("hci_daitien"), m_szDaitien);
	m_hms_careinfoTbl.SetValue(_T("hci_tieutien"), m_szTieutien);
}
...
int CHMSTreatmentCareInputDialog::SetMode(int nMode){
 		...
 		case VM_ADD: 
			...
			m_nTreatIdx = 0;
			m_wndDauhieunhiemkhuan.EnableWindow(false);
			m_wnddanhmucnhiemkhuan108.EnableWindow(false);
 			break;
 		case VM_EDIT: 
 			EnableControls(TRUE);
 			EnableButtons(TRUE, 0, 1, 2, -1);
...
void CHMSTreatmentCareInputDialog::OnYesSelect()
{
	UpdateData(true);
	m_wndDauhieunhiemkhuan.EnableWindow(true);
	m_wnddanhmucnhiemkhuan108.EnableWindow(true);
	m_nYes = 0;
	m_nNo = 1;
	UpdateData(false);
}
void CHMSTreatmentCareInputDialog::OnNoSelect()
{
	UpdateData(true);
	m_wndDauhieunhiemkhuan.EnableWindow(false);
	m_wnddanhmucnhiemkhuan108.EnableWindow(false);
...
*/
int CHMSTreatmentCareInputDialog::OnDaitienCheckValue()
{
	return 0;
}
int CHMSTreatmentCareInputDialog::OnTieutienCheckValue()
{
	return 0;
}