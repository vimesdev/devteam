...
void CTMBenhAnDialog::OnCreateComponents(){
	...
	int x = 535;
	m_wndUpdate.Create(this, _T("&Update"), 535, 615, 615, 640);
	m_wndSave.Create(this, _T("&Save"), 620, 615, 700, 640);
	m_wndCancel.Create(this, _T("&Cancel"), 705, 615, 790, 640);
...
// 	m_wndTab.Add(_T("III. Phan Ket Luan"), &m_wndPhanKetLuan);
	...
	if (m_nCancerTime == 0 && pMF->m_nCancerTime > 0)
        m_nCancerTime = pMF->m_nCancerTime;
...
// 	m_wndPhanKetLuan.OnSavePhanKetLuan();
	...
	if (pMF->m_UserInfo.su_groupid == _T("A"))
	{
		...
		if(dlg.DoModal() != IDOK)
		return;
		szSQL.Format(_T(" update hms_doc_emr set HDE_CREATEDBY = '%s' ") \
		_T(" where hde_docno=%ld and hde_type IN('BENH_AN_KL')") \
		_T(" and hde_treattime=%ld"), dlg.m_szDoctor, pMF->m_nDocumentNo, m_nTreatTime);
		pMF->ExecSQL(szSQL);
		//_msg(_T("%s"), szSQL);
	}
...
void CTMBenhAnDialog::OnPrintSelect(){
	...
	m_wndChuyenKhoa.UpdateData(true);
	CString szForm = m_wndChuyenKhoa.m_szMedicalTypeKey;
	szForm.AppendFormat(_T(".html"));
	info.Add(m_wndChuyenKhoa.m_szMedicalTypeKey);
	_tprintf(_T("\r\nCTMBenhAnDialog::OnPrintSelect: %s"), szForm);
	pMF->AddProp(L"patient_name", pMF->m_wndPatientDocument.m_szPatientName);
	if (m_nCancerTime > 1)
	{
...
BOOL CTMBenhAnDialog::PreTranslateMessage(MSG* pMsg)
{
