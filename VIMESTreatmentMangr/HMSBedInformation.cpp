...
{
	 ((CHMSBedInformation*)pWnd)->OnByTreatTimeSelect();
}

static int _OnDynamicBedInformationViewFnc(CWnd *pWnd){
	 return ((CHMSBedInformation*)pWnd)->OnDynamicBedInformationView();
} 
...
void CHMSBedInformation::OnCreateComponents()
{
	...
	m_wndByTreatTime.Create(this, _T("View theo lần điều trị"), 430, 180, 615, 205);
	...
	//m_wndDynBed.Create(this, CRect(0, 385, 600, 400));
...
void CHMSBedInformation::OnSetWindowEvents()
{
	...
	m_wndList.AddEvent(0, _T("-"));
	m_wndList.AddEvent(6, _T("Xem thông tin giường động"), _OnDynamicBedInformationViewFnc);	
	...
	m_wndByTreatTime.SetEvent(WE_CLICK, _OnByTreatTimeSelectFnc);
...
void CHMSBedInformation::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Check(pDX, m_wndByTreatTime.GetDlgCtrlID(), m_bByTreatTime);
...
void CHMSBedInformation::GetDataToScreen()
{
	...
	if(!rs.IsEOF())
	{
		...
		if (GetMode() == VM_VIEW)
		{
			if (m_szDeptID != m_szCurrentDeptID)
			{
				OnPriceBedLoadData();
			}
		}
...
int CHMSBedInformation::OnListApplyDynamicPrice(){
	...
	dlg.m_szFromDate1 = m_szAdmitDate;
	dlg.m_szToDate1 = m_szOutDate;

	dlg.m_szFromDate2 = m_szAdmitDate;
	dlg.m_szToDate2 = m_szOutDate;

	dlg.m_szFromDate3 = m_szAdmitDate;
	dlg.m_szToDate3 = m_szOutDate;

	dlg.m_szFromDate4 = m_szAdmitDate;
	dlg.m_szToDate4 = m_szOutDate;

	dlg.m_szFromDate5 = m_szAdmitDate;
	dlg.m_szToDate5 = m_szOutDate;
	
	dlg.m_szAdmitDate = m_szAdmitDate;
	dlg.m_szOutDate = m_szOutDate;
...
int CHMSBedInformation::OnListUnusedDynamicPrice()
{
	...
	if(m_szDynPrice != _T("Y"))
	{
		ShowMessageBox(_T("Bệnh nhân không có giường động, không xóa được!"));
		return -1;
	}
	...
	if(pMF->IsPatientDischarge())
	{
		ShowMessageBox(_T("Hồ sơ bệnh án đã kết thúc, không cho phép xóa!"));
		return -1;
	}
	...
	rs.ExecSQL(szSQL);
	if(rs.GetIntValue() > 0){
		ShowMessageBox(_T("Bệnh nhân đã thanh toán tiền giường, không thao tác được"), MB_OK);
		return 0;
	}
	int nMsg = ShowMessageBox(_T("Bạn muốn xóa giường động không?(Y/N) "), MB_YESNO|MB_DEFBUTTON2);
	if(nMsg == IDNO)
		return 0;
	...
	pMF->ExecSQL(szSQL);
	szSQL.Format(_T("UPDATE hms_bed ") \
				_T(" SET hb_dynprice='N' ") \
				_T(" WHERE hb_docno=%ld and hb_idx=%d and hb_status='T' ")
				, pMF->m_nDocumentNo, m_nBedIdx);
	pMF->ExecSQL(szSQL);
	pMF->Commit();
	OnListLoadData();
...
void CHMSBedInformation::OnByTreatTimeSelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	UpdateData();
	OnListLoadData();	
...
int CHMSBedInformation::OnDeleteHMSBedInformation(){
	...
	if(m_szDynPrice == _T("Y"))
	{	
		ShowMessageBox(_T("Giường đã có giường động bạn không thể xóa.Vui lòng xóa giường động trước !"));
		return -1;
	}
...
void CHMSBedInformation::RefreshData(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	_tprintf(_T("\nbTreat: %d|GetCheck:%d"), m_bByTreatTime, m_wndByTreatTime.GetCheck());
	UpdateData();
	_tprintf(_T("\nbTreat: %d|GetCheck:%d"), m_bByTreatTime, m_wndByTreatTime.GetCheck());
	OnListLoadData();
...
int CHMSBedInformation::OnDynamicBedInformationView()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
void CHMSBedInformation::OnResizeLayout()
{
	...
	AddResize(&m_wndBedInformation, 100, 100);

	AddBottom(&m_wndBedDiagram, 0, true);
	AddBottom(&m_wndAdd);
