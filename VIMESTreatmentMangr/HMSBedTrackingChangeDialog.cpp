...
CHMSBedTrackingChangeDialog::CHMSBedTrackingChangeDialog(CWnd *pParent, long nBedIdx, bool bMove):
	CGuiDialog(pParent){
...
void CHMSBedTrackingChangeDialog::OnInitializeComponents(){
	...
	m_wndCurrentRoom.SetLimitText(135);
	m_wndCurrentRoom.SetReadOnly(true);
	m_wndCurrentBed.SetLimitText(135);
	m_wndCurrentBed.SetReadOnly(true);
	m_wndAdmitDate.SetCheckValue(true);
	m_wndEndDate.SetCheckValue(false);
	m_wndTreatNumber.SetLimitText(5);
	m_wndTreatNumber.SetCheckValue(false);
	m_wndNewRoom.SetCheckValue(true);
	m_wndNewRoom.LimitText(135);
	m_wndNewBed.SetCheckValue(true);
	m_wndNewBed.LimitText(135);
	m_wndDoctor.SetCheckValue(true);
	m_wndDoctor.LimitText(135);
	m_wndNurse.SetCheckValue(false);
	m_wndNurse.LimitText(135);


	m_wndNewRoom.InsertColumn(0, _T("ID"), CFMT_TEXT, 50);
	m_wndNewRoom.InsertColumn(1, _T("Name"), CFMT_TEXT, 200);


	m_wndNewBed.InsertColumn(0, _T("ID"), CFMT_TEXT, 50);
	m_wndNewBed.InsertColumn(1, _T("Name"), CFMT_TEXT, 200);
	...
	m_wndNurse.InsertColumn(0, _T("ID"), CFMT_TEXT, 90);
...
void CHMSBedTrackingChangeDialog::GetDataToScreen(){
	...
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T(" SELECT hb_refidx as refidx, hb_roomid AS roomid,") \
_T("   hb_bedid         AS bedid,") \
_T("   (SELECT DISTINCT hrl_name") \
_T("   FROM hms_roomlist") \
_T("   WHERE hrl_deptid = hb_deptid") \
_T("   AND hrl_id       =hb_roomid") \
_T("   ) AS roomname,") \
_T("   (SELECT DISTINCT hbl_name") \
_T("   FROM hms_bedlist") \
_T("   WHERE hbl_deptid = hb_deptid") \
_T("   AND hbl_roomid   =hb_roomid") \
_T("   AND hbl_id       =hb_bedid") \
_T("   )            AS bedname,") \
_T("   hb_admitdate AS admitdate,") \
_T("   hb_enddate   AS enddate,") \
_T("   hb_treatqty  AS treatnumber,") \
_T("   hb_doctor as doctor,") \
_T("   hb_nurse as nurse") \
_T(" FROM hms_bed") \
_T(" WHERE hb_docno=%ld and hb_idx=%ld "), m_nDocumentNo, m_nBedIdx);
	...
	if(!rs.IsEOF())
	{
		rs.GetValue(_T("roomname"), m_szCurrentRoom);
		rs.GetValue(_T("bedname"), m_szCurrentBed);
		rs.GetValue(_T("admitdate"), m_szAdmitDate);
		rs.GetValue(_T("enddate"), m_szEndDate);
		rs.GetValue(_T("treatnumber"), m_nTreatNumber);
		rs.GetValue(_T("refidx"), m_nRefIdx);
		rs.GetValue(_T("roomid"), m_nRoomID);
		rs.GetValue(_T("doctor"), m_szDoctorKey);
		rs.GetValue(_T("nurse"), m_szNurseKey);
...
int CHMSBedTrackingChangeDialog::SetMode(int nMode){
		...
		m_wndEndDate.SetCheckValue(m_bMove);
		m_wndTreatNumber.SetCheckValue(m_bMove);
		m_wndEndDate.EnableWindow(m_bMove);
		m_wndTreatNumber.EnableWindow(m_bMove);
...
long CHMSBedTrackingChangeDialog::OnNewRoomLoadData(){
	...
	szWhere.Format(_T(" and hrl_deptid='%s' "), pMF->GetDepartmentID());
	if(m_wndNewRoom.IsSearchKey() && str2int(m_szNewRoomKey) > 0){
	 szWhere.Format(_T(" and hrl_id=%d "), str2int(m_szNewRoomKey));
	};
	
	m_wndNewRoom.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT hrl_id as id, hrl_name as name ") \
		_T("FROM hms_roomlist WHERE hrl_active='Y' %s ORDER BY id "), szWhere);
...
long CHMSBedTrackingChangeDialog::OnNewBedLoadData(){
	...
	szWhere.Format(_T(" and hbl_deptid='%s' and hbl_roomid=%d"), pMF->GetDepartmentID(), str2int(m_szNewRoomKey));

	if(m_wndNewBed.IsSearchKey() && str2int(m_szNewBedKey) > 0){
	 szWhere.AppendFormat(_T(" and hbl_id='%s' "), str2int(m_szNewBedKey));
	};
	m_wndNewBed.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT hbl_id as id, hbl_name as name ") \
		_T(" FROM hms_bedlist WHERE hbl_active='Y'  %s ORDER BY id "), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
...
long CHMSBedTrackingChangeDialog::OnDoctorLoadData(){
	...
	return pMF->LoadDoctorList(&m_wndDoctor, m_szDoctorKey, pMF->GetDepartmentID(), szFilter);
/*
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
long CHMSBedTrackingChangeDialog::OnNurseLoadData(){
	...
	CString szFilter;
	szFilter.Format(_T(" and su_groupid ='N' "));
	return pMF->LoadDoctorList(&m_wndNurse, m_szNurseKey, pMF->GetDepartmentID(), szFilter);
/*
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
void CHMSBedTrackingChangeDialog::OnSaveSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnSaveHMSBedTrackingChangeDialog();
} 
...
int CHMSBedTrackingChangeDialog::OnSaveHMSBedTrackingChangeDialog(){
	...
	tbl.AddField(_T("roomid"), dfInteger);
	tbl.AddField(_T("bedidx"), dfLong);
	tbl.AddField(_T("admitdate"), dfDateTime);
	tbl.AddField(_T("enddate"), dfDateTime);
	tbl.AddField(_T("treatnumber"), dfInteger);
	tbl.AddField(_T("newroomid"), dfInteger);
	tbl.AddField(_T("newbedid"), dfInteger);
	tbl.AddField(_T("doctor"), dfText, 20);
	tbl.AddField(_T("nurse"), dfText, 20);
	tbl.AddField(_T("move"), dfText, 1);

	tbl.SetValue(_T("docno"), m_nDocumentNo);
	tbl.SetValue(_T("deptid"), pMF->GetDepartmentID());
	tbl.SetValue(_T("refidx"), m_nRefIdx);
	tbl.SetValue(_T("roomid"), m_nRoomID);
	tbl.SetValue(_T("bedidx"), m_nBedIdx);
	tbl.SetValue(_T("admitdate"), m_szAdmitDate);
	tbl.SetValue(_T("enddate"), m_szEndDate);
	tbl.SetValue(_T("treatnumber"), m_nTreatNumber);
	tbl.SetValue(_T("newroomid"), m_szNewRoomKey);
	tbl.SetValue(_T("newbedid"), m_szNewBedKey);
	tbl.SetValue(_T("doctor"), m_szDoctorKey);
	tbl.SetValue(_T("nurse"), m_szNurseKey);

	tbl.SetValue(_T("move"), m_bMove?_T("Y"):_T("N"));

	szSQL.Format(_T("HMS_BED_CHANGE(%s) "), tbl.FormatSQL());
	...
	else
	{
		switch(ret){
			case 0:
				break;
			default:
				ShowMessageBox(_T("\x43\xF3 l\x1ED7i khi \x63\x1EADp nh\x1EADt \x64\x1EEF li\x1EC7u"));
		};
	}
