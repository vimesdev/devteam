...
void CHMSServicePackageDetailDialog::OnCreateComponents(){
	...
	m_wndOrderQtyLabel.Create(this, _T("Order Qty"), 5, 95, 125, 120);
	m_wndOrderQty.Create(this,130, 95, 250, 120); 
	m_wndRealQtyLabel.Create(this, _T("Real Qty"), 255, 95, 375, 120);
	...
	m_wndTerminate.Create(this, _T("&Terminate"), 334, 558, 414, 583);
...
void CHMSServicePackageDetailDialog::OnInitializeComponents(){
	...
	m_wndList.InsertColumn(0, _T("Date"), CFMT_TEXT, 80);
	m_wndList.InsertColumn(1, _T("Quantity"), CFMT_NUMBER, 80);
	m_wndList.InsertColumn(2, _T("Description"), CFMT_TEXT, 200);
	m_wndList.InsertColumn(3, _T("Food Ordered"), CFMT_TEXT, 100);
...
/*
	...
	while(!rs.IsEOF()){ 
		m_wndList.AddItems(
			...
			rs.GetValue(_T("FoodOrdered")), NULL);
		rs.MoveNext();
	}
	m_wndList.EndLoad(); 
	return nCount;
