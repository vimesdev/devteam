...
void CHMSServicePackageFamilyFoodOrderDialog::OnCreateComponents(){
	...
	m_wndLunchLabel.Create(this, _T(""), 0, 0, 0, 0);
	m_wndLunchLabel.ShowWindow(SW_HIDE);
	m_wndLunchLabel.EnableWindow(FALSE);
	m_wndLunch.Create(this, 0, 0, 0, 0);
	m_wndLunch.ShowWindow(SW_HIDE);
	m_wndLunch.EnableWindow(FALSE);
	m_wndDinnerLabel.Create(this, _T(""), 0, 0, 0, 0);
	m_wndDinnerLabel.ShowWindow(SW_HIDE);
	m_wndDinnerLabel.EnableWindow(FALSE);
	m_wndDinner.Create(this, 0, 0, 0, 0);
	m_wndDinner.ShowWindow(SW_HIDE);
...
void CHMSServicePackageFamilyFoodOrderDialog::OnInitializeComponents(){
	...
	ShowMessageBox(_T("Để biết người nhà BN đã được báo vào phiếu nào -> nhấn vào nút $DS đã báo$ trên Form này. Người nhà cần phải được cấp thẻ xác nhận lưu trú! "));
...
_T("  %s ") \
_T(" AND   NVL(hfl_subitem,'N') = 'NN' and hfe_status not in ('R', 'S', 'O', 'C')") \
...
void CHMSServicePackageFamilyFoodOrderDialog::OnApplySelect(){
	...
	for (int i=0; i < m_wndSelectedList.GetItemCount(); i++)
	{
		...
		long nExtra_id = str2long(m_wndSelectedList.GetItemText(i,7));
		CString szVipA;
		...
		if(szVipA == _T("A"))
			...
			szVipA.Empty();
		szSQL.Format(_T("HMS_FOODORDER_APPLY_V1_DOCNO('%s', '%s', %ld, '%s',%ld,'%s','%s', %ld) "),
			pMF->GetCurrentUser(), szOrderDate, m_nOID, pMF->m_szDept,nDocNo,m_szBreakfastKey,szVipA,nExtra_id );
