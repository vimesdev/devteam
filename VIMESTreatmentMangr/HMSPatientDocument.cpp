...
#include "HMSBirthCertificateDialog.h"
...
#include "HMSCreatininHisStorery.h"
...
static void _OnHasAllergyLabelSelectFnc(CWnd* pWnd) {
	CHMSPatientDocument* pVw = (CHMSPatientDocument*)pWnd;
	pVw->OnHasAllergyLabelSelect();
}
...
static long _OnTreatHistoryLoadDataFnc(CWnd* pWnd) {
	return ((CHMSPatientDocument*)pWnd->GetParent())->OnTreatHistoryLoadData();
}
static void _OnTreatHistoryDblClickFnc(CWnd* pWnd) {
	((CHMSPatientDocument*)pWnd->GetParent())->OnTreatHistoryDblClick();
}
static void _OnTreatHistorySelectChangeFnc(CWnd* pWnd, int nOldItem, int nNewItem) {
	((CHMSPatientDocument*)pWnd->GetParent())->OnTreatHistorySelectChange(nOldItem, nNewItem);
}
...
static int _OnPrintBirthCetificateFnc(CWnd* pWnd) {
	((CHMSPatientDocument*)pWnd)->OnPrintBirthCetificate();
	return 0;
}
...
static int _OnViewRelativeInformationFnc(CWnd* pWnd) {
	((CHMSPatientDocument*)pWnd)->OnViewRelativeInformation();
	return 0;
}

static int _OnShowCreatininFnc(CWnd* pWnd) {
	((CHMSPatientDocument*)pWnd)->OnShowCreatinin();
	return 0;
}

static int _OnCreateClinicalRecordHtmlFnc(CWnd* pWnd)
{
	((CHMSPatientDocument*)pWnd)->OnCreateClinicalRecordHtml();
	return 0;
}

static void _OnUpdatePatientInfor_LabelSelectFnc(CWnd* pWnd)
{
	((CHMSPatientDocument*)pWnd)->OnUpdatePatientInforSelect();
}

static int _OnDeptListRollbackFnc(CWnd* pWnd)
{
	return ((CHMSPatientDocument*)pWnd)->OnDeptListRollback();
}
static int _OnEditInforFnc(CWnd* pWnd)
{
	((CHMSPatientDocument*)pWnd)->OnEditInfor();
	return 0;
}

static void _OnViewExamHistSelectFnc(CWnd* pWnd) {
	CHMSPatientDocument* pVw = (CHMSPatientDocument*)pWnd;
	pVw->OnViewExamHistSelect();
}

static int _OnLoadListByPatientNoInforFnc(CWnd *pWnd)
{
    ((CHMSPatientDocument *)pWnd)->OnLoadListByPatientNoInfor();
    return 0;
}

static int _OnRefreshInforFnc(CWnd *pWnd)
{
    ((CHMSPatientDocument *)pWnd)->OnRefreshInfor();
    return 0;
}
...
void CHMSPatientDocument::OnCreateComponents() {
	m_wndProfiles.Create(this, _T("Thông tin hành chính"), 5, 5, 425, 265);
	m_wndHasAllergyLabel.Create(this, _T("Bệnh nhân có tiền sử dị ứng"), 205, 5, 425, 30);
	...
	m_wndAddress.Create(this, 120, 150, 420, 200, FALSE, TRUE, TRUE);
	...
	m_wndMainDiseaseDiagnosticLabel.Create(this, L"Chẩn đoán", 10, 385, 115, 410);
	m_wndMainDiseaseDiagnostic.Create(this, 120, 385, 420, 465, FALSE, TRUE, TRUE);
	m_wndRelationDiseaseLabel.Create(this, _T("Bệnh kèm theo"), 10, 440, 115, 465);
	m_wndRelationDisease.Create(this, 120, 440, 420, 465, FALSE, TRUE, TRUE);
	...
	m_wndAntibioticUse.Create(this, _T("HDSD Kháng sinh"), 10, 440, 115, 465);


	m_wndDeptList.Create(this, 5, 505, 425, 643);
	m_wndDeptList.SetItemHeight(45);

	//m_wndInfoTab.Create(this, CRect(5, 505, 400, 643));
	//m_wndDeptList.Create(&m_wndInfoTab, 5, 5, 400, 643);
	//m_wndTreatHistory.Create(&m_wndInfoTab, 5, 5, 400, 643);
	//m_wndDiseaseHistory.Create(&m_wndInfoTab, 5, 5, 400, 643);

	m_wndRelationDisease.ShowWindow(SW_HIDE);
	m_wndRelationDiseaseLabel.ShowWindow(SW_HIDE);

	m_wndTreatmentTab.Create(this, 430, 5, 1020, 580);
	...
	*/
	...
	m_wndExamHistory.Create(this, _T("Lịch sử khám"), 290, 270, 380, 295);
	//m_wndExamHistory.SetTextColor(RGB(255, 0, 0));
...
void CHMSPatientDocument::OnInitializeComponents() {
	...
	m_wndCareView.Create(&m_wndTreatmentTab);
	...
	//m_wndPatientEMR.Create(&m_wndTreatmentTab);

	m_wndTreatmentTab.SetPadding(CSize(8, 8));
	m_wndInfusion.Create(&m_wndTreatmentTab);
	
	

	m_wndTreatmentTab.Add(_T("Tổng quan"), &m_wndOverview);
	m_wndTreatmentTab.Add(_T("Giường"), &m_wndBed);
	m_wndTreatmentTab.Add(_T("Xem bệnh trình"), &m_wndExamine);
	m_wndTreatmentTab.Add(_T("Xem phiếu chăm sóc"), &m_wndCareView);
	m_wndTreatmentTab.Add(_T("Cận lâm sàng"), &m_wndParaclinicalOrder);
	m_wndTreatmentTab.Add(_T("Phẫu thuật, thủ thuật"), &m_wndOperationOrder);
	m_wndTreatmentTab.Add(_T("Thuốc, vật tư"), &m_wndDrug);
	m_wndTreatmentTab.Add(_T("Truyền dịch"), &m_wndInfusion);
	m_wndTreatmentTab.Add(_T("Dịch vụ trọn gói"), &m_wndTreatPackageView);
	...
	//	m_wndTreatmentTab.Add(_T("Hospital Fee"), &m_wndFeeDocument2);	
		...
		//m_wndTreatmentTab.Add(_T("Hồ sơ BAĐT"), &m_wndPatientEMR);



		//m_wndInfoTab.Add(L"Khoa", &m_wndDeptList);
		//m_wndInfoTab.Add(L"Tiền sử điều trị", &m_wndTreatHistory);
		//m_wndInfoTab.Add(L"Disease history", &m_wndDiseaseHistory);
		//m_wndInfoTab.SetPadding(CSize(10, 10));
		//m_wndInfoTab.SetCurSel(0);
	...
	m_wndHasAllergyLabel.SetTextColor(RGB(255, 0, 0));
	m_wndHasAllergyLabel.ShowWindow(SW_HIDE);
	m_wndPatientNameLabel.SetHyperlink(true);

	m_wndDeptList.InsertColumn(0, _T("Department"), CFMT_TEXT, 150);
	m_wndDeptList.InsertColumn(1, _T("Admit Date"), CFMT_TEXT, 120);
	m_wndDeptList.InsertColumn(2, _T("Discharge Date"), CFMT_TEXT, 120);
	m_wndDeptList.InsertColumn(3, _T("Status"), CFMT_TEXT, 70);
	m_wndDeptList.InsertColumn(4, _T("refidx"), CFMT_NUMBER, 0);
	m_wndDeptList.InsertColumn(5, _T("treattime"), CFMT_NUMBER, 70);
	m_wndDeptList.InsertColumn(6, _T("cancertime"), CFMT_NUMBER, 0);
    m_wndDeptList.InsertColumn(7, _T("docno"), CFMT_NUMBER, 0);


	m_wndTreatHistory.InsertColumn(0, _T("Doc No"), CFMT_TEXT, 80);
	m_wndTreatHistory.InsertColumn(1, _T("Dept"), CFMT_TEXT, 70);
	m_wndTreatHistory.InsertColumn(2, _T("Admit Date"), CFMT_TEXT, 125);
	m_wndTreatHistory.InsertColumn(3, _T("Discharge Date"), CFMT_TEXT, 125);
	m_wndTreatHistory.InsertColumn(4, _T("treattime"), CFMT_NUMBER, 70);



	m_wndDiseaseHistory.InsertColumn(0, _T("Doc No"), CFMT_TEXT, 70);
	m_wndDiseaseHistory.InsertColumn(1, _T("Date"), CFMT_TEXT, 80);
	...
	m_wndExamine.m_pWndBenhTrinh = this;
	m_wndCareView.m_pWndCareView = this;
...
void CHMSPatientDocument::OnSetWindowEvents() {
	...
	{
		m_wndDeptList.AddEvent(1, _T("Khôi phục lại lượt điều trị"), _OnDeptListRollbackFnc);
		m_wndDeptList.AddEvent(0, _T("-"), NULL);
		m_wndDeptList.AddEvent(0, _T("-"), NULL);
		m_wndDeptList.AddEvent(16, _T("Sửa thông tin vào/ra"), _OnEditInforFnc);

	}
    m_wndDeptList.AddEvent(0, _T("-"), NULL);
    m_wndDeptList.AddEvent(17, _T("Xem lịch sử (theo mã BN)"), _OnLoadListByPatientNoInforFnc);

    m_wndDeptList.AddEvent(0, _T("-"), NULL);
    m_wndDeptList.AddEvent(18, _T("Refresh"), _OnRefreshInforFnc);
	...
	m_wndHasAllergyLabel.SetEvent(WE_CLICK, _OnHasAllergyLabelSelectFnc);
	...
	//Nhấn vào để cập nhập thông tin hành chính trong khoa
	m_wndPatientNameLabel.SetEvent(WE_CLICK, _OnUpdatePatientInfor_LabelSelectFnc);

	m_wndExamHistory.SetEvent(WE_CLICK, _OnViewExamHistSelectFnc);
	...
	*/
	...
	AddEvent(0, _T("-"));
	AddEvent(16, _T("Cấp giấy chứng sinh"), _OnPrintBirthCetificateFnc);
	AddEvent(0, _T("-"));
	AddEvent(13, _T("In phiếu hẹn"), _OnAppointmentReTreatmentFnc);
	//AddEvent(0, _T("-"));
	//AddEvent(12, _T("Print Forms Dialog"), _OnPrintFormsFnc);

	AddEvent(0, _T("-"));
	AddEvent(17, _T("Xem thông tin người nhà bệnh nhân"), _OnViewRelativeInformationFnc);
	AddEvent(0, _T("-"));
	AddEvent(18, _T("Xem chỉ số Creatinin"), _OnShowCreatininFnc);

	AddEvent(0, _T("-"));
	AddEvent(19, _T("Bệnh án điều trị mới"), _OnCreateClinicalRecordHtmlFnc);
	...
	//m_wndDocumentNo.SetTextColor(RGB(255, 0, 0));
	SetWindowFont(&m_wndExamHistory, GetFaceName(), GetFaceSize() + 2, true);
	m_wndExamHistory.SetTextColor(RGB(255, 128, 0));
	...
	if (rect.Height() <= 768)
	{
		//ResizeControl(this, &m_wndAddress, 25);
		//ResizeControl(this, &m_wndMainDiseaseDiagnostic, 50);
		m_wndDeptList.SetItemHeight(30);
		m_wndTreatHistory.SetItemHeight(30);
	...
	m_wndElectronicRecord.Create(this, _T("Mẫu biểu chuyên môn"), 855, 610, 995, 640);
	m_wndExtButton.Create(this, _T("@"), 1000, 610, 1025, 640);
	m_wndHasAllergyLabel.Create(this, _T("Bệnh nhân có tiền sử dị ứng"), 5, 480, 425, 500);
	*/
...
void CHMSPatientDocument::GetDataToScreen() {
	...
	szSQL.Format(_T(" SELECT hcr_recordno as recordno,	hp_patientno as patientno,") \
		...
		_T("	trunc_date(htr_updateddate) as updated_date, hd_isinpatient, htr_combinedtreat, ") \
		_T("	htr_ctdeptid, htr_treattime, hd_istransplant, hcr_cancer AS cancer, htr_cancer_time, ") \
		_T("	hd_hasallergy as hasallergy") \
		_T(" FROM hms_patient") \
	...
	szSQL.Format(_T(" SELECT") \
		_T(" HTM_TREAT_MOVEMENT_ID as treatmove_id,") \
		_T(" htm_refidx as refidx,") \
		_T(" HTM_PERFORM_DEPTID as deptid,") \
		_T(" htm_status as status") \
		_T(" FROM HMS_TREAT_MOVEMENT WHERE htm_status = 'O' AND htm_docno = %ld AND HTM_PERFORM_DEPTID='%s' AND htm_refidx =%d "), pMF->m_nDocumentNo, pMF->m_szDept, pMF->m_nRefIndex);

	rsl.ExecSQL(szSQL);

	if (!rsl.IsEOF())
	{
		rsl.GetValue(_T("deptid"), m_szSendDept);
		rsl.GetValue(_T("status"), m_szSendDeptStatus);
	}
	...
	if (!rs.IsEOF())
	{
		...
		pMF->AddProp(L"patient_name", m_szPatientName);
		pMF->m_szPatientName = m_szPatientName;
		...
		//Bổ sung điều kiện bệnh nhân điều trị kết hợp trong table HMS_TREAT_MOVEMENT: Trạng thái O && khoa gửi đến = khoa đăng nhập

		if ((tmpStr == _T("Y") && ctdeptid == pMF->m_szDept) || (m_szSendDept == pMF->m_szDept && m_szSendDeptStatus == _T("O")))
		{
			pMF->m_szOutpatient = _T("Y");
			m_bCombinedTreatment = true;
		}

		if (m_szDepartment_ID != pMF->m_szDept && !m_bCombinedTreatment && m_szSendDept != pMF->m_szDept && m_szSendDeptStatus != _T("O"))
		{
		...
		rs.GetValue(_T("htr_cancer_time"), pMF->m_nCancerTime);
		rs.GetValue(_T("hasallergy"), tmpStr);
		m_wndHasAllergyLabel.ShowWindow(SW_HIDE);
		if (tmpStr == _T("Y"))
		{
			m_wndHasAllergyLabel.ShowWindow(SW_SHOW);
		}
	}

	m_wndTreatmentTab.SetCurSel(0);
	//reload data mẫu biểu when select other patient
	CHMSOverview* pView = (CHMSOverview*)m_wndTreatmentTab.GetWindowAt(0);
	if (pView)
	{
		pView->OnTabSelectChange(0, 0);
	}

	OnDeptListLoadData();
	//OnTreatHistoryLoadData();
	//OnDiseaseHistoryLoadData();
	SetMode(VM_VIEW);

	//khám chuyên khoa disable khi hồ sơ kết thúc
...
void CHMSPatientDocument::OnSuggestionBtnSelect()
{
	...
	CRecord rs(&pMF->m_db);
	CRecord rsx(&pMF->m_db);
	CString szSQL, szStatus, tmpStr, szFromDept, szFromDeptStatus;
	...
	szSQL.Format(_T("SELECT count(*) ") \
		...
		_T(" AND htr_idx > %d and htr_status <>'A' ")
		, m_nDocumentNo, pMF->m_nRefIndex);
	rs.ExecSQL(szSQL);
	if (rs.GetIntValue())
	{
		ShowMessageBox(_T("Lượt điều trị này đã kết thúc, không thể thao tác!"));
		return;
	}
	//_msg(_T("%s"), m_szSendDeptStatus);
	//Kiểm tra khi kết thúc hồ sơ, nếu còn phiếu chuyển khám chuyên khoa trạng thái O thì yêu cầu phải đóng lại	

	szSQL.Format(_T(" SELECT") \
		_T(" HTM_TREAT_MOVEMENT_ID as treatmove_id,") \
		_T(" htm_refidx as refidx,") \
		_T(" HTM_SEND_DEPTID as deptid,") \
		_T(" htm_status as status") \
		_T(" FROM HMS_TREAT_MOVEMENT WHERE htm_status = 'O' AND htm_docno = %ld AND HTM_SEND_DEPTID='%s' AND htm_refidx =%d "), pMF->m_nDocumentNo, pMF->m_szDept, pMF->m_nRefIndex);

	rsx.ExecSQL(szSQL);

	if (!rsx.IsEOF())
	{
		rsx.GetValue(_T("deptid"), szFromDept);
		rsx.GetValue(_T("status"), szFromDeptStatus);
	}

	if (m_bCombinedTreatment && (szFromDeptStatus == _T("O")))
	{
		ShowMessageBox(_T("Có phiếu gửi khám chuyên khoa chưa kết thúc, vui lòng kết thúc trước!"));
	...
	//Bệnh nhân thuộc đối tượng quân đôi thì yêu cầu nhập thông tin sĩ quan, quân nhân chuyên nghiệp....
	CString szisarmobjectpat;

	szSQL.Format(_T("SELECT IS_ARM_OBJECT_PATIENT(%ld) as isarmobjectpat FROM dual"), m_nDocumentNo);
	rs.ExecSQL(szSQL);

	rs.GetValue(_T("isarmobjectpat"), szisarmobjectpat);

	if (szisarmobjectpat == _T("N"))
	{
		ShowMessageBox(_T("Bệnh nhân thuộc đối tượng quân nhân, cần nhập thông tin: Sĩ quan/quân nhân chuyên nghiệp/CNVQP/HSQCS!"));
	...
	//Check bệnh nhân bảo hiểm chưa nhập thông tin mã đối tượng khám chữa bệnh
	/*
	if (pMF->GetObjectType() == _T("I") || pMF->GetObjectType() == _T("C"))
	{
		CString szmadoituongkcb;

		szSQL.Format(_T("SELECT CHECK_MA_DOITUONG_KCB(%ld) as szmadoituongkcb FROM dual"), m_nDocumentNo);
		rs.ExecSQL(szSQL);

		rs.GetValue(_T("szmadoituongkcb"), szmadoituongkcb);

		if (szmadoituongkcb == _T("N"))
		{
			ShowMessageBox(_T("Cần cập nhật thông tin mã đối tượng KCB cho bệnh nhân!"), MB_ICONERROR);
			return;
		}
	}
	*/
	...
	szSQL.Format(_T("select count(*) from VIMES.hms_fee_extra ") \
		...
		_T("	and hfe_status not in ('T', 'R', 'C')  "), pMF->m_nDocumentNo, pMF->m_nRefIndex, m_szDepartment_ID);
	rs.ExecSQL(szSQL);

	if (rs.GetIntValue() > 0)
	{
		ShowMessageBox(_T("Bệnh nhân có gói dịch vụ chưa chốt (trạng thái khác T), yêu cầu chốt gói trước khi kết thúc hồ sơ"), MB_OK);
		return;
	}

	szSQL.Format(
        _T(" select count(*) from hms_operation ")
        _T(" where ho_docno=%ld and ho_refidx = %d and ")
...
void CHMSPatientDocument::OnTreatmentTabSelectChange(int nOld, int nNew) {
	...
	case 0:
		...
		break;
	case 1:
		m_wndBed.RefreshData();
		break;
	case 2:
		m_wndExamine.RefreshData();
		break;
	case 3:
		m_wndCareView.RefreshData();
		break;
	case 4:
...
void CHMSPatientDocument::OnDeptListSelectChange(int nOldItem, int nNewItem) {
    ...
    if (m_bViewByPatientNo)
    {
        pMF->m_nDocumentNo = str2long(m_wndDeptList.GetItemText(nNewItem, 7));
    }
    else
    {
	...
	if (m_wndOverview.IsWindowVisible())
	{
		m_wndOverview.RefreshData();
	}

	if (m_wndExamine.IsWindowVisible())
    {
        m_wndExamine.RefreshData();
    }
	if (m_wndCareView.IsWindowVisible())
    {
        m_wndCareView.RefreshData();
    }

	if (m_wndInfusion.IsWindowVisible())
    {
        m_wndInfusion.RefreshData();
    }
...
long CHMSPatientDocument::OnDeptListLoadData() 
{
	...
	if (m_bViewByPatientNo)
    {
        szSQL.Format(_T(" SELECT 	htr_deptid as deptid, htr_docno as docno, ")
                     _T(" htr_docno||'/'||(select sd_name from sys_dept where ")
                     _T("sd_id=htr_deptid) as department,") \
		_T(" 	TO_CHAR(htr_admitdate, 'DD/MM/YYYY HH24:MI') as admitdate,")
        _T(" 	case when htr_status='T' then to_char(htr_dischargedate, ")
        _T("'DD/MM/YYYY HH24:MI') ELSE NULL end as dischargedate,")
        ...
        _T(" WHERE htr_patientno=%ld ORDER BY htr_docno, htr_idx "),
                     pMF->m_nPatientNo);
    }
    else
    {
        szSQL.Format(_T(" SELECT 	htr_deptid as deptid, htr_docno as docno, ")
                     _T(" 	(select sd_name from sys_dept where ")
                     _T("sd_id=htr_deptid) as department,") \
		_T(" 	TO_CHAR(htr_admitdate, 'DD/MM/YYYY HH24:MI') as admitdate,")
        _T(" 	case when htr_status='T' then to_char(htr_dischargedate, ")
        _T("'DD/MM/YYYY HH24:MI') ELSE NULL end as dischargedate,")
        ...
        _T(" FROM hms_treatment_record")
        _T(" LEFT JOIN hms_clinical_record ON(hcr_docno = htr_docno)")
        _T(" WHERE htr_docno=%ld ORDER BY htr_idx "),
                     m_nDocumentNo);
    }
...
void CHMSPatientDocument::OnTreatHistoryDblClick() {
	int nSel = m_wndTreatHistory.GetCurSel();
	if (nSel < 0) return;
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();

}
void CHMSPatientDocument::OnTreatHistorySelectChange(int nOldItem, int nNewItem) {

}

long CHMSPatientDocument::OnTreatHistoryLoadData() {
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	CString szDischargeDate;

	m_wndTreatHistory.BeginLoad();

	int nCount = 0;
	int nRefIdx, nCurrSel = 0;
	szSQL.Format(_T(" SELECT htr_docno as docno, htr_deptid as deptid, ") \
		...
		_T(" 	TO_CHAR(htr_admitdate, 'DD/MM/YYYY HH24:MI') as admitdate,") \
		_T(" 	case when htr_status='T' then to_char(htr_dischargedate, 'DD/MM/YYYY HH24:MI') ELSE NULL end as dischargedate,") \
		_T(" 	htr_status as status,") \
		_T(" 	htr_idx as refidx, htr_treattime, hcr_cancer, htr_cancer_time ") \
		_T(" FROM hms_treatment_record") \
		_T(" LEFT JOIN hms_clinical_record ON(hcr_docno = htr_docno)") \
		_T(" WHERE htr_patientno=%ld ORDER BY htr_idx "), m_nPatientNo);
	nCount = rs.ExecSQL(szSQL);
	//_msg(_T("%s"), szSQL);
	while (!rs.IsEOF())
	{
...
void CHMSPatientDocument::OnDiseaseHistoryDblClick() {
	int nSel = m_wndDiseaseHistory.GetCurSel();
	if (nSel < 0) return;
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
...
void CHMSPatientDocument::LoadData(long nPatientNo, long nDocNo, int nIndex) {
	...
	GetDataToScreen();
	_tprintf(_T("\ncancer: %s"), pMF->m_szCancer);
	if (pMF->m_szCancer == _T("Y"))
	{
		m_wndBed.m_wndByTreatTime.SetCheck(TRUE);
	}
	else
	{
		m_wndBed.m_wndByTreatTime.SetCheck(FALSE);
	}
	m_wndOverview.RefreshData();
	//m_wndBed.RefreshData();
...
bool CHMSPatientDocument::OnCheckTerminateDocument(long nDocument) {
	...
	szSQL.Format(_T("SELECT * FROM HMS_PHARMAORDER_VIEW WHERE hpo_docno=%ld AND hpo_orderstatus in('O','S') and nvl(hpo_outpatient_order,'N') <>'Y' "), nDocument);
	rs.ExecSQL(szSQL);
	if (!rs.IsEOF())
	{
		...
		{
			TranslateString(_T("Đơn thuốc PTTT [%s] trong số phiếu [%s] ngày tạo [%s] chưa cấp. Không thể kết thúc"), tmpStr);
		...
		else
		{
			...
			TranslateString(_T("Prescription [%s] in the daily order [%s] date [%s] khoa [%s] don't issue. Can not terminate bed"), tmpStr);
			szMsg.Format(tmpStr, rs.GetValue(_T("hpo_orderid")), rs2.GetValue(_T("mt_orderno")), CDate::Convert(rs.GetValue(_T("hpo_orderdate")), yyyymmdd, ddmmyyyy), rs.GetValue(_T("hpo_deptid")));
		}
		ShowMessageBox(szMsg, MB_OK);
		return false;
	}
...
void CHMSPatientDocument::OnRecordSummaryDischarge()
{
	...
	if (pMF->IsDocumentReadOnly() && (!pMF->CheckPermission(_T("20.01"))))
	{
		ShowMessageBox(_T("Cannot process with current status"));
		return;
	}
...
void CHMSPatientDocument::OnResizeLayout()
{
	...
	//	AddLayoutControls(WS_RESIZEX, 0, 0, 20, 0,&m_wndDepartments,NULL);
		...
		//AddLayoutControls(WS_RESIZEX|WS_RESIZEY, 00, 0, 100, 100,&m_wndTreatmentTab,NULL);
		//AddLayoutControls(WS_REPOSX|WS_REPOSY, 0, 100, 0, 0, &m_wndTreatmentTracking, &m_wndPatientCare,&m_wndClinicalRecord, &m_wndElectronicRecord, &m_wndExtButton, NULL);
}
...
void CHMSPatientDocument::OnTreatmentTrackingSelect()
{
	...
	/*
	CReportUtils ru;
	if (ru.CheckAllergy(m_nDocumentNo) < 0)
	{
		return;
	}
	*/
...
void CHMSPatientDocument::OnClinicalRecordSelect()
{
	...
	if (szStatus == _T("A"))
	{
		ShowMessageBox(_T("Bệnh nhân chưa được tiếp nhập vào khoa, không thể cập nhật bệnh án được!"));
		return;
	}
	...
	//Nếu đổi bệnh án mà bệnh án trong hms_doc_emr không thay đổi theo thì thông báo
	int nCount = 0;
	szSQL.Format(_T("CHANGE_RECORD_NO_NOTICE(%ld, %ld) "), m_nDocumentNo, pMF->m_nTreatTime);
	nCount = str2int(pMF->ExecDML(szSQL));

	if (nCount > 0)
		ShowMessageBox(_T("Số bệnh án của bệnh nhân đã bị thay đổi!"), MB_ICONWARNING);


	CTMBenhAnDialog dlg(this, VM_EDIT, m_nDocumentNo, pMF->m_szRecordNo, m_szCancer, m_nHtrIdx, m_nCancerTime);
	if (dlg.DoModal() == IDOK)
	{
...
void CHMSPatientDocument::OnElectronicRecordSelect()
{
	...
	CHMSOldTreatmentHtmlDialog dlg(this, _T(""));
	if (dlg.DoModal() == IDOK)
	{
...
#include "../HMSCore/BankCustomer.h"
...
int CHMSPatientDocument::OnPrintBirthCetificate()
{
	if (m_nDocumentNo <= 0)
		return -1;
	CHMSBirthCertificateDialog dlg(this);
	dlg.m_nDocNo = m_nDocumentNo;
	dlg.DoModal();
	return 1;
}
...
int CHMSPatientDocument::OnViewRelativeInformation()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CString szSQL;
	CRecord rs(&pMF->m_db);
	...
	if (m_nDocumentNo <= 0)
		return 0;
	CViewRelativeInformation dlg(this);
	dlg.DoModal();
	SetMode(VM_VIEW);
	return 0;
}

int CHMSPatientDocument::OnShowCreatinin()
{
	CHMSCreatininHisStorery dlg(this);
	dlg.DoModal();
	return 0;
}

int CHMSPatientDocument::OnCreateClinicalRecordHtml()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	if (m_nDocumentNo <= 0)
		return -1;
	...
	pChuyenkhoa->Init(GetMode(), pMF->m_nDocumentNo, pMF->m_szRecordNo, m_nHtrIdx, pMF->m_nTreatTime);
	dlg.DoModal();
	return 0;
}

void CHMSPatientDocument::OnHasAllergyLabelSelect()
{
	CString szType = _T("KB_KHAITHACTIENSUDIUNG");
	CHMSSurgeryOperationHtmlDialog dlg(this, szType, TRUE);
	dlg.DoModal();
}

void CHMSPatientDocument::OnUpdatePatientInforSelect()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();

	/*if (!pMF->CheckPermission(_T("admin.TM.100.2")))
	{
		ShowMessageBox(_T("Không có quyền thao tác, liên hệ admin (cấp quyền = admin.TM.100.2)"));
		return;
	}*/
...
int CHMSPatientDocument::OnDeptListRollback()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CString szSQL, tmpStr;
	int ret = 10;
	...
	//Hồ sơ kết thúc rồi thì thôi
	szSQL.Format(_T("SELECT count(*) FROM HMS_CLINICAL_RECORD WHERE hcr_docno=%ld AND hcr_status='T' "), pMF->m_nDocumentNo);
	rs.ExecSQL(szSQL);

	if (rs.GetIntValue() > 0)
	{
		ShowMessageBox(_T("Hồ sơ bệnh án đã kết thúc, không cho phép thao tác!"), MB_OK);
		return -1;
	}
	//Hồ sơ đã xác nhận phí rồi thì thôi
	szSQL.Format(_T("SELECT count(*) FROM hms_fee_approve WHERE hfe_docno=%ld and hfe_refidx=%d "),
		m_nDocumentNo, pMF->m_nRefIndex);
	...
	//Đúng ông user quản trị mới được phép thao tác

	if (pMF->m_UserInfo.su_groupid == _T("A"))

		ret = ShowMessageBox(_T("Bạn có muốn cập nhật lại trạng thái điều trị của khoa đang chọn không? Chức năng này dành cho user quản trị, sau khi dùng xong cần khôi phục lại như cũ(Y/N)"), MB_YESNO | MB_DEFBUTTON2);
	if (ret == IDNO)
		return 0;

	szSQL.Format(_T(" HMS_TREATMENT_ROLLBACK_ADMIN(%ld, %d, '%s')"), pMF->m_nDocumentNo, pMF->m_nRefIndex, pMF->GetCurrentUser());
	ret = str2int(pMF->ExecDML(szSQL));

	if (ret == -1)
	{
		ShowMessageBox(_T("Bệnh nhân chưa vào điều trị"));
		return 0;
	}
	if (ret == -2)
	{
		ShowMessageBox(_T("Lượt điều trị này đã được xác nhận phí hoặc thanh toán"));
		return 0;
	}
	OnDeptListLoadData();
	return 0;
}
void CHMSPatientDocument::OnEditInfor()
{
	int nSel = m_wndDeptList.GetCurSel();
	if (nSel < 0)
		return;
	...
	CHMSEditParaInfor dlg(this);
	dlg.m_nDocumentNo = m_nDocumentNo;
	dlg.m_nTreattime = pMF->m_nTreatTime;
	dlg.m_nRef = pMF->m_nRefIndex;
	dlg.DoModal();
}

void CHMSPatientDocument::OnViewExamHistSelect()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
...
void CHMSPatientDocument::OnSize(UINT nType, int cx, int cy)
{
	...
	rc = rect;
	rc.left += 425;
	m_wndTreatmentTab.MoveWindow(rc);
	rc = rect;
	rc.left = 5;
	rc.top = 505;
	
	
	if (rcDesktop.Height() <= 768)
    {
        m_wndDeptList.SetItemHeight(30);
        ...
        rc.left = 120;
        rc.top = rect.top + 385;
        rc.bottom = rect.bottom - 165;
        m_wndMainDiseaseDiagnostic.MoveWindow(rc);

        rc.right = 110;
        rc.left = 5;
        rc.top = rect.top + 405;
        //rc.bottom = rect.bottom - 150 - 15;
        m_wndAntibioticUse.MoveWindow(rc);

        rc.left = 10;
        rc.top = rect.top + 430;
        rc.right = 110;
        rc.bottom = rc.top + 25;
        m_wndSuggestionLabel.MoveWindow(rc);

        rc.left = 120;
        rc.right = 320;
        rc.top = rect.top + 430;
        rc.bottom = rc.top + 25;
        m_wndSuggestion.MoveWindow(rc);

        rc.left = 325;
        rc.right = rc.left + 100;
        rc.top = rect.top + 430;
        rc.bottom = rc.top + 25;
        m_wndSuggestionBtn.MoveWindow(rc);

		rc.right = 425;
        rc.bottom = rect.bottom - 5;
        rc.top = rect.top + 455;
        rc.left = 5;
        // m_wndInfoTab.MoveWindow(rc);
        m_wndDeptList.MoveWindow(rc);
    }
	else
	{
        rc.right = 425;
...
void CHMSPatientDocument::OnViewEMR() {
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
    pMF->ViewEMRSignature();    
}
void CHMSPatientDocument::UpdateLayout(float scale)
{
	if (!m_wndDeptList.GetSafeHwnd())
		return;
	cout << scale << endl;
	if (scale < 1.0)
	{
		m_wndDeptList.SetItemHeight(30);
		m_wndDeptList.SetScale(scale);
	}
	else
	{
		m_wndDeptList.SetScale(1.0);
		m_wndDeptList.SetItemHeight(40);
	}
	m_wndDeptList.RedrawWindow();
}

void CHMSPatientDocument::CheckCCCD(LPCTSTR lpszCard)
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
...
int CHMSPatientDocument::ProcessIdCardQrInput(CString szInfo)
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CString szSQL;
...
int CHMSPatientDocument::OnLoadListByPatientNoInfor() 
{
    UpdateData(true);
    m_bViewByPatientNo = true;
    UpdateData(false);
    OnDeptListLoadData();   
    return 0;
}

int CHMSPatientDocument::OnRefreshInfor()
{
    UpdateData(true);
    m_bViewByPatientNo = false;
    UpdateData(false);
...
/*
void CHMSPatientDocument::SetCardInfo(JSONVALUE& jData, CardInfo& stcCard)
{
	bool bValue;
...
void CHMSPatientDocument::UpdateRelativeInfo(CString idCard)
{
    if (m_nDocumentNo <= 0 || !m_wndFeeDocument.IsWindowVisible())
        return;
    CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();

    CBankCustomer dlg(this);
    dlg.m_bCheckCCCD = TRUE;
    dlg.m_szIDCardFM = idCard;
    dlg.m_nDocumentNo = m_nDocumentNo;
    dlg.m_szDeptID = m_szDept;
    dlg.m_szPatientName = m_szPatientName;
    dlg.m_szStatus = m_szStatus;
    dlg.m_nTreatIdx = pMF->m_nTreatTime;
    dlg.DoModal();
}