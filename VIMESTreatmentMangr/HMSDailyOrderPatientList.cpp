...
static int _OnPrintDailyDrugExFnc(CWnd *pWnd){
	 ((CHMSDailyOrderPatientList*)pWnd)->OnPrintDailyDrugEx();
	 return 0;
}
...
void CHMSDailyOrderPatientList::OnCreateComponents(){
	m_wndList.Create(this,0, 0, 395, 355); 
	m_wndSearchLabel.Create(this, _T("Search"), 5, 360, 65, 385);
	m_wndSearch.Create(this,70, 360, 155, 385); 
	m_wndTotalItemsLabel.Create(this, _T("Số BN/ Phiếu chia"), 160, 360, 275, 385);
	m_wndTotalItems.Create(this,280, 360, 315, 385);
	m_wndDelim.Create(this, _T("/"), 320, 360, 345, 385);
	m_wndOrderCount.Create(this,350, 360, 385, 385); 
...
//	m_wndTotalItems.SetCheckValue(true);
	...
	m_wndList.InsertColumn(9, _T("Updated By"), CFMT_TEXT, 0);
	m_wndList.InsertColumn(10, _T("RefBatOrderID"), CFMT_NUMBER, 0);
...
//	m_wndList.AddEvent(1, _T("Delete"), _OnListDeleteItemFnc, 0, VK_DELETE, 0);
	...
	m_wndList.AddEvent(0, _T("-"), NULL);
	m_wndList.AddEvent(6, _T("In phiếu lĩnh vật tư A3"), _OnPrintDailyDrugExFnc);
...
void CHMSDailyOrderPatientList::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Text(pDX, m_wndOrderCount.GetDlgCtrlID(), m_nOrderCount);
}
...
CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	...
	szSQL.Format(_T(" SELECT distinct hpo_docno as docno, HPO_UPDATEDBY as updatedby,") \
	_T(" 	trim(hp_surname||' '||hp_midname||' '||hp_firstname) as pname,") \
	_T(" 	first_value(hpo_roomid||'.'||hpo_bedid) ") \
    _T("	    over (partition by hpo_docno, trunc_date(hpo_orderdate) order by hpo_orderid desc) as bedid,") \
	//_T(" 	trunc_date(hpo_orderdate) as orderdate,  HPO_TRANSACTION_ID as RefBatOrderID") \//
...
int CHMSDailyOrderPatientList::OnUndoPatientPrescription(){
	...
	CString szSQL, m_szAddBy;

	int nSel = m_wndList.GetCurSel();
	if(nSel < 0)
		return 0;
	long nDocumentNo = str2long(m_wndList.GetItemText(nSel, 1));
	m_szAddBy = m_wndList.GetItemText(nSel, 9);

	if (pMF->GetCurrentUser() != m_szAddBy)
		{	
		CString tmpStr;
		tmpStr.Format(_T("Phiếu được add bởi [%s] không cho phép hủy phiếu"), m_szAddBy);
...
void CHMSDailyOrderPatientList::OnPrintDailyDrugEx()
{
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	int nReforderid =0;
	int nSel = m_wndList.GetCurSel();
	...
	nReforderid = ToLong(m_wndList.GetItemText(nSel, 10));	
	CPrintForms printer;
	printer.TM_OnPrintDailyDrugEx(m_nDocumentNo, m_nBatchID, m_szOrderDate.Left(10));
}
...
{
	m_Layout.Resize(&m_wndList, 100, 100);
	return;
