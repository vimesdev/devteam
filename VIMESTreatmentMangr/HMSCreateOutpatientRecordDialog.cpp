...
void CHMSCreateOutpatientRecordDialog::OnCreateComponents()
{
	...
	m_wndDepartmentAdmitedInformation.Create(this, _T("Department Admited Information"), 5, 275, 640, 445);
	...
	m_wndOutpatient.Create(this, _T("Outpatient treat"), 5, 450, 130, 475);
	m_wndEmergency.Create(this, _T("Emergency"), 135, 450, 210, 475);
	m_wndSave.Create(this, _T("&Save"), 485, 450, 560, 475);
	m_wndCancel.Create(this, _T("&Cancel"), 565, 450, 640, 475);*/

	m_wndAdmissionInformation.Create(this, _T("Admission Information"), 5, 125, 640, 325);
	m_wndContactInformation.Create(this, _T("Contact Information"), 5, 5, 640, 120);
	m_wndDepartmentAdmitedInformation.Create(this, _T("Department Admited Information"), 5, 329, 640, 569);
	m_wndDeptLabel.Create(this, _T("Dept"), 295, 354, 365, 379);
	m_wndDept.Create(this,370, 354, 635, 379); 
	m_wndRelativeLabel.Create(this, _T("Relative"), 10, 30, 160, 55);
	m_wndRelative.Create(this,165, 30, 365, 55); 
	m_wndRelationLabel.Create(this, _T("Relation"), 370, 30, 470, 55);
	m_wndRelation.Create(this,475, 30, 635, 55); 
	...
	m_wndTransferDiagnostic.Create(this,165, 210, 635, 235); 
	m_wndExamineDiseaseLabel.Create(this, _T("Examine Disease"), 10, 240, 160, 265);
	m_wndExamineDisease.Create(this,165, 240, 635, 320, true, false, true); 
	m_wndInDateLabel.Create(this, _T("In Date"), 10, 354, 160, 379);
	m_wndInDate.Create(this,165, 354, 290, 379); 
	m_wndOutpatient.Create(this, _T("Outpatient treat"), 5, 575, 130, 600);
	m_wndEmergency.Create(this, _T("Emergency"), 135, 575, 220, 600);
	m_wndAdmissionDiseaseLabel.Create(this, _T("Admission Disease"), 10, 384, 160, 409);
	m_wndAdmissionDisease.Create(this,165, 384, 365, 409); 
	m_wndDoctorLabel.Create(this, _T("Doctor"), 370, 384, 470, 409);
	m_wndDoctor.Create(this,475, 384, 635, 409); 
	m_wndMainDiseaseLabel.Create(this, _T("Main Disease"), 10, 414, 160, 439);
	m_wndMainDisease.Create(this,165, 414, 635, 506, TRUE, FALSE, TRUE); 
	m_wndRelationDiseaseLabel.Create(this, _T("Relation Disease"), 10, 510, 160, 535);
	m_wndRelationDisease.Create(this,165, 510, 635, 565, TRUE, FALSE, TRUE); 
	m_wndSave.Create(this, _T("&Save"), 485, 575, 560, 600);
	m_wndCancel.Create(this, _T("&Cancel"), 565, 575, 640, 600);
...
//	m_wndRelationDisease.SetCheckValue(true);
	...
	m_wndDept.InsertColumn(2, _T("Description"), CFMT_TEXT, 350);
...
void CHMSCreateOutpatientRecordDialog::GetDataToScreen()
{
	...
	szSQL.Format(_T(" SELECT 	")
		...
		_T("	htr_emergency as emergency, htr_ward as ward, hcr_cancer cancer, htr_doctor as doctor ") \
		_T(" FROM hms_doc") \
...
//		return -1;
	...
	if (m_szCancer == _T("Y") )
		szSQL.Format(_T("HMS_CANCER_RECORD_CREATE_V2(%ld, '%s', TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS'), '%s', '%s', '%s', '%s', '%s')"),
		m_nDocumentNo, pMF->GetCurrentUser(), m_szInDate, m_szDeptKey, m_bOutpatient?_T("Y"):_T("N"), m_szDoctorKey, m_szAdmissionDiseaseKey, m_szMainDiseaseKey);
	else
		szSQL.Format(_T("HMS_TREATMENT_RECORD_CREATE_V2(%ld, '%s', TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS'),'Y','%s', '%s', '%s', '%s')"),
		m_nDocumentNo, pMF->GetCurrentUser(), m_szInDate,szDeptId, m_szDoctorKey, m_szAdmissionDiseaseKey, m_szMainDiseaseKey);
	...
	//Gọi hàm HMS_FIX_TREATTIME để tính lại treattime nếu bị sai
		
		szSQL.Format(_T("HMS_FIX_TREATTIME(%ld)"),m_nDocumentNo);
		int vcount = str2int(pMF->ExecDML(szSQL));
		if (vcount > 0) 
		{
			szSQL.Format(_T("UPDATE_ICD_CANCER_DEPT(%ld, '%s', %d, %d, '%s')"), m_nDocumentNo, m_szAdmissionDiseaseKey, vcount, res, pMF->m_szDept);
