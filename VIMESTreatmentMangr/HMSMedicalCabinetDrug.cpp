...
#include "HMSMedicalCabinetDrugOrderEntryDlg.h"
...
int CHMSMedicalCabinetDrug::OnDrugListDeleteItem(){
	...
	CList<CMFCGridItem*, CMFCGridItem*> lstSelected;
	std::vector<int> lstDeleted;
	...
	for (it = lstDeleted.begin(); it != lstDeleted.end(); it++)
	{
		m_wndDrugList.DeleteItem(*it);
	}
...
void CHMSMedicalCabinetDrug::OnAddDrugSelect(){
	...
	CHMSMedicalCabinetDrugOrderEntryDlg* newPopup = new CHMSMedicalCabinetDrugOrderEntryDlg(this, VM_ADD);
...
void CHMSMedicalCabinetDrug::Refresh(long nOrderID){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	_tprintf(_T("Refresh OrderID:%d"), nOrderID);
...
void CHMSMedicalCabinetDrug::OnResizeLayout()
{
	...
	return;
	AddLayoutControls(WS_REPOSX|WS_REPOSY, 100, 100, 0, 0,&m_wndAddDrug,&m_wndDeleteDrug,&m_wndConfirm,&m_wndPrint,NULL);

	AddLayoutControls(WS_REPOSY, 0, 100, 0, 0,&m_wndSheetOrderReturn,&m_wndStorageDetail,NULL);

	AddLayoutControls(WS_RESIZEX|WS_RESIZEY, 0, 0, 100, 100,&m_wndDrugInformation,&m_wndDrugList,NULL);

	AddLayoutControls(WS_RESIZEX, 0, 0, 100, 0,&m_wndPatientInformation,&m_wndPatientList,NULL);

}

LRESULT CHMSMedicalCabinetDrug::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
