...
void CHMSReturnDrugDialog::GetDataToScreen(){
	...
	//	UpdateData(FALSE);
		...
		szSQL.Format(
			_T("SELECT coalesce(sum(hpolr_qtyreverse), 0) ") \
			_T("FROM hms_ipharmaorderline_r ") \
			_T("WHERE hpolr_orderid=%ld and hpolr_product_id=%ld and hpolr_status <> 'O' ") \
			_T(" AND hpolr_product_item_id = %ld"),
			m_nOrderID, m_nProduct_ID, m_nProductItemID);
		rs.ExecSQL(szSQL);
...
int CHMSReturnDrugDialog::OnSaveHMSReturnDrugDialog(){
	...
	//if(m_nIssueQuantity != m_nOrderQuantity){
 		CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
 		CString szSQL; 
		szSQL.Format(_T("HMS_IPHARMAORDERLINE_CONFIRM2(%ld, %ld, %.3f, %.3f, '%s', %ld) "), 
			m_nOrderID, m_nProduct_ID, m_nUnitPrice,  (m_nOrderQuantity- m_nIssueQuantity), pMF->GetCurrentUser(), m_nProductItemID);
_fmsg(_T("%s"), szSQL);
