...
void CPhanKetLuan_UngThuTuLan2::OnCreateComponents(){
	...
	m_wndhuongdieutriLabel.Create(this, _T("5. Hướng điều trị:"), 470, 360, 620, 385);
	m_wndhuongdieutri.Create(this,470, 386, 930, 461, 1, 0, 1); 
	m_wndtienluongLabel.Create(this, _T("6. Tiên lượng:"), 5, 462, 155, 487);
...
void CPhanKetLuan_UngThuTuLan2::GetDataToScreen(){
	...
	CString szFilter;
	szFilter.Empty();
	szFilter.Format(_T(" AND hde_deptid = '%s'"), pMF->GetCurrentDepartmentID());
	...
	else
	{
		...
		szSQL.Format(_T("SELECT COUNT(*) AS solan ") \
					_T(" FROM hms_doc_emr ") \
					_T(" WHERE hde_docno=%ld and hde_name in ('%s', 'BANGT') ") \
					_T(" and hde_type='BENH_AN_KL_UTTL2' AND hde_treattime = %d AND hde_deptid = '%s'")
					, m_nDocumentNo, m_szRecordNo, m_nTreatTime, pMF->GetCurrentDepartmentID());
		rs.ExecSQL(szSQL);
		...
		if(n > 0)
		{
			for (int i = 0; i < n; i++)
			{
				szSQL.Format(_T("SELECT to_char(hde_value) as hde_value ") \
							_T(" FROM hms_doc_emr ") \
							_T(" WHERE hde_docno=%ld and hde_name in ('%s', 'BANGT') ") \
							_T(" and hde_type='BENH_AN_KL_UTTL2' and hde_refidx = %d ") \
							_T(" AND hde_treattime = %d AND hde_deptid = '%s'")
							, m_nDocumentNo, m_szRecordNo, i, m_nTreatTime, pMF->GetCurrentDepartmentID());
				rs.ExecSQL(szSQL);
