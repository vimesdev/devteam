...
CPhanKhamChuyenKhoaInput::CPhanKhamChuyenKhoaInput(CString szForm, long nDocumentNo, int nTreatTime, bool isHtml)
{
	m_nDlgWidth = 580;
	m_nDlgHeight = 800;
	m_pWndFocus = NULL;
	...
	m_szType = _T("BENH_AN_CHUYEN_KHOA");
	m_isHtml = isHtml;
	m_pHTMLForms = NULL;
...
void CPhanKhamChuyenKhoaInput::OnCreateComponents()
{
	...
	{
_tprintf(_T("\r\nHTMLayout"));
		if(m_pHTMLForms == NULL)
		{
			m_pHTMLForms = new CHTMLayoutCtrlEx();
		}
		if(m_pHTMLForms->GetSafeHwnd() == NULL)
		{
			CRect rect;
			GetClientRect(&rect);
			rect.DeflateRect(5, 5, 5, 5);
			BOOL res = m_pHTMLForms->Create(WS_CHILD|WS_VISIBLE|WS_BORDER, rect, this, _GetNextID());
			if(res)
_tprintf(_T("\r\nCreate HTMLayout"));
		}
		return;
...
void CPhanKhamChuyenKhoaInput::GetDataToScreen(){
	...
	int n = 0;
	CString szFilter, szWhere;
	szFilter.Empty();

	if(m_isHtml && m_pHTMLForms)
	{
		CString szFile;
		szFile.Format(_T("./REPORTS/HMS/BENHAN/%s.HTML"), m_szFormID);
_tprintf(_T("\r\n%s"), szFile);

_tprintf(_T("\r\n%s: %s"),m_szType, m_szFormID);

		m_pHTMLForms->m_szType = m_szType;
		m_pHTMLForms->m_nDocumentNo = m_nDocumentNo;
		m_pHTMLForms->m_szDeptID = pMF->m_szDept;
		m_pHTMLForms->m_nRefIndex = pMF->m_nRefIndex;
		m_pHTMLForms->m_nRoomID = pMF->m_nRoomID;
		m_pHTMLForms->m_nOptIdx = m_nTreatTime;
		m_pHTMLForms->m_szForm = m_szFormID;
		m_pHTMLForms->Load(szFile);
		
		//m_pHTMLForms->LoadData_V2();
		m_pHTMLForms->EnableControls(false);
	...
	bool res = pMF->GetEMRData(m_nDocumentNo, m_nTreatTime, m_szType, m_szFormID, szFilter, szData);
	if(res)
	{
	...
	{
		szSQL.Format(_T("SELECT hde_name, to_char(hde_value) as hde_value FROM hms_doc_emr WHERE hde_docno=%ld and hde_type='BENH_AN_CHUYEN_KHOA' %s ORDER BY hde_refidx"), m_nDocumentNo, szWhere);
		rs.ExecSQL(szSQL);
		szData.Empty();
		while(!rs.IsEOF())
		{
...
{ 
 		...
 		case VM_ADD: 
 			...
 			SetDefaultValues(); 
			if(m_pHTMLForms)
				m_pHTMLForms->EnableControls(true);
 			break; 
 		case VM_EDIT: 
 			EnableControls(TRUE); 
...
int CPhanKhamChuyenKhoaInput::OnSaveCPhanKhamChuyenKhoaInput()
{
	...
	if (m_isHtml && m_pHTMLForms)
	{
		...
		dom::element root = m_pHTMLForms->GetRootElement();
		if (htmlayout::get_values(root, values))
		{
			...
			for (htmlayout::named_values::const_iterator it = values.begin(); it != values.end(); ++it)
			{
				...
				if (value.length() > 0)
				{
					m_jData[name.c_str()] = value.c_str();
				}
	...
	bool res = pMF->SaveEMRData(m_szType, m_szFormID, m_nDocumentNo, pMF->m_szDept, pMF->m_nRefIndex, 0, 0, szData);
	...
	for (int i =0; i < strList.GetCount();i++)
	{
		tmpStr = strList[i];
		tmpStr.Replace(_T("'"), _T("''"));
		szSQL.Format(_T("HMS_DOC_EMR_CREATE('BENH_AN_CHUYEN_KHOA', %ld, '%s', %d, %d, '%s', '%s', '%s', %d, %d) "),
			m_nDocumentNo, pMF->m_szDept, n++, 0, pMF->GetCurrentUser(), m_szFormID, tmpStr, 0, m_nTreatTime);
	...
	szSQL.Format(_T("DELETE FROM hms_doc_emr WHERE hde_docno=%ld and hde_name='%s' and hde_type='BENH_AN_CHUYEN_KHOA' AND hde_htridx = %d and hde_refidx >=%d "), m_nDocumentNo, m_szFormID, m_nTreatTime, n);
	pMF->ExecSQL(szSQL);
...
void CPhanKhamChuyenKhoaInput::Refresh(int nTreatTime)
{
	m_nTreatTime = nTreatTime;
	GetDataToScreen();
	SetMode(VM_VIEW);
}
void CPhanKhamChuyenKhoaInput::OnSize(UINT nType, int cx, int cy)
{
	CGuiView::OnSize(nType, cx, cy);
	if(m_pHTMLForms != NULL && m_pHTMLForms->GetSafeHwnd())
	{
		CRect rect;
		GetClientRect(&rect);
		m_pHTMLForms->MoveWindow(rect);
	}
