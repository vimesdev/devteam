...
static void _OnAdmissionICDSelectChangeFnc(CWnd* pWnd, int nOldItemSel, int nNewItemSel) {
	((CHMSDepartmentAdmissionDialog*)pWnd)->OnAdmissionICDSelectChange(nOldItemSel, nNewItemSel);
}
static void _OnAdmissionICDSelendokFnc(CWnd* pWnd) {
	((CHMSDepartmentAdmissionDialog*)pWnd)->OnAdmissionICDSelendok();
}
/*static void _OnAdmissionICDSetfocusFnc(CWnd *pWnd){
	((CHMSDepartmentAdmissionDialog *)pWnd)->OnAdmissionICDKillfocus();
}*/
/*static void _OnAdmissionICDKillfocusFnc(CWnd *pWnd){
	((CHMSDepartmentAdmissionDialog *)pWnd)->OnAdmissionICDKillfocus();
}*/
static long _OnAdmissionICDLoadDataFnc(CWnd* pWnd) {
	return ((CHMSDepartmentAdmissionDialog*)pWnd)->OnAdmissionICDLoadData();
}
/*static void _OnAdmissionICDAddNewFnc(CWnd *pWnd){
	((CHMSDepartmentAdmissionDialog *)pWnd)->OnAdmissionICDAddNew();
}*/
...
static void _OnMainDiseaseSelectFnc(CWnd* pWnd) {
	((CHMSDepartmentAdmissionDialog*)pWnd)->OnMainDiseaseSelect();
}
...
static long _OnPhanloaibenhcovid19LoadDataFnc(CWnd* pWnd) {
	return ((CHMSDepartmentAdmissionDialog*)pWnd)->OnPhanloaibenhcovid19LoadData();
}
static long _OnLoaidieutriLoadDataFnc(CWnd* pWnd) {
	return ((CHMSDepartmentAdmissionDialog*)pWnd)->OnLoaidieutriLoadData();
}
static long _OnLoaiKCBLoadDataFnc(CWnd* pWnd)
{
	return ((CHMSDepartmentAdmissionDialog*)pWnd)->OnLoaiKCBLoadData();
}
static long _OnPhanloaiUngthuLoadDataFnc(CWnd* pWnd)
{
	return ((CHMSDepartmentAdmissionDialog*)pWnd)->OnPhanloaiUngthuLoadData();
}
...
{
	/*m_wndDepartmentAdmitedInformation.Create(this, _T("Department Admited Information"), 5, 485, 755, 660);
	...
	m_wndPatientProfiles.Create(this, _T("Patient Profiles"), 5, 5, 755, 300);
	...
	m_wndLoaiKCBLabel.Create(this, _T("Mã loại KCB"), 294, 510, 374, 535);
	m_wndLoaiKCB.Create(this,379, 510, 490, 535);

	m_wndDoctorLabel.Create(this, _T("Doctor"), 495, 510, 565, 535);
	m_wndDoctor.Create(this,570, 510, 749, 535);

	m_wndAdmissionICDLabel.Create(this, _T("Admission ICD"), 10, 540, 160, 565);
	m_wndAdmissionICD.Create(this,165, 540, 365, 565);
	m_wndAdmissionDiseaseLabel.Create(this, _T("Admission Disease"), 370, 540, 490, 565);
	m_wndAdmissionDisease.Create(this,495, 540, 749, 565);
	m_wndMainDiseaseLabel.Create(this, _T("Main Disease"), 10, 570, 160, 595);
	m_wndMainDisease.Create(this,165, 570, 749, 595, 1, 0, 1);
	m_wndRelationDiseaseLabel.Create(this, _T("Relation Disease"), 10, 600, 160, 625);
	m_wndRelationDisease.Create(this,165, 600, 749, 625);
	m_wndIssueNumberLabel.Create(this, _T("Issue Number"), 10, 630, 160, 655);
	m_wndIssueNumber.Create(this,165, 630, 192, 655);

	m_wndNoteLabel.Create(this, _T("Note"), 0, 0, 0, 0);
	m_wndNote.Create(this,0, 0, 0, 0);


	m_wndLoaidieutriLabel.Create(this, _T("Loại điều trị"), 197, 630, 277, 655);
	m_wndLoaidieutri.Create(this,282, 630, 430, 655);

	m_wndPhanloaibenhcovidLabel.Create(this, _T("Phân loại BN Covid"), 434, 630, 568, 655);
	m_wndPhanloaibenhcovid19.Create(this,573, 630, 749, 655);

	m_wndOutpatient.Create(this, _T("Outpatient treat"), 5, 665, 135, 690);
	m_wndEmergency.Create(this, _T("Emergency"), 140, 665, 220, 690);
	m_wndCancer.Create(this, _T("Cancer"), 225, 665, 320, 690);
	m_wndCancerTime.Create(this,325, 665, 370, 690);
	m_wndHasAllergy.Create(this, _T("Has Allergy"), 375, 665, 500, 690);
	m_wndSave.Create(this, _T("&Save"), 509, 665, 589, 690);
	m_wndCancel.Create(this, _T("&Cancel"), 594, 665, 669, 690);
	m_wndPrint.Create(this, _T("&Print"), 674, 665, 749, 690);
	m_wndPhanloaibenhcovidLabel.SetTextColor(RGB(255,0,0));
	m_wndHasAllergy.ShowWindow(SW_HIDE);*/

	m_wndPatientProfiles.Create(this, _T("Patient Profiles"), 5, 5, 905, 300);
	m_wndDepartmentAdmitedInformation.Create(this, _T("Department Admited Information"), 5, 485, 905, 660);
	m_wndAdmissionInformation.Create(this, _T("Admission Information"), 5, 305, 905, 480);
	m_wndExamDateLabel.Create(this, _T("Exam Date"), 10, 30, 160, 55);
	m_wndExamDate.Create(this, 165, 30, 271, 55);
	m_wndExamDeptLabel.Create(this, _T("Exam Dept"), 370, 30, 490, 55);
	m_wndExamDept.Create(this, 495, 30, 590, 55);
	m_wndExamRoomLabel.Create(this, _T("Exam Room"), 595, 30, 665, 55);
	m_wndExamRoom.Create(this, 670, 30, 905, 55);
	m_wndPatientNameLabel.Create(this, _T("Patient Name"), 10, 60, 160, 85);
	m_wndPatientName.Create(this, 165, 60, 365, 85);
	m_wndBirthDateLabel.Create(this, _T("BirthDate"), 370, 60, 490, 85);
	m_wndBirthDate.Create(this, 495, 60, 590, 85);
	...
	m_wndEthnic.Create(this, 165, 90, 365, 115);
	m_wndOccupationLabel.Create(this, _T("Occupation"), 370, 90, 490, 115);
	m_wndOccupation.Create(this, 495, 90, 905, 115);
	m_wndProvinceLabel.Create(this, _T("Province"), 10, 120, 160, 145);
	m_wndProvince.Create(this, 165, 120, 365, 145);
	m_wndDistrictLabel.Create(this, _T("District"), 370, 120, 490, 145);
	m_wndDistrict.Create(this, 495, 120, 905, 145);
	m_wndVillageLabel.Create(this, _T("Village"), 10, 150, 160, 175);
	m_wndVillage.Create(this, 165, 150, 365, 175);
	m_wndDetailAddressLabel.Create(this, _T("Detail Address"), 370, 150, 490, 175);
	m_wndDetailAddress.Create(this, 495, 150, 905, 175);
	m_wndWorkplaceLabel.Create(this, _T("Workplace"), 10, 180, 160, 205);
	m_wndWorkplace.Create(this, 165, 180, 905, 205);
	m_wndRankLabel.Create(this, _T("Rank"), 10, 210, 160, 235);
	m_wndRank.Create(this, 165, 210, 365, 235);
	m_wndPositionLabel.Create(this, _T("Position"), 370, 210, 490, 235);
	m_wndPosition.Create(this, 495, 210, 905, 235);
	m_wndRelativeLabel.Create(this, _T("Relative"), 10, 240, 160, 265);
	m_wndRelative.Create(this, 165, 240, 365, 265);
	m_wndRelationLabel.Create(this, _T("Relation"), 370, 240, 490, 265);
	m_wndRelation.Create(this, 495, 240, 905, 265);
	m_wndContactAddressLabel.Create(this, _T("Contact Address"), 10, 270, 160, 295);
	m_wndContactAddress.Create(this, 165, 270, 365, 295);
	m_wndContactPhoneLabel.Create(this, _T("Contact Phone"), 370, 270, 490, 295);
	m_wndContactPhone.Create(this, 495, 270, 905, 295);
	m_wndRecordNoLabel.Create(this, _T("Admission No"), 10, 330, 160, 355);
	m_wndRecordNo.Create(this, 165, 330, 265, 355);
	m_wndTimes.Create(this, _T("Times"), 270, 330, 330, 355);
	m_wndTreatTime.Create(this, 335, 330, 365, 355);
	...
	m_wndAdmitedDirection.Create(this, 165, 360, 365, 385);
	m_wndTransferPlaceLabel.Create(this, _T("Transfer Place"), 370, 360, 490, 385);
	m_wndTransferPlace.Create(this, 495, 360, 905, 385);
	m_wndTransferHospitalLabel.Create(this, _T("Trans Hospital"), 10, 390, 160, 415);
	m_wndTransferHospital.Create(this, 165, 390, 905, 415);
	m_wndTransferDiagnosticLabel.Create(this, _T("Transfer Diagnostic"), 10, 420, 160, 445);
	m_wndTransferDiagnostic.Create(this, 165, 420, 905, 445);
	m_wndExamineDiseaseLabel.Create(this, _T("Examine Disease"), 10, 450, 160, 475);
	m_wndExamineDisease.Create(this, 165, 450, 905, 475);
	m_wndInDateLabel.Create(this, _T("In Date"), 10, 510, 160, 535);
	m_wndInDate.Create(this, 165, 510, 290, 535);
	m_wndDoctorLabel.Create(this, _T("Doctor"), 495, 510, 565, 535);
	m_wndDoctor.Create(this, 570, 510, 905, 535);
	m_wndOutpatient.Create(this, _T("Outpatient treat"), 5, 665, 135, 690);
	m_wndEmergency.Create(this, _T("Emergency"), 140, 665, 220, 690);
	m_wndCancer.Create(this, _T("Cancer"), 225, 665, 320, 690);
	m_wndCancerTime.Create(this, 325, 665, 370, 690);
	...
	m_wndAdmissionICD.Create(this, 165, 540, 365, 565);
	m_wndAdmissionDiseaseLabel.Create(this, _T("Admission Disease"), 370, 540, 490, 565);
	m_wndAdmissionDisease.Create(this, 495, 540, 905, 565);
	m_wndMainDiseaseLabel.Create(this, _T("Main Disease"), 10, 570, 160, 595);
	m_wndMainDisease.Create(this, 165, 570, 905, 595);
	m_wndRelationDiseaseLabel.Create(this, _T("Relation Disease"), 10, 600, 160, 625);
	m_wndRelationDisease.Create(this, 165, 600, 905, 625);
	m_wndIssueNumberLabel.Create(this, _T("Issue Number"), 10, 630, 160, 655);
	m_wndIssueNumber.Create(this, 165, 630, 192, 655);
	...
	m_wndLoaidieutriLabel.Create(this, _T("Loại điều trị"), 197, 630, 277, 655);
	m_wndLoaidieutri.Create(this, 280, 630, 413, 655);
	m_wndPhanloaibenhcovidLabel.Create(this, _T("Phân loại Covid"), 414, 630, 520, 655);
	m_wndPhanloaibenhcovid19.Create(this, 521, 630, 654, 655);
	m_wndSave.Create(this, _T("&Save"), 665, 665, 745, 690);
	m_wndCancel.Create(this, _T("&Cancel"), 750, 665, 825, 690);
	m_wndPrint.Create(this, _T("&Print"), 830, 665, 905, 690);
	m_wndLoaiKCBLabel.Create(this, _T("Mã loại KCB"), 294, 510, 374, 535);
	m_wndLoaiKCB.Create(this, 379, 510, 490, 535);
	m_wndPhanloaiUngthuLabel.Create(this, _T("Phân loại ung thư"), 657, 630, 775, 655);
	m_wndPhanloaiUngthu.Create(this, 780, 630, 905, 655);
	m_wndPhanloaiUngthuLabel.SetTextColor(RGB(255, 0, 0));
	m_wndPhanloaibenhcovidLabel.SetTextColor(RGB(255, 0, 0));
	m_wndHasAllergy.ShowWindow(SW_HIDE);
}
void CHMSDepartmentAdmissionDialog::OnInitializeComponents() {
	...
	m_wndContactPhone.SetLimitText(128);
	...
	//m_wndExamineDisease.SetCheckValue(true);
	m_wndAdmissionICD.SetCheckValue(true);
	m_wndAdmissionICD.LimitText(254);
	...
	//	m_wndRelationDisease.SetCheckValue(true);
		//m_wndPhanloaibenhcovid19.SetCheckValue(true);
	m_wndLoaidieutri.SetCheckValue(true);

	if (pMF->GetProp(_T("hms_use_130")) == _T("Y"))
		m_wndLoaiKCB.SetCheckValue(true);

	m_wndLoaiKCB.InsertColumn(0, _T("ID"), CFMT_NUMBER, 50);
	m_wndLoaiKCB.InsertColumn(1, _T("Description"), CFMT_TEXT, 400);
	...
	m_wndAdmissionICD.InsertColumn(0, _T("ID"), CFMT_NUMBER, 50);
	m_wndAdmissionICD.InsertColumn(1, _T("Name"), CFMT_TEXT, 400);
	...
	m_wndLoaidieutri.InsertColumn(1, _T("Name"), CFMT_TEXT, 350);

	m_wndPhanloaibenhcovid19.InsertColumn(0, _T("ID"), CFMT_TEXT, 80);
	m_wndPhanloaibenhcovid19.InsertColumn(1, _T("Name"), CFMT_TEXT, 350);
	m_wndPhanloaiUngthu.InsertColumn(0, _T("ID"), CFMT_TEXT, 80);
	...
	m_hms_clinicalTbl.AddField(_T("p_outpatient"), dfText, 15);
	...
	m_hms_clinicalTbl.AddField(_T("p_admit_disease"), dfText, 254);
	m_hms_clinicalTbl.AddField(_T("p_hcr_phanloaibenhcovid19"), dfText, 2);
	m_hms_clinicalTbl.AddField(_T("p_hcr_phanloaiungthu"), dfText, 3);
	...
	m_hms_treatment_recordTbl.AddField(_T("htr_treatment_type"), dfText, 15);
	m_szTitle = m_szTiles;
...
void CHMSDepartmentAdmissionDialog::OnSetWindowEvents() {
	...
	m_wndAdmissionICD.SetEvent(WE_SELENDOK, _OnAdmissionICDSelendokFnc);
	//m_wndAdmissionICD.SetEvent(WE_SETFOCUS, _OnAdmissionICDSetfocusFnc);
	//m_wndAdmissionICD.SetEvent(WE_KILLFOCUS, _OnAdmissionICDKillfocusFnc);
	m_wndAdmissionICD.SetEvent(WE_SELCHANGE, _OnAdmissionICDSelectChangeFnc);
	m_wndAdmissionICD.SetEvent(WE_LOADDATA, _OnAdmissionICDLoadDataFnc);
	//m_wndAdmissionICD.SetEvent(WE_ADDNEW, _OnAdmissionICDAddNewFnc);
	...
	m_wndMainDiseaseLabel.SetHyperlink(true);
	m_wndMainDiseaseLabel.SetEvent(WE_CLICK, _OnMainDiseaseSelectFnc);
	...
	m_wndPhanloaibenhcovid19.SetEvent(WE_LOADDATA, _OnPhanloaibenhcovid19LoadDataFnc);
...
void CHMSDepartmentAdmissionDialog::OnDoDataExchange(CDataExchange* pDX) {
	...
	DDX_TextEx(pDX, m_wndAdmissionICD.GetDlgCtrlID(), m_szAdmissionICDKey);
	...
	DDX_Text(pDX, m_wndAdmissionDisease.GetDlgCtrlID(), m_szAdmissionDisease);
	DDX_TextEx(pDX, m_wndLoaidieutri.GetDlgCtrlID(), m_szLoaidieutriKey);
	DDX_TextEx(pDX, m_wndPhanloaibenhcovid19.GetDlgCtrlID(), m_szPhanloaibenhcovid19Key);
	DDX_Check(pDX, m_wndHasAllergy.GetDlgCtrlID(), m_bHasAllergy);
...
void CHMSDepartmentAdmissionDialog::UpdateJson(BOOL bSave) {
	if (bSave)
	{
		...
		m_jData[_T("AdmissionICD")] = m_szAdmissionICDKey;
	...
	else
	{
		...
		m_jData[_T("AdmissionICD")].GetValue(m_szAdmissionICDKey);
...
void CHMSDepartmentAdmissionDialog::GetDataToScreen()
{
	...
	szSQL.Format(_T(" SELECT hp_patientno, hp_surname||' '||hp_midname||' '||hp_firstname as pname, ") \
		...
		_T(" hd_transplace, hd_transdiagn, hd_cardno, ") \
		...
		_T("	nvl(hd_outpatient,'N') as hd_outpatient, ") \
		_T("	nvl(htr_outpatient,'N') as outpatient, ") \
		_T("	htr_mainicd as mainicd, htr_brace_no, htr_brace_note,") \
		_T("	htr_maindisease as maindisease, ") \
		_T("	htr_emergency as emergency, htr_ward as ward, hcr_cancer, ") \
		_T("	hcr_cancer_time, hcr_treattime as treattime, htr_cancer_time,  ") \
		_T("	hcr_treatdoctor as doctor, hcr_admitdisease as admit_disease, ") \
		_T("	hcr_phanloaibenhcovid19, HCR_PHANLOAIUNGTHU, htr_treatment_type, hd_hasallergy as hasallergy, hd_ma_loai_kcb") \
		_T(" FROM hms_patient ") \
	...
	if (!rs.IsEOF())
	{
		rs.GetValue(_T("admit_disease"), m_szAdmissionDisease);
		...
		rs.GetValue(_T("htr_cancer_time"), m_nCancerTime);
		rs.GetValue(_T("hcr_phanloaibenhcovid19"), m_szPhanloaibenhcovid19Key);
		rs.GetValue(_T("hcr_phanloaiungthu"), m_szPhanloaiUngthuKey);
		rs.GetValue(_T("htr_treatment_type"), m_szLoaidieutriKey);
		//Truong hop khong co du lieu trong hms_treatment_record
		if (m_nCancerTime <= 0)
		{
		...
		if (m_bCancer && m_nCancerTime <= 0)
		{
			szSQL.Format(_T("SELECT coalesce(max(hcr_cancer_time),0)+1 ") \
				...
				m_nPatientNo, m_nDocumentNo);
			CRecord rsx(&pMF->m_db);
			rsx.ExecSQL(szSQL);
			int nCancerTime = rsx.GetIntValue();
		...
		rs.GetValue(_T("admiticd"), m_szAdmissionICDKey);
		//rs.GetValue(_T("mainicd"), m_szAdmissionICDKey);
		...
		m_bHasAllergy = tmpStr == _T("Y") ? TRUE : FALSE;
		m_bRegisted = true;
		if (GetMode() != VM_VIEW)
		{
...
void CHMSDepartmentAdmissionDialog::GetScreenToData() {
	...
	m_hms_clinicalTbl.SetValue(_T("p_outpatient"), m_bOutpatient ? _T("Y") : _T("N"));
	m_hms_clinicalTbl.SetValue(_T("p_emergency"), m_bEmergency ? _T("Y") : _T("N"));
	m_hms_clinicalTbl.SetValue(_T("p_cancer"), m_bCancer ? _T("Y") : _T("N"));
	m_hms_clinicalTbl.SetValue(_T("p_cancer_time"), m_nCancerTime);
	m_hms_clinicalTbl.SetValue(_T("p_admit_icd10"), m_szAdmissionICDKey);
	...
	m_hms_clinicalTbl.SetValue(_T("p_admit_disease"), m_szAdmissionDisease);
	m_hms_clinicalTbl.SetValue(_T("p_hcr_phanloaibenhcovid19"), m_szPhanloaibenhcovid19Key);
	m_hms_clinicalTbl.SetValue(_T("p_hcr_phanloaiungthu"), m_szPhanloaiUngthuKey);
	...
	m_hms_treatment_recordTbl.SetValue(_T("htr_mainicd"), m_szAdmissionICDKey);
	...
	m_hms_treatment_recordTbl.SetValue(_T("htr_treatment_type"), m_szLoaidieutriKey);
}
void CHMSDepartmentAdmissionDialog::SetDefaultValues() {
	m_szAdmissionDisease.Empty();
	m_szDoctorKey.Empty();
	m_szExamDate.Empty();
	m_szExamDept.Empty();
	m_szExamRoom.Empty();
...
int CHMSDepartmentAdmissionDialog::SetMode(int nMode) {
	...
	//m_wndOutpatient.EnableWindow(FALSE);
	if (m_bCancer)
		m_wndCancerTime.EnableWindow(TRUE);


	/*if ((pMF->GetCurrentDepartmentID()).Left(5) == _T("COVID") || (pMF->GetCurrentDepartmentID()).Left(2) == _T("A4"))
	{
		m_wndPhanloaibenhcovid19.EnableWindow(true);
	}
	else
	m_wndPhanloaibenhcovid19.EnableWindow(false);*/
	if (pMF->GetCurrentDepartmentID() == _T("A17") || pMF->GetCurrentDepartmentID() == _T("B11") || pMF->GetCurrentDepartmentID() == _T("A15 NEW"))
	{
		m_wndLoaidieutri.EnableWindow(true);
	}
	else
		m_wndLoaidieutri.EnableWindow(false);

	m_wndPhanloaibenhcovid19.EnableWindow(true);
	m_wndPhanloaiUngthu.EnableWindow(true);
...
long CHMSDepartmentAdmissionDialog::OnPhanloaibenhcovid19LoadData()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
	if (m_wndPhanloaibenhcovid19.IsSearchKey() && !m_szPhanloaibenhcovid19Key.IsEmpty())
	{
		szWhere.Format(_T(" AND ss_code='%s'"), m_szPhanloaibenhcovid19Key);
	};
	m_wndPhanloaibenhcovid19.DeleteAllItems();
	int nCount = 0;
	szSQL.Format(_T("SELECT ss_code as id, ss_desc as name FROM sys_sel WHERE ss_id='hms_phanloaibenhcovid19' %s ORDER BY cast(ss_code as integer) "), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while (!rs.IsEOF()) {
		m_wndPhanloaibenhcovid19.AddItems
		(
			rs.GetValue(_T("id")),
...
long CHMSDepartmentAdmissionDialog::OnPhanloaiUngthuLoadData()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
long CHMSDepartmentAdmissionDialog::OnLoaidieutriLoadData()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT ss_code as id, ss_desc as name FROM sys_sel WHERE ss_id='hms_phanloai_dieutri' %s ORDER BY cast(ss_code as integer) "), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while (!rs.IsEOF()) {
...
int CHMSDepartmentAdmissionDialog::OnInDateCheckValue() {
	...
	if (m_nIndex > 1)
		szSQL.Format(_T("SELECT to_char(htr_dischargedate, 'YYYY/MM/DD HH24:MI') as cmpdate FROM hms_treatment_record WHERE htr_docno=%ld and htr_idx=%d"), m_nDocumentNo, m_nIndex - 1);
	else
		szSQL.Format(_T("SELECT to_char(hd_enddate, 'YYYY/MM/DD HH24:MI') as cmpdate FROM hms_doc WHERE hd_docno=%ld "), m_nDocumentNo);
	rs.ExecSQL(szSQL);
...
int CHMSDepartmentAdmissionDialog::OnCancerTimeCheckValue() {
	...
	if (m_bCancer)
	{
		...
		rsx.ExecSQL(szSQL);
		int nCancerTime = rsx.GetIntValue();
		if (nCancerTime > m_nCancerTime)
		{
			CString szMsg;
			szMsg.Format(_T("Bệnh nhân đã được cập nhật số lần điều trị ung thư [%d]"), nCancerTime);
			m_wndCancerTime.SetToolTipMessage(szMsg);
			return -1;
		}
...
void CHMSDepartmentAdmissionDialog::OnAdmissionICDSelectChange(int nOldItemSel, int nNewItemSel) {
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();

}
void CHMSDepartmentAdmissionDialog::OnAdmissionICDSelendok() {

}
/*void CHMSDepartmentAdmissionDialog::OnAdmissionICDSetfocus(){

}*/
/*void CHMSDepartmentAdmissionDialog::OnAdmissionICDKillfocus(){

}*/
long CHMSDepartmentAdmissionDialog::OnAdmissionICDLoadData() {
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	return pMF->LoadICD(&m_wndAdmissionICD, m_szAdmissionICDKey);
	/*
		CRecord rs(&pMF->m_db);
		CString szSQL, szWhere;
		if(m_wndAdmissionICD.IsSearchKey() && !m_szAdmissionICDKey.IsEmpty()){
		 szWhere.Format(_T(" and id='%s' "), m_szAdmissionICDKey
	};
		m_wndAdmissionICD.DeleteAllItems();
		int nCount = 0;
		szSQL.Format(_T("SELECT fld1 as id, fld2 as name FROM tbl WHERE 1=1 %s ORDER BY id "), szWhere
		nCount = rs.ExecSQL(szSQL);
		while(!rs.IsEOF()){
			m_wndAdmissionICD.AddItems(
			rs.MoveNext();
		}
		return nCount;
	*/
...
/*void CHMSDepartmentAdmissionDialog::OnAdmissionICDAddNew(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
void CHMSDepartmentAdmissionDialog::OnMainDiseaseSelect()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	UpdateData(TRUE);
	CHMSMainDiseaseUpdateDialog dlg(this);
	dlg.m_nDocumentNo = m_nDocumentNo;
	dlg.m_nOrderId = m_nIndex;
...
int CHMSDepartmentAdmissionDialog::OnSaveHMSDepartmentAdmissionDialog() {
	...
	/*if (m_bCancer && m_szPhanloaiUngthuKey.IsEmpty())
	{
		ShowMessageBox(_T("Bệnh nhân có bệnh án ung thư, bạn cần chọn phân loại ung thư"), MB_ICONERROR);
		m_wndPhanloaiUngthu.SetFocus();
		return -1;
	}*/
	...
	if (m_szObjectType == L"I" || m_szObjectType == L"C")
	{
		szSQL.Format(L"SELECT * FROM hms_cardinfo WHERE hci_docno=%ld and hci_cardno = '%s'",
			m_nDocumentNo, m_szCardNo);
		rs.ExecSQL(szSQL);
		if (rs.IsEOF())
		{
			CHMSCardViewDialog dlg(this, VM_ADD);
	...
	if (ret > 0)
	{
		...
		szSQL.Format(_T("UPDATE hms_doc SET hd_ma_loai_kcb='%s' WHERE hd_docno=%ld "), m_szLoaiKCBKey, m_nDocumentNo);
		pMF->ExecSQL(szSQL);

		szSQL.Format(_T("UPDATE_ICD_CANCER_DEPT(%ld, '%s', %d, %d, '%s')"), m_nDocumentNo, m_szAdmissionICDKey, m_nTreatTime, m_nIndex, pMF->m_szDept);

		int tmpvcount = str2int(pMF->ExecDML(szSQL));
		if (tmpvcount > 0)
		{
			ShowMessageBox(_T("Gửi bệnh nhân đến khu khám ung thư!"), MB_OK);
		}
		SetMode(VM_VIEW);
		CGuiDialog::OnOK();
	}
...
CString CHMSDepartmentAdmissionDialog::PrepareRecordNo(CString szInc) {
	...
	szSQL.Format(_T("HMS_CREATE_RECORDNO(%ld,'%s','%s')"), m_nDocumentNo, m_szOutPatient, szInc);
	...
	if (m_szOutPatient == _T("Y"))
	{
		...
		szSQL.Format(_T(" SELECT") \
			...
			_T("		AND extract(year from hcr_admitdate)=extract(year from sysdate)"), m_nDocumentNo);
		rs.ExecSQL(szSQL);
		if (!rs.IsEOF())
		{
			rs.GetValue(_T("numinward"), m_nNumInward);
	...
	if (!bIsRegister)
	{
		...
		if (szOutpatient == _T("Y") && szPrefix.Find(_T("NGT")) == -1)
		{
			szRecord = _T("NGT");
		}
		szRecord.AppendFormat(_T("%s"), szPrefix);
		szWhere.Format(_T(" AND substr(hcr_recordno, 0, instr(hcr_recordno, '-')-1) = '%s'"), szRecord);
		...
		szSQL.Format(_T(" SELECT COALESCE(MAX(hcr_numinward),0)+1") \
			...
			_T(" WHERE nvl(htr_outpatient,'N')                  = '%s' %s ") \
			_T(" AND extract(YEAR FROM hcr_admitdate)=extract(YEAR FROM SYSDATE)"), szOutpatient, szWhere);

		rs.ExecSQL(szSQL);
		if (!rs.IsEOF())
		{
			m_nNumInward = (long)rs.GetDoubleValue();
			szRecordNo.AppendFormat(_T("%s-%ld"), szRecord, m_nNumInward);
			m_nTreatTime = 1;
		}
	}
...
long CHMSDepartmentAdmissionDialog::PrepareNumber(CString szPrefix) {
	...
	szSQL.Format(_T(" SELECT COALESCE(MAX(hcr_numinward),0)+1") \
		...
		_T(" AND extract(YEAR FROM hcr_admitdate)=extract(YEAR FROM SYSDATE)"), m_szOutPatient, szWhere);
	rs.ExecSQL(szSQL);
	if (!rs.IsEOF())
	{
...
long CHMSDepartmentAdmissionDialog::OnLoaiKCBLoadData()
{
	CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
	CString szFilter;
	szFilter.Format(_T(" and ss_default in ('A','I')  "));
