...
void CHMSPrescriptionApprove::OnCreateComponents(){
	...
	m_wndIDLabel.Create(this, _T("ID"), 380, 60, 480, 85);
	m_wndID.Create(this,485, 60, 585, 85); 
	m_wndTotalPatientLabel.Create(this, _T("Total Patient"), 590, 60, 690, 85);
	m_wndTotalPatient.Create(this,695, 60, 795, 85); 
...
void CHMSPrescriptionApprove::OnInitializeComponents(){
	...
	m_wndList.InsertColumn(5, _T("Total Cost"), CFMT_MONEY, 100);
	m_wndList.InsertColumn(6, _T("Deposit"), CFMT_MONEY, 100);
	m_wndList.InsertColumn(7, _T("Processed"), CFMT_TEXT|CFMT_CENTER, 0);
	m_wndList.InsertColumn(8, _T("Approved By"), CFMT_TEXT, 0);
	m_wndList.InsertColumn(9, _T("Approved Date"), CFMT_TEXT, 0);
	m_wndList.InsertColumn(10, _T("Dept"), CFMT_TEXT, 0);
	m_wndList.InsertColumn(11, _T("Patdept"), CFMT_MONEY, 0);
	m_wndList.InsertColumn(12, _T("Transaction_ID"), CFMT_MONEY, 0);
...
void CHMSPrescriptionApprove::OnSetWindowEvents(){
	...
	AddEvent(1, _T("Add	Ctrl+A"), _OnAddHMSPrescriptionApproveFnc, 0, 'A', VK_CONTROL);
	AddEvent(2, _T("Edit	Ctrl+E"), _OnEditHMSPrescriptionApproveFnc, 0, 'E', VK_CONTROL);
	AddEvent(3, _T("Delete	Ctrl+D"), _OnDeleteHMSPrescriptionApproveFnc, 0, 'D', VK_CONTROL);
	AddEvent(4, _T("Save	Ctrl+S"), _OnSaveHMSPrescriptionApproveFnc, 0, 'S', VK_CONTROL);
	AddEvent(0, _T("-"));
	AddEvent(5, _T("Cancel	Ctrl+T"), _OnCancelHMSPrescriptionApproveFnc, 0, 'T', VK_CONTROL);
	SetMode(VM_NONE);
...
long CHMSPrescriptionApprove::OnOrderListLoadData(){
	...
	szSQL.Format(_T("SELECT mt_orderno, mt_transaction_id, mt_storage_id, msl_code, mt_department_to_id ") \
		...
		_T("and msl_category='S' and mt_orderno like('%s%s%s%') ") \
		_T(" and trunc_date(mt_orderdate) BETWEEN cast_string2date('%s') and cast_string2date('%s') ") \
		_T(" %s ") \
		_T("ORDER BY mt_transaction_id "),
		_T("%"), m_szSheetNo, _T("%"), m_szFromDate, m_szToDate, szWhere);
...
long CHMSPrescriptionApprove::OnListLoadData(){
	...
	_T("	hpo_processedby, ") \
	_T("	sum(hpo_amount) as amount, ") \
	_T("	(SELECT SUM(hfe_amount) FROM hms_fee_deposit WHERE hfe_docno=hpo_docno)  as Deposit, ") \
	_T("	(SELECT Round(SUM(hms_fee_view.hfe_patdebt)) FROM hms_fee_view WHERE hfe_docno=hpo_docno and hfe_status not in('P','C') and hfe_category<>'Y' ) AS patdebt ") \
	_T(" FROM hms_ipharmaorder") \
	_T(" LEFT JOIN hms_doc ON(hd_docno=hpo_docno) ") \
	_T(" LEFT JOIN hms_patient ON(hd_patientno=hp_patientno)") \
	_T(" WHERE hpo_transaction_id =%ld ") \
	_T(" and hpo_ordertype IN('P','D')  ") \
