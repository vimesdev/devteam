...
CHMSAdviseChatMessage::CHMSAdviseChatMessage(CWnd *pParent, long nOrderId):
...
void CHMSAdviseChatMessage::OnCreateComponents(){
	...
	m_wndMessage.Create(this,6, 490, 381, 540, TRUE, FALSE, TRUE); 
...
void CHMSAdviseChatMessage::OnInitializeComponents(){
	...
	m_wndChatMessage.InsertColumn(0, _T(""), CFMT_LEFT, 400);
}
...
long CHMSAdviseChatMessage::OnChatMessageLoadData(){
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT createdby, to_char(createddate, 'HH24:MI:SS DD/MM/YYYY') as createddate, message FROM hms_advise_message WHERE order_id=%ld ORDER BY createddate"), m_nOrderId);
	nCount = rs.ExecSQL(szSQL);
	CString szTitle, szMessage, tmpStr;
	while(!rs.IsEOF()){ 
		

		CGridListItem *pNewItem = new CGridListItem();

		rs.GetValue(_T("message"), szMessage);
		szMessage.Replace(_T("\r\n"), _T("<br>"));
		tmpStr.Format(_T("<font color=\"#C00000\" face-size=\"14\"><b>%s  </b></font><b><i>[%s]</b></i><p>%s</p>"),
			rs.GetValue(_T("createdby")),
			rs.GetValue(_T("createddate")),
			szMessage);
		pNewItem->Add(tmpStr, 450, CELLTYPE_HTML, DT_WORDBREAK);
		int nItem = m_wndChatMessage.Add(pNewItem);
...
void CHMSAdviseChatMessage::OnSendSelect(){
	...
	CString szSQL;
	CDbfMap tbl;
	tbl.SetTableName(_T("hms_advise_message"));
	tbl.AddField(_T("order_id"), dfLong);
	tbl.AddField(_T("createdby"), dfText, 15);
	tbl.AddField(_T("message"), dfText, 2000);
	tbl.SetValue(_T("order_id"), m_nOrderId);
	tbl.SetValue(_T("createdby"), pMF->GetCurrentUser());
	tbl.SetValue(_T("message"), m_szMessage);
