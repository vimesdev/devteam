...
static void _OnAllDeptSelectFnc(CWnd *pWnd){
	 ((CHMSParaclinicResultDialog*)pWnd)->OnAllDeptSelect();
}
...
CHMSParaclinicResultDialog::CHMSParaclinicResultDialog(CWnd *pParent, long nDocumentNo, bool bAllDept):
	CGuiDialog(pParent){
...
void CHMSParaclinicResultDialog::OnAllDeptSelect(){
	CGuiMainFrame *pMF = (CGuiMainFrame*) AfxGetMainWnd();
	UpdateData(TRUE);
	OnItemLoadData();
	UpdateData(FALSE);
...
long CHMSParaclinicResultDialog::OnItemLoadData(){
	...
	szWhere.AppendFormat(_T(" and trunc(hpc_orderdate) between to_DATE('%s', 'YYYY-MM-DD') and to_DATE('%s', 'YYYY-MM-DD') "), m_szFromDate, m_szToDate);
