...
/*
#if !defined(W2A) // wchar to multi-uchar string converter (current locale)
#define W2A aux::w2a
#endif

#if !defined(A2W) // multi-uchar to wchar string converter (current locale)
#define A2W aux::a2w
#endif
...
namespace pod
{
  ...
  template <typename T>
    ...
    typedef buffer<uchar> byte_buffer; 
...
{ 
  ...
  inline bool towcs(const uchar *utf8, size_t length, pod::wchar_buffer& outbuf)
  {
    if(!utf8 || length == 0) return true;
    const uchar* pc = (const uchar*)utf8;
    const uchar* last = pc + length;
    unsigned int b;
    ...
    while (pc < last) 
    {
      ...
      if ((b & 0x80) == 0)
      {
        // 1-uchar sequence: 000000000xxxxxxx = 0xxxxxxx
        ;
      } 
      else if ((b & 0xe0) == 0xc0) 
      {
        // 2-uchar sequence: 00000yyyyyxxxxxx = 110yyyyy 10xxxxxx
      ...
      {
        // 3-uchar sequence: zzzzyyyyyyxxxxxx = 1110zzzz 10yyyyyy 10xxxxxx
      ...
      {
        // 4-uchar sequence: 11101110wwwwzzzzyy + 110111yyyyxxxxxx = 11110uuu 10uuzzzz 10yyyyyy 10xxxxxx
...
//#pragma warning( suppress:4127 ) //  warning C4127: conditional expression is constant
        ...
        {
          assert(0); // what? wchar_t is single uchar here?
        }
      } 
      else 
      {
        assert(0); //bad start for UTF-8 multi-uchar sequence"
        ++num_errors;
        b = '?';
      }
      outbuf.push( wchar_t(b) );
  ...
  inline bool fromwcs(const wchar_t* wcs, size_t length, pod::byte_buffer& outbuf)
  {
    ...
    while((c = getc_utf16(wcs,length)) != 0) 
    {
      ...
      {
        outbuf.push(uchar((c >> 6) | 0xc0));
        outbuf.push(uchar((c & 0x3f) | 0x80));
      } 
      else if (c < (1 << 16)) 
      {
        outbuf.push(uchar((c >> 12) | 0xe0));
        outbuf.push(uchar(((c >> 6) & 0x3f) | 0x80));
        outbuf.push(uchar((c & 0x3f) | 0x80));
      } 
      else if (c < (1 << 21)) 
      {
        outbuf.push(uchar((c >> 18) | 0xf0));
        outbuf.push(uchar(((c >> 12) & 0x3f) | 0x80));
        outbuf.push(uchar(((c >> 6) & 0x3f) | 0x80));
        outbuf.push(uchar((c & 0x3f) | 0x80));
      }
      else 
    ...
    ostream_t& operator << (const wchar_t* wstr)
    {
      ...
      while((c = getc_utf16(wstr,length)) != 0) 
      {
        ...
        if (c < (1 << 7)) 
        {
         ...
         T::push (uchar((c >> 6) | 0xc0));
         T::push (uchar((c & 0x3f) | 0x80));
        } 
        else if (c < (1 << 16)) {
         T::push (uchar((c >> 12) | 0xe0));
         T::push (uchar(((c >> 6) & 0x3f) | 0x80));
         T::push (uchar((c & 0x3f) | 0x80));
        } 
        else if (c < (1 << 21)) 
        {
         T::push (uchar((c >> 18) | 0xf0));
         T::push (uchar(((c >> 12) & 0x3f) | 0x80));
         T::push (uchar(((c >> 6) & 0x3f) | 0x80));
         T::push (uchar((c & 0x3f) | 0x80));
        }
      }
...
namespace aux 
{
  ...
  public:
    explicit utf2w(const uchar* utf8, size_t length = 0)
    { 
      if(utf8)
      {
    ...
    { 
      if(utf8)
      {
        ...
        utf8::towcs((const uchar*)utf8, length ,buffer);
      }
    }
    ~utf2w() {}
  ...
  public:
    ...
    ~w2utf() {}
    operator const uchar*() { return buffer.data(); }
