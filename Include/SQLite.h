...
#include <afxext.h>
	...
	class SQLiteTable; // Forward declaration
	class SQLiteTablePtr; // Forward declaration

	// Main wrapper
	class AFX_EXT_CLASS SQLiteDatabase
	{
	public:
		SQLiteDatabase(void);
		...
		void ClearError() { m_iLastError=0; };
		...
		SQLiteTablePtr QuerySQL2( LPCTSTR strSQL );
	...
	{
		friend class SQLiteDatabase; // I allow Database class to enter data directly.
	public:
		SQLiteTable(void){ m_iRows=m_iCols=0;	m_iPos=-1; };
		virtual ~SQLiteTable() {};
	...
	class AFX_EXT_CLASS SQLiteTablePtr
	{
	public:
		// Default constructor
		SQLiteTablePtr( );
		...
		SQLiteTable* Detach();
		SQLiteTable* GetTablePtr(){ return m_pTable;}
