#ifndef HMSUPDATEPROMOTIONFEEDIALOG_H
#define HMSUPDATEPROMOTIONFEEDIALOG_H
...
class CHMSPromotionFeeDialog : public CGuiDialog
{
protected:
...
public:
	...
	CGuiRadioButton	m_wndNolevel;
	CGuiLabel		m_wndDiscountLimitLabel;
	CGuiComboBox	m_wndDiscountLimit;
	CGuiListCtrl	m_wndDiscountInvoiceList;
	CString	m_szSerialNo;
	long	m_nReceiptNo;
	CString	m_szReceiptDate;
	CString	m_szDiscountLimitKey;
	double	m_nAmount;
	...
	void			OnLevel1Select(); 
	void			OnLevel2Select(); 
	void			OnNolevelSelect();
	long			OnDiscountLimitLoadData();
	void			OnDiscountLimitSelendok();
	CHMSPromotionFeeDialog(CWnd *pParent);
	~CHMSPromotionFeeDialog();
	...
	int OnAddHMSFeeDiscountDialog(); 
	int OnEditHMSFeeDiscountDialog(); 
	int OnDeleteHMSFeeDiscountDialog(); 
	int OnSaveHMSFeeDiscountDialog(); 
	int OnCancelHMSFeeDiscountDialog(); 
	int OnHMSFeeDiscountDialogListLoadData();
	void OnDiscountInvoiceListSelectChange(int nOldItem, int nNewItem);
	int			OnDiscountInvoiceListDeleteItem();
	long		OnDiscountInvoiceListLoadData();
};
#endif
