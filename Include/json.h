...
typedef std::wstring *pwstring;
typedef std::string  *pstring;

// json object is a collection of name-value pair
typedef std::map<std::wstring, JSONVALUE> JSONOBJECT, *LPJSONOBJECT;
typedef std::vector<std::wstring> JSONOBJECTORDER, *LPJSONOBJECTORDER;
...
public:
        ...
        JSONVALUE(const std::wstring& src);
        ...
        JSONVALUE& operator =(const std::wstring& src);
        ...
        const wchar_t* ToString(std::wstring& sz, const DWORD nFlag = 0, const JSONFORMAT* pFormat = NULL);
        bool Parse(const std::wstring& sz, const DWORD nFlag = JSON_FLAG_LOOSE, JSONERROR *pError = NULL);

        // conversion, ansi string version
        const char* ToString(std::string& sz, const DWORD nFlag = 0, const JSONFORMAT* pFormat = NULL);
		LPCTSTR ToString(CString& sz, const DWORD nFlag = 0, const JSONFORMAT* pFormat = NULL);
        bool Parse(const std::string& sz, const DWORD nFlag = JSON_FLAG_LOOSE, JSONERROR *pError = NULL);
		bool Parse(const CString& sz, const DWORD nFlag = JSON_FLAG_LOOSE, JSONERROR *pError = NULL);
...
protected:
        // string
        const wchar_t* FormatString(std::wstring &sz, const std::wstring &szSrc, const DWORD nFlag, const bool bName);
        const wchar_t* TrimString(std::wstring& sz, const int nLR);
        bool IsHexChar(const wchar_t c);
        const wchar_t* EscapeChar(const wchar_t c, const wchar_t cQuote);
        wchar_t UnescapeChar(const std::wstring& sz);
        int UnescapeString(std::wstring& sz);
        const wchar_t* ConvertAW(std::wstring& dest, const char* src, const UINT srccp = CP_UTF8, const bool bRemoveBom = true);
        const char* ConvertWA(std::string& dest, const wchar_t* src, const UINT destcp = CP_UTF8, const bool bRemoveBom = true);
        int JsonFromString(JSONVALUE* dst, const std::wstring& sz, const DWORD nFlag);
        
        bool HasFlag(const DWORD nFlag, const DWORD nSrc);
        void SetError(JSONERROR *pError, const std::wstring& sz, const std::wstring::size_type pos, LPCWSTR pszDesc);

        // value
        void SetString(JSONVALUE* p);
        void SetObject(JSONVALUE* p);
        void SetArray(JSONVALUE* p);
        void ObjectToString(std::wstring& sz, const std::wstring& szKey, const JSONVALUE& jVal, const DWORD nFlag, const JSONFORMAT& formatNew,
                bool& bFirstItem, const bool bPrettyPrint, const std::wstring& szIndent, const std::wstring& cIndent);
