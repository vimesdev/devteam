...
public:
    struct Result {
        ...
        std::string data;
        std::string error;
		Result(bool bValid, std::string strData, std::string strError):
		valid(bValid), data(strData), error(strError)
		{
		}
    };
	...
	virtual Result load( const std::wstring & name ) = 0;
};
...
public:
	...
	Result load( const std::wstring & name );
};
...
public:
	...
	void add( const std::wstring & name, const std::wstring & body );
    Result load( const std::string & name );
	Result load( const std::wstring & name );
};
