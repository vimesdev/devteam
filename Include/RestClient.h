...
#define CLIENT_USERAGENT "restclient-agent/1.0"
...
#if _AFXDLL
#define    RESTDLLEXPORT _declspec(dllexport)

#elif defined(_AFXLIB)
#define    RESTDLLEXPORT _declspec(dllimport)
#else
#define    RESTDLLEXPORT WINAPI
#endif

#define PRINT_LOG [](const std::string& strLogMsg) { std::cout << strLogMsg << std::endl;  }

extern std::string kAccesstoken;
extern std::mutex   kMtxCurlSession; // mutex used to manage API global operations
extern std::string  kCertificationAuthorityFile;
extern int kCurlSession;   // Count of the actual sessions


class RESTDLLEXPORT CRestClient
{
public:
	// Public definitions
	...
	struct ProgressFnStruct
	{
		...
		double dLastRunTime;
		CURL*  pCurl;
		/* owner of the CRestClient object. can be used in the body of the progress
	...
	* error log messages printing by not using the flag ALL_FLAGS or ENABLE_LOG */
	CRestClient(LogFnCallback oLogger=PRINT_LOG);
	virtual ~CRestClient();

	// copy constructor and assignment operator are disabled
	CRestClient(const CRestClient& Copy) = delete;
	CRestClient& operator=(const CRestClient& Copy) = delete;
	...
	static int GetCurlSessionCount() { return kCurlSession; }
	...
	// SSL certs
	static const std::string& GetCertificateFile() { return kCertificationAuthorityFile; }
	static void SetCertificateFile(const std::string& strPath) { kCertificationAuthorityFile = strPath; }
...
#define LOG_WARNING_OBJECT_NOT_CLEANED          "[HTTPClient][Warning] Object was freed before calling " \
                                                "CRestClient::CleanupSession(). The API session was cleaned though."
...
#pragma comment(lib, "../lib/third_party/libcurl.lib")
