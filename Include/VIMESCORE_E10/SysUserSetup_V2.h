#ifndef SYSUSERSETUP_V2_H
#define SYSUSERSETUP_V2_H
...
class AFX_EXT_CLASS CSysUserSetup_V2 : public CGuiView
{
...
public:
	...
	CGuiLabel		m_wndPosisionLabel;
	CGuiComboBox	m_wndPosision;
	...
	CGuiTextCtrl	m_wndSignCert;
	CGuiButton		m_wndGroupPer;
	...
	//void			OnSignCertChange(); 
	//void			OnSignCertSetfocus(); 
	//void			OnSignCertKillfocus(); 
	int			OnSignCertCheckValue(); 
	void		OnPermListSearch();
	void		OnPermListSearchByName();
	int			OnSoBHYTCheckValue();
	long		OnPosisionLoadData();
	void			OnGroupPerSelect(); 
