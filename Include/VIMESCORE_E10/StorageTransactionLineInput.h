#ifndef STORAGETRANSACTIONLINEINPUT_H
#define STORAGETRANSACTIONLINEINPUT_H
...
#include "GuiPopup.h"
#include "DbField.h"
class CStorageTransactionLineInput : public CGuiPopup{
protected:
	bool	IsService(int nStorageID);
public:
	long	m_nOrderID;
	long	m_nOrderLine;
	int		m_nStorageID;
	...
	double	m_nSoldier;
	double	m_nPolicy;
	double	m_nSoldierInsurnace;
	double	m_nOnHand;
	double	m_nOtherInsurance;
	double	m_nService;
	double	m_nOther;
	double	m_nSumQuantity;
	...
	CStorageTransactionLineInput(CWnd *pParent, int nMode);
	~CStorageTransactionLineInput();
