...
void CRegimenOrder::OnCreateComponents(){
	...
	m_wndDateDayLabel.Create(this, _T("DateDay"), 465, 170, 565, 195);
	...
	m_wndSoluotDTLabel.Create(this, _T("SoluotDT"), 220, 200, 320, 225);
	m_wndSoluotDT.Create(this,325, 200, 460, 225); 
	m_wndDanhGiaLabel.Create(this, _T("DanhGia"), 465, 200, 565, 225);
	m_wndDanhGia.Create(this,570, 200, 628, 226); 
...
void CRegimenOrder::OnSetWindowEvents(){
	...
	AddEvent(1, _T("Add	Ctrl+A"), _OnAddRegimenOrderFnc, 0, 'A', VK_CONTROL);
	AddEvent(2, _T("Edit	Ctrl+E"), _OnEditRegimenOrderFnc, 0, 'E', VK_CONTROL);
	AddEvent(3, _T("Delete	Ctrl+D"), _OnDeleteRegimenOrderFnc, 0, 'D', VK_CONTROL);
	AddEvent(4, _T("Save	Ctrl+S"), _OnSaveRegimenOrderFnc, 0, 'S', VK_CONTROL);
	AddEvent(0, _T("-"));
	AddEvent(5, _T("Cancel	Ctrl+T"), _OnCancelRegimenOrderFnc, 0, 'T', VK_CONTROL);
	SetMode(VM_NONE);

}
void CRegimenOrder::OnDoDataExchange(CDataExchange* pDX){
	DDX_Text(pDX, m_wndCreatedby.GetDlgCtrlID(), m_szCreatedby);
	DDX_Text(pDX, m_wndCreateddate.GetDlgCtrlID(), m_szCreateddate);
