...
static long _OnPhanloaidtLoadDataFnc(CWnd *pWnd){
	return ((CHMSExtraPatientPackageDialog *)pWnd)->OnPhanloaidtLoadData();
}
...
void CHMSExtraPatientPackageDialog::OnCreateComponents(){
	...
	m_wndYobLabel.Create(this, _T("Yob"), 10, 60, 130, 85);
	m_wndYob.Create(this,135, 60, 260, 85); 
	m_wndPackageInformation.Create(this, _T("Package Information"), 5, 5, 645, 299);
	m_wndSexLabel.Create(this, _T("Sex"), 265, 60, 385, 85);
	m_wndSex.Create(this,390, 60, 515, 85); 
	m_wndAddressLabel.Create(this, _T("Address"), 10, 90, 130, 115);
	m_wndAddress.Create(this,135, 90, 640, 115); 
	...
	m_wndBedDiagram.Create(this, _T("Bed Diagram"), 265, 210, 385, 235);
	m_wndBed.Create(this,390, 210, 640, 235); 
	m_wndPhanloaidtLabel.Create(this, _T("Phân loại lưu trú"), 10, 240, 130, 265);
	m_wndPhanloaidt.Create(this,135, 240, 640, 265); 
	m_wndDescriptionLabel.Create(this, _T("Description"), 10, 270, 130, 295);
	m_wndDescription.Create(this,135, 270, 640, 295); 
	m_wndSave.Create(this, _T("&Save"), 480, 305, 560, 330);
	m_wndCancel.Create(this, _T("&Cancel"), 565, 305, 645, 330);
}
void CHMSExtraPatientPackageDialog::OnInitializeComponents(){
	...
	m_wndPhanloaidt.SetCheckValue(true);
...
//	m_wndEndDate.SetMax(CDateTime(pMF->GetSysDateTime()));
	...
	m_wndPhanloaidt.InsertColumn(0, _T("ID"), CFMT_TEXT, 90);
...
_tprintf(_T("\r\n%s"), m_szPackageKey);
	...
	szWhere.AppendFormat(_T(" and hfl_subgroup <> 'PKG_OLD' "));
...
long CHMSExtraPatientPackageDialog::OnPhanloaidtLoadData()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
int CHMSExtraPatientPackageDialog::OnSaveHMSExtraPatientPackageDialog(){
	...
	szSQL.Format(_T("HMS_PATIENT_CREATENEW_EXTRA(%ld, '%s','%s', '%s') "), m_nFindDocument, pMF->GetDepartmentID(), pMF->GetCurrentUser(), m_szPhanloaidtKey);
	//_msg(_T("%s"), szSQL);
	...
	tbl.AddField(_T("p_suspectcovid"), dfText, 2);
	...
	tbl.SetValue(_T("p_suspectcovid"), m_szPhanloaidtKey);
