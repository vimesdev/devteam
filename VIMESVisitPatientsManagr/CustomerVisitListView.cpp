...
static int _OnCustomerNameCheckValueFnc(CWnd *pWnd){
	return ((CCustomerVisitListView *)pWnd)->OnCustomerNameCheckValue();
} 
...
/*static void _OnDocumentNoChangeFnc(CWnd *pWnd){
	((CCustomerVisitListView *)pWnd)->OnDocumentNoChange();
} */
/*static void _OnDocumentNoSetfocusFnc(CWnd *pWnd){
	((CCustomerVisitListView *)pWnd)->OnDocumentNoSetfocus();} */ 
/*static void _OnDocumentNoKillfocusFnc(CWnd *pWnd){
	((CCustomerVisitListView *)pWnd)->OnDocumentNoKillfocus();
} */
static int _OnDocumentNoCheckValueFnc(CWnd *pWnd){
	return ((CCustomerVisitListView *)pWnd)->OnDocumentNoCheckValue();
} 
...
static void _OnRegistedOptionSelectFnc(CWnd *pWnd){
	  ((CCustomerVisitListView*)pWnd)->OnRegistedOptionSelect();
}
...
static long _OnListLoadDataFnc(CWnd *pWnd){
	return ((CCustomerVisitListView*)pWnd)->OnListLoadData();
} 
static void _OnListDblClickFnc(CWnd *pWnd){
	((CCustomerVisitListView*)pWnd)->OnListDblClick();
} 
static void _OnListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CCustomerVisitListView*)pWnd)->OnListSelectChange(nOldItem, nNewItem);
} 
static int _OnListDeleteItemFnc(CWnd *pWnd){
	 return ((CCustomerVisitListView*)pWnd)->OnListDeleteItem();
} 

static int _OnListRegisterItemFnc(CWnd *pWnd){
	 return ((CCustomerVisitListView*)pWnd)->OnListRegisterItem();
} 

static int _OnListEditItemFnc(CWnd *pWnd){
	 return ((CCustomerVisitListView*)pWnd)->OnListEditItem();
} 

static int _OnListTerminateItemFnc(CWnd *pWnd){
	 return ((CCustomerVisitListView*)pWnd)->OnListTerminateItem();
} 
static int _OnListPrintItemFnc(CWnd *pWnd){
	 return ((CCustomerVisitListView*)pWnd)->OnListPrintItem();
}

static int _OnListRollbackItemFnc(CWnd *pWnd){
	 return ((CCustomerVisitListView*)pWnd)->OnListRollbackItem();
}

static int _OnListRevokeItemFnc(CWnd *pWnd){
	 return ((CCustomerVisitListView*)pWnd)->OnListRevokeItem();
} 
...
void CCustomerVisitListView::OnCreateComponents()
{
	...
	m_wndCardTypeLabel.Create(this, _T("Card Type"), 360, 30, 445, 55);
	...
	m_wndPendingOption.Create(this, _T("Được cấp thẻ"), 115, 90, 225, 115);
	m_wndTerminateOption.Create(this, _T("Đã trả thẻ"), 230, 90, 345, 115);
...
//	m_wndToDate.SetMax(CDate(pMF->GetSysDate()));
	...
	m_wndList.InsertColumn(3, _T("Tên khách"), CFMT_TEXT, 140);
	m_wndList.InsertColumn(4, _T("Điện thoại"), CFMT_TEXT, 100);
	m_wndList.InsertColumn(5, _T("Số thẻ"), CFMT_TEXT, 100);
	m_wndList.InsertColumn(6, _T("Thẻ mở rộng"), CFMT_TEXT, 100);
	m_wndList.InsertColumn(7, _T("Loại thẻ"), CFMT_TEXT, 70);
	m_wndList.InsertColumn(8, _T("Đặt cọc"), CFMT_NUMBER, 70);
	m_wndList.InsertColumn(9, _T("Ghi chú"), CFMT_TEXT, 200);
	m_wndList.InsertColumn(10, _T("Thời gian vào"), CFMT_DATETIME, 120);
	m_wndList.InsertColumn(11, _T("Thời gian ra"), CFMT_DATETIME, 120);
	m_wndList.InsertColumn(12, _T("Status"), CFMT_TEXT|CFMT_CENTER, 50);
	m_wndList.InsertColumn(13, _T("Người cấp"), CFMT_TEXT, 70);
	m_wndList.InsertColumn(14, _T("Người trả"), CFMT_TEXT, 70);
	m_wndList.InsertColumn(15, _T("Người bảo lãnh"), CFMT_TEXT, 140);
	m_wndList.InsertColumn(16, _T("Diễn giải"), CFMT_TEXT, 200);
...
void CCustomerVisitListView::OnSetWindowEvents(){
	...
	m_wndList.AddEvent(1, _T("Cấp phát thẻ\tF2"), _OnListRegisterItemFnc,  0, 0, VK_F2);		
	m_wndList.AddEvent(0, _T("-"), NULL);
	m_wndList.AddEvent(2, _T("Sửa thông tin"), _OnListEditItemFnc);
	m_wndList.AddEvent(0, _T("-"), NULL);
	m_wndList.AddEvent(3, _T("Xóa thẻ"), _OnListDeleteItemFnc);
	m_wndList.AddEvent(0, _T("-"), NULL);
	m_wndList.AddEvent(4, _T("Trả thẻ\tF5"), _OnListTerminateItemFnc,0, 0, VK_F5);
...
/*void CCustomerVisitListView::OnCustomerNameChange(){
	
} */
/*void CCustomerVisitListView::OnCustomerNameSetfocus(){
	
} */
/*void CCustomerVisitListView::OnCustomerNameKillfocus(){
	
} */
int CCustomerVisitListView::OnCustomerNameCheckValue(){
	OnListLoadData();
	return 0;
} 
...
/*void CCustomerVisitListView::UIDKillfocus(){
...
void CCustomerVisitListView::OnRegistedOptionSelect(){
	UpdateData(TRUE);
	m_wndFromDate.EnableWindow(TRUE);
	m_wndToDate.EnableWindow(TRUE);
	OnListLoadData();
...
void CCustomerVisitListView::OnListDblClick(){
	int nSel = m_wndList.GetCurSel();
	if(nSel < 0)
		return;
	CCustomerVisitRegisterDialog dlg(this, VM_VIEW);
	dlg.m_nUID = m_nUID;
	if(dlg.DoModal() == IDOK)
	{
		OnListLoadData();
...
void CCustomerVisitListView::OnListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	if(nNewItem < 0)
		return;
...
int CCustomerVisitListView::OnListRegisterItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnRegisterSelect();
	 return 0;
} 

int CCustomerVisitListView::OnListEditItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
int CCustomerVisitListView::OnListDeleteItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	int nSel = m_wndList.GetCurSel();
	if(nSel < 0)
		return -1;
...
int CCustomerVisitListView::OnListTerminateItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
int CCustomerVisitListView::OnListRevokeItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
int CCustomerVisitListView::OnListPrintItem(long nId)
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	//Phan in can lam lai
	long m_nUID = 0;
...
long CCustomerVisitListView::OnListLoadData(){
	...
	{
		szWhere.AppendFormat(_T(" and hcv_card_type = '%s' "), m_szCardTypeKey);
	}
	
	if(!m_szCardNumber.IsEmpty())
	{
		szWhere.AppendFormat(_T(" and HCV_CARD_NO = '%s' "), m_szCardNumber);
	}

	if(!m_szDepartmentKey.IsEmpty())
	{
		szWhere.AppendFormat(_T(" and hcv_deptid='%s' "), m_szDepartmentKey);
	}	

	if(!m_szCustomerName.IsEmpty())
	{
		szWhere.AppendFormat(_T(" and lower(hcv_name) like(chr(37)||lower('%s')||chr(37)) "),
			m_szCustomerName);
	}
	...
	{
		szWhere.AppendFormat(_T(" and hcv_status='O' "));
	...
	szSQL.Format(_T(" SELECT") \
				_T("     hcv_uid,") \
				//_T("     hcv_name,") \//
				_T("     hcv_name ||'-'|| hcv_docno as hcv_name,") \
				_T("     hcv_phone,") \
				_T("     hcv_reason,") \
				_T("     hcv_guardian,") \
				_T("     hcv_card_type,") \
	...
	while(!rs.IsEOF()){ 
		...
		if(szStatus == _T("O"))
			szStatusDesc = _T("Đã cấp thẻ");
		if(szStatus == _T("T"))
			szStatusDesc = _T("Đã trả thẻ");

		tmpStr.Format(_T("%d"), ++nIndex);
		
		m_wndList.AddItems(
			tmpStr,
			rs.GetValue(_T("hcv_uid")),
			rs.GetValue(_T("hcv_deptid")), 
			rs.GetValue(_T("hcv_name")),
			rs.GetValue(_T("hcv_phone")),
			rs.GetValue(_T("HCV_CARD_NO")),
			rs.GetValue(_T("HCV_CARD_NO_EX")),
			rs.GetValue(_T("ss_desc")),
			rs.GetValue(_T("hcv_amount")),
			rs.GetValue(_T("hcv_note")), 
			rs.GetValue(_T("hcv_time_in")), 
			rs.GetValue(_T("hcv_time_out")), 
			rs.GetValue(_T("hcv_status")),
			rs.GetValue(_T("HCV_CREATEDBY")),
			rs.GetValue(_T("hcv_termby")),
			rs.GetValue(_T("hcv_guardian")), 
			rs.GetValue(_T("hcv_reason")), 			
			NULL);
...
BOOL CCustomerVisitListView::PreTranslateMessage(MSG *pMsg)
{
...
CString CCustomerVisitListView::GetOrderStatus()
{
	CMainFrame* pMF = (CMainFrame*) AfxGetMainWnd();
...
void CCustomerVisitListView::OnResizeLayout()
{
...
int CCustomerVisitListView::OnListRollbackItem()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	return 0;
	int nSel = m_wndList.GetCurSel();
