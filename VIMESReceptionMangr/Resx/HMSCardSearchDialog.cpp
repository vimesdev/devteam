...
int CHMSCardSearchDialog::OnAddHMSCardSearchDialog(){
 	if(GetMode() == VM_ADD || GetMode() == VM_EDIT)  
 		return -1; 
 	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd(); 
 	if(!pMF->SetStartWork(this,_T("Add HMSCardSearchDialog"))) 
 		return -1; 
 	SetMode(VM_ADD);
return 0; 
...
int CHMSCardSearchDialog::OnEditHMSCardSearchDialog(){
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd(); 
 	if(!pMF->SetStartWork(this,_T("Edit HMSCardSearchDialog"))) 
 		return -1; 
 	SetMode(VM_EDIT);
return 0;  
...
int CHMSCardSearchDialog::OnSaveHMSCardSearchDialog(){
 ...
 _fmsg(_T("%s"), szSQL); 
 	...
 	if(ret > 0) 
 	{ 
 		//OnHMSCardSearchDialogListLoadData(); 
 		SetMode(VM_VIEW); 
 		pMF->FinishWork(this); 
 	} 
 	else 
