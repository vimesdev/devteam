...
static int _OnSetDiscountPatientFnc(CWnd *pWnd)
{
    return ((CHMSRegistration *)pWnd)->OnSetDiscountPatient();
}
...
>>>>>>>>> Temporary merge branch 2
    ...
    */
    ...
    m_wndExamList.InsertColumn(10, _T("Room ID"), CFMT_TEXT | CFMT_CENTER, 0);
    ...
    m_wndProvill.InsertColumn(3, _T("Ghi chú"), CFMT_TEXT, 150);
...
void CHMSRegistration::OnSetWindowEvents()
{
    ...
    // Thêm chức năng đánh dấu bệnh nhân được miễn giảm của khoa PTTM
    // Phân quyền nếu cần thiết -> ghi log lại
    //
    if (pMF->m_szDept == _T("TTTM") || pMF->m_szDept == _T("TMNT"))
    {
        m_wndExamList.AddEvent(5, _T("Phân loại bệnh nhân được miễn giảm"),
                               _OnSetDiscountPatientFnc);
    }
...
void CHMSRegistration::GetScreenToData()
{
    ...
    if ((bAddExam) || nMode == VM_EDIT)
    {
        ...
        if (m_nQMSidx > 0)
            m_hms_examTbl.SetValue(_T("he_zone"), _T("ONL"));
    }
}
...
int CHMSRegistration::SetMode(int nMode)
{
    ...
    case VM_ADDPAT:
        ...
        // GiangDH load data tu dang ki Online
        if (m_nQMSidx > 0)
        {

            szSQL.Format(
                _T("SELECT qms_cardid, qms_patientname, hms_getage(sysdate, qms_birthdate) as age, " )
                _T(" qms_birthdate, qms_roomid, qms_prov_id, qms_prov_id,")
                _T("qms_dist_id, qms_vill_id, qms_occupation, qms_reason || ' ")
                _T("' || qms_comment as note,")
                _T(" qms_address, qms_contact, qms_idcard, qms_patientno ")
                _T(" FROM qms_patient WHERE qms_idx = %d"),
                m_nQMSidx);
            rs.ExecSQL(szSQL);
            if (!rs.IsEOF())
            {
                rs.GetValue(_T("qms_cardid"), m_szCardNo);
                rs.GetValue(_T("qms_patientname"), m_szPatientName);
                rs.GetValue(_T("age"), m_szAge);
                rs.GetValue(_T("qms_birthdate"), m_szBirthDate);
                rs.GetValue(_T("qms_roomid"), m_szRoomKey);
                // rs.GetValue(_T("qms_examtype"), m_szExamTypeKey);
                // rs.GetValue(_T("qms_examdate"), m_szExamDate);
                rs.GetValue(_T("qms_prov_id"), m_szProvillKey);
                rs.GetValue(_T("qms_dist_id"), m_szDistrictKey);
                rs.GetValue(_T("qms_vill_id"), m_szVillageKey);
                rs.GetValue(_T("qms_address"), m_szDetailAddress);
                rs.GetValue(_T("qms_contact"), m_szPhone);
                rs.GetValue(_T("qms_idcard"), m_szIDNo);
                rs.GetValue(_T("qms_occupation"), m_szOccupationKey);
                rs.GetValue(_T("note"), m_szPatientNote);
                rs.GetValue(_T("qms_patientno"), m_nPatientNo);
            }
            if (m_nPatientNo > 0)
            {
                CString szSQLP, szObj;
                szSQLP.Format(
                    _T(" SELECT hc_idx, hd_object, hd_army_objectid, hd_rank ")
                    _T("FROM (SELECT distinct hc_idx, hd_object, ")
                    _T("hd_army_objectid, hd_rank FROM hms_doc ")
                    _T(" LEFT JOIN hms_card ON (hc_patientno = hd_patientno) ")
                    _T(" WHERE hd_patientno = %ld  ORDER BY hc_idx DESC) ")
                    _T("WHERE rownum < 2 "),
                    m_nPatientNo);
                ...
                if (!rs.IsEOF())
                {
                    // m_szObjectKey = int2str(rs.GetIntValue());
                    rs.GetValue(_T("hd_army_objectid"), m_szArmObjectKey);
...
int CHMSRegistration::OnRoomListLoadData()
{
    ...
    szSQL.Format(_T(" SELECT * FROM (SELECT he_deptid,")
                 _T("   hrl_name AS name,")
                 ...
                 _T("   WHERE 1=1 %s AND he_deptid='%s' ")
                 _T("   ) Tbl")
...
long CHMSRegistration::OnExamListLoadData()
{
    ...
    szSQL.Format(
        ...
        _T("           Trunc(SYSDATE) - Trunc(he_examdate) AS count_examdate,")
        _T("           (SELECT DISTINCT '['")
        _T("                            ||hrl_deptid")
        _T("                            ||'] '")
        _T("                            ||hrl_name")
        _T("              FROM hms_roomlist")
        _T("             WHERE hrl_deptid = he_deptid AND hrl_id = he_roomid) ")
        _T("AS name,")
        ...
        _T("              FROM sys_user")
        _T("             WHERE su_userid = he_doctor) AS doctor,")
        _T("           he_status AS status,")
        _T("           he_diagnostic AS disease,")
        _T("           CASE WHEN Nvl(hdf_acceptedfee, 'N') = 'Y' THEN ")
        _T("Cast('Đã thanh toán' AS NVARCHAR2(21))")
        _T("           ELSE Cast('Chưa thanh toán' AS NVARCHAR2(21))")
        _T("           END AS payment, he_roomid as room_id")
        _T("      FROM hms_doc")
    ...
    while (!rs.IsEOF())
    {
        ...
        int nItem = m_wndExamList.AddItems(
            ...
            rs.GetValue(_T("HE_FEEIDX")), rs.GetValue(L"room_id"), NULL);
...
int CHMSRegistration::ProcessIdCardQrInput(CString szInfo)
{
    ...
    if (szCheckQROnline != _T("Y"))
    {
        ...
        if (dlg.DoModal() == IDOK)
        {
            ...
            if (info.GetSize() >= 5)
            {
                CString szAddress, tmpStr;
                info.GetAt(5, szAddress);
                
                CString szSQL;
                szSQL.Format(L"get_address_code('%s') ", szAddress);
                CString result = pMF->ExecDML(szSQL);
                if (!result.IsEmpty())
                {
                    CStringToken addr(result, L".");
                    addr.GetAt(0, m_szProvillKey);
                    addr.GetAt(2, m_szDistrictKey);
                    addr.GetAt(2, m_szVillageKey);
                    addr.GetAt(3, m_szDetailAddress);
                    UpdateData(false);
                }
...
int CHMSRegistration::OnVillageLoadData()
{
    ...
    CString szFilter = _T(""), szDistKey = m_szDistrictKey;
...
int CHMSRegistration::OnCardNoCheckValue()
{
    ...
    CString szSQL;
    szSQL.Format(_T("SELECT * FROM hms_card WHERE hc_cardno='%s' "),
                 m_szCardNo);
...
int CHMSRegistration::OnCardNoButtonSelect()
{
    ...
    if (szHasCard == _T("Y"))
    {
        ...
        _fmsg(L"dlg cardno : %s", m_szCardNo);
        if (!m_szCardNo.IsEmpty())
        {
...
int CHMSRegistration::OnSaveHMSRegistration()
{
    ...
    // nPatientNo); 			ShowMessageBox(szMsg, MB_ICONWARNING|MB_OK);
    // return -1;
    //		}
    ...
    if ((bAddExam) || nMode == VM_EDIT)
    {
        ...
        else
        {
            if (m_wndExamList.GetItemCount() > 0)
            {
                m_szCurRoom = m_wndExamList.GetItemText(0, 10);
            }
        }
    ...
    if (bAddExam)
    {
        ...
        // GiangDH: kiểm tra nếu là đăng kí khám onl thì cập nhật lại docno
        // patientno vào QMS theo qms_idx
        if (m_nQMSidx > 0)
        {
            szSQL.Format(_T("UPDATE qms_patient SET qms_docno = %ld, ")
                         _T("qms_trangthai = 'Y',")
                         _T("qms_patientno = %ld WHERE qms_idx = %d "),
                         m_nDocumentNo, m_nPatientNo, m_nQMSidx);
            _tprintf(L"%s", szSQL);
            pMF->ExecSQL(szSQL);
        }
    }
    else if (nMode == VM_EDIT)
    {
        ...
        if (szHealthExam == _T("Y"))
        {
            szSQL = m_hms_docTbl.GetUpdateSQL(
                ...
                _T("xissueplace, hd_admitdate,hd_enddate, HD_HEALTHEXAM, hd_admitdept"));
        }
        else
            ...
            szSQL = m_hms_docTbl.GetUpdateSQL(
                ...
                _T("status,hd_cardno, ")
                _T("hd_cardidx,hd_xobject,hd_xcardno,hd_xissuedate,hd_")
                _T("xissueplace, hd_admitdate,hd_enddate, hd_admitdept"));
        szSQL += szWhere;
    ...
    // m_szCardNo); 		int ret = str2int(pMF->ExecDML(szSQL));
    // _msg(_T("%s"), szSQL);
...
int CHMSRegistration::OnSaveHMSRegistration2()
{
    ...
    else if (nMode == VM_EDIT)
    {
        ...
        szSQL = m_hms_docTbl.GetUpdateSQL(
            ...
            _T("hd_xobject,hd_xcardno,hd_xissuedate,hd_xissueplace, hd_admitdept"));
        szSQL += szWhere;
...
void CHMSRegistration::PrintReceipt(bool m_bPreview, bool m_bDirect)
{
    ...
    szSQL.Format(
        ...
        _T("IVF_GIAYGIOITHIEU,")
        _T("	(SELECT NVL(hrl_address, NULL) FROM hms_roomlist WHERE ")
        _T("hrl_deptid=he_deptid AND hrl_id=he_roomid) as diachi, ")
        _T("    HD_PASSWD as OnlineCode, hp_sin as cccd")
        _T(" FROM hms_patient")
        _T(" LEFT JOIN hms_doc ON(hd_patientno=hp_patientno)")
    ...
    rs.GetValue(_T("OnlineCode"), szOnlineCode);
    ...
    rpt.GetReportFooter()->SetValue(_T("diachi"), rs.GetValue(_T("diachi")));
    ...
    rpt.GetReportFooter()->SetValue(_T("OnlineCode"), szOnlineCode);
    ...
    CString szUrlLinkPatient;
    szUrlLinkPatient.Format(_T("%s/?docno=%ld&phone=%s"),
                            pMF->GetProp(_T("hms_web_url_patient")),
                            m_nDocumentNo, m_szPhone);
    //_msg(_T("1: %s"), pMF->GetProp(_T("hms_web_url_patient")));
    //_msg(_T("2: %s"), pMF->GetProp(_T("hms_web_url_patient")));

    if (szUrlLinkPatient.IsEmpty())
        szUrlLinkPatient = _T("err: 99");
    rpt.GetReportFooter()->SetValue(_T("UrlLink"), szUrlLinkPatient);
    rpt.GetReportHeader()->SetValue(_T("UrlLink"), szUrlLinkPatient);

    rpt.GetReportFooter()->SetValue(_T("Passwd"), szOnlineCode);
    rpt.GetReportHeader()->SetValue(_T("Passwd"), szOnlineCode);

    rpt.GetReportFooter()->SetValue(_T("LinkOnline"),
                                    pMF->GetProp(_T("hms_web_url_patient")));
    rpt.GetReportHeader()->SetValue(_T("LinkOnline"),
                                    pMF->GetProp(_T("hms_web_url_patient")));
        ...
        if (nMaloi == 0)
        {
            if (bShowHis)
            {
                ...
                for (int i = 0; i < jj.Size(); i++)
                {
                    ...
                    // m_sCardInfo.szExpdate) <=5)) 		_msg(_T(" vua di
                    // kham cach day 5 ngay"));
                    //
                    // }
...
// Chức năng cho khoa C13 được chuyển đổi kiểu khám -> thu tiền dịch vụ
#include "HMSSetDisCountPatientDialog.h"
int CHMSRegistration::OnSetDiscountPatient()
{
    CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
    CRecord rs(&pMF->m_db);
    CString szSQL;
    ...
    {
        ShowMessageBox(_T("Chỉ hỗ trợ đối tượng bệnh nhân dịch vụ!"));
        return -2;
    }

    if (!pMF->CheckPermission(_T("RM.100.13")))
    {
        ShowMessageBox(_T("Liên hệ admin để cấp quyền RM.100.13 và sử dụng ")
                       _T("chức năng này!"));
        return -4;
    }
    ...
    {
        ShowMessageBox(
            _T("Chỉ User của trung tâm thẩm mỹ được sử dụng chức năng này!"));
        return -5;
    }
    CHMSSetDisCountPatientDialog dlg(pMF);
