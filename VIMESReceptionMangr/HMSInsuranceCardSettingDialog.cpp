...
void CHMSInsuranceCardSettingDialog::OnInitializeComponents(){
	...
	m_wndList.InsertColumn(0, _T("Id"), CFMT_NUMBER, 70);
	...
	m_wndList.InsertColumn(6, _T("Off Line"), CFMT_TEXT|CFMT_CENTER, 50);
	m_wndList.InsertColumn(7, _T("Active"), CFMT_TEXT|CFMT_CENTER, 50);
...
long CHMSInsuranceCardSettingDialog::OnListLoadData(){
	...
	szSQL.Format(_T("SELECT hc_cardno as cardno, trunc_date(hd_admitdate) as admitdate,") \
		...
		_T("WHERE hc_patientno=%ld  and hc_isprimary ='Y' ") \
...
void CHMSInsuranceCardSettingDialog::OnAddCardSelect(){
	...
	CHMSCardEntryDialog dlg(pMF, VM_ADD);
...
void CHMSInsuranceCardSettingDialog::OnApplySelect(){
	...
	if(nSel < 0) 
	{
		ShowMessage(147, MB_OK| MB_ICONASTERISK);
		return;
	}
	UpdateData(true);
	int nCardIdx = str2int(m_wndList.GetItemText(nSel, 0));
	szSQL.Format(_T("SELECT hc_cardno, hc_regdate, hc_expdate, hc_discount FROM hms_card WHERE hc_patientno=%ld and hc_idx=%d and hd_isprimary='Y' "), m_nPatientNo, nCardIdx);
	rs.ExecSQL(szSQL);
	if(!rs.IsEOF()){
		...
		if(ret >= 0)
		{
			...
			if(m_wndList.GetItemCount() > 1)
			{
				for (int i =0; i < m_wndList.GetItemCount(); i++){
					...
					if(i != nSel){
						...
						_T("FROM hms_doc ") \
						_T("WHERE hd_patientno=%ld and (hd_cardidx=%ld or hd_xcardidx=%ld) "), m_nPatientNo, nCardIdx, nCardIdx);
			...
			CString m_szStatus;
			szSQL.Format(_T("SELECT max(hc_idx) FROM hms_card WHERE hc_patientno=%ld and hc_isprimary='Y' "), m_nPatientNo);
			rs.ExecSQL(szSQL);
			int nMaxCardIdx = rs.GetIntValue();
