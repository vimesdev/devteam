...
CHMSFeeDepositDialog::CHMSFeeDepositDialog(CWnd *pParent):
	CGuiDialog(pParent){
...
void CHMSFeeDepositDialog::OnSetWindowEvents(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	SetWindowText(_T("Deposit receipt"));
...
void CHMSFeeDepositDialog::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT * FROM hms_fee_deposit ") \
		_T("WHERE hfe_docno=%ld and hfe_org_id='%s' "), 
		m_nDocumentNo, pMF->GetModuleID()
		);
	rs.ExecSQL(szSQL);
	SetDefaultValues();
	if(!rs.IsEOF())
	...
	{
		m_nReceiptNo = pMF->GetLastRecvNo();
...
void CHMSFeeDepositDialog::GetScreenToData(){
	...
	m_hms_fee_depositTbl.SetValue(_T("hfe_patientno"), pMF->m_wndRegistration.m_nPatientNo);
	m_hms_fee_depositTbl.SetValue(_T("hfe_docno"), pMF->m_nDocumentNo);	
	...
	m_hms_fee_depositTbl.SetValue(_T("hfe_org_id"), _T("RM"));
}
...
int CHMSFeeDepositDialog::OnReceiptNoCheckValue(){
	...
	if(GetMode() == VM_EDIT)
	{
		szSQL.Format(_T("SELECT * FROM hms_fee_deposit WHERE hfe_docno<>%ld and trunc_date(hfe_date)=to_date('%s', 'yyyy/mm/dd hh24:mi:ss') and hfe_serialno='%s' and hfe_receiptno=%ld"), 
	...
	{
		szSQL.Format(_T("SELECT * FROM hms_fee_deposit WHERE trunc_date(hfe_date)=to_date('%s', 'yyyy/mm/dd hh24:mi:ss') and hfe_serialno='%s' and hfe_receiptno=%ld"), 		
...
int CHMSFeeDepositDialog::OnSaveHMSFeeDepositDialog(){
	...
	/*if(GetMode() == VM_ADD || GetMode() != VM_VIEW ) 
 		return -1; */
	...
	if(GetMode() == VM_ADD)
	{
		szSQL.Format(_T("HMS_FEE_DEPOSIT_INSERT(%s) "), m_hms_fee_depositTbl.FormatSQL());
 	...
 	{ 		
		if(m_bPrintReceipt){
			PrintDeposit(m_nInvoiceNo, true);
		}
		CGuiDialog::OnOK();
		return 1;
	}
...
void CHMSFeeDepositDialog::PrintDeposit(long nInvoiceNo, bool bPreview)
{	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	CReport rpt;
	...
	int p =1;
		...
		for (int i =0; i < p; i++)
		{
			...
			CString szDeptName = pMF->GetDepartmentName(tmpStr);
