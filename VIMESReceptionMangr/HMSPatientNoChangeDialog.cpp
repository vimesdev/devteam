...
static int _OnYOBCheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnYOBCheckValue();
} 
static int _OnSexCheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnSexCheckValue();
}
static int _OnAddressCheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnAddressCheckValue();
}
static int _OnInsCardCheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnInsCardCheckValue();
}
static int _OnYOB2CheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnYOB2CheckValue();
}
static int _OnSex2CheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnSex2CheckValue();
}
static int _OnAddress2CheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnAddress2CheckValue();
}
static int _OnInsCard2CheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientNoChangeDialog *)pWnd)->OnInsCard2CheckValue();
} 
...
void CHMSPatientNoChangeDialog::OnCreateComponents()
{
	...
	m_wndOldDocumentNo.Create(this,134, 29, 318, 54); 
	m_wndOldDocNoLabel.Create(this, _T("Old Document No"), 10, 30, 130, 55);
	m_wndPatientNo1Label.Create(this, _T("Từ mã BN"), 324, 30, 409, 55);
	m_wndNewPatientInformation.Create(this, _T("Thông tin bệnh nhân mới"), 5, 185, 597, 359);
	m_wndPatientNo1.Create(this,414, 30, 593, 55); 
	m_wndPatientInformation.Create(this, _T("Thông tin bệnh nhân cũ"), 5, 5, 597, 179);
	m_wndPatientNameLabel.Create(this, _T("Patient Name"), 10, 59, 130, 84);
	m_wndPatientName.Create(this,134, 60, 593, 85); 
	m_wndYOBLabel.Create(this, _T("Năm sinh"), 10, 90, 130, 115);
	m_wndYOB.Create(this,134, 90, 318, 115); 
	m_wndSexLabel.Create(this, _T("Sex"), 324, 90, 409, 115);
	m_wndSex.Create(this,414, 90, 593, 115); 
	m_wndAddressLabel.Create(this, _T("Address"), 9, 120, 129, 145);
	m_wndAddress.Create(this,134, 120, 593, 145); 
	m_wndInsCardLabel.Create(this, _T("Thẻ BH"), 9, 150, 129, 175);
	m_wndInsCard.Create(this,134, 150, 593, 175); 
	m_wndNewDocumentNoLabel.Create(this, _T("New Document No"), 9, 210, 129, 235);
	m_wndNewDocumentNo.Create(this,134, 210, 318, 235); 
	m_wndPatientNo2Label.Create(this, _T("Sang mã BN"), 324, 210, 409, 235);
	m_wndPatientNo2.Create(this,414, 210, 593, 235); 
	m_wndPatientName2Label.Create(this, _T("Patient Name"), 9, 240, 129, 265);
	m_wndPatientName2.Create(this,134, 240, 593, 265); 
	m_wndYOB2Label.Create(this, _T("Năm sinh"), 10, 270, 130, 295);
	m_wndYOB2.Create(this,134, 270, 318, 295); 
	m_wndSex2Label.Create(this, _T("Sex"), 324, 270, 409, 295);
	m_wndSex2.Create(this,414, 270, 593, 295); 
	m_wndAddress2Label.Create(this, _T("Địa chỉ"), 9, 300, 129, 325);
	m_wndAddress2.Create(this,134, 300, 593, 325); 
	m_wndInsCard2Label.Create(this, _T("Thẻ BH"), 9, 330, 129, 355);
	m_wndInsCard2.Create(this,134, 330, 593, 355); 
	m_wndOK.Create(this, _T("&OK"), 427, 365, 507, 390);
	m_wndCancel.Create(this, _T("&Cancel"), 512, 365, 592, 390);
}
...
//	m_wndPatientName2.SetCheckValue(true);
	...
	m_wndYOB2.SetReadOnly(true);
	m_wndSex.SetReadOnly(true);
	m_wndSex2.SetReadOnly(true);
	m_wndAddress.SetReadOnly(true);
	m_wndAddress2.SetReadOnly(true);
	m_wndInsCard.SetReadOnly(true);
	m_wndInsCard2.SetReadOnly(true);
}
void CHMSPatientNoChangeDialog::OnSetWindowEvents(){
	...
	m_wndInsCard.SetEvent(WE_CHECKVALUE, _OnInsCardCheckValueFnc);
	m_wndYOB2.SetEvent(WE_CHECKVALUE, _OnYOB2CheckValueFnc);	
	m_wndSex2.SetEvent(WE_CHECKVALUE, _OnSex2CheckValueFnc);	
	m_wndAddress2.SetEvent(WE_CHECKVALUE, _OnAddress2CheckValueFnc);	
	m_wndInsCard2.SetEvent(WE_CHECKVALUE, _OnInsCard2CheckValueFnc);
	m_wndNewDocumentNo.SetEvent(WE_CHECKVALUE, _OnNewDocumentNoCheckValueFnc);	
...
void CHMSPatientNoChangeDialog::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Text(pDX, m_wndInsCard.GetDlgCtrlID(), m_szInsCard);
	DDX_Text(pDX, m_wndYOB2.GetDlgCtrlID(), m_szYOB2);
	DDX_Text(pDX, m_wndSex2.GetDlgCtrlID(), m_szSex2);
	DDX_Text(pDX, m_wndAddress2.GetDlgCtrlID(), m_szAddress2);
	DDX_Text(pDX, m_wndInsCard2.GetDlgCtrlID(), m_szInsCard2);
...
int CHMSPatientNoChangeDialog::OnYOBCheckValue(){
	return 0;
}
int CHMSPatientNoChangeDialog::OnSexCheckValue(){
	return 0;
}
int CHMSPatientNoChangeDialog::OnAddressCheckValue(){
	return 0;
}
int CHMSPatientNoChangeDialog::OnInsCardCheckValue(){
	return 0;
}

int CHMSPatientNoChangeDialog::OnYOB2CheckValue(){
	return 0;
}
int CHMSPatientNoChangeDialog::OnSex2CheckValue(){
	return 0;
}
int CHMSPatientNoChangeDialog::OnAddress2CheckValue(){
	return 0;
}
int CHMSPatientNoChangeDialog::OnInsCard2CheckValue(){
	return 0;
} 
...
int CHMSPatientNoChangeDialog::OnOldDocumentNoCheckValue()
{
	...
	szSQL.Format(_T(" SELECT hd_patientno as patientno,") \
		...
		//_T(" 	hp_dtladdr||'-'||hms_getaddress(nvl(hd_provid, hp_provid), nvl(hd_distid, hp_distid), nvl(hd_villid, hp_villid)) AS address,") \//
		_T(" 	lower(hms_getaddress(nvl(hd_provid, hp_provid), nvl(hd_distid, hp_distid), nvl(hd_villid, hp_villid))) AS address,") \
		_T(" 	substr(HD_CARDNO, 1, 15) as inscard") \
		_T(" FROM hms_patient") \
	...
	if(!rs.IsEOF())
	{
		...
		rs.GetValue(_T("inscard"), m_szInsCard);
	}
	UpdateData(FALSE);
...
int CHMSPatientNoChangeDialog::OnNewDocumentNoCheckValue(){
	...
	szSQL.Format(_T(" SELECT 	hd_patientno as patientno,") \
		_T(" 	hp_surname||' '||hp_midname||' '||hp_firstname as pname,") \
		_T(" 	to_char(hp_birthdate, 'DD/MM/YYYY') as yob2,") \
		_T(" 	sys_sel_getname('sys_sex', hp_sex) as sex2,") \
		_T(" 	lower(hms_getaddress(nvl(hd_provid, hp_provid), nvl(hd_distid, hp_distid), nvl(hd_villid, hp_villid))) AS address2,") \
		_T(" 	substr(HD_CARDNO, 1, 15) as inscard2") \
		_T(" FROM hms_patient") \
		_T(" LEFT JOIN hms_doc ON(hd_patientno=hp_patientno)") \
	...
	if(!rs.IsEOF())
	{
		...
		rs.GetValue(_T("yob2"), m_szYOB2);
		rs.GetValue(_T("sex2"), m_szSex2);
		rs.GetValue(_T("address2"), m_szAddress2);
		rs.GetValue(_T("inscard2"), m_szInsCard2);
...
bool CHMSPatientNoChangeDialog::IsTreatmentTerminate(int nDocNoNew, int nDocNoOld){
	...
	bool docnew=false, docold=false, outdoc=false;
	int ret=0;
...
void CHMSPatientNoChangeDialog::OnOKSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	UpdateData(true);
    CString szPatientName, szPatientName2;
	if(m_nNewDocumentNo <= 0)
		return;

	if(m_nOldDocumentNo <= 0)
		return;
	//Nếu vô tình gõ 2 số hồ sơ giống nhau -> Return
	if (m_nNewDocumentNo == m_nOldDocumentNo)
	{
		ShowMessageBox(_T("Số hồ sơ giống nhau, không cần đổi!"), MB_OK);
		return;
	}
	//Nếu họ tên BN khác nhau -> Return
    StringLower(m_szPatientName, szPatientName);
    StringLower(m_szPatientName2, szPatientName2);
	if(szPatientName != szPatientName2)
	{
		ShowMessageBox(_T("Tên bệnh nhân khác nhau không được phép đổi"), MB_OK);
		return;
	}
	//Nếu ngày tháng năm sinh khác nhau -> Return
	if(m_szYOB != m_szYOB2)
	{
		ShowMessageBox(_T("Ngày tháng năm sinh của BN khác nhau không được phép đổi"), MB_OK);
		return;
	}
	//Nếu địa chỉ khác nhau -> Return
	// Comment đoạn này vào vì đối tượng C12 (A Son)

	/*
	if(m_szAddress != m_szAddress2)
	{
		ShowMessageBox(_T("Địa chỉ của BN khác nhau không được phép đổi"), MB_OK);
		return;
	}
	//Nếu bệnh nhân có thẻ BH và 2 thẻ khác nhau -> Return
	if	(
		(m_szInsCard != m_szInsCard2)
		&& 
		(!m_szInsCard.IsEmpty())
		&& 
		(!m_szInsCard2.IsEmpty())
		)
	{
		ShowMessageBox(_T("Số thẻ của BN khác nhau không được phép đổi"), MB_OK);
		return;
	}
	*/
	...
	//Nếu hồ sơ đã vào viện thì -> Return
	if (IsTreatmentTerminate(nNewDoc, nOldDoc)) 
		return ;
	
	if(ShowMessageBox(_T("Bạn chắc chắn ghép mã không? Hãy kiểm tra để đảm bảo thông tin bệnh nhân: Họ và tên, tuổi, giới, địa chỉ được chính xác! Hoặc nhờ ban CNTT hỗ trợ (Yes/No)") , MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO)
 		return; 

	CString szSQL;
	szSQL.Format(_T("HMS_MERGEPATIENT(%ld,%ld) "), m_nPatientNo1, m_nPatientNo2, pMF->GetCurrentUser());	
	int ret = ToInt(pMF->ExecDML(szSQL));
	//_fmsg(_T("%s"), szSQL);
	if(ret > 0)
	{
		AfxMessageBox(_T("Đã ghép mã bệnh nhân thành công, hãy check lại") , MB_OK|MB_ICONINFORMATION);
		CGuiDialog::OnOK();
	}
	else
	{
		ShowMessageBox(_T("Không thể ghép mã bệnh nhân!") , MB_OK|MB_ICONERROR);
	}
