...
void CHMSTreatmentPlanDlg::OnCreateComponents(){
	m_wndTreatmentPlan.Create(this, _T("Treatment Plan"), 5, 5, 800, 570);
	m_wndPatientNoLabel.Create(this, _T("Patient No"), 10, 30, 110, 55);
	m_wndPatientNo.Create(this,115, 30, 195, 55); 
	m_wndPatientNameLabel.Create(this, _T("Patient Name"), 200, 30, 280, 55);
	m_wndPatientName.Create(this,285, 30, 435, 55); 
	m_wndDocIDLabel.Create(this, _T("Doc ID"), 440, 30, 540, 55);
	m_wndDocID.Create(this,545, 30, 625, 55); 
	m_wndDetectTimeLabel.Create(this, _T("TG ph\xE1t hi\x1EC7n"), 630, 30, 710, 55);
	m_wndDetectTime.Create(this,715, 30, 795, 55); 
	m_wndSymptomLabel.Create(this, _T("Tri\x1EC7u \x63h\x1EE9ng"), 10, 60, 110, 85);
	m_wndSymptom.Create(this,115, 60, 435, 85); 
	m_wndSurgeryLabel.Create(this, _T("Surgery"), 440, 60, 540, 85);
	m_wndSurgery.Create(this,546, 60, 796, 85); 
	m_wndAdditionSurgeryLabel.Create(this, _T("PT \x62\x1ED5 sung"), 10, 90, 110, 115);
	m_wndAdditionSurgery.Create(this,115, 90, 435, 115); 
	m_wndAfterSurgeryLabel.Create(this, _T("S\x61u ph\x1EABu thu\x1EADt"), 440, 90, 540, 115);
	m_wndAfterSurgery.Create(this,546, 90, 796, 115); 
	m_wndGPBLLabel.Create(this, _T("GPBL"), 10, 120, 110, 145);
	m_wndGPBL.Create(this,115, 120, 435, 145); 
	m_wndRelapseRiskLabel.Create(this, _T("Nguy \x63\x1A1 t\xE1i ph\xE1t"), 440, 120, 540, 145);
	m_wndRelapseRisk.Create(this,545, 120, 795, 145); 
	
	m_wndDiseaseCombineLabel.Create(this, _T("\x42\x1EC7nh k\x1EBFt h\x1EE3p"), 10, 150, 110, 175);
	m_wndDiseaseCombine.Create(this,115, 150, 795, 175); 
	m_wndUsingDrugLabel.Create(this, _T("Thu\x1ED1\x63 \x111\x61ng \x64\xF9ng"), 10, 180, 110, 205);
	m_wndUsingDrug.Create(this,115, 180, 795, 205); 
	m_wndFamilyLabel.Create(this, _T("Gi\x61 \x111\xECnh"), 10, 210, 110, 235);
	m_wndFamily.Create(this,115, 210, 195, 235); 
	m_wndCQKhacLabel.Create(this, _T("CQ Khac"), 200, 210, 280, 235);
	m_wndCQKhac.Create(this,285, 210, 435, 235); 
	m_wndNeckGanglionLabel.Create(this, _T("H\x1EA1\x63h \x63\x1ED5"), 440, 210, 540, 235);
	m_wndNeckGanglion.Create(this,545, 210, 795, 235); 
	m_wndTSHLabel.Create(this, _T("TSH"), 10, 240, 110, 265);
	m_wndTSH.Create(this,115, 240, 195, 265); 
	m_wndCanxiLabel.Create(this, _T("Canxi"), 200, 240, 280, 265);
	m_wndCanxi.Create(this,285, 240, 435, 265); 
	m_wndKaliLabel.Create(this, _T("Kali"), 440, 240, 540, 265);
	m_wndKali.Create(this,545, 240, 625, 265); 
	m_wndTestThaiLabel.Create(this, _T("Test Thai"), 630, 240, 710, 265);
	m_wndTestThai.Create(this,715, 240, 795, 265); 
	m_wndTgATgChangeLabel.Create(this, _T("\x42i\x1EBFn \x111\x1ED5i Tg-\x41Tg"), 10, 270, 110, 295);
	m_wndTgATgChange.Create(this,115, 270, 435, 295); 
	m_wndSideEffectLabel.Create(this, _T("T/\x64 ph\x1EE5 I\x31\x33\x31"), 440, 270, 540, 295);
	m_wndSideEffect.Create(this,545, 270, 795, 295); 
	m_wndTgLabel.Create(this, _T("Tg"), 10, 300, 110, 325);
	m_wndTg.Create(this,115, 300, 195, 325); 
	m_wndATg.Create(this,285, 300, 435, 325); 
	m_wndATgLabel.Create(this, _T("A-Tg"), 200, 300, 280, 325);
	m_wndDose.Create(this,545, 300, 625, 325); 
	m_wndDoseLabel.Create(this, _T("Dose"), 440, 300, 540, 325);
	m_wndTimeLabel.Create(this, _T("Time"), 630, 300, 710, 325);
	m_wndTime.Create(this,715, 300, 795, 325); 
	m_wndOBloodChemistryLabel.Create(this, _T("SH m\xE1u kh\xE1\x63"), 10, 330, 110, 355);
	m_wndOBloodChemistry.Create(this,115, 330, 195, 355); 
	m_wndBloodWeirdo.Create(this,200, 330, 435, 355); 
	m_wndBloodFormulaLabel.Create(this, _T("\x43\xF4ng th\x1EE9\x63 m\xE1u"), 440, 330, 540, 355);
	m_wndBloodFormula.Create(this,545, 330, 795, 355); 
	m_wndUltraSonicNeckLabel.Create(this, _T("Si\xEAu \xE2m \x63\x1ED5"), 10, 360, 110, 385);
	m_wndUltraSonicNeck.Create(this,115, 360, 435, 385); 
	m_wndUltraSonicAbdomenLabel.Create(this, _T("Si\xEAu \xE2m \x1ED5 \x62\x1EE5ng"), 440, 360, 540, 385);
	m_wndUltraSonicAbdomen.Create(this,545, 360, 795, 385); 
	m_wndImagingAfterTreatLabel.Create(this, _T("\x58H s\x61u \x110T/\x43\x110"), 10, 390, 175, 415);
	m_wndIATSpotLabel.Create(this, _T("t\x1EA1i \x63h\x1ED7"), 200, 390, 280, 415);
	m_wndIATSpot.Create(this,285, 390, 435, 415); 
	m_wndIATAllLabel.Create(this, _T("to\xE0n th\xE2n"), 440, 390, 540, 415);
	m_wndIATAll.Create(this,545, 390, 795, 415); 
	m_wndImaging2MCILabel.Create(this, _T("\x58H\x43\x110 \x32m\x43i"), 10, 420, 175, 445);
	m_wndI2MSpotLabel.Create(this, _T("t\x1EA1i \x63h\x1ED7"), 200, 420, 280, 445);
	m_wndI2MSpot.Create(this,285, 420, 435, 445); 
	m_wndI2MAllLabel.Create(this, _T("to\xE0n th\xE2n"), 440, 420, 540, 445);
	m_wndI2MAll.Create(this,545, 420, 795, 445); 
	m_wndBISPECTLabel.Create(this, _T("SPECT"), 200, 450, 280, 475);
	m_wndBISPECT.Create(this,285, 450, 435, 475); 
	m_wndBoneImagingLabel.Create(this, _T("\x58\x1EA1 h\xECnh \x78\x1B0\x1A1ng"), 10, 450, 175, 475);
	m_wndBICTLabel.Create(this, _T("CT"), 440, 450, 540, 475);
	m_wndBICT.Create(this,545, 450, 795, 475); 
	m_wndDiagnoseLabel.Create(this, _T("\x43h\x1EA9n \x111o\xE1n"), 10, 480, 110, 505);
	m_wndDiagnose.Create(this,115, 480, 520, 505); 
	m_wndStageLabel.Create(this, _T("Gi\x61i \x111o\x1EA1n"), 545, 480, 625, 505);
	m_wndStage.Create(this,630, 480, 795, 505); 
	m_wndAttackDiseaseLabel.Create(this, _T("\x42\x1EC7nh k\xE8m th\x65o"), 10, 510, 110, 535);
	m_wndAttackDisease.Create(this,115, 510, 795, 535); 
	m_wndExpectedTherapeuticDoseLabel.Create(this, _T("\x44\x1EF1 ki\x1EBFn li\x1EC1u \x111i\x1EC1u tr\x1ECB"), 10, 540, 110, 565);
	m_wndExpectedTherapeuticDose.Create(this,115, 540, 435, 565); 
	m_wndTreatmentReasonLabel.Create(this, _T("L\xFD \x64o \x111i\x1EC1u tr\x1ECB"), 440, 540, 540, 565);
	m_wndTreatmentReason.Create(this,545, 540, 795, 565); 
	...
	m_wndUpdate.Create(this, _T("Update"), 465, 575, 545, 600);
	m_wndPrint.Create(this, _T("Print"), 380, 575, 460, 600);
}
void CHMSTreatmentPlanDlg::OnInitializeComponents(){
	...
	m_tblTreatmentPlan.AddField(_T("htp_patientno"), dfText, 20);
	...
	m_tblTGATG.AddField(_T("htc_patientno"), dfText, 15);
...
void CHMSTreatmentPlanDlg::OnSetWindowEvents(){
	...
	{
		CString szSQL;
		m_szPatientNo.Format(_T("%ld"), pMF->m_nPNo);
		szSQL.Format(_T("GET_PATIENTNAME_BYPNO('%s')"),m_szPatientNo);
...
void CHMSTreatmentPlanDlg::GetScreenToData(){
	...
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T(" SELECT coalesce(max(htp_idx), 0)+1 as nIdx FROM HMS_TREATMENT_PLAN WHERE htp_patientno = %ld "), pMF->m_nPNo);
	rs.ExecSQL(szSQL);
	...
	m_tblTreatmentPlan.SetValue(_T("htp_patientno"), pMF->m_nPNo);
	...
	m_tblTGATG.SetValue(_T("htc_patientno"), pMF->m_nPNo);
...
void CHMSTreatmentPlanDlg::OnPrintSelect(){
	...
	szSQL.Format(_T("select htp_docid, GET_PATIENTNAME_BYPNO(htp_patientno) as pname, hp_birthdate as age, htp_patientno,") \
		_T(" HMS_GETSEX(hp_sex) as sex, hta_phone, htp_detect_time as ngayphathien, htp_symptom as trieuchung, htp_surgery as phauthuat, ") \
		...
		_T(" htp_testthai as thai, htp_tgatg_change, htp_tg as tg, htp_atg as atg, htp_dose as dose, htp_time, htp_other_blood as shmaukhac, ") \
		...
		_T(" where htp_patientno = %ld and htp_idx = (select max(htp_idx) from hms_treatment_plan where htp_patientno = %ld)"), pMF->m_nPNo, pMF->m_nPNo);
	_fmsg(_T("%s"), szSQL);
	...
	szSQL2.Format(_T("select type,L1, L2, L3, L4, L5, L6, L7, L8, LS9, L10 ") \
					_T(" FROM( ") \
					_T("	SELECT * FROM ( ") \
					_T("	SELECT 'TG' as type, htc_idx, htc_tg FROM hms_tgatg_change WHERE htc_patientno = '%s') ") \
					_T("	PIVOT ( ") \
					_T("	sum(htc_tg) ") \
					...
					_T(" UNION ALL ") \
					_T("	SELECT * FROM ( ") \
					_T("	SELECT 'A-Tg' as type, htc_idx, htc_atg FROM hms_tgatg_change WHERE htc_patientno = '%s') ") \
					_T("	PIVOT ( ") \
					_T("	sum(htc_atg) ") \
					_T("	FOR htc_idx IN (1 as L1, 2 as L2, 3 as L3, 4 as L4, 5 as L5, 6 as L6, 7 as L7, 8 as L8, 9 as LS9, 10 as L10) ") \
					_T("	) ") \
					_T(" UNION ALL ") \
					_T("	SELECT * FROM ( ") \
					_T("	SELECT 'Dose' as type, htc_idx, htc_dose FROM hms_tgatg_change WHERE htc_patientno = '%s') ") \
					_T("	PIVOT ( ") \
					_T("	sum(htc_dose) ") \
					_T("	FOR htc_idx IN (1 as L1, 2 as L2, 3 as L3, 4 as L4, 5 as L5, 6 as L6, 7 as L7, 8 as L8, 9 as LS9, 10 as L10) ") \
					_T("	) ") \
					_T(" UNION ALL ") \
					_T("	SELECT * FROM ( ") \
					_T("	SELECT 'Time' as type, htc_idx, htc_time FROM hms_tgatg_change WHERE htc_patientno = '%s') ") \
					_T("	PIVOT ( ") \
					_T("	sum(htc_time) ") \
					_T("	FOR htc_idx IN (1 as L1, 2 as L2, 3 as L3, 4 as L4, 5 as L5, 6 as L6, 7 as L7, 8 as L8, 9 as LS9, 10 as L10) ") \
					_T("	) )") , szTmpPatientNo, szTmpPatientNo, szTmpPatientNo, szTmpPatientNo);
	rs2.ExecSQL(szSQL2);
	...
	szSQL3.Format(_T("select GET_OBJECTNAME(hd_object) as obj, GET_TRANSFERHOSPITALNAME(hd_transplaceid) as transplaceid, hd_admitdate, ") \
				...
				_T(" where hp_patientno = '%s' and hd_admitdate = (select max(hd_admitdate) from hms_doc where hd_patientno = '%s') ") \
				_T(" and he_updateddate = (select max(he_updateddate) from hms_exam where he_patientno = '%s') ") \
				_T(" and hcr_admitdate = (select max(hcr_admitdate) from hms_clinical_record where hcr_patientno = '%s')"), m_szPatientNo, m_szPatientNo, m_szPatientNo, m_szPatientNo);
	...
	rpt.GetReportHeader()->SetValue(_T("Phone"), rs.GetValue(_T("hta_phone")));
	//noigui
	...
	rpt.GetReportHeader()->SetValue(_T("benhkethop"), rs.GetValue(_T("htp_disease_combine")));
	...
	rpt.GetReportHeader()->SetValue(_T("tdphu131"), rs.GetValue(_T("tdphu131")));
	...
	rpt.GetReportFooter()->SetValue(_T("dtcpall"), rs.GetValue(_T("htp_ita_all")));
...
int CHMSTreatmentPlanDlg::OnSaveHMSTreatmentPlanDlg(){
	...
	//_msg(_T("m_szPatientNo %s,m_szPatientName %s,m_szDocID %s,m_szDetectTime %s,m_szSymptom %s,m_szSurgery %s,m_szAdditionSurgery %s,m_szAfterSurgery %s,m_szGPBL %s,m_szDiseaseCombine %s,m_szUsingDrug %s,m_szFamilyKey %s,m_szCQKhac %s,m_szTSH %s,m_szCanxi %s,m_szKali %s,m_szTestThai %s,m_szTgATgChange %s,m_szSideEffect %s,m_szTg %s,m_szATg %s,m_szDose %s,m_szTime %s,m_szOBloodChemistryKey %s,m_szBloodWeirdo %s,m_szBloodFormulaKey %s,m_szUltraSonicNeck %s,m_szUltraSonicAbdomen %s,m_szIATSpot %s,m_szIATAll %s,m_szI2MSpot %s,m_szI2MAll %s,m_szBISPECT %s,m_szBICT %s,m_szDiagnose %s,	m_szStage %s,m_szAttackDisease %s,m_szExpectedTherapeuticDose %s,m_szTreatmentReason %s,m_nIDx %s,	m_szRelapseRisk %s,m_szNeckGanglion %s"),m_szPatientNo,m_szPatientName,m_szDocID,m_szDetectTime,m_szSymptom,m_szSurgery,m_szAdditionSurgery,m_szAfterSurgery,m_szGPBL,m_szDiseaseCombine,m_szUsingDrug,m_szFamilyKey,m_szCQKhac,m_szTSH,m_szCanxi,m_szKali,m_szTestThai,m_szTgATgChange,m_szSideEffect,m_szTg,m_szATg,m_szDose,m_szTime,m_szOBloodChemistryKey,m_szBloodWeirdo,m_szBloodFormulaKey,m_szUltraSonicNeck,m_szUltraSonicAbdomen,m_szIATSpot,m_szIATAll,m_szI2MSpot,m_szI2MAll,m_szBISPECT,m_szBICT,m_szDiagnose,	m_szStage,m_szAttackDisease,m_szExpectedTherapeuticDose,m_szTreatmentReason,m_nIDx,	m_szRelapseRisk,m_szNeckGanglion);
 	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
 	...
 	if(GetMode() == VM_ADD){
		...
		szWhere.Format(_T(" WHERE htp_patientno = %ld AND htp_idx = %ld"), pMF->m_nPNo, m_nIDx);
 		szSQL = m_tblTreatmentPlan.GetUpdateSQL(_T("htp_idx,htp_patientno"));
 		szSQL += szWhere;

		szWhere2.Format(_T(" WHERE htc_idx = %ld AND htc_patientno = %ld"), m_nIDx, pMF->m_nPNo);
