...
void CHMSSurgerySchedule::OnCreateComponents()
{
	...
	m_wndSurgeryScheduleInfo.Create(this, _T("Duty Schedule Information"), 5, 160, 1010, 615);
	...
	m_wndCalendar.Create(NULL, NULL, WS_CHILD | WS_VISIBLE, CRect(10, 180, 1005, 615), this, IDC_CALCTRL);
...
void CHMSSurgerySchedule::OnInitializeComponents()
{
	...
	m_wndTodayScheduleList.InsertColumn(0, _T("Doctor"), CFMT_TEXT, 100);
	m_wndTodayScheduleList.InsertColumn(1, _T("Doctor1"), CFMT_TEXT, 100);
	m_wndTodayScheduleList.InsertColumn(2, _T("Nurses1"), CFMT_TEXT, 95);
	m_wndTodayScheduleList.InsertColumn(3, _T("Nurses2"), CFMT_TEXT, 95);
	m_wndTodayScheduleList.InsertColumn(4, _T("Nurses3"), CFMT_TEXT, 95);
	m_wndTodayScheduleList.InsertColumn(5, _T("Nurses4"), CFMT_TEXT, 95);
	m_wndTodayScheduleList.InsertColumn(6, _T("Nurses5"), CFMT_TEXT, 95);
	m_wndTodayScheduleList.InsertColumn(7, _T("Nurses6"), CFMT_TEXT, 95);
...
long CHMSSurgerySchedule::OnTodayScheduleListLoadData()
{
	...
	szSQL.Format(_T(" SELECT (SELECT su_name FROM sys_user WHERE su_userid=hssl_doctor) AS Doctor,") \
				...
				_T(" AND rownum=1") \
				_T(" ORDER BY hss_starttime, hss_endtime DESC"), pMF->GetSysDate());
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF())
	{ 
		m_wndTodayScheduleList.AddItems(
			rs.GetValue(_T("Doctor")), 
			rs.GetValue(_T("Doctor1")), 
			rs.GetValue(_T("Nurse1")), 
			rs.GetValue(_T("Nurse2")), 
			rs.GetValue(_T("Nurse3")), 
			rs.GetValue(_T("Nurse4")), 
			rs.GetValue(_T("Nurse5")),
			rs.GetValue(_T("Nurse6")), NULL);
		rs.MoveNext();
	}
...
void CHMSSurgerySchedule::OnPrintSelect()
{
	...
	tmpStr.Format(_T("In Lịch trực"));
	menu.AppendMenu(MF_BYPOSITION, 1, tmpStr);
	menu.AppendMenu(MF_SEPARATOR);
	tmpStr.Format(_T("In báo cáo giao ban"));
...
void CHMSSurgerySchedule::OnLoadData()
{
	...
	szSQL.Format(_T("SELECT hssl_refidx AS ID, ")\
				 ...
				 _T("WHERE trunc_date(hss_starttime) BETWEEN to_date('%s', 'YYYY-MM-DD') AND to_date('%s', 'YYYY-MM-DD')"),
				 szFromDate, szToDate);
	rs.ExecSQL(szSQL);
...
{
	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
	CHMSSurgeryScheduleInfoDialog dlg(this);
	COleDateTime dtDate;
	CString szDate;
	BOOL bAddMode = FALSE;
	...
	if (pData->pItem == NULL)
	{
		...
		szSQL.Format(_T("SELECT Count(*) FROM hms_surgery_schedule ")\
					 _T("WHERE trunc_date(hss_starttime)=to_date('%s', 'YYYY-MM-DD')"), szTemp);
		rs.ExecSQL(szSQL);
		if (rs.GetIntValue() > 0)
			return;
...
void CHMSSurgerySchedule::OnDeleteItem()
{
	if (m_nOrderID > 0)
	{
		...
		CRecord rs(&pMF->m_db);
		CString szSQL;
		szSQL.Format(_T("SELECT hss_status AS Status FROM hms_surgery_schedule WHERE hss_idx=%ld"), m_nOrderID);
		rs.ExecSQL(szSQL);
		...
		{
			szSQL.Format(_T("DELETE FROM hms_surgery_schedule_line WHERE hssl_refidx=%ld"), m_nOrderID);
			int ret = pMF->ExecSQL(szSQL);
			if (ret >= 0)
			{
				szSQL.Format(_T("DELETE FROM hms_surgery_schedule WHERE hss_idx=%ld"), m_nOrderID);
				pMF->ExecSQL(szSQL);
...
CString CHMSSurgerySchedule::GetSubject(long nIdx)
{
	...
	if (!rs.IsEOF())
	{
		rs.GetValue(_T("Doctor"), szTemp);
		if (!rs.GetValue(_T("Doctor1")).Trim().IsEmpty())
			szTemp += _T(", ");
		szTemp.AppendFormat(_T("%s"), rs.GetValue(_T("Doctor1")));
		szDoctors += szTemp;

		rs.GetValue(_T("Nurse1"), szTemp);
		if (!rs.GetValue(_T("Nurse2")).Trim().IsEmpty())
			szTemp += _T(", ");
		szTemp.AppendFormat(_T("%s"), rs.GetValue(_T("Nurse2")));
		szNurses += szTemp;

		rs.GetValue(_T("Nurse3"), szTemp);
		if (!rs.GetValue(_T("Nurse4")).Trim().IsEmpty())
			szTemp += _T(", ");
		szTemp.AppendFormat(_T("%s"), rs.GetValue(_T("Nurse4")));
		if (!rs.GetValue(_T("Nurse5")).Trim().IsEmpty())
			szTemp += _T(", ");
		szTemp.AppendFormat(_T("%s"), rs.GetValue(_T("Nurse5")));
		if (!rs.GetValue(_T("Nurse6")).Trim().IsEmpty())
			szTemp += _T(", ");
		szTemp.AppendFormat(_T("%s"), rs.GetValue(_T("Nurse6")));
		szNurses1 += szTemp;
	}
...
CString CHMSSurgerySchedule::GetColumnName(CGuiListCtrlEx *list, int nCol)
{
	CString szName;
	int nTotalCol;
	szName.Empty();
...
void CHMSSurgerySchedule::PrintDutySchedule()
{
	...
	{
		szWhere2.AppendFormat(_T(" or ho_pdeptid IN (%s) "), pMF->m_UserInfo.su_hms_xdepts);
	}
	...
	szSQL.Format(_T(" SELECT ") \
				...
				_T(" WHERE (ho_pdeptid ='%s' %s) AND NVL(hfl_categoryid, 0) NOT IN  ('500', '501') ") \
				...
				_T(" ORDER BY ho_type, ho_performdate")
				, pMF->m_szDepartment, szWhere2, szFromDate, szToDate, 
				szToDate.Left(10), szFromDate.Left(10));
...
//_fmsg(_T("%s"), szSQL);
	...
	szSQL.Format(_T("SELECT sum(case when ho_normal = '1' then 1 else 0 end) as L1,") \
				_T("		sum(case when ho_normal = '2' then 1 else 0 end) as L2,") \
				_T("		sum(case when ho_normal = '3' then 1 else 0 end) as L3,") \
				_T("		sum(case when ho_normal = '4' then 1 else 0 end) as L4,") \
				_T("		sum(case when ho_normal = 'S' then 1 else 0 end) as L5") \
				_T(" FROM hms_patient") \
				_T(" LEFT JOIN hms_doc ON(hd_patientno=hp_patientno)") \
				_T(" LEFT JOIN hms_clinical_record ON(hcr_docno=hd_docno)") \
				_T(" LEFT JOIN hms_operation ON(ho_docno=hcr_docno and ho_patientno=hp_patientno)") \
				_T(" LEFT JOIN hms_fee_list ON(hfl_feeid=ho_itemid)") \
				_T(" LEFT JOIN hms_fee_group ON(hfg_id=hfl_groupid)") \
				_T(" WHERE ho_pdeptid='%s' AND NVL(hfl_categoryid, 0) NOT IN  ('500', '501') ") \
				_T(" AND ho_status = 'T' AND ((ho_performdate BETWEEN to_date('%s', 'YYYY-MM-DD HH24:MI:SS') AND to_date('%s', 'YYYY-MM-DD HH24:MI:SS') ")\
				_T(" AND ho_performdate <> to_date('%s 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) ") \
				_T(" OR ho_performdate = to_date('%s 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) ") \
				_T(" ORDER BY ho_type, ho_performdate")
				, pMF->m_szDepartment, szFromDate, szToDate, szToDate.Left(10), szFromDate.Left(10));
	rs.ExecSQL(szSQL);
	for (int i = 1; i <= 5; i++)
	{
		tmpStr.Format(_T("L%d"), i);
		rpt.GetReportFooter()->SetValue(tmpStr, rs.GetValue(tmpStr));
	}
	EndWaitCursor();
	rpt.PrintPreview();

}
void CHMSSurgerySchedule::PrintDutySchedule_GB()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szSQL;
	CString szFromDate, szToDate;
	...
	static CReport rpt;
	CString tmpStr, tmpStr1, szWhere2;
	if(!rpt.Init(_T("Reports/HMS/PTTT/DUTIESREPORT_GB.RPT")) )
	{
		return;
	}
	...
	szSQL.Format(_T(" SELECT ") \
				...
				_T("        ho_type as category, ") \
				_T("        (SELECT ss_desc as name FROM sys_sel WHERE ss_code=ho_type AND ss_id='hms_operation_type') as categeryname,") \
				_T("        (SELECT ss_desc as name FROM sys_sel WHERE ss_code=ho_emergencytype AND ss_id='hms_operation_type_line') as emergency_type,ho_emergencytype,") \
				_T("        ho_subcategory as subcategory,ho_type,ho_deptid as pdeptid,ho_diagnostic as diagnostic,ho_performdate ") \
				_T(" FROM hms_patient") \
				_T(" LEFT JOIN hms_doc ON(hd_patientno=hp_patientno)") \
				_T(" LEFT JOIN hms_clinical_record ON(hcr_docno=hd_docno)") \
				_T(" LEFT JOIN hms_operation ON(ho_docno=hcr_docno and ho_patientno=hp_patientno)") \
				_T(" LEFT JOIN hms_fee_list ON(hfl_feeid=ho_itemid)") \
				_T(" LEFT JOIN hms_fee_group ON(hfg_id=hfl_groupid)") \
				_T(" WHERE WHERE (ho_pdeptid ='%s' %s) AND NVL(hfl_categoryid, 0) <> '500' and nvl(hfl_tyc,'N') <> 'TYC'  ") \
				_T(" AND ho_status = 'T' and ho_type=2 AND ho_performdate BETWEEN to_date('%s', 'YYYY-MM-DD HH24:MI:SS') AND to_date('%s', 'YYYY-MM-DD HH24:MI:SS') ")\
				_T(" ORDER BY ho_type,ho_emergencytype, ho_performdate")
				, pMF->m_szDepartment, szWhere2, szFromDate, szToDate);
...
_fmsg(_T("%s"), szSQL);
	...
	CString szEmergencyNew,szEmergencyOld;
	CString szType;
	szEmergencyNew.Empty();
	szEmergencyOld.Empty();
	while(!rs.IsEOF())
	{
		rs.GetValue(_T("categeryname"), szNewLine);
		rs.GetValue(_T("emergency_type"), szEmergencyNew);
		rs.GetValue(_T("ho_type"),szType );
		...
		/*if(szNewLine != szOldLine && !szNewLine.IsEmpty())
		{
			...
			if(szEmergencyNew.IsEmpty() || szEmergencyNew==_T("") )
				{
					szEmergencyNew= _T("Chưa phân loại");
				}
			if( szEmergencyNew != szEmergencyOld || szEmergencyOld.IsEmpty() )
			{
				
				rptDetail = rpt.AddDetail(rpt.GetGroupHeader(1));
				rptDetail->SetValue(_T("GroupName"), szEmergencyNew);
				nIndex=1;
				
			}
			szEmergencyOld = szEmergencyNew;
		...
		rs.GetValue(_T("ho_performdate"), tmpStr);

		rptDetail->SetValue(_T("14"), CDateTime::Convert(tmpStr, yyyymmdd | hhmm, ddmmyyyy | hhmm));
		...
		rs.GetValue(_T("ho_emergencytype"),m);
			if(m >= 0 && m < 4)
		{
			nTotalEmergency[m] ++;
		}	
	...
	for (int i = 0; i < 4; i++)
	{
		tmpStr.Format(_T("E%d"), i);
		rpt.GetReportFooter()->SetValue(tmpStr, int2str(nTotalEmergency[i]));
	}
	...
	szSQL.Format(_T("SELECT sum(case when ho_normal = '1' then 1 else 0 end) as L1,") \
				...
				_T(" LEFT JOIN hms_fee_list ON(hfl_feeid=ho_itemid)") \
				_T(" LEFT JOIN hms_fee_group ON(hfg_id=hfl_groupid)") \
				_T(" WHERE ho_pdeptid='%s' AND NVL(hfl_categoryid, 0) <> '500' and nvl(hfl_tyc,'N') <> 'TYC'  ") \
				_T(" AND ho_status = 'T' and ho_type=2 AND ho_performdate BETWEEN to_date('%s', 'YYYY-MM-DD HH24:MI:SS') AND to_date('%s', 'YYYY-MM-DD HH24:MI:SS') ")\
				_T(" ORDER BY ho_type,ho_emergencytype, ho_performdate")
				, pMF->m_szDepartment, szFromDate, szToDate);
	rs.ExecSQL(szSQL);
	for (int i = 1; i <= 5; i++)
	{
		tmpStr.Format(_T("L%d"), i);
