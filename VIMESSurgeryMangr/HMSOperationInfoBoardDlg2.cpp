...
long CHMSOperationInfoBoardDlg::OnRoomLoadData(){
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT hrl_id as id, hrl_name as name FROM hms_roomlist WHERE hrl_deptid='B5'  %s ORDER BY hrl_id "), szWhere);
...
long CHMSOperationInfoBoardDlg::OnListLoadData(){
	...
	szSQL.Format(_T("SELECT hob_docno, get_patientname(hob_docno) as pname, hob_roomid, hob_deptid, hob_rettime, hob_retdept, ") \
		...
		_T(" hob_operation_table FROM HMS_OPERATION_BOARD WHERE to_date(hob_date) = to_date('%s', 'yyyy/mm/dd')") \
		_T(" and hob_deptid not in ('PTTYC', 'TYC')"), pMF->GetSysDate());
	_fmsg(_T("%s"), szSQL);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndList.AddItems(
