...
static void _OnObjectSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CrptBaocaosudungthuochuongthan* )pWnd)->OnObjectSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnObjectSelendokFnc(CWnd *pWnd){
	((CrptBaocaosudungthuochuongthan *)pWnd)->OnObjectSelendok();
}
/*static void _OnObjectSetfocusFnc(CWnd *pWnd){
	((CrptBaocaosudungthuochuongthan *)pWnd)->OnObjectKillfocus();
}*/
/*static void _OnObjectKillfocusFnc(CWnd *pWnd){
	((CrptBaocaosudungthuochuongthan *)pWnd)->OnObjectKillfocus();
}*/
static long _OnObjectLoadDataFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan *)pWnd)->OnObjectLoadData();
}
/*static void _OnObjectAddNewFnc(CWnd *pWnd){
	((CrptBaocaosudungthuochuongthan *)pWnd)->OnObjectAddNew();
}*/
...
static long _OnStockListLoadDataFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnStockListLoadData();
} 
static void _OnStockListDblClickFnc(CWnd *pWnd){
	((CrptBaocaosudungthuochuongthan*)pWnd)->OnStockListDblClick();
} 
static void _OnStockListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CrptBaocaosudungthuochuongthan*)pWnd)->OnStockListSelectChange(nOldItem, nNewItem);
} 
static int _OnStockListDeleteItemFnc(CWnd *pWnd){
	 return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnStockListDeleteItem();
} 
static long _OnObjectListLoadDataFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnObjectListLoadData();
} 
static void _OnObjectListDblClickFnc(CWnd *pWnd){
	((CrptBaocaosudungthuochuongthan*)pWnd)->OnObjectListDblClick();
} 
static void _OnObjectListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CrptBaocaosudungthuochuongthan*)pWnd)->OnObjectListSelectChange(nOldItem, nNewItem);
} 
static int _OnObjectListDeleteItemFnc(CWnd *pWnd){
	 return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnObjectListDeleteItem();
} 
...
static int _OnListReloadDataFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnListReloadData();
}

static int _OnStockListCheckAllFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnStockListCheckAll();
} 

static int _OnStockListUnCheckAllFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnStockListUnCheckAll();
}

static int _OnObjectListCheckAllFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnObjectListCheckAll();
} 

static int _OnObjectListUnCheckAllFnc(CWnd *pWnd){
	return ((CrptBaocaosudungthuochuongthan*)pWnd)->OnObjectListUnCheckAll();
}
...
void CrptBaocaosudungthuochuongthan::OnCreateComponents()
{
	...
	//m_wndFromDateLabel.Create(this, _T("From Date"), 10, 85, 112, 110);
	//m_wndFromDate.Create(this,117, 85, 237, 110); 
	//m_wndToDateLabel.Create(this, _T("To Date"), 242, 85, 322, 110);
	//m_wndToDate.Create(this,327, 85, 447, 110); 
	//m_wndStockLabel.Create(this, _T("Stock"), 10, 115, 112, 140);
	//m_wndStock.Create(this,117, 115, 447, 140); 
	////m_wndPrintPreview.Create(this, _T("Print Pre&view"), 200, 485, 280, 510);
	//m_wndExport.Create(this, _T("&Export"), 285, 485, 365, 510);
	//m_wndClose.Create(this, _T("&Close"), 370, 485, 450, 510);
	//m_wndListFac.Create(this,10, 175, 445, 475); 
	//m_wndObject.SetCheckBox(true);
	//m_wndObjectLabel.Create(this, _T("Object"), 10, 145, 112, 170);
	//m_wndObject.Create(this,117, 145, 447, 170); 
	//m_wndTitle.Create(this, _T("\x44S \x62\x1EC7nh nh\xE2n s\x1EED \x64\x1EE5ng thu\x1ED1\x63 \x111\x1ED9\x63, h\x1B0\x1A1ng th\x1EA7n, g\xE2y nghi\x1EC7n"), 5, 5, 450, 55);
	...
	m_wndExport.Create(this, _T("&Export"), 245, 545, 345, 570);
...
void CrptBaocaosudungthuochuongthan::OnObjectSelectChange(int nOldItemSel, int nNewItemSel){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
void CrptBaocaosudungthuochuongthan::OnObjectSelendok(){
	 
}
/*void CrptBaocaosudungthuochuongthan::OnObjectSetfocus(){
	
}*/
/*void CrptBaocaosudungthuochuongthan::OnObjectKillfocus(){
	
}*/
long CrptBaocaosudungthuochuongthan::OnObjectLoadData()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
/*void CrptBaocaosudungthuochuongthan::OnObjectAddNew(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
...
void CrptBaocaosudungthuochuongthan::OnStockListDblClick(){
	
} 
void CrptBaocaosudungthuochuongthan::OnStockListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
int CrptBaocaosudungthuochuongthan::OnStockListDeleteItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	 return 0;
} 
long CrptBaocaosudungthuochuongthan::OnStockListLoadData()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
void CrptBaocaosudungthuochuongthan::OnObjectListDblClick(){
	
} 
void CrptBaocaosudungthuochuongthan::OnObjectListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
int CrptBaocaosudungthuochuongthan::OnObjectListDeleteItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	 return 0;
} 
long CrptBaocaosudungthuochuongthan::OnObjectListLoadData()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
long CrptBaocaosudungthuochuongthan::OnListFacLoadData(){
	...
	szSQL.Format(_T(" SELECT distinct mp_code as ID, mp_name as Name, adu_name as Unit") \
				...
				_T("       AND msl_storage_id in(%s) AND msl_isactive='Y'") \
				_T(" ORDER BY mp_name"), m_szGroups, pMF->m_szStorages);
...
void CrptBaocaosudungthuochuongthan::OnPrintBaocaodanhsachbenhnhansudungthuochuongthan(bool m_nPreview)
{
	...
	_T(" 	ho_cardno as cardNo, ho_deptid as DeptId,	") \
	_T(" %s")	\
	_T(" FROM hms_patient") \
	...
	_T(" GROUP BY ho_opera_table, ho_docno, hp_surname, hp_midname, hp_firstname, hd_admitdate, hp_birthdate, ho_cardno, ho_deptid, pmi_id") \
	_T(" ) TblTemp ") \
	...
	szSQL.Format(_T(" SELECT (select distinct hst_name from hms_surgery_table where hst_idx=tableOpera) as tableOpera,") \
				...
				_T("  LEFT JOIN m_product_item ON(mpi_product_item_id=hpol_product_item_id)") \
				_T("  LEFT JOIN m_product ON(mp_product_id=hpol_product_id)") \
				_T("  WHERE ho_pdeptid='%s' and ho_status in('A','I','D','T') and hpo_storage_id in(%s)") \
				_T("        and ho_performdate between to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') AND to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss')") \
				...
				_T(" ORDER BY tableOpera, docno"),
				szSumDept,
				szCaseDrugID, pMF->m_szDepartment, szStocks, m_szFromDate, m_szToDate, m_szGroups, szWhere,
				szCaseDrugID, pMF->m_szDepartment, szStocks, m_szFromDate, m_szToDate, m_szGroups, szWhere,
				szTotalSum);
...
void CrptBaocaosudungthuochuongthan::OnExportBaocaodanhsachbenhnhansudungthuochuongthan()
{
	...
	xls.SetCellText(0, 1, pMF->m_CompanyInfo.sc_pname, FMT_TEXT, true, 12);
	xls.SetCellText(0, 2, pMF->m_CompanyInfo.sc_name, FMT_TEXT, true, 12);
...
int CrptBaocaosudungthuochuongthan::OnListReloadData()
{
	OnListFacLoadData();
	return 0;
}

int CrptBaocaosudungthuochuongthan::OnStockListCheckAll()
{
	for (int i = 0; i < m_wndStockList.GetItemCount(); i++)
	{
		if (!m_wndStockList.GetCheck(i))
...
int CrptBaocaosudungthuochuongthan::OnStockListUnCheckAll()
{
	for (int i = 0; i < m_wndStockList.GetItemCount(); i++)
	{
		if (m_wndStockList.GetCheck(i))
...
int CrptBaocaosudungthuochuongthan::OnObjectListCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (!m_wndObjectList.GetCheck(i))
...
int CrptBaocaosudungthuochuongthan::OnObjectListUnCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (m_wndObjectList.GetCheck(i))
