...
void CHMSTreatmentAppointment::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT hta_docno, hta_docid, GET_PATIENTNAME(hta_docno) as pname,hta_shape, hta_metastasis, hta_tgatg, hta_sche_treatment, hta_sche_diagnose, hta_sche_i131, ") \
		...
		_T(" FROM HMS_TREATMENT_APPOINTMENT WHERE HTA_DOCNO = '%s' AND HTA_IDX = %ld"), m_szDocumentNo, m_nIndex);
	rs.ExecSQL(szSQL);
	if(!rs.IsEOF())
	{
		rs.GetValue(_T("hta_docno"), pMF->m_nDocNo);
...
void CHMSTreatmentAppointment::GetScreenToData(){
	...
	m_tbl_appointment.SetValue(_T("hta_docno"), pMF->m_nDocNo);
...
void CHMSTreatmentAppointment::OnAppointmentListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	m_szDocumentNo = m_wndAppointmentList.GetItemText(nNewItem, 2);
...
long CHMSTreatmentAppointment::OnAppointmentListLoadData(){
	...
	int nCount = 0;
	int nIdx = 1;
	szSQL.Format(_T("SELECT hta_idx, hta_docno, GET_PATIENTNAME(hta_docno) as pname, hta_appoint_date, hta_sche_treatment FROM HMS_TREATMENT_APPOINTMENT WHERE HTA_DOCNO = %ld"), pMF->m_nDocNo);
	_fmsg(_T("%s"), szSQL);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
...
int CHMSTreatmentAppointment::OnSaveHMSTreatmentAppointment(){
 	...
 	if(GetMode() == VM_ADD){
 		...
 		CString szWhere;
		szWhere.Format(_T(" WHERE hta_docno = %ld AND hta_idx = %ld"), pMF->m_nDocNo, m_nIndex);
 		szSQL = m_tbl_appointment.GetUpdateSQL(_T("hta_docno, hta_idx"));
 		szSQL += szWhere;
 	}
 _fmsg(_T("%s"), szSQL);
 	int ret = pMF->ExecSQL(szSQL);
...
void CHMSTreatmentAppointment::OnLoadBasicInfo()
{
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
	...
	szSQL.Format(_T("SELECT trim(hp_surname||' '||hp_midname||' '||hp_firstname) as pname, hd_telephone FROM hms_patient ") \
		...
		_T(" WHERE hd_docno = %ld "), pMF->m_nDocNo);
	...
	if(!rs.IsEOF())
	{
		...
		m_szDocumentNo.Format(_T("%ld"), pMF->m_nDocNo);
		UpdateData(FALSE);
	}
