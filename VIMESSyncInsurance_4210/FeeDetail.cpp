...
void CFeeDetail::OnCreateComponents()
{
	...
	m_wndcontrol_1.InsertColumn(0, _T("STT"), CFMT_TEXT, 80);
	m_wndcontrol_1.InsertColumn(1, _T("Item"), CFMT_TEXT, 100);
	m_wndcontrol_1.InsertColumn(2, _T("Fee Name"), CFMT_TEXT, 250);
	m_wndcontrol_1.InsertColumn(3, _T("Unit"), CFMT_TEXT, 50);
	m_wndcontrol_1.InsertColumn(4, _T("Content"), CFMT_TEXT, 0);
	m_wndcontrol_1.InsertColumn(5, _T("Qty"), CFMT_NUMBER, 60);
	m_wndcontrol_1.InsertColumn(6, _T("Price"), CFMT_TEXT, 150);
	m_wndcontrol_1.InsertColumn(7, _T("Tỷ lệ TT"), CFMT_NUMBER, 50);
	m_wndcontrol_1.InsertColumn(8, _T("Amout"), CFMT_NUMBER, 150);
	m_wndcontrol_1.InsertColumn(9, _T("DeptID"), CFMT_TEXT, 100);
	m_wndcontrol_1.InsertColumn(10, _T("ma nhom"), CFMT_TEXT, 0);
	m_wndcontrol_1.InsertColumn(11, _T("tt_thau"), CFMT_TEXT, 100);
	m_wndcontrol_1.InsertColumn(12, _T("goi_vtyt"), CFMT_TEXT, 100);
...
long CFeeDetail::Oncontrol_1LoadData(){
	...
	/*szSQL.Format(_T("select case when ma_nhom in ('10','11') then ma_vat_tu else ma_dich_vu end as ma_dich_vu, ") \
				_T("	case when ma_nhom in ('10','11') then ten_vat_tu else ten_dich_vu end as ten_dich_vu, ") \
				_T("	don_vi_tinh,sum(so_luong) as so_luong,don_gia,thanh_tien,ma_nhom,tt_thau, TYLE_TT ") \
				_T("		from bh_bang_ctdv where ma_lk='%s' and thanh_tien >0 ") \
				_T("		GROUP BY ma_dich_vu,ten_dich_vu,don_vi_tinh,don_gia,thanh_tien,ma_nhom,ma_vat_tu,tt_thau,ten_vat_tu, TYLE_TT ") \
				_T("		order by ma_nhom,ma_dich_vu "),m_szMa_lk);*/

	szSQL.Format(_T("select stt, case when ma_nhom in ('10','11') then ma_vat_tu else ma_dich_vu end as ma_dich_vu, ") \
				_T("	case when ma_nhom in ('10','11') then ten_vat_tu else ten_dich_vu end as ten_dich_vu, ") \
				_T("	don_vi_tinh,so_luong as so_luong,don_gia,thanh_tien,ma_nhom,tt_thau, TYLE_TT, goi_vtyt ") \
				_T("		from bh_bang_ctdv where ma_lk='%s' and thanh_tien >0 ") \
				_T("		order by stt "),m_szMa_lk);
	int nCount = 0;
	int nIndex = 1;
	...
	if(n_thanh_tien_total > 0)
	{
		...
		int nItem = m_wndcontrol_1.AddItems(_T(""),_T(""),_T(""),_T(""),_T(""),_T(""),tmpStr,_T(""),ToString(n_thanh_tien_total), NULL);
		m_wndcontrol_1.SetItemBkColor(nItem, RGB(192, 192, 192), FALSE);
		m_wndcontrol_1.SetItemTextColor(nItem, RGB(0, 0, 255), FALSE);
		m_wndcontrol_1.MergeCell(nItem, 0, nItem, 4, DT_LEFT|DT_VCENTER|DT_SINGLELINE, false, true);		
	}
	m_wndcontrol_1.EndLoad(); 

	/*for (int i=0; i< m_wndcontrol_1.GetItemCount()-1;i++)
	{	
		if(m_wndcontrol_1.GetItemText(i,10)!=_T("10"))
		szSQL.Format(_T(" SELECT count(*) from HMS_CIRCULAR_MAP where hcm_map_id='%s' "),m_wndcontrol_1.GetItemText(i,1));

		if(m_wndcontrol_1.GetItemText(i,10)==_T("10"))
		szSQL.Format(_T(" SELECT count(*) from m_product_extra_info where mpei_ma_nhom_vtyt='%s' "),m_wndcontrol_1.GetItemText(i,1));
		rs.ExecSQL(szSQL);
		nQty = rs.GetIntValue();
		if(nQty <=0)
