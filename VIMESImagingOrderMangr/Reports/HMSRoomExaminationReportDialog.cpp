...
long CHMSRoomExaminationReportDialog::OnRoomListLoadData(){	
	...
	if (m_nOutPatient)
	{
		szSQL.Format(_T(" select hrl_id as id, hrl_name as name from hms_roomlist left join sys_dept on(sd_id=hrl_deptid) where sd_type in('KB') order by hrl_id "),szDeptID);
	}
...
_fmsg(_T("%s"), szSQL);
	...
	StringUpper(pMF->m_CompanyInfo.sc_pname, tmpStr);
	xls.SetCellText(0,1, tmpStr,FMT_TEXT|FMT_CENTER,true);
	xls.SetCellMergedColumns(0,2,3);
	StringUpper(pMF->m_CompanyInfo.sc_name, tmpStr);
	xls.SetCellText(0,2, tmpStr,FMT_TEXT|FMT_CENTER,true);

	xls.SetCellMergedColumns(0,3,6);
	xls.SetCellText(0, 3, _T("\x42\xC1O \x43\xC1O TH\x45O TI\xCAU \x42\x1EA2N"),FMT_TEXT|FMT_CENTER,true,18,0);
	...
	xls.SetCellText(0,5, tmpStr,FMT_TEXT);
	...
	xls.SetCellText(1, nRow, _T("T\xEAn \x78\xE9t nghi\x1EC7m"),FMT_TEXT|FMT_CENTER,true);
	xls.SetCellText(2, nRow, _T("\x110\x1A1n v\x1ECB"),FMT_TEXT|FMT_CENTER,TRUE);
	...
	xls.SetCellText(5, nRow, _T("Th\xE0nh ti\x1EC1n"),FMT_TEXT|FMT_CENTER,true);	
	...
	while(!rs.IsEOF())
	{
		...
		if(szNewLine != szOldLine && !szNewLine.IsEmpty()){
			...
			{	nRow++;
				...
				xls.SetCellText(5, nRow, tmpStr, FMT_NUMBER1,true,10,0);
				ttlCost += grpCost;				
				grpCost = 0;
				nIndex=1;
			}
