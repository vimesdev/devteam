...
void CHMSInstrumentSetting::OnCreateComponents(){
	...
	m_wndSendTypeLabel.Create(this, _T("Send Type"), 220, 109, 290, 131);
	m_wndSendType.Create(this,295, 109, 375, 131); 
	m_wndTestGroupLabel.Create(this, _T("Test Group"), 380, 109, 450, 131);
	m_wndTestGroup.Create(this,455, 109, 536, 131); 
	m_wndProtocolLabel.Create(this, _T("Protocol"), 541, 109, 627, 131);
	m_wndProtocol.Create(this,632, 109, 781, 131); 
...
void CHMSInstrumentSetting::OnInitializeComponents(){
	...
	m_wndSendType.InsertColumn(0, _T("ID"), CFMT_NUMBER, 50);
...
/*
	...
	m_wndInstrumentList.DeleteAllItems(); 
	int nCount = 0;
...
int CHMSInstrumentSetting::OnAddHMSInstrumentSetting(){
 	if(GetMode() == VM_ADD || GetMode() == VM_EDIT)  
 		return -1; 
 	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd(); 
 	if(!pMF->SetStartWork(this,_T("Add HMSInstrumentSetting"))) 
 		return -1; 
 	SetMode(VM_ADD);
return 0; 
...
int CHMSInstrumentSetting::OnEditHMSInstrumentSetting(){
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd(); 
 	if(!pMF->SetStartWork(this,_T("Edit HMSInstrumentSetting"))) 
 		return -1; 
 	SetMode(VM_EDIT);
return 0;  
...
int CHMSInstrumentSetting::OnSaveHMSInstrumentSetting(){
 	...
 	CString szSQL; 
 	if(GetMode() == VM_ADD){ 
 		//7szSQL = m_hms_instrumentTbl.GetInsertSQL(); 
 	} 
