...
void CLIMSInputDataDialog::OnInitializeComponents(){
	...
	m_wndRefCode.InsertColumn(0, _T("ID"), CFMT_NUMBER, 50);
...
int CLIMSInputDataDialog::OnAddLIMSInputDataDialog(){
 	if(GetMode() == VM_ADD || GetMode() == VM_EDIT)  
 		return -1; 
 	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd(); 
 	if(!pMF->SetStartWork(this,_T("Add LIMSInputDataDialog"))) 
 		return -1; 
 	SetMode(VM_ADD);
return 0; 
...
int CLIMSInputDataDialog::OnEditLIMSInputDataDialog(){
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd(); 
 	if(!pMF->SetStartWork(this,_T("Edit LIMSInputDataDialog"))) 
 		return -1; 
 	SetMode(VM_EDIT);
return 0;  
...
int CLIMSInputDataDialog::OnSaveLIMSInputDataDialog(){
 	...
 	CString szSQL; 
 	if(GetMode() == VM_ADD){ 
 		//7szSQL = m_lims_instrument_dataTbl.GetInsertSQL(); 
 	} 
