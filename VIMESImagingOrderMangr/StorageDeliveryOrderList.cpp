...
//	m_wndToDepartment.SetCheckValue(true);
	...
	m_wndToDepartment.ShowWindow(SW_HIDE);
...
long CStorageDeliveryOrderList::OnFromStorageLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szWhere;
	szWhere.AppendFormat(_T(" and msl_type = 'C' and msl_dept_id ='%s'"), pMF->m_szDept);
...
long CStorageDeliveryOrderList::OnListLoadData(){
	...
	szWhere.AppendFormat(_T(" and mt_org_id in('%s') "), pMF->GetModuleID());

	szWhere.AppendFormat(_T(" and mt_doctype in('BOO','TST') "));
	szWhere.AppendFormat(_T(" and mt_department_id='%s' "), pMF->m_szDept);
...
void CStorageDeliveryOrderList::OnViewSelect(){
	...
	CString szTransaction_ID = m_wndList.GetItemText(nSel, 0);
	CGuiTabCtrl *pTab = (CGuiTabCtrl*) ((CStorageDeliveryOrderView*) m_wndOrder)->GetParent();
	((CStorageDeliveryOrderView*) m_wndOrder)->Load(str2long(szTransaction_ID));
	pTab->SetCurSel(1);
