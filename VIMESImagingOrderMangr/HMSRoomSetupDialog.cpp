...
{
	/*m_wndRoomList.Create(this,5, 5, 805, 325); 
	m_wndTypeLabel.Create(this, _T("Type"), 5, 329, 85, 354);
	m_wndType.Create(this,91, 329, 465, 354); 
	m_wndSectionLabel.Create(this, _T("Section"), 470, 329, 550, 354);
	m_wndSection.Create(this,555, 329, 805, 354); 
	m_wndExamTypeLabel.Create(this, _T("Exam Type"), 5, 359, 85, 384);
	m_wndExamType.Create(this,91, 359, 465, 384); 
	m_wndZoneLabel.Create(this, _T("Báo cáo khu"), 470, 359, 550, 384);
	m_wndZone.Create(this,555, 359, 805, 384); 
	m_wndNameLabel.Create(this, _T("Name"), 5, 389, 85, 414);
	m_wndName.Create(this,90, 389, 645, 414);
	m_wndIndexLabel.Create(this, _T("Số thứ tự"), 650, 389, 730, 414);
	m_wndIndexNo.Create(this,735, 389, 805, 414); 
	m_wndGroupLabel.Create(this, _T("Group"), 5, 419, 85, 444);
	m_wndGroupType.Create(this,90, 419, 805, 444); 
	m_wndAdressLabel.Create(this, _T("Địa chỉ"), 5, 449, 85, 474);
	m_wndAddress.Create(this,90, 449, 805, 474); 
	m_wndSOD.Create(this, _T("SOD"), 614, 479, 694, 504);
	m_wndActive.Create(this, _T("Active"), 699, 479, 804, 504);
	m_wndAdd.Create(this, _T("&Add"), 434, 510, 504, 535);
	m_wndEdit.Create(this, _T("&Edit"), 509, 510, 579, 535);
	m_wndDelete.Create(this, _T("&Delete"), 584, 510, 654, 535);
	m_wndSave.Create(this, _T("&Save"), 659, 510, 729, 535);
	m_wndCancel.Create(this, _T("&Cancel"), 734, 510, 804, 535);*/
	...
	m_wndCancel.Create(this, _T("&Close"), 734, 525, 804, 550);
...
void CHMSRoomSetupDialog::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT * FROM hms_roomlist WHERE hrl_id=%d and hrl_deptid='%s'"), m_nID, pMF2->m_szDeptID);
	rs.ExecSQL(szSQL);
	if(!rs.IsEOF()){
...
void CHMSRoomSetupDialog::GetScreenToData(){
	...
	if(GetMode() == VM_ADD)
	{
		...
		szSQL.Format(_T("select coalesce(max(hrl_id), 0)+1 FROM hms_roomlist WHERE hrl_deptid='%s' "), pMF2->m_szDeptID);
		rs.ExecSQL(szSQL);
	...
	m_hms_roomlistTbl.SetValue(_T("hrl_deptid"), pMF2->m_szDeptID);
...
long CHMSRoomSetupDialog::OnRoomListLoadData(){
	...
	szSQL.Format(_T("SELECT r.*, ") \
				...
				_T(" ORDER BY hrl_id "), pMF2->m_szDeptID);
	nCount = rs.ExecSQL(szSQL);
...
int CHMSRoomSetupDialog::OnSaveHMSRoomSetupDialog(){
 	...
 	if(GetMode() == VM_ADD){ 
 		...
 		CString szWhere; 
 		szWhere.Format(_T(" WHERE hrl_id=%ld and hrl_deptid='%s' "), m_nID, pMF->m_szDeptID); 
