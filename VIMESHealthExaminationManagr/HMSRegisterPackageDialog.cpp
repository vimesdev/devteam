...
void CHMSRegisterPackageDialog::OnCreateComponents(){
	...
	m_wndOrderQtyLabel.Create(this, _T("Số ngày dự kiến"), 270, 180, 370, 205);
	...
	m_wndQuantityLabel.Create(this, _T("Số ngày thực tế"), 270, 210, 370, 235);
	...
	m_wndNote.Create(this,115, 330, 470, 420, TRUE, FALSE, TRUE); 
	m_wndApply.Create(this, _T("&Apply"), 300, 432, 385, 457);
	m_wndClose.Create(this, _T("&Close"), 390, 432, 475, 457);
...
void CHMSRegisterPackageDialog::GetDataToScreen(){
	...
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T(" SELECT hfe_cardnumber, hfe_cardtype, hfe_status, ") \
_T("   hfe_orderdate,") \
_T("   hfe_enddate,") \
...
int CHMSRegisterPackageDialog::OnCardNumberCheckValue(){
	...
	CString szSQL;
	CRecord rs(&pMF->m_db);
	szSQL.Format(_T("SELECT count(*) FROM hms_smartcard WHERE hsc_cardid='%s' and hsc_docno <> %ld"),
