...
static long _OnVisitListLoadDataFnc(CWnd *pWnd){
	return ((CPatientList*)pWnd)->OnVisitListLoadData();
} 
static void _OnVisitListDblClickFnc(CWnd *pWnd){
	((CPatientList*)pWnd)->OnVisitListDblClick();
} 
static void _OnVisitListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CPatientList*)pWnd)->OnVisitListSelectChange(nOldItem, nNewItem);
} 
static int _OnVisitListDeleteItemFnc(CWnd *pWnd){
	 return ((CPatientList*)pWnd)->OnVisitListDeleteItem();
} 
...
void CPatientList::OnCreateComponents(){
	...
	m_wndList.Create(this,5, 160, 955, 460); 
...
void CPatientList::OnVisitListDblClick(){
	
} 
void CPatientList::OnVisitListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
int CPatientList::OnVisitListDeleteItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	 return 0;
} 
long CPatientList::OnVisitListLoadData(){
/*
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
