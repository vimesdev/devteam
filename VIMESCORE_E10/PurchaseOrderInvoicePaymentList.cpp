...
static int _OnListRollbackItemFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnListRollbackItem();
}
static int _OnListAddItemFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnListAddItem();
}
static int _OnListEditItemFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnListEditItem();
}

static int _OnListViewItemFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnListViewItem();
}
...
static int _OnAddPurchaseOrderInvoiceListFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnAddPurchaseOrderInvoiceList();
} 
static int _OnEditPurchaseOrderInvoiceListFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnEditPurchaseOrderInvoiceList();
} 
static int _OnDeletePurchaseOrderInvoiceListFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnDeletePurchaseOrderInvoiceList();
} 
static int _OnSavePurchaseOrderInvoiceListFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnSavePurchaseOrderInvoiceList();
} 
static int _OnCancelPurchaseOrderInvoiceListFnc(CWnd *pWnd){
	 return ((CPurchaseOrderInvoicePaymentList*)pWnd)->OnCancelPurchaseOrderInvoiceList();
} 
CPurchaseOrderInvoicePaymentList::CPurchaseOrderInvoicePaymentList()
{
	...
	SetDefaultValues();
	m_szType = _T("POO");
	SetWindowName(_T("PurchaseOrderInvoicePaymentList"));
}
CPurchaseOrderInvoicePaymentList::~CPurchaseOrderInvoicePaymentList(){
}
...
int CPurchaseOrderInvoicePaymentList::OnListRollbackItem(){
	CMainFrame_E10 *pMF = (CMainFrame_E10*) AfxGetMainWnd();
	CString szSQL;
	int nRes = 0;
	if (m_nID < 0) return -1;
	...
	nRes += pMF->ExecSQL(szSQL);
	*/
	int retMsg = ShowMessageBox(_T("Do you want to rollback order?(Y/N)"), MB_YESNO|MB_DEFBUTTON1);
	if(retMsg == IDNO)
		return -1;
	szSQL.Format(_T("purchase_payment_rollback('%s', %ld)"), pMF->GetCurrentUser(), m_nID);
...
int CPurchaseOrderInvoicePaymentList::OnListAddItem(){
	CMainFrame_E10 *pMF = (CMainFrame_E10*) AfxGetMainWnd();
	OnAddSelect();
	 return 0;
}
int CPurchaseOrderInvoicePaymentList::OnListEditItem(){
	CMainFrame_E10 *pMF = (CMainFrame_E10*) AfxGetMainWnd();
	OnEditSelect();
	 return 0;
}

int CPurchaseOrderInvoicePaymentList::OnListViewItem(){
	CMainFrame_E10 *pMF = (CMainFrame_E10*) AfxGetMainWnd();
	OnViewSelect();
	 return 0;
}
...
_T(" WHERE nvl(pp_type,'HD') ='HD' and trunc(pp_acctdate) BETWEEN TO_DATE('%s','YYYY-MM-DD') AND TO_DATE('%s','YYYY-MM-DD') ") \
...
void CPurchaseOrderInvoicePaymentList::OnViewSelect(){
	...
	CPurchaseOrderInvoicePaymentDialog dlg(pMF, VM_VIEW);
	dlg.m_nID = m_nID;
	if(dlg.DoModal() == IDOK){
		OnListLoadData();
	}
...
void CPurchaseOrderInvoicePaymentList::OnAddSelect(){
	...
	{
		CPurchaseOrderInvoicePaymentDialog dlg(pMF, VM_ADD);
		if(dlg.DoModal() == IDOK){
			Refresh();	
		}
	}
...
void CPurchaseOrderInvoicePaymentList::OnEditSelect(){
	...
	if(pMF->GetModuleID() != _T("GL"))
	{
		CPurchaseOrderInvoicePaymentDialog dlg(pMF, VM_EDIT);
		dlg.m_nID  = m_nID;
		if(dlg.DoModal() == IDOK){
			Refresh();	
		}
...
int CPurchaseOrderInvoicePaymentList::OnAddPurchaseOrderInvoiceList(){
...
int CPurchaseOrderInvoicePaymentList::OnEditPurchaseOrderInvoiceList(){
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CMainFrame_E10 *pMF = (CMainFrame_E10 *) AfxGetMainWnd(); 
 	SetMode(VM_EDIT);
...
int CPurchaseOrderInvoicePaymentList::OnDeletePurchaseOrderInvoiceList(){
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CMainFrame_E10 *pMF = (CMainFrame_E10 *)AfxGetMainWnd(); 
...
int CPurchaseOrderInvoicePaymentList::OnSavePurchaseOrderInvoiceList(){
 	if(GetMode() != VM_ADD && GetMode() != VM_EDIT) 
 		return -1; 
 	if(!IsValidateData()) 
 		return -1; 
...
int CPurchaseOrderInvoicePaymentList::OnCancelPurchaseOrderInvoiceList(){
 	if(GetMode() == VM_EDIT) 
 	{ 
 		SetMode(VM_VIEW); 
 	} 
...
int CPurchaseOrderInvoicePaymentList::OnPurchaseOrderInvoiceListListLoadData(){
	return 0;
}


void CPurchaseOrderInvoicePaymentList::Refresh(){
	int nSel = m_wndList.GetCurSel();
	OnListLoadData();
	m_wndList.SetCurSel(nSel);

}BEGIN_MESSAGE_MAP(CPurchaseOrderInvoicePaymentList, CGuiView)
ON_WM_SETFOCUS()
END_MESSAGE_MAP()

void CPurchaseOrderInvoicePaymentList::OnSetFocus(CWnd* pOldWnd)
{
...
CString CPurchaseOrderInvoicePaymentList::GetCurrentStatus()
{
	CString szSQL;
	CMainFrame_E10 *pMF = (CMainFrame_E10 *) AfxGetMainWnd(); 
	CRecord rs(&pMF->m_db);
	szSQL.Format(_T("SELECT pp_status as status FROM purchase_payment WHERE pp_purchase_payment_id=%ld "), m_nID);
	rs.ExecSQL(szSQL);
	return rs.GetStringValue();
}
void CPurchaseOrderInvoicePaymentList::OnResizeLayout() {
