#ifndef PURCHASEORDERDIALOGEX_V2_H
#define PURCHASEORDERDIALOGEX_V2_H
...
class CPurchaseOrderDialogEx_V2 : public CGuiDialog
{
protected:
	bool	m_bRefresh;
	bool	m_bNonStock;
...
public:
	...
	bool		IsConsign();
	CPurchaseOrderDialogEx_V2(CWnd *pParent, int nMode, bool bNonStock=false);
	~CPurchaseOrderDialogEx_V2();
	...
	int OnAddPurchaseOrderDialogEx(); 
	int OnEditPurchaseOrderDialogEx(); 
	int OnDeletePurchaseOrderDialogEx(); 
	int OnSavePurchaseOrderDialogEx(); 
	int OnDiscardPurchaseOrderDialogEx(); 
	int OnPurchaseOrderDialogExListLoadData(); 
	void UpdateJson(BOOL bSave);
	void CreateTabViews();
	void	RecalcAmount();
	void	Refresh();
