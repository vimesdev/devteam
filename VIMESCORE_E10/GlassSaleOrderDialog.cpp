...
void CGlassSaleOrderDialog::OnCreateComponents(){
	...
	m_wndApprovedDateLabel.Create(this, _T("Acct Date"), 711, 90, 811, 115);
	m_wndApprovedDate.Create(this,816, 90, 956, 115); 
...
//	m_wndList.InsertColumn(7, _T("Mfg Name"), CFMT_TEXT, 100);
	...
	m_wndItemName.InsertColumn(5, _T(""), CFMT_TEXT, 0);
...
void CGlassSaleOrderDialog::UpdateJson(BOOL bSave){
	...
	m_jData[_T("ApprovedDate")] =  m_szApprovedDate;
	...
	m_jData[_T("ApprovedDate")].GetValue(m_szApprovedDate);
...
void CGlassSaleOrderDialog::GetDataToScreen(){
	...
	if(!rs.IsEOF()){
		...
		rs.GetValue(_T("so_docno"), m_nDocumentNo);
		...
		else 
		{
			if(m_szStatus == _T("S")){
				...
				m_wndApproval.SetWindowText(tmpStr);
			}
			else
			{
...
int CGlassSaleOrderDialog::OnDocumentNoCheckValue(){
	...
	{
		bool flag = true;
		m_wndCustomerName.EnableWindow(flag);
		m_wndYob.EnableWindow(flag);
		m_wndGender.EnableWindow(flag);
	}
	return 0;
} 
...
int CGlassSaleOrderDialog::OnSaveGlassSaleOrderDialog(){
	...
	if(m_nSaleOrderID <= 0)
	{
		...
		m_sale_orderTbl.SetValue(_T("so_docno"), m_nDocumentNo);
...
void CGlassSaleOrderDialog::GetPatientInfo()
{
	UpdateData(TRUE);
	CMainFrame_E10* pMF = (CMainFrame_E10*) AfxGetMainWnd();
	CString szSQL;
	CRecord rs(&pMF->m_db);
	//Them cacs field can lay ra tai day
	szSQL.Format(_T("SELECT hp_surname||' '||hp_midname||' '||hp_firstname as pname, hp_birthdate as yob, hp_sex as gender ") \
...
_tprintf(_T("\r\n%s"), szSQL);
	...
	if(!rs.IsEOF())
	{
		rs.GetValue(_T("pname"), m_szCustomerName);
		rs.GetValue(_T("yob"), m_szYob);
		rs.GetValue(_T("gender"), m_szGenderKey);
		m_szTypeKey = _T("I");
		flag = false;

		szSQL.Format(_T("SELECT * FROM hms_advancedexmeye ") \
			_T(" WHERE haee_docno=%ld "), m_nDocumentNo);
		rs.ExecSQL(szSQL);
		if(!rs.IsEOF())
		{
			rs.GetValue(_T("HAEE_UNGLASSLEFT"), m_szMatTrai);
			rs.GetValue(_T("HAEE_UNGLASSRIGHT"), m_szMatPhai);

			rs.GetValue(_T("HAEE_GLASSLEFT"), m_szCKMattrai);
			rs.GetValue(_T("HAEE_GLASSRIGHT"), m_szCKMatphai);
			rs.GetValue(_T("HAEE_EYEBALLLEFT"), m_szNAMattrai);
			rs.GetValue(_T("HAEE_EYEBALLRIGHT"), m_szNAMatphai);
		}
