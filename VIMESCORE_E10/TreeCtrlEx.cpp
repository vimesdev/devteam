...
HTREEITEM CTreeCtrlEx::FindItem(const CString& name)  
{  
   HTREEITEM root = GetRootItem();  
   while(root != NULL)  
   {  
      HTREEITEM hFound = FindItem(name, this, root);  
      if (hFound)  
         return hFound;   
...
HTREEITEM CTreeCtrlEx::FindItem(const CString& name, CTreeCtrl* tree, HTREEITEM hRoot)  
{  
    // check whether the current item is the searched one  
    CString find_name;
	CString text = tree->GetItemText(hRoot);  
	StringLower(tree->GetItemText(hRoot), text);
	StringLower(name, find_name);

	if (text.Find(find_name) != -1)  
        return hRoot;   
...
HTREEITEM CTreeCtrlEx::FindItem2(CTreeCtrlEx *pTree, HTREEITEM hItem, const CString& szSearch){
  //if (hItem == NULL)
  //{
  ...
  //}
  //Check if current item found
  CString tmpStr, szDest, szDest2;
  tmpStr = pTree->GetItemText(hItem);
  StringLower(tmpStr, szDest);
  StringLower(szSearch, szDest2);
_debug(_T("%s"), szDest);
	  _debug(_T("%s"), szDest2);
	  _debug(_T("%d"), szDest.Find(szDest2));
  if (szDest.Find(szDest2) != -1)
  {
	return hItem; 
  }
  //
  if (pTree->ItemHasChildren(hItem))
  {
	...
	while (hItem)
	{
		HTREEITEM hFound = pTree->FindItem2(pTree, hItem, szSearch);
		if (hFound)
			return hFound;
...
HTREEITEM CTreeCtrlEx::FindNextItem(CTreeCtrlEx *pTree, HTREEITEM hItem, HTREEITEM hItemSelected, const CString& szSearch, BOOL& bReturn){

	//Without selected item, select the first item found
  ...
  //Reach selected item, select the immediate item that match
  ...
  { 
    ...
    while (hItem)
    {
      HTREEITEM hFound = pTree->FindNextItem(pTree, hItem, hItemSelected, szSearch, bReturn);
      if (hFound)
		return hFound;
      hItem = pTree->GetNextItem(hItem, TVGN_NEXT);
    }
