...
//	m_wndOrderList.ModifyStyle(0, LVS_NOSORTHEADER);
	...
	m_wndAssignedList.InsertColumn(8, _T(""), CFMT_TEXT, 0); //orderline
...
*/	
	...
	szSQL.Format(_T(" SELECT DISTINCT hpcl_perform_deptid,") \
	_T("   hpcl_proomid,") \
	_T("   hpcl_receptno") \
	...
	while(!rs.IsEOF())
	{
		...
		rptDetail->SetValue(_T("RoomName"), pMF->GetRoomName(rs.GetValue(_T("hpcl_perform_deptid")), ToInt(tmpStr)));
		rs.GetValue(_T("hpcl_proomid"), tmpStr);
