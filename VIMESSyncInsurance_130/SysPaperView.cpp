...
void CSysPaperView::OnCreateComponents(){
	...
	m_wndDischargeSheet.Create(this, _T("Discharge Sheet"), 220, 30, 430, 55);
    m_wndBirthCertificate.Create(this, _T("Birth Certificate"), 220, 60, 430,
                                 85);
	m_wndInsuranceLeave.Create(this, _T("Insurance Leave"), 220, 92, 430, 117);
	...
	m_wndMaternityLeave.Create(this, _T("Maternity Leave"), 435, 30, 680, 55);
	m_wndProfileSumery.Create(this, _T("Profile Sumery"), 435, 60, 680, 85);
...
void CSysPaperView::OnInitializeComponents(){
    ...
    m_wndListPatient.InsertColumn(6, _T("Giấy chứng nhận NV hưởng BHXH"),
                                  CFMT_TEXT, 100);
    m_wndListPatient.InsertColumn(7, _T("Giấy nghỉ dưỡng thai"), CFMT_TEXT,
                                  100);
...
void CSysPaperView::OnSetWindowEvents(){
    ...
    AddEvent(1, _T("Add	Ctrl+A"), _OnAddSysPaperViewFnc, 0, 'A',
             VK_CONTROL);
    AddEvent(2, _T("Edit	Ctrl+E"), _OnEditSysPaperViewFnc, 0, 'E',
             VK_CONTROL);
    AddEvent(3, _T("Delete	Ctrl+D"), _OnDeleteSysPaperViewFnc, 0, 'D',
             VK_CONTROL);
    AddEvent(4, _T("Save	Ctrl+S"), _OnSaveSysPaperViewFnc, 0, 'S',
             VK_CONTROL);
	AddEvent(0, _T("-"));
    AddEvent(5, _T("Cancel	Ctrl+T"), _OnCancelSysPaperViewFnc, 0, 'T',
             VK_CONTROL);
	SetMode(VM_ADD);
	m_wndDirectory.EnableWindow(false);
    m_szDirectory = AfxGetApp()->GetProfileString(_T("VIMES_SYSINSURANCE_HSCT"),
                                                  _T("Drirectory"));
...
int CSysPaperView::OnFromDateCheckValue() { return 0; }
...
int CSysPaperView::OnToDateCheckValue() { return 0; }
...
long CSysPaperView::OnListPatientLoadData(){
    ...
    szSQL.Format(
        ...
        _T("      case when hc_docno >0 and hc_isthaisan='N' then 'X' else ")
        _T("null end as CT07,  ")
        _T("      case when hc_docno >0 and hc_isthaisan='Y' then 'X' else ")
        _T("null end as CT06  ")
        _T("  FROM")
        ...
        _T("      and hcr_dischargedate BETWEEN timestamp '%s' ")
        _T("         AND timestamp '%s'")
        _T(" UNION ALL")
        _T(" SELECT")
        ...
        _T("  WHERE 1=1 ")
        _T("     and hd_enddate BETWEEN timestamp '%s'")
        _T("     AND timestamp '%s' and  length(hd_cardno)>=15  ")
        _T("      and hd_status='T' and hd_suggestion not in ('D','C') and ")
        _T("(hbc_docno >0 or  hc_docno >0)  ) as tbl ")
        _T("   where 1=1 %s    "),
        m_szFromDate, szToDate, m_szFromDate, szToDate, szWhere);
	...
	while(!rs.IsEOF()){ 
		...
		nCount = m_wndListPatient.AddItems(
            tmpStr, rs.GetValue(_T("docno")), rs.GetValue(_T("pname")),
            rs.GetValue(_T("CT03")), rs.GetValue(_T("CT04")),
            rs.GetValue(_T("CT05")), rs.GetValue(_T("CT07")),
            rs.GetValue(_T("CT06")), NULL);
...
int CSysPaperView::OnDirectoryCheckValue() { return 0; }
void CSysPaperView::OnbrowseSelect(){
    ...
    if (!szPath.IsEmpty()) {
		...
		UpdateData(false);
        AfxGetApp()->WriteProfileString(_T("VIMES_SYSINSURANCE_HSCT"),
...
int CSysPaperView::OnSysPaperViewListLoadData() { return 0; }
