...
void CFeeDetail::OnCreateComponents(){
	...
	m_wndcontrol_1.InsertColumn(9, _T("tt_thau"), CFMT_TEXT, 100);
...
long CFeeDetail::Oncontrol_1LoadData(){
	...
	szSQL.Format(_T("select case when ma_nhom in ('10','11') then ma_vat_tu else ma_dich_vu end as ma_dich_vu, ") \
				...
				_T("	don_vi_tinh,sum(so_luong) as so_luong,don_gia,thanh_tien,ma_nhom,tt_thau ") \
				_T("		from bh_bang_ctdv where ma_lk='%s' and thanh_tien >0 ") \
				_T("		GROUP BY ma_dich_vu,ten_dich_vu,don_vi_tinh,don_gia,thanh_tien,ma_nhom,ma_vat_tu,tt_thau,ten_vat_tu ") \
