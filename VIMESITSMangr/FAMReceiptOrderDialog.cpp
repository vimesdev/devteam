...
CFAMReceiptOrderDialog::CFAMReceiptOrderDialog(CWnd *pParent, int nMode):
	CGuiDialog(pParent){
...
long CFAMReceiptOrderDialog::OnWarehouseLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szFilter;
	szFilter.Format(_T(" and COALESCE(sl_type,'WA') = 'WA' "));
...
void CFAMReceiptOrderDialog::OnAddSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnAddFAMReceiptOrderDialog();
} 
void CFAMReceiptOrderDialog::OnEditSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	if(m_szStatus != _T("P"))
	{
		OnEditFAMReceiptOrderDialog();
	}
	else
...
void CFAMReceiptOrderDialog::OnImportSelect(){
	...
	if(retMsg == IDYES)
	{
		...
		{
			ShowMessageBox(_T("Cannot post order"));
		}
	}
...
void CFAMReceiptOrderDialog::CreateNewOrder()
{
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
