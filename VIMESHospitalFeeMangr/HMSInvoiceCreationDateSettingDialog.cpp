...
void CHMSInvoiceCreationDateSettingDialog::GetDataToScreen(){
	...
	if(!rs.IsEOF())
	{
		rs.GetValue(_T("hfc_recvdate"), m_szReceiptDate);
	}
	else
...
void CHMSInvoiceCreationDateSettingDialog::OnOKSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnSaveHMSInvoiceCreationDateSettingDialog();
} 
...
int CHMSInvoiceCreationDateSettingDialog::OnSaveHMSInvoiceCreationDateSettingDialog(){
 	...
 	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
 	CString szSQL; 
	szSQL.Format(_T("UPDATE hms_fee_control SET hfc_recvdate=TO_TIMESTAMP('%s', 'YYYY-MM-DD HH24:MI:SS') WHERE hfc_userid='%s' "), m_szReceiptDate, pMF->GetCurrentUser());
 	int ret = pMF->ExecSQL(szSQL); 
 	if(ret > 0) 
 	{ 
 	} 
 	...
 	{
		szSQL.Format(_T("INSERT INTO hms_fee_control(hfc_userid,hfc_recvdate) VALUES('%s', '%s') "), pMF->GetCurrentUser(), m_szReceiptDate);
		pMF->ExecSQL(szSQL); 	
 	} 
 	return ret; 
