...
/*static void _OnReceiptNoChangeFnc(CWnd *pWnd){
	((CHMSAcceptInsuranceDialog *)pWnd)->OnReceiptNoChange();
} */
/*static void _OnReceiptNoSetfocusFnc(CWnd *pWnd){
	((CHMSAcceptInsuranceDialog *)pWnd)->OnReceiptNoSetfocus();} */ 
/*static void _OnReceiptNoKillfocusFnc(CWnd *pWnd){
	((CHMSAcceptInsuranceDialog *)pWnd)->OnReceiptNoKillfocus();
} */
static int _OnReceiptNoCheckValueFnc(CWnd *pWnd){
	return ((CHMSAcceptInsuranceDialog *)pWnd)->OnReceiptNoCheckValue();
} 
...
void CHMSAcceptInsuranceDialog::OnCreateComponents(){
	...
	m_wndReceiptNoLabel.Create(this, _T("Receive No"), 10, 540, 120, 565);
	m_wndReceiptNo.Create(this,125, 540, 225, 565); 
...
//	m_wndDescription.LimitText(35);
	...
	m_wndAcceptDate.SetReadOnly(true);
	...
	{
		m_wndList.InsertColumn(6, _T("Insurance Paid"), CFMT_MONEY, 90);
		m_wndDiscountLabel.SetWindowText(_T("Insurance Paid"));
	}
	else
		m_wndList.InsertColumn(6, _T("Discount"), CFMT_MONEY, 90);
	m_wndList.InsertColumn(7, _T("Accept"), CFMT_MONEY, 90);
...
//	m_wndDescription.SetEvent(WE_LOADDATA, _OnDescriptionLoadDataFnc);
	...
	if(GetMode() == VM_ADD)
	{
		m_szAcceptDate = pMF->m_szRecvDate;
		UpdateData(false);
		m_wndAccept.SetFocus();
	}
	else
		GetDataToScreen();
...
long CHMSAcceptInsuranceDialog::OnListLoadData(){
	...
	szSQL.Format(_T(" SELECT") \
		...
		_T(" 		hfe_unitprice-hfe_diffcost/hfe_quantity as unitprice,") \
				...
				rs.GetValue(_T("deptid"), tmpStr);
					...
					if(szParentGroup == szNewGroup.Left(szParentGroup.GetLength()))
					{
						if(szNewGroup.Left(2) ==_T("B1") && szOldGroup != szNewGroup){
							...
							szSQL.Format(_T("SELECT hfg_name FROM hms_fee_group WHERE rtrim(hfe_group,'0')='%s' "), szNewGroup);
...
/*void CHMSAcceptInsuranceDialog::OnReceiptNoChange(){
	
} */
/*void CHMSAcceptInsuranceDialog::OnReceiptNoSetfocus(){
	
} */
/*void CHMSAcceptInsuranceDialog::OnReceiptNoKillfocus(){
	
} */
int CHMSAcceptInsuranceDialog::OnReceiptNoCheckValue(){
	return 0;
} 
...
void CHMSAcceptInsuranceDialog::OnDischargeSelect(){
	...
	szSQL.Format(_T("select hms_fee_cancelinvoice(%ld, %ld, '%s') "),
		m_nDocumentNo, m_nReceiptNo, pMF->GetCurrentUser());
//_fmsg(_T("%s"), szSQL);
	rs.ExecSQL(szSQL);
	if(rs.GetIntValue() <= 0)
	{
