#ifndef HMSFEEDEPOSITDIALOG_H
#define HMSFEeDEPOSITDIALOG_H
...
class CHMSFeeDepositDialog : public CGuiDialog{
protected:
	//m_szType: bao gom cac gia tri
	//A: Admission(tam ung vao vien);
	//B: Treatment (Tam ung trong khoa dieu tri); 
	//C: Discharge(Tam ung ra vien);
	CString		m_szType;
...
public:
	...
	void			OnOKSelect(); 
	void			OnCancelSelect(); 
	CHMSFeeDepositDialog(CWnd *pParent, int nMode);
	~CHMSFeeDepositDialog();
	...
	int OnAddHMSFeeDepositDialog(); 
	int OnEditHMSFeeDepositDialog(); 
	int OnDeleteHMSFeeDepositDialog(); 
	int OnSaveHMSFeeDepositDialog(); 
	int OnCancelHMSFeeDepositDialog(); 
	int OnHMSFeeDepositDialogListLoadData(); 
};
#endif
