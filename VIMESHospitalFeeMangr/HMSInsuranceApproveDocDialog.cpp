...
void CHMSInsuranceApproveDocDialog::OnCreateComponents(){
	...
	m_wndApprovedDate.Create(this,115, 30, 235, 55); 
...
//	m_wndApprovedBy.SetCheckValue(true);
	m_wndApprovedBy.SetReadOnly(true);
...
void CHMSInsuranceApproveDocDialog::OnSetWindowEvents(){
	...
	m_szApprovedBy = pMF->GetCurrentUser();
...
void CHMSInsuranceApproveDocDialog::OnOKSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnSaveHMSInsuranceApproveDocDialog();
} 
...
int CHMSInsuranceApproveDocDialog::OnSaveHMSInsuranceApproveDocDialog(){
 	...
 	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
 	CString szSQL;
	szSQL.Format(_T("HMS_FEE_INSAPPROVEDOC(%ld, TO_TIMESTAMP('%s','YYYY-MM-DD HH24:MI:SS'), '%s') "), pMF->m_nDocumentNo, m_szApprovedDate, m_szApprovedBy);
	int ret = str2int(pMF->ExecDML(szSQL));
	if(ret <= 0)
	{
		ShowMessageBox(_T("Kh\xF4ng \x64uy\x1EC7t \x111\x1B0\x1EE3\x63 h\x1ED3 s\x1A1 \x42HYT"));
		return -1;
	}
	CGuiDialog::OnOK();
 	return 0;
