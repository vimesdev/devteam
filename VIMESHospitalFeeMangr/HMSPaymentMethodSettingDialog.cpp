...
void CHMSPaymentMethodSettingDialog::OnInitializeComponents(){
	...
	m_wndDescription.ModifyStyle(0, SS_CENTERIMAGE);
...
void CHMSPaymentMethodSettingDialog::OnSetWindowEvents(){
	...
	m_szPaymentMethodKey = AfxGetApp()->GetProfileString(_T("FEE"), _T("PAYMENT_METHOD"), _T("TM1"));
	m_bRequest = AfxGetApp()->GetProfileInt(_T("FEE"), _T("ISREQUEST"), 0);
	m_szDeptKey = AfxGetApp()->GetProfileString(_T("FEE"), _T("REQUEST_DEPT"), _T(""));
	if(!m_bRequest)
	{
		m_szDeptKey.Empty();
	}
...
void CHMSPaymentMethodSettingDialog::OnPaymentMethodSelectChange(int nOldItemSel, int nNewItemSel){
	...
	UpdateData(true);
	CString szDesc;
	if(m_szPaymentMethodKey == _T("TM1"))
	{
		szDesc.Format(_T("%s:\r\n\x42\x1EC7nh nh\xE2n n\x1ED9p ti\x1EC1n tr\x1EF1\x63 ti\x1EBFp \x63ho k\x1EBF to\xE1n vi\x1EC7n ph\xED"), m_wndPaymentMethod.GetCurrent(1));
		
	}
	if(m_szPaymentMethodKey == _T("QUY"))
	{
		szDesc.Format(_T("%s:\r\n\x42\x1EC7nh nh\xE2n s\x1EBD qu\x61 qu\x1EF9 n\x1ED9p ti\x1EC1n \x63ho k\x1EBF to\xE1n qu\x1EF9"), m_wndPaymentMethod.GetCurrent(1));
...
long CHMSPaymentMethodSettingDialog::OnDeptLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	m_wndDept.AddItems(_T("TYC"), _T("Khoa khám bệnh theo yêu cầu"), NULL);
	m_wndDept.AddItems(_T("PTTYC"), _T("Khoa PT theo yêu cầu"), NULL);
...
void CHMSPaymentMethodSettingDialog::OnOKSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	UpdateData();
	AfxGetApp()->WriteProfileString(_T("FEE"), _T("PAYMENT_METHOD"), m_szPaymentMethodKey);
	AfxGetApp()->WriteProfileInt(_T("FEE"), _T("ISREQUEST"), m_bRequest);
	AfxGetApp()->WriteProfileString(_T("FEE"), _T("REQUEST_DEPT"), m_szDeptKey);
	CGuiDialog::OnOK();
} 
