...
void CHMSDiscountVoucherLineDialog::OnInitializeComponents(){
	...
	m_wndReceipt.InsertColumn(1, _T("Staff"), CFMT_TEXT, 80);
	m_wndReceipt.InsertColumn(2, _T("Date"), CFMT_TEXT, 90);
	m_wndReceipt.InsertColumn(3, _T("Amount"), CFMT_TEXT, 90);
...
long CHMSDiscountVoucherLineDialog::OnListLoadData(){
	...
	if (m_szReceiptKey.IsEmpty())
		szWhere.AppendFormat(_T(" and hfe_status not in('C','R') "));
	else
		szWhere.AppendFormat(_T(" and hfe_status ='P' and hfe_invoiceno=%ld "), str2long(m_szReceiptKey));

	szSQL.Format(_T(" SELECT hfe_type                AS ftype,") \
_T("   hfe_itemid                   AS fitemid,") \
_T("   hfe_desc                     AS fdesc,") \
_T("   hfe_unit                     AS funit,") \
_T("   hfe_unitprice                AS funitprice,") \
...
_T("   hfe_desc"), m_nDocumentNo, szWhere);
...
long CHMSDiscountVoucherLineDialog::OnReceiptLoadData(){
	...
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
	if(m_wndReceipt.IsSearchKey() && str2long(m_szReceiptKey) > 0){
	 szWhere.Format(_T(" and hfe_invoiceno=%ld "), m_szReceiptKey);
	};
	m_wndReceipt.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T(" SELECT hfe_invoiceno       AS finvoiceno,") \
_T("   hfe_staff                AS fstaff,") \
_T("   trunc(hfe_date)                 AS fdate,") \
_T("   (hfe_payment+hfe_refund) AS fpaid") \
_T(" FROM hms_fee_invoice") \
_T(" WHERE hfe_docno = %ld ") \
_T(" AND hfe_status  ='P' ORDER BY hfe_invoiceno"), m_nDocumentNo);

	nCount = rs.ExecSQL(szSQL);
	m_wndReceipt.AddItems(_T(""), pMF->GetCurrentUser(), pMF->GetSysDate(), _T("0"), NULL);

	while(!rs.IsEOF()){ 
		m_wndReceipt.AddItems(
			rs.GetValue(_T("finvoiceno")), 
			rs.GetValue(_T("fstaff")), 
			rs.GetValue(_T("fdate")), 
			rs.GetValue(_T("fpaid")), 
			NULL);
		rs.MoveNext();
	}
	return nCount;
