...
static void _OnOKSelectFnc(CWnd *pWnd){
	CHMSChuyennguonTTbenhnhanquancovid19 *pVw = (CHMSChuyennguonTTbenhnhanquancovid19 *)pWnd;
	pVw->OnOKSelect();
} 
...
static long _OnListLoadDataFnc(CWnd *pWnd){
	return ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnListLoadData();
} 
static void _OnListDblClickFnc(CWnd *pWnd){
	((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnListDblClick();
} 
static void _OnListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnListSelectChange(nOldItem, nNewItem);
} 
static int _OnListDeleteItemFnc(CWnd *pWnd){
	 return ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnListDeleteItem();
} 
static int _OnAddHMSChuyennguonTTbenhnhancovid19Fnc(CWnd *pWnd){
	 return ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnAddHMSChuyennguonTTbenhnhancovid19();
} 
static int _OnEditHMSChuyennguonTTbenhnhancovid19Fnc(CWnd *pWnd){
	 return ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnEditHMSChuyennguonTTbenhnhancovid19();
} 
static int _OnDeleteHMSChuyennguonTTbenhnhancovid19Fnc(CWnd *pWnd){
	 return ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnDeleteHMSChuyennguonTTbenhnhancovid19();
} 
static int _OnSaveHMSChuyennguonTTbenhnhancovid19Fnc(CWnd *pWnd){
	 return ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnSaveHMSChuyennguonTTbenhnhancovid19();
} 
static int _OnCancelHMSChuyennguonTTbenhnhancovid19Fnc(CWnd *pWnd){
	 return ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnCancelHMSChuyennguonTTbenhnhancovid19();
}

static void _OnTatcaSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnTatcaSelect();
}
static void _OnPhikhamSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnPhikhamSelect();
}
static void _OnPhiCLSSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnPhiCLSSelect();
}
static void _OnPhithuocSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnPhithuocSelect();
}
static void _OnPhivattuSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnPhivattuSelect();
}
static void _OnTienanSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnTienanSelect();
}
static void _OnGoidichvuSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnGoidichvuSelect();
}
static void _OnTiengiuongSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnTiengiuongSelect();
}
static void _OnPhiPTTTSelectFnc(CWnd *pWnd){
	  ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnPhiPTTTSelect();
}
static void _OnChitinhmucphingansachttSelectFnc(CWnd *pWnd){
	 ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnChitinhmucphingansachttSelect();
}
...
static void _OnLocxncovid19SelectFnc(CWnd *pWnd){
	 ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnLocxncovid19Select();
}
...
static void _OnLoccpbntraSelectFnc(CWnd *pWnd){
	 ((CHMSChuyennguonTTbenhnhanquancovid19*)pWnd)->OnLoccpbntraSelect();
}

static long _OnnguonthanhtoanLoadDataFnc(CWnd *pWnd){
	return ((CHMSChuyennguonTTbenhnhanquancovid19 *)pWnd)->OnnguonthanhtoanLoadData();
}


CHMSChuyennguonTTbenhnhanquancovid19::CHMSChuyennguonTTbenhnhanquancovid19(CWnd *pParent):
	CGuiDialog(pParent)
{
	m_nDlgWidth = 605;
	m_nDlgHeight = 485;
...
CHMSChuyennguonTTbenhnhanquancovid19::~CHMSChuyennguonTTbenhnhanquancovid19()
{
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnCreateComponents()
{
	
	m_wndBoloc.Create(this, _T("CHỌN CÁC MỤC PHÍ THEO NHÓM"), 5, 550, 659, 675);
	m_wndList.Create(this,10, 25, 954, 514); 
	m_wndNguonthantoanLabel.Create(this, _T("Chọn nguồn thanh toán"), 10, 519, 250, 544);
	m_wndnguonthanhtoan.Create(this,255, 519, 954, 544); 
	m_wndPaymentSource.Create(this, _T("CHUYỂN NGUỒN THANH TOÁN BỆNH NHÂN"), 5, 5, 959, 549);
	...
	m_wndNguonthantoanLabel.SetTextColor(RGB(255,0,0));

}
void CHMSChuyennguonTTbenhnhanquancovid19::OnInitializeComponents(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();

	m_wndnguonthanhtoan.SetCheckValue(true);
	m_wndList.InsertColumn(0, _T("STT"), CFMT_TEXT, 30);
	m_wndList.InsertColumn(1, _T("Mã phí"), CFMT_NUMBER, 100);
	m_wndList.InsertColumn(2, _T("Mã nhóm"), CFMT_TEXT, 150);	
	m_wndList.InsertColumn(3, _T("Tên mục phí"), CFMT_TEXT, 500);
	m_wndList.InsertColumn(4, _T("Amount"), CFMT_MONEY, 100);
	m_wndList.InsertColumn(5, _T("Nguồn thanh toán"), CFMT_TEXT, 0);
	m_wndList.InsertColumn(6, _T("T/G chỉ định"), CFMT_TEXT, 100);
	m_wndList.InsertColumn(7, _T("Khoa chỉ định"), CFMT_TEXT, 80);
	m_wndList.InsertColumn(8, _T("Mã nhóm phí"), CFMT_TEXT, 0);

	m_wndnguonthanhtoan.InsertColumn(0, _T("ID"), CFMT_TEXT, 80);
	m_wndnguonthanhtoan.InsertColumn(1, _T("Name"), CFMT_TEXT, 350);

}
void CHMSChuyennguonTTbenhnhanquancovid19::OnSetWindowEvents(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	...
	m_wndnguonthanhtoan.SetEvent(WE_LOADDATA, _OnnguonthanhtoanLoadDataFnc);

	SetMode(VM_NONE);	
	OnListLoadData();
	OnnguonthanhtoanLoadData();

}
void CHMSChuyennguonTTbenhnhanquancovid19::OnDoDataExchange(CDataExchange* pDX)

{
	...
	DDX_TextEx(pDX, m_wndnguonthanhtoan.GetDlgCtrlID(), m_sznguonthanhtoanKey);

}
void CHMSChuyennguonTTbenhnhanquancovid19::GetDataToScreen(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
void CHMSChuyennguonTTbenhnhanquancovid19::GetScreenToData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();

}
void CHMSChuyennguonTTbenhnhanquancovid19::SetDefaultValues()
{
	m_nTatca=0;
	m_nPhikham=1;
	m_nPhiCLS=1;
...
int CHMSChuyennguonTTbenhnhanquancovid19::SetMode(int nMode)
{
		...
		m_wndnguonthanhtoan.EnableWindow(true);
 		UpdateData(FALSE);
 		return nOldMode;
}

long CHMSChuyennguonTTbenhnhanquancovid19::OnnguonthanhtoanLoadData()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
	if(m_wndnguonthanhtoan.IsSearchKey() && !m_sznguonthanhtoanKey.IsEmpty())
	{
		szWhere.Format(_T(" AND ss_code='%s'"), m_sznguonthanhtoanKey);
	};
	m_wndnguonthanhtoan.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT ss_code as id, ss_desc as name FROM sys_sel WHERE ss_id='hms_nguontt_benhnhan' %s ORDER BY ss_id "), szWhere);
	nCount = rs.ExecSQL(szSQL);
	//_msg(_T("%s"), szSQL);
	while(!rs.IsEOF()){ 
		m_wndnguonthanhtoan.AddItems
			(
			rs.GetValue(_T("id")), 
...
void CHMSChuyennguonTTbenhnhanquancovid19::OnOKSelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	...
	for (int i =0; i < m_wndList.GetItemCount(); i++)
	{
		...
		szSQL.Format(_T("HMS_FEE_CHANGE_PAYSOURCE_108(%ld, %ld, %ld, '%s', %d, '%s') "),
			m_nDocumentNo, m_nInvoiceNo, nOderlineID, pMF->GetCurrentUser(), nPatpaid, m_sznguonthanhtoanKey);
...
void CHMSChuyennguonTTbenhnhanquancovid19::OnTatcaSelect()
{
	UpdateData(TRUE);
	OnListLoadData();	
	UpdateData(FALSE);
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnPhikhamSelect()
{
	UpdateData(TRUE);
	OnListLoadData();	
	UpdateData(FALSE);
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnPhiCLSSelect()
{
	UpdateData(TRUE);
	OnListLoadData();
	UpdateData(FALSE);
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnPhithuocSelect()
{
	UpdateData(TRUE);
	OnListLoadData();
	UpdateData(FALSE);	
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnPhivattuSelect()
{
	UpdateData(TRUE);
	OnListLoadData();
	UpdateData(FALSE);	
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnTienanSelect()
{
	UpdateData(TRUE);
	OnListLoadData();
	UpdateData(FALSE);	
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnGoidichvuSelect()
{
	UpdateData(TRUE);
	OnListLoadData();
	UpdateData(FALSE);	
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnTiengiuongSelect()
{
	UpdateData(TRUE);
	OnListLoadData();
	UpdateData(FALSE);	
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnPhiPTTTSelect()
{
	UpdateData(TRUE);
	OnListLoadData();
	UpdateData(FALSE);
}
void CHMSChuyennguonTTbenhnhanquancovid19::OnChitinhmucphingansachttSelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	UpdateData(TRUE);
	OnListLoadData();
...
void CHMSChuyennguonTTbenhnhanquancovid19::OnLocxncovid19Select()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	UpdateData(TRUE);
	OnListLoadData();
...
void CHMSChuyennguonTTbenhnhanquancovid19::OnLoccpbntraSelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	UpdateData(TRUE);
	OnListLoadData();
...
void CHMSChuyennguonTTbenhnhanquancovid19::OnCloseSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CGuiDialog::OnCancel();
} 
void CHMSChuyennguonTTbenhnhanquancovid19::OnListDblClick()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd();
	/*CString szItemText;	
	int nCheck = 0;
...
void CHMSChuyennguonTTbenhnhanquancovid19::OnListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	
} 
int CHMSChuyennguonTTbenhnhanquancovid19::OnListDeleteItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	 return 0;
} 
long CHMSChuyennguonTTbenhnhanquancovid19::OnListLoadData(){
	...
	/*if (!m_sznguonthanhtoanKey.IsEmpty())
	{
		szWhere.AppendFormat(_T("AND hfe_category not in '%s'"), m_sznguonthanhtoanKey);	
	}*/
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnAddHMSChuyennguonTTbenhnhancovid19(){
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnEditHMSChuyennguonTTbenhnhancovid19(){
 	if(GetMode() != VM_VIEW)
 		return -1;
 	CMainFrame *pMF = (CMainFrame *) AfxGetMainWnd();
 	SetMode(VM_EDIT);
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnDeleteHMSChuyennguonTTbenhnhancovid19(){
 	if(GetMode() != VM_VIEW)
 		return -1;
 	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd();
 	CString szSQL;
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnSaveHMSChuyennguonTTbenhnhancovid19(){
 	if(GetMode() != VM_ADD && GetMode() != VM_EDIT)
 		return -1;
 	if(!IsValidateData())
 		return -1;
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnCancelHMSChuyennguonTTbenhnhancovid19(){
 	if(GetMode() == VM_EDIT)
 	{
 		SetMode(VM_VIEW);
 	} 
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnHMSChuyennguonTTbenhnhancovid19ListLoadData(){
	return 0;
}

int CHMSChuyennguonTTbenhnhanquancovid19::OnMarkAllSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	for(int i = 0; i < m_wndList.GetItemCount(); i++)
	{
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnUnMarkSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	for(int i = 0; i < m_wndList.GetItemCount(); i++)
	{
...
int CHMSChuyennguonTTbenhnhanquancovid19::OnMarkFoodFeeSelect()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CString szItemText;
	int nCheck = 0;
	for (int i=0; i<m_wndList.GetItemCount(); i++)	
	{
		szItemText=m_wndList.GetItemText(i,8);
