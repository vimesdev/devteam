...
static int _OnEditCardDateFnc(CWnd *pWnd){
	return ((CHMSInsuranceCardSettingDialog*)pWnd)->OnEditInsCardDate();
}
...
{
	m_wndInsuranceCardInformation.Create(this, _T("Insurance Card Information"), 5, 5, 605, 235);
	m_wndApplyDateLabel.Create(this, _T("Apply Date"), 10, 30, 100, 55);
	m_wndApplyDate.Create(this,105, 30, 195, 55); 
	m_wndOffLine.Create(this, _T("Off Line"), 200, 30, 285, 55);
	m_wndActive.Create(this, _T("Active"), 290, 30, 365, 55);
	//m_wndApply.Create(this, _T("&Apply"), 370, 30, 440, 55);
	//m_wndAddCard.Create(this, _T("&Add Card"), 445, 30, 520, 55);
	m_wndApply.Create(this, _T("&Apply"), 445, 30, 520, 55);
	m_wndClose.Create(this, _T("&Close"), 525, 30, 600, 55);
	m_wndList.Create(this,10, 60, 600, 230); 

	m_wndEditCard.Create(this, _T(""), 0, 0, 0, 0);
	m_wndEditCard.ShowWindow(SW_HIDE);
	m_wndEditCard.EnableWindow(FALSE);
}
void CHMSInsuranceCardSettingDialog::OnInitializeComponents()
{
	...
	m_wndList.InsertColumn(1, _T("Date"), CFMT_DATE, 80);
	m_wndList.InsertColumn(2, _T("Card No"), CFMT_TEXT, 160);
...
*/
	...
	m_wndList.SetWindowText(_T("Insurance Card Entry"));
	m_wndList.AddEvent(1, _T("Edit Card Date"), _OnEditCardDateFnc);
...
void CHMSInsuranceCardSettingDialog::OnListSelectChange(int nOldItem, int nNewItem){
	...
	tmpStr = m_wndList.GetItemText(nSel, 6);
	if(tmpStr == _T("Y"))
		m_bOffLine = true;
	else
		m_bOffLine = false;
...
long CHMSInsuranceCardSettingDialog::OnListLoadData(){
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT hc_cardno as cardno, ") \
				...
				_T("ORDER BY hd_admitdate DESC, hc_idx DESC"), m_nPatientNo);
...
return;
	if(dlg.DoModal()==IDOK){
		szSQL.Format(_T("SELECT max(hc_idx)+1 FROM hms_card WHERE hc_patientno=%ld"), m_nPatientNo);
		rs.ExecSQL(szSQL);
		...
		m_hms_cardTbl.SetValue(_T("hc_cardno"), dlg.m_szCardNo); 
		m_hms_cardTbl.SetValue(_T("hc_idx"), nMaxCardIdx); 
		m_hms_cardTbl.SetValue(_T("hc_regdate"), dlg.m_szRegistrationDate); 
		m_hms_cardTbl.SetValue(_T("hc_expdate"), dlg.m_szExpiryDate); 
		m_hms_cardTbl.SetValue(_T("hc_regcode"), dlg.m_szRegistrationPlaceKey); 
		m_hms_cardTbl.SetValue(_T("hc_company"), dlg.m_szCompany); 
		m_hms_cardTbl.SetValue(_T("hc_code"), dlg.m_szCode); 
		int nDisrate = dlg.m_nDiscount;
		CString szOffLine = dlg.m_bOffLine?_T("Y"):_T("N");
		if(szOffLine == _T("Y") && pMF->m_nInsOffLinePayment > 0)
				nDisrate = pMF->m_nInsOffLinePayment;

		m_hms_cardTbl.SetValue(_T("hc_discount"), nDisrate);
		m_hms_cardTbl.SetValue(_T("hc_groupid"), dlg.m_nGroupID);
		m_hms_cardTbl.SetValue(_T("hc_active"), _T("N"));
		szSQL = m_hms_cardTbl.GetInsertSQL();
		int ret = pMF->ExecSQL(szSQL);
		if(ret >= 0)
		{
			pMF->BeginTransaction();
			szSQL.Format(_T("DELETE FROM hms_xcard WHERE hxc_docno=%ld"), m_nDocumentNo);
			pMF->ExecSQL(szSQL);
			szSQL.Format(_T("INSERT INTO hms_xcard SELECT hd_patientno, hd_docno, hd_cardidx, hd_disrate, hd_insline, hd_insregdate FROM hms_doc WHERE hd_docno=%ld"), m_nDocumentNo);
			pMF->ExecSQL(szSQL);

			szSQL.Format(_T("UPDATE hms_doc SET hd_insregdate=date('%s'), hd_disrate=%d, hd_cardidx=%d, hd_cardno='%s', hd_hasxcard='Y', hd_insline='%s' WHERE hd_docno=%ld"),
				m_szApplyDate, dlg.m_nDiscount, nMaxCardIdx, dlg.m_szCardNo, dlg.m_bOffLine?_T("Y"):_T("N"), m_nDocumentNo);
			pMF->ExecSQL(szSQL);
			szSQL.Format(_T("UPDATE hms_card SET hc_active='N' WHERE hc_patientno=%ld and hc_idx<>%d"), m_nPatientNo, nMaxCardIdx);
			pMF->ExecSQL(szSQL);
			pMF->Commit();
...
void CHMSInsuranceCardSettingDialog::OnApplySelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szRegDate, szExpDate, szCardNo,szOffLine;
	...
	if(nSel < 0) 
	{
		ShowMessageBox(_T("\xC1p \x64\x1EE5ng th\xE0nh \x63\xF4ng \x63h\x1EBF \x111\x1ED9 \x42HYT."), MB_OK| MB_ICONASTERISK);
		return;
	}
	UpdateData(true);
...
int CHMSInsuranceCardSettingDialog::OnEditInsCardDate()
{
	int nSel = m_wndList.GetCurSel();
	...
	long nCardIdx = str2long(m_wndList.GetItemText(nSel, 0));

	CHMSEditCardDateDialog dlg(this);
	dlg.m_nCardIdx = nCardIdx;
	dlg.m_nDocNo = m_nDocumentNo;
	dlg.m_nPatientNo = m_nPatientNo;
	dlg.m_szCardNo = m_wndList.GetItemText(nSel, 2);
	dlg.m_szRegistrationDate = m_wndList.GetItemText(nSel, 4);
	dlg.m_szExpiryDate = m_wndList.GetItemText(nSel, 5);
	CString tmpStr = m_wndList.GetItemText(nSel, 6);

	if (tmpStr == _T("N"))
		dlg.m_bOutLine = FALSE;
	else
		dlg.m_bOutLine = TRUE;
