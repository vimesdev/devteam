...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CString szLabel;
	TranslateString(_T("PHÂN HỆ QUẢN LÝ HẬU CẦN"), szLabel);
	SetWindowText(szLabel);
	
	CRecord rs(&m_db);
	CString szSQL, tmpStr;
	szSQL.Format(_T("select sd_org_id from sys_dept where sd_id = '%s'"), GetDepartmentID());
	rs.ExecSQL(szSQL);
	tmpStr = rs.GetStringValue();
	if (!tmpStr.IsEmpty())
	{
		m_szOriginModuleID = m_szModuleID;
		m_szModuleID = tmpStr;
		
	}
	szSQL.Format(_T("SELECT count(*) FROM m_storagelist WHERE msl_dept_id='%s' and msl_org_id IN ('%s') "), GetDepartmentID(), GetModuleID());
	rs.ExecSQL(szSQL);
	//_msg(_T("%s"), szSQL);
	
	//if(rs.GetIntValue() <= 0 && ( GetCurrentUser() != _T("haucan") ))
	if(rs.GetIntValue() <= 0)
	{
...
//	m_wndPurchaseDesktop.ShowWindow(SW_HIDE);
	m_wndDeptIndexSetup_HC.Create(GetPane());
	m_wndDeptIndexSetup_HC.ShowWindow(SW_SHOW);
	...
	obj= pDocPane->AddPane(_T("Purchases"), _T("Suppliers management system"), NULL);
		...
		//	obj->Add(_T("Issue Payments"), _T(""), &m_wndPurchaseOrderList);
			...
			obj->Add(_T("HC-Items"), _T(""), &m_wndProduct);
...
//			obj->Add(_T("Resources"), _T(""), NULL);
		...
		if(obj){
			...
			obj->Add(_T("HC Orders"), _T(""), &m_wndPrescription);
			m_wndSaleOrderList.Create(pDocPane);
			obj->Add(_T("Retain HC Orders"), _T(""), &m_wndSaleOrderList);
			m_wndPrescriptionReturnList.Create(pDocPane);
			obj->Add(_T("HC Return Orders"), _T(""), &m_wndPrescriptionReturnList);
		...
		*/
		...
		if (obj)
		{
			...
			obj->Add(_T("Department Index"), _T(""), &m_wndDeptIndexSetup_HC);
