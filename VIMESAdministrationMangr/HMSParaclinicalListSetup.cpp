#include "HMSParaclinicalListSetup.h"
//#include "stdafx.h"
#include "MainFrm.h"
#include "HMSParaclinicalSetupDialog.h"
#include "HMSParaclinicalSetupDialog_new.h"
#include "HMSParaclinicalSetupDialog_new2.h"
#include "HMSAdditionFeeSetupDialog.h"
#include "HMSSelectionListDialog.h"
#include "HMSUpdatePacsGroupDialog.h"
#include "HMSUpdateChildItemDialog.h"
#include "HMSUpdateExtraInforItemDialog.h"

static int _OnParaclinicalListLoadDataFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnParaclinicalListLoadData();
} 
static int _OnParaclinicalListDblClickFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnParaclinicalListDblClick();
} 
static int _OnParaclinicalListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnParaclinicalListSelectChange(nOldItem, nNewItem);
} 
static int _OnParaclinicalListDeleteItemFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnParaclinicalListDeleteItem();
} 
static int _OnFeeListLoadDataFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnFeeListLoadData();
} 
static int _OnFeeListDblClickFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnFeeListDblClick();
} 
static int _OnFeeListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnFeeListSelectChange(nOldItem, nNewItem);
} 
static int _OnFeeListDeleteItemFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnFeeListDeleteItem();
} 

static int _OnFeeListSortIndexFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnFeeListSortIndex();
} 
static int _OnListShowLogItemFnc(CWnd *pWnd){
	return ((CHMSParaclinicalListSetup*)pWnd)->OnListShowLogItem();
}
static int _OnFindChangeFnc(CWnd *pWnd){
	return ((CHMSParaclinicalListSetup *)pWnd)->OnFindChange();
}

static int _OnSetSubGroupFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnSetSubGroup();
} 
static int _OnSetSubItemFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnSetSubItem();
} 
static int _OnListEditOldPriceFnc(CWnd* pWnd){
	((CHMSParaclinicalListSetup*)pWnd)->OnListEditOldPrice();
	return 0;
}
static int _OnListCopyItemFeeFnc(CWnd* pWnd){
	((CHMSParaclinicalListSetup*)pWnd)->OnListCopyItemFee();
	return 0;
}

static int _OnListEditItemFeeFnc(CWnd* pWnd){
	((CHMSParaclinicalListSetup*)pWnd)->OnListEditItemFee();
	return 0;
}

static int _OnUpdatePacsGroupFnc(CWnd* pWnd){
	((CHMSParaclinicalListSetup*)pWnd)->OnUpdatePacsGroup();
	return 0;
}

static int _OnFilterSelectFnc(CWnd *pWnd){
	CHMSParaclinicalListSetup *pVw = (CHMSParaclinicalListSetup *)pWnd;
	return pVw->OnFilterSelect();
}

static int _OnAddSelectFnc(CWnd *pWnd){
	CHMSParaclinicalListSetup *pVw = (CHMSParaclinicalListSetup *)pWnd;
	return pVw->OnAddSelect();
} 
static int _OnEditSelectFnc(CWnd *pWnd){
	CHMSParaclinicalListSetup *pVw = (CHMSParaclinicalListSetup *)pWnd;
	return pVw->OnEditSelect();
} 
static int _OnDeleteSelectFnc(CWnd *pWnd){
	CHMSParaclinicalListSetup *pVw = (CHMSParaclinicalListSetup *)pWnd;
	return pVw->OnDeleteSelect();
} 
static int _OnApplyDateLabelSelectFnc(CWnd *pWnd){
	CHMSParaclinicalListSetup *pVw = (CHMSParaclinicalListSetup *)pWnd;
	return pVw->OnApplyDateLabelSelect();
} 
static int _OnApplyDateCheckValueFnc(CWnd *pWnd){
	CHMSParaclinicalListSetup *pVw = (CHMSParaclinicalListSetup *)pWnd;
	return pVw->OnApplyDateCheckValue();
} 
static int _OnAddHMSParaclinicalListSetupFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnAddHMSParaclinicalListSetup();
} 
static int _OnEditHMSParaclinicalListSetupFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnEditHMSParaclinicalListSetup();
} 
static int _OnDeleteHMSParaclinicalListSetupFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnDeleteHMSParaclinicalListSetup();
} 
static int _OnSaveHMSParaclinicalListSetupFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnSaveHMSParaclinicalListSetup();
} 
static int _OnCancelHMSParaclinicalListSetupFnc(CWnd *pWnd){
	 return ((CHMSParaclinicalListSetup*)pWnd)->OnCancelHMSParaclinicalListSetup();
} 
static void _OnInactiveSelectFnc(CWnd *pWnd){
	return ((CHMSParaclinicalListSetup*)pWnd)->OnInactiveSelect();
}

static void _OnDurationFilterSelectFnc(CWnd *pWnd){
	return ((CHMSParaclinicalListSetup*)pWnd)->OnDurationFilterSelect();
}


static void _OnGenderFilterSelectFnc(CWnd *pWnd){
	return ((CHMSParaclinicalListSetup*)pWnd)->OnGenderFilterSelect();
}

static int _OnUpdateChildItemListFnc(CWnd* pWnd){
	((CHMSParaclinicalListSetup*)pWnd)->OnUpdateChildItemList();
	return 0;
}
static int _OnUpdateExtraInforItemListFnc(CWnd* pWnd){
	((CHMSParaclinicalListSetup*)pWnd)->OnUpdateExtraInforItemList();
	return 0;
}


CHMSParaclinicalListSetup::CHMSParaclinicalListSetup(){

	m_nDlgWidth = 800;
	m_nDlgHeight = 590;
	SetDefaultValues();
}
CHMSParaclinicalListSetup::~CHMSParaclinicalListSetup(){
}
void CHMSParaclinicalListSetup::OnCreateComponents(){
	m_wndSearchInformation.Create(this, _T("Search Information"), 260, 5, 805, 60);
	m_wndFindLabel.Create(this, _T("&Find"), 265, 30, 335, 55);
	m_wndFind.Create(this,340, 30, 570, 55); 
	m_wndParaclinicalList.Create(this,5, 5, 255, 585); 
	m_wndFeeList.Create(this,260, 65, 805, 435); 
	m_wndApplyDateLabel.Create(this, _T("Apply Date"), 575, 30, 685, 55);
	m_wndApplyDate.Create(this, 690, 30, 770, 55);
	m_wndAdd.Create(this, _T("&Add"), 565, 560, 640, 585);
	m_wndEdit.Create(this, _T("&Edit"), 645, 560, 720, 585);
	m_wndDelete.Create(this, _T("&Delete"), 725, 560, 800, 585);
	m_wndFilter.Create(this, _T("@"), 775, 30, 800, 55);
	m_wndInactive.Create(this, _T("Inactive"), 260, 560, 370, 585);
	m_wndDurationFilter.Create(this, _T("Thời hạn"), 375, 560, 460, 585);
	m_wndGenderFilter.Create(this, _T("Giới tính"), 465, 560, 560, 585);
	m_wndMapCircular.m_nStyle = 1;
	m_wndMapCircular.Create(this, CRect(260, 440, 805, 555));
}
void CHMSParaclinicalListSetup::OnInitializeComponents(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	CRecord rs(&pMF->m_db);
	CString szSQL, tmpStr, szCode;
	TCHAR chOld=_T(' '), ch;
	//1: Nhom can lam sang
	szSQL.Format(_T("SELECT * FROM hms_fee_group WHERE substr(hfg_id, 1, 2) IN('B1','B2','B3') ORDER BY hfg_index, hfg_id "));
	
	rs.ExecSQL(szSQL);
	m_arCode.RemoveAll();
	while(!rs.IsEOF()){
		rs.GetValue(_T("hfg_id"), szCode);
		ch = szCode[2];
		if(ch == _T('0'))
		{
			rs.GetValue(_T("hfg_name"), tmpStr);
			m_wndParaclinicalList.AddCaption(tmpStr);
			m_arCode.Add(CString(_T("")));
		}
		else
		{
			m_wndParaclinicalList.Add(rs.GetValue(_T("hfg_name")), _T(""), NULL);
			m_arCode.Add(szCode);
		}
		rs.MoveNext();
	};
	m_wndParaclinicalList.ModifyStyle(0, WS_BORDER|WS_VSCROLL);
	m_wndFind.SetLimitText(35);
	m_wndFind.ModifyStyle(WS_TABSTOP, 0);

//	m_wndFind.SetCheckValue(true);
	m_wndFeeList.InsertColumn(0, _T("Map ID"), CFMT_TEXT, 90);
	m_wndFeeList.InsertColumn(1, _T("ID"), CFMT_TEXT, 85);
	m_wndFeeList.InsertColumn(2, _T("Name"), CFMT_TEXT, 200);
	m_wndFeeList.InsertColumn(3, _T("Unit"), CFMT_TEXT, 50);
	m_wndFeeList.InsertColumn(4, _T("Service Price"), CFMT_MONEY, 85);
	m_wndFeeList.InsertColumn(5, _T("Insurance Price"), CFMT_MONEY, 85);
	m_wndFeeList.InsertColumn(6, _T("Policy Price"), CFMT_MONEY, 85);
	m_wndFeeList.InsertColumn(7, _T("Return Dept"), CFMT_MONEY, 85);
	m_wndFeeList.InsertColumn(8, _T("Dept ID"), CFMT_TEXT, 50);
	m_wndFeeList.InsertColumn(9, _T("Apply Date"), CFMT_DATE, 80);
	m_wndFeeList.InsertColumn(10, _T("Salary"), CFMT_MONEY, 85);
	m_wndFeeList.InsertColumn(11, _T("Nhóm BH"), CFMT_TEXT, 350);
	m_wndFeeList.SetAllowDrag(true);
	m_wndFeeList.ModifyStyle(LVS_SINGLESEL, 0);
	m_wndMapCircular.OnInitDialog();
	
}
void CHMSParaclinicalListSetup::OnSetWindowEvents(){
	m_wndParaclinicalList.SetCallback(_OnParaclinicalListSelectChangeFnc);
	m_wndParaclinicalList.SetFontSize(GetFaceSize());
	//m_wndParaclinicalList.SetEvent(WE_LOADDATA, _OnParaclinicalListLoadDataFnc);
	//m_wndParaclinicalList.SetEvent(WE_DBLCLICK, _OnParaclinicalListDblClickFnc);
//	m_wndParaclinicalList.AddEvent(1, _T("Delete"), _OnParaclinicalListDeleteItemFnc, 0, VK_DELETE, 0);
	m_wndFeeList.SetEvent(WE_SELCHANGE, _OnFeeListSelectChangeFnc);
	m_wndFeeList.SetEvent(WE_LOADDATA, _OnFeeListLoadDataFnc);
	m_wndFeeList.SetEvent(WE_DBLCLICK, _OnFeeListDblClickFnc);
	m_wndFeeList.SetWindowText(_T("Fee Entry"));
	m_wndFeeList.AddEvent(1, _T("Subgroup Setting"), _OnSetSubGroupFnc);
	m_wndFeeList.AddEvent(2, _T("Subitem Setting"), _OnSetSubItemFnc);
	m_wndFeeList.AddEvent(0, _T("-"), NULL);
	m_wndFeeList.AddEvent(3, _T("Delete"), _OnFeeListDeleteItemFnc, 0, VK_DELETE, 0);
	m_wndFeeList.AddEvent(4, _T("Sort Index"), _OnFeeListSortIndexFnc);
	m_wndFeeList.AddEvent(5, _T("Show table of fixed charges"), _OnListShowLogItemFnc);
	m_wndFeeList.AddEvent(0, _T("-"), NULL);
	m_wndFeeList.AddEvent(6, _T("Edit old price"), _OnListEditOldPriceFnc);
	m_wndFeeList.AddEvent(0, _T("-"), NULL);
	m_wndFeeList.AddEvent(7, _T("Dupplicate Item Fee"), _OnListCopyItemFeeFnc);

	m_wndFeeList.AddEvent(0, _T("-"), NULL);
	m_wndFeeList.AddEvent(8, _T("Thiết lập mục xét nghiệm được gõ số lượng!"), _OnListEditItemFeeFnc);

	m_wndFeeList.AddEvent(0, _T("-"), NULL);
	m_wndFeeList.AddEvent(9, _T("Thiết lập mục phí phân loại theo PACS"), _OnUpdatePacsGroupFnc);

	m_wndFeeList.AddEvent(0, _T("-"), NULL);
	m_wndFeeList.AddEvent(10, _T("Thiết lập mục phí nhóm con SHPT / GPBL"), _OnUpdateChildItemListFnc);

	m_wndFeeList.AddEvent(0, _T("-"), NULL);
	m_wndFeeList.AddEvent(11, _T("Thiết lập số lượng, ghi chú cho mục phí"), _OnUpdateExtraInforItemListFnc);

	m_wndFind.SetEvent(WE_CHANGE, _OnFindChangeFnc);
	m_wndFilter.SetEvent(WE_CLICK, _OnFilterSelectFnc);
	m_wndInactive.SetEvent(WE_CLICK, _OnInactiveSelectFnc);
	m_wndDurationFilter.SetEvent(WE_CLICK, _OnDurationFilterSelectFnc);
	m_wndGenderFilter.SetEvent(WE_CLICK, _OnGenderFilterSelectFnc);

	m_wndAdd.SetEvent(WE_CLICK, _OnAddSelectFnc);
	m_wndEdit.SetEvent(WE_CLICK, _OnEditSelectFnc);
	m_wndDelete.SetEvent(WE_CLICK, _OnDeleteSelectFnc);
	m_wndApplyDateLabel.SetEvent(WE_CLICK, _OnApplyDateLabelSelectFnc);
	m_wndApplyDate.SetEvent(WE_CHECKVALUE, _OnApplyDateCheckValueFnc);
	EnableButtons(TRUE, -1);
}
void CHMSParaclinicalListSetup::OnDoDataExchange(CDataExchange* pDX){
	DDX_Text(pDX, m_wndFind.GetDlgCtrlID(), m_szFind);
	DDX_Check(pDX, m_wndApplyDateLabel.GetDlgCtrlID(), m_bApplyDate);
	DDX_TextEx(pDX, m_wndApplyDate.GetDlgCtrlID(), m_szApplyDate);
	DDX_Check(pDX, m_wndInactive.GetDlgCtrlID(), m_bInactive);
	DDX_Check(pDX, m_wndDurationFilter.GetDlgCtrlID(), m_bDurationFilter);
	DDX_Check(pDX, m_wndGenderFilter.GetDlgCtrlID(), m_bGenderFilter);
}
void CHMSParaclinicalListSetup::GetDataToScreen(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 

}
void CHMSParaclinicalListSetup::GetScreenToData(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 

}
void CHMSParaclinicalListSetup::SetDefaultValues(){
	m_bApplyDate = FALSE;
	m_szApplyDate.Empty();
	m_bInactive = FALSE;

}
int CHMSParaclinicalListSetup::SetMode(int nMode){ 
 		int nOldMode = GetMode(); 
 		CGuiView::SetMode(nMode); 
 		CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd(); 
 		CString szSQL; 
 		CRecord rs(&pMF->m_db); 
  		switch(nMode){ 
 		case VM_ADD: 
 			EnableControls(TRUE); 
 			EnableButtons(TRUE, 0, -1); 
 			SetDefaultValues(); 
 			break; 
 		case VM_EDIT: 
 			EnableControls(TRUE); 
 			EnableButtons(TRUE, 3, 4, -1); 
 			break; 
 		case VM_VIEW: 
 			EnableControls(FALSE); 
 			EnableButtons(TRUE, 0, 1, 2, -1); 
 			break; 
 		case VM_NONE: 
 			EnableControls(FALSE); 
 			EnableButtons(TRUE, 0, 6, -1); 
 			SetDefaultValues(); 
 			break; 
 		}; 
		m_wndFilter.EnableWindow(TRUE);
 		UpdateData(FALSE); 
 		return nOldMode; 
 	} 
int CHMSParaclinicalListSetup::OnParaclinicalListDblClick(){
	 return 0;
} 
int CHMSParaclinicalListSetup::OnParaclinicalListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	if(nNewItem < 0 || nNewItem >= m_arCode.GetCount())
	{
		EnableButtons(false, -1);
	//	m_szCode.Empty();
		return -1;
	}
	EnableButtons(true, 0, -1);
	m_szCode = m_arCode.GetAt(nNewItem);
	OnFeeListLoadData();
	return 0;
} 
int CHMSParaclinicalListSetup::OnParaclinicalListDeleteItem(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	 return 0;
} 
int CHMSParaclinicalListSetup::OnParaclinicalListLoadData(){
/*
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	CRecord rs(&pMF->m_db);
	CString szSQL;
	m_wndParaclinicalList.BeginLoad(); 
	m_wndParaclinicalList.DeleteAllItems(); 
	int nCount = 0;
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndParaclinicalList.AddItems(
		rs.MoveNext();
	}
	m_wndParaclinicalList.EndLoad(); 
	return nCount;
*/
	return 0;
}
int CHMSParaclinicalListSetup::OnFeeListDblClick(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	
	int nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return 0;
	CString szID = m_wndFeeList.GetItemText(nSel, 1);
	//CHMSAdditionFeeSetupDialog dlg(pMF);

	CHMSParaclinicalSetupDialog_new dlg(pMF);
	dlg.m_szType = _T("T");
	dlg.m_szID = szID;
	dlg.m_pWnd = this;
	dlg.SetMode(VM_EDIT);
	if(dlg.DoModal() == IDOK)
	{
		int nSel = m_wndFeeList.GetCurSel();
		OnFeeListLoadData();
		m_wndFeeList.SetCurSel(nSel);
	}
	 return 0;
} 
int CHMSParaclinicalListSetup::OnFeeListSelectChange(int nOldItem, int nNewItem){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	int nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0)
		return -1;

	m_szFeeID = m_wndFeeList.GetItemText(nSel, 1);
	SetMode(VM_VIEW);
	m_wndMapCircular.m_szFeeID = m_szFeeID;
	m_wndMapCircular.m_szName = m_wndFeeList.GetItemText(nNewItem, 2);
	m_wndMapCircular.GetDataToScreen();
	return 0;
} 
int CHMSParaclinicalListSetup::OnFeeListDeleteItem(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	OnDeleteHMSParaclinicalListSetup();
	return 0;
} 
int CHMSParaclinicalListSetup::OnFeeListSortIndex(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	CString szSQL, szID;
	for (int i =0; i < m_wndFeeList.GetItemCount(); i++){
		szID = m_wndFeeList.GetItemText(i, 1);
		szSQL.Format(_T("UPDATE hms_fee_list SET hfl_line=%d WHERE hfl_groupid='%s' and hfl_feeid='%s' "), i+1, m_szCode, szID);
		pMF->ExecSQL(szSQL);
	}
	return 0;
} 
int CHMSParaclinicalListSetup::OnFeeListLoadData(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	UpdateData(true);
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere, szSubItem, szSubGroup, tmpStr, szItype, szPacscode, szChildItem;
	m_wndFeeList.BeginLoad(); 
	int nCount = 0;
	if(!m_szFind.IsEmpty())
	{
		szWhere.Format(_T(" AND lower(hfl_name) like(chr(37)||lower('%s')||chr(37)) "), m_szFind);
	}
	if (m_bApplyDate && !m_szApplyDate.IsEmpty())
		szWhere.AppendFormat(_T(" AND hfl_applydate = to_date('%s', 'yyyy/mm/dd hh24:mi:ss')"), m_szApplyDate);
	if (m_bInactive)
		szWhere.AppendFormat(_T(" AND hfl_active = 'N'"));
	else 
		szWhere.AppendFormat(_T(" AND hfl_active = 'Y'"));
	if (m_bDurationFilter)
		szWhere.AppendFormat(_T(" AND nvl(hfl_apply_duration,0) > 0 "));
	if (m_bGenderFilter)
		szWhere.AppendFormat(_T(" AND nvl(hfl_apply_gender,'N') in ('F','M') "));

	
	szSQL.Format(	_T(" SELECT *") \
					_T(" FROM hms_fee_list") \
					_T(" LEFT JOIN hms_circular_map ON (hfl_feeid = hcm_fee_id)") \
					_T(" LEFT JOIN sys_sel ON (hfl_manhom = ss_code and ss_id = 'hms_4210_manhom')") \
					_T(" WHERE hfl_groupid='%s' %s ") \
					_T(" ORDER BY hfl_line, hfl_feeid"), m_szCode, szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		rs.GetValue(_T("hfl_subgroup"), szSubGroup);
		rs.GetValue(_T("hfl_subitem"), szSubItem);
		rs.GetValue(_T("hfl_itype"), szItype);
		rs.GetValue(_T("HFL_PACSNAME"), szPacscode);
		rs.GetValue(_T("hfl_is_mother_group"), szChildItem);
		tmpStr.Empty();
		if(szSubGroup == _T("Y"))
			tmpStr = _T("(*) ");
		if(!szSubGroup.IsEmpty() && szSubGroup != _T("Y"))
			tmpStr = _T("  ");
		if(szSubItem == _T("Y"))
			tmpStr = _T("[+] ");
		if(!szSubItem.IsEmpty() && szSubItem != _T("Y"))
			tmpStr = _T(" - ");
		int nItem = m_wndFeeList.AddItems(
			rs.GetValue(_T("hcm_map_id")),
			rs.GetValue(_T("hfl_feeid")), 
			tmpStr + rs.GetValue(_T("hfl_name")), 
			rs.GetValue(_T("hfl_unit")), 
			rs.GetValue(_T("hfl_servprice")), 
			rs.GetValue(_T("hfl_insprice")), 
			rs.GetValue(_T("hfl_polprice")),
			rs.GetValue(_T("hfl_retprice")),
			rs.GetValue(_T("hfl_deptid")),
			rs.GetValue(_T("hfl_applydate")),
			rs.GetValue(_T("hfl_salary")),
			rs.GetValue(_T("ss_desc")),
			NULL);
		if(szSubGroup == _T("Y"))
			m_wndFeeList.SetItemBkColor(nItem, RGB(200, 200, 255), FALSE);
		if(szSubItem == _T("Y"))
			m_wndFeeList.SetItemBkColor(nItem, RGB(240, 240, 255), FALSE);
        	
		if (szItype == 'I')
		{
			m_wndFeeList.SetSubItemBkColor(nItem, 1, RGB(244, 164, 96), FALSE);
			m_wndFeeList.SetSubItemTextColor(nItem, 1, RGB(255, 255, 255), FALSE);
		}
		//_msg(_T("%s"), szPacscode);
		if (!szPacscode.IsEmpty())
		{
			m_wndFeeList.SetSubItemBkColor(nItem, 1, RGB(0, 139, 0), FALSE);
			m_wndFeeList.SetSubItemTextColor(nItem, 1, RGB(255, 255, 255), FALSE);
		}

		if (!szChildItem.IsEmpty())
		{
			m_wndFeeList.SetSubItemBkColor(nItem, 2, RGB(0, 0, 128), FALSE);
			m_wndFeeList.SetSubItemTextColor(nItem, 2, RGB(255, 255, 255), FALSE);
		}

		rs.MoveNext();
	}
	m_wndFeeList.EndLoad(); 
	return nCount;

}

int CHMSParaclinicalListSetup::OnSetSubGroup(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	int nSel;
	nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return 0;
	CString szSQL, szID;
	CRecord rs(&pMF->m_db);
	CHMSSelectionListDialog dlg(this);
	
	szID = m_wndFeeList.GetItemText(nSel, 1);
	szSQL.Format(_T("SELECT htp_itemid, hfl_name FROM hms_testprofile LEFT JOIN hms_fee_list ON(hfl_groupid='%s' and htp_itemid=hfl_feeid) WHERE htp_profileid='%s' "), m_szCode, szID);
	rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){
		CHMSSelectionListDialog::SELITEM si;
		rs.GetValue(_T("htp_itemid"), si.szID);
		rs.GetValue(_T("hfl_name"), si.szName);
		dlg.m_arraySelection.Add(si);
		rs.MoveNext();
	}
	
	dlg.m_szSQL.Format(_T("select hfl_feeid as id, hfl_name as name ") \
		_T("from hms_fee_list ") \
		_T("where hfl_groupid='%s' and hfl_feeid <> '%s' and (hfl_subgroup<>'Y' or hfl_subgroup is null) and hfl_subitem IS NULL ") \
		_T("order by hfl_line "), m_szCode, szID);
	if(dlg.DoModal() == IDOK){
		pMF->BeginTransaction();
		if(dlg.m_arraySelection.GetCount() > 0)
		{
			szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='Y', hfl_subitem='' WHERE hfl_groupid='%s' AND hfl_feeid='%s' "), m_szCode, szID);
			pMF->ExecSQL(szSQL);

			szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='', hfl_subitem='' WHERE hfl_groupid='%s' AND hfl_feeid in(select htp_itemid from hms_testprofile where htp_profileid='%s')  "), m_szCode, szID);
			pMF->ExecSQL(szSQL);
			szSQL.Format(_T("DELETE FROM hms_testprofile WHERE htp_profileid='%s' "), szID);
			pMF->ExecSQL(szSQL);

			for (int i =0; i < dlg.m_arraySelection.GetCount(); i++){
				CHMSSelectionListDialog::SELITEM si;
				si = dlg.m_arraySelection[i];
				szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='YY', hfl_subitem='' WHERE hfl_groupid='%s' AND hfl_feeid='%s' "), m_szCode, si.szID);
				pMF->ExecSQL(szSQL);
				szSQL.Format(_T("INSERT INTO hms_testprofile VALUES('%s', '%s') "), szID, si.szID);
				pMF->ExecSQL(szSQL);
			}
		}
		else
		{
			szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='', hfl_subitem='' WHERE hfl_groupid='%s' AND hfl_feeid='%s' "), m_szCode, szID);
			pMF->ExecSQL(szSQL);
			szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='', hfl_subitem='' WHERE hfl_groupid='%s' AND hfl_feeid in(select htp_itemid from hms_testprofile where htp_profileid='%s') "), m_szCode, szID);
			pMF->ExecSQL(szSQL);
			szSQL.Format(_T("DELETE FROM hms_testprofile WHERE htp_profileid='%s' "), szID);
			pMF->ExecSQL(szSQL);
		}
		pMF->Commit();
	}
	return 0;
}

int CHMSParaclinicalListSetup::OnSetSubItem(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	int nSel;
	nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return 0;
	CString szSQL, szID;
	CRecord rs(&pMF->m_db);
	CHMSSelectionListDialog dlg(this);
	
	szID = m_wndFeeList.GetItemText(nSel, 1);
	szSQL.Format(_T("SELECT * FROM hms_fee_list WHERE hfl_groupid='%s' AND hfl_subitem='%s' "), m_szCode, szID);
	rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){
		CHMSSelectionListDialog::SELITEM si;
		rs.GetValue(_T("hfl_feeid"), si.szID);
		rs.GetValue(_T("hfl_name"), si.szName);
		dlg.m_arraySelection.Add(si);
		rs.MoveNext();
	}
	
	dlg.m_szSQL.Format(_T("select hfl_feeid as id, hfl_name as name from hms_fee_list ") \
		_T("where hfl_groupid='%s' and hfl_feeid <> '%s' and (hfl_subgroup is null or hfl_subgroup<>'Y') ") \
		_T(" and (hfl_subitem<>'Y' or hfl_subitem is null) order by hfl_line "), m_szCode, szID);

	if(dlg.DoModal() == IDOK){
		pMF->BeginTransaction();
		szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='N', hfl_subitem='Y' WHERE hfl_groupid='%s' AND hfl_feeid='%s' "), m_szCode, szID);
		pMF->ExecSQL(szSQL);
		szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='N', hfl_subitem='' WHERE hfl_groupid='%s' AND hfl_subitem='%s' "), m_szCode, szID);
		pMF->ExecSQL(szSQL);
		for (int i =0; i < dlg.m_arraySelection.GetCount(); i++){
			CHMSSelectionListDialog::SELITEM si;
			si = dlg.m_arraySelection[i];
			szSQL.Format(_T("UPDATE hms_fee_list SET hfl_subgroup='', hfl_subitem='%s' WHERE hfl_groupid='%s' AND hfl_feeid='%s' "), szID, m_szCode, si.szID);
			pMF->ExecSQL(szSQL);
		}
		pMF->Commit();
	}
	return 0;
}

void CHMSParaclinicalListSetup::OnListEditOldPrice(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	
	int nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return;
	CString szID = m_wndFeeList.GetItemText(nSel, 1);
	CHMSParaclinicalSetupDialog_new2 dlg(pMF);
	dlg.m_szType = _T("T");
	dlg.m_szID = szID;
	dlg.m_pWnd = this;
	dlg.SetMode(VM_EDIT);
	if(dlg.DoModal() == IDOK){
		int nSel = m_wndFeeList.GetCurSel();
		OnFeeListLoadData();
		m_wndFeeList.SetCurSel(nSel);
	}
	 return ;
}

int CHMSParaclinicalListSetup::OnFilterSelect(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	CRect rect;
	m_wndFilter.GetWindowRect(&rect);
	rect.right-=325;
	rect.bottom -= 305;
	 return 0;
}
int CHMSParaclinicalListSetup::OnAddSelect(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	OnAddHMSParaclinicalListSetup();
	 return 0;
} 
int CHMSParaclinicalListSetup::OnEditSelect(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	OnEditHMSParaclinicalListSetup();
	return 0;
} 
int CHMSParaclinicalListSetup::OnDeleteSelect(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	OnDeleteHMSParaclinicalListSetup();
	 return 0;
} 
int CHMSParaclinicalListSetup::OnAddHMSParaclinicalListSetup(){
	int nSel = m_wndParaclinicalList.GetCurSel();
	if(nSel < 0)
		return -1;
	CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd();
	CHMSParaclinicalSetupDialog_new dlg(this);
	dlg.m_szType = _T("T");
	CString szText;
	m_wndParaclinicalList.GetText(nSel, szText);
	dlg.SetMode(VM_ADD);
	dlg.m_szParentGroup = m_szCode;
	dlg.DoModal();
	return 0; 
} 
int CHMSParaclinicalListSetup::OnEditHMSParaclinicalListSetup(){
	if(m_szCode.IsEmpty())
		return -1;
	int nCurSel = m_wndFeeList.GetCurSel();
	if(nCurSel < 0)
		return -1;
	CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd();
	CHMSParaclinicalSetupDialog_new dlg(this);
	dlg.m_szType = _T("T");
	dlg.SetMode(VM_EDIT);
	dlg.m_szParentGroup = m_szCode;
	dlg.m_szID = m_wndFeeList.GetItemText(nCurSel, 1);
	dlg.m_szDepartmentKey = m_wndFeeList.GetItemText(nCurSel, 8);
	dlg.DoModal();
	return 0;
} 
int CHMSParaclinicalListSetup::OnDeleteHMSParaclinicalListSetup(){
	//int nSel = m_wndParaclinicalList.GetCurSel();
	//if(nSel < 0)
	//	return -1;
	//CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd();
	//CHMSParaclinicalSetupDialog_new dlg(this);
	//CString szText;
	//m_wndParaclinicalList.GetText(nSel, szText);
	//dlg.SetMode(VM_ADD);
	//dlg.m_szParentGroup = m_szCode;
	//dlg.DoModal();
	//return 0; 
 	if(GetMode() != VM_VIEW) 
 		return -1; 
 	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
 	CString szSQL, tmpStr; 
	CRecord rs(&pMF->m_db);

 	if(ShowMessage(1, MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO) 
 		return -1; 
	
	int nID = -1;
	for (int i = m_wndFeeList.GetItemCount()-1; i>=0; i--){
		if(m_wndFeeList.GetItemState(i, LVIS_SELECTED))
		{
			CString szID = m_wndFeeList.GetItemText(i, 1);
			tmpStr.Format(_T("This is item fee active. Can not delete item fee [%s]!"), szID);
			szSQL.Format(_T(" select count(*) from hms_testorderline  where hpcl_itemid ='%s'"), szID);
			rs.ExecSQL(szSQL);
			if (rs.GetIntValue() > 0){
				ShowMessageBox(tmpStr, MB_OK|MB_ICONWARNING);
				return -1;
			}
			szSQL.Format(_T(" select count(*) from hms_pacsorderline  where hpcl_itemid ='%s'"), szID);
			rs.ExecSQL(szSQL);
			if (rs.GetIntValue() > 0){
				ShowMessageBox(tmpStr, MB_OK|MB_ICONWARNING);
				return -1;
			}

 			szSQL.Format(_T("DELETE FROM hms_fee_list WHERE  hfl_feeid='%s' ") , szID); 
			pMF->ExecSQL(szSQL); 
			m_wndFeeList.DeleteItem(i);
			nID++;
		}
	}
 	if(nID >= 0){ 
 		SetMode(VM_NONE); 
		OnCancelHMSParaclinicalListSetup(); 
		return nID;
 	}
	return 0;
 } 
int CHMSParaclinicalListSetup::OnSaveHMSParaclinicalListSetup(){
 	if(GetMode() != VM_ADD && GetMode() != VM_EDIT) 
 		return -1; 
 	if(!IsValidateData()) 
 		return -1; 
 	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
 	CString szSQL; 
 	if(GetMode() == VM_ADD){ 
// 		szSQL = m_tblTbl.GetInsertSQL(); 
 	} 
 	else if(GetMode() == VM_EDIT){ 
 		CString szWhere; 
 		szWhere.Format(_T(" WHERE")); 
 	//	szSQL = m_tblTbl.GetUpdateSQL(_T("createdby,createddate")); 
 		szSQL += szWhere; 
 	} 
 //_fmsg(_T("%s"), szSQL); 
 	int ret = pMF->ExecSQL(szSQL); 
 	if(ret > 0) 
 	{ 
 		//OnHMSParaclinicalListSetupListLoadData(); 
 		SetMode(VM_VIEW); 
 		 
 	} 
 	else 
 	{ 
 	} 
 	return ret; 
 	return 0; 
}
int CHMSParaclinicalListSetup::OnCancelHMSParaclinicalListSetup(){
 	if(GetMode() == VM_EDIT) 
 	{ 
 		SetMode(VM_VIEW); 
 	} 
 	else 
 	{ 
 		SetMode(VM_NONE); 
 	} 
 	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
 	 
 	return 0;
} 
int CHMSParaclinicalListSetup::OnHMSParaclinicalListSetupListLoadData(){
	return 0;
}

int CHMSParaclinicalListSetup::OnFindChange(){
	
	UpdateData(true);
	OnFeeListLoadData();
	return 0;
}
#include "HMSFeeListEditDetailDialog.h"
int CHMSParaclinicalListSetup::OnListShowLogItem(){
	CHMSFeeListEditDetailDialog dlg(this);
	dlg.m_szFeeID=m_szFeeID;
	dlg.DoModal();
	return 0;
}

int CHMSParaclinicalListSetup::OnApplyDateLabelSelect(){
	UpdateData(true);
	if (m_bApplyDate)
		m_wndApplyDate.EnableWindow(m_bApplyDate);
	return OnFeeListLoadData();
}

int CHMSParaclinicalListSetup::OnApplyDateCheckValue(){
	OnFeeListLoadData();
	return 0;
}

void CHMSParaclinicalListSetup::OnInactiveSelect(){
	OnFeeListLoadData();
}

void CHMSParaclinicalListSetup::OnDurationFilterSelect(){
	OnFeeListLoadData();
}

void CHMSParaclinicalListSetup::OnGenderFilterSelect(){
	OnFeeListLoadData();
}
int CHMSParaclinicalListSetup::OnListCopyItemFee()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
 	CString szSQL, tmpStr; 
	CRecord rs(&pMF->m_db);
	
	int nID = -1;
	for (int i = m_wndFeeList.GetItemCount()-1; i>=0; i--){
		if(m_wndFeeList.GetItemState(i, LVIS_SELECTED))
		{
			CString szID = m_wndFeeList.GetItemText(i, 1);
			
			szSQL.Format(_T("hms_auto_copy_itemfee('%s','%s') "),szID,pMF->GetCurrentUser());
			pMF->ExecDML(szSQL);
		}
	}
	OnFeeListLoadData();
	return 1;
}

int CHMSParaclinicalListSetup::OnListEditItemFee()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd(); 
 	CString szSQL, tmpStr; 
	CRecord rs(&pMF->m_db);

	int nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return 0;
	CString szID = m_wndFeeList.GetItemText(nSel, 1);
	
	int ret = ShowMessageBox(_T("Chức năng này sẽ cập nhật đề mục cho phép nhập số lượng khi chỉ định, OK?"),MB_YESNO);
		if(ret == IDNO)
		return -1;
		else
	pMF->BeginTransaction();
	{
		szSQL.Format(_T("UPDATE hms_fee_list SET hfl_itype ='I' WHERE hfl_feeid='%s'"), szID);
	    pMF->ExecSQL(szSQL);		
	}
	pMF->Commit();
	
	OnFeeListLoadData();
	return 0;
}

int CHMSParaclinicalListSetup::OnUpdatePacsGroup()
{
	int nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return 0;
	CString szID = m_wndFeeList.GetItemText(nSel, 1);

	CHMSUpdatePacsGroupDialog dlg(this);
	
	dlg.m_szID = szID;
	dlg.SetMode(VM_EDIT);
	if(dlg.DoModal() == IDOK)
	{
		int nSel = m_wndFeeList.GetCurSel();
		OnFeeListLoadData();
		m_wndFeeList.SetCurSel(nSel);
	}
	 return 0;
}

int CHMSParaclinicalListSetup::OnUpdateChildItemList()
{
	int nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return 0;
	CString szID = m_wndFeeList.GetItemText(nSel, 1);

	CHMSUpdateChildItemDialog dlg(this);
	
	dlg.m_szID = szID;
	dlg.SetMode(VM_EDIT);
	if(dlg.DoModal() == IDOK)
	{
		int nSel = m_wndFeeList.GetCurSel();
		OnFeeListLoadData();
		m_wndFeeList.SetCurSel(nSel);
	}
	 return 0;
}

int CHMSParaclinicalListSetup::OnUpdateExtraInforItemList()
{
	int nSel = m_wndFeeList.GetCurSel();
	if(nSel < 0) return 0;
	CString szID = m_wndFeeList.GetItemText(nSel, 1);

	CHMSUpdateExtraInforItemDialog dlg(this);
	
	dlg.m_szID = szID;
	dlg.SetMode(VM_EDIT);
	if(dlg.DoModal() == IDOK)
	{
		int nSel = m_wndFeeList.GetCurSel();
		OnFeeListLoadData();
		m_wndFeeList.SetCurSel(nSel);
	}
	 return 0;
}