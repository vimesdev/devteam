...
static int _OnGroupsearchCheckValueFnc(CWnd *pWnd){
	return ((CHMSTestProfileDialog *)pWnd)->OnGroupsearchCheckValue();
}
static int _OnUsersearchCheckValueFnc(CWnd *pWnd){
	return ((CHMSTestProfileDialog *)pWnd)->OnUsersearchCheckValue();
}
...
static void _OnGroupsearchChangeFnc(CWnd *pWnd)
{
	((CHMSTestProfileDialog *)pWnd)->OnGroupsearchChange();
}

static void _OnUsersearchChangeFnc(CWnd *pWnd)
{
	((CHMSTestProfileDialog *)pWnd)->OnUsersearchChange();
}
...
void CHMSTestProfileDialog::OnCreateComponents()
{
	...
	m_wndTestProfiles.Create(this, _T("Test Profiles"), 5, 5, 982, 570);
	m_wndProfileList.Create(this,5, 54, 488, 564); 
	m_wndItemList.Create(this,492, 54, 977, 504); 
	m_wndGroupLabel.Create(this, _T("Group"), 492, 510, 592, 535);
	m_wndGroup.Create(this,597, 510, 977, 535); 
	m_wndNameLabel.Create(this, _T("Name"), 492, 540, 592, 565);
	m_wndName.Create(this,597, 540, 877, 565); 
	m_wndQtyLabel.Create(this, _T("Qty"), 882, 540, 932, 565);
	m_wndQty.Create(this,937, 540, 977, 565); 
	m_wndAdd.Create(this, _T("&Add"), 562, 575, 642, 600);
	m_wndDelete.Create(this, _T("&Delete"), 647, 575, 727, 600);
	m_wndSave.Create(this, _T("&Save"), 732, 575, 812, 600);
	m_wndCancel.Create(this, _T("&Cancel"), 817, 575, 897, 600);
	m_wndClose.Create(this, _T("&Close"), 902, 575, 982, 600);
	m_wndGroupsearchLabel.Create(this, _T("Nhóm"), 8, 28, 88, 53);
	m_wndGroupsearch.Create(this,93, 28, 488, 53); 
	m_wndUser.Create(this, _T("Người tạo"), 492, 28, 572, 53);
	m_wndUsersearch.Create(this,577, 27, 880, 52); 
	m_wndSearch.Create(this, _T("Search"), 885, 27, 977, 52);
...
void CHMSTestProfileDialog::OnInitializeComponents(){
	...
	m_wndItemList.InsertColumn(1, _T("Name"), CFMT_TEXT, 250);
	m_wndItemList.InsertColumn(2, _T("Qty"), CFMT_TEXT, 50);
	m_wndItemList.InsertColumn(3, _T("Unit Price"), CFMT_MONEY, 80);	
	m_wndItemList.InsertColumn(4, _T("Ins Price"), CFMT_MONEY, 80);
	m_wndItemList.InsertColumn(5, _T("itemid"), CFMT_TEXT, 0);


	m_wndGroup.InsertColumn(0, _T("ID"), CFMT_NUMBER, 50);
	m_wndGroup.InsertColumn(1, _T("Description"), CFMT_TEXT, 220);


	m_wndProfileList.InsertColumn(0, _T("ID"), CFMT_NUMBER, 60);
	m_wndProfileList.InsertColumn(1, _T("Group Name"), CFMT_TEXT, 300);
	m_wndProfileList.InsertColumn(2, _T("Created By"), CFMT_TEXT, 100);

	m_wndName.InsertColumn(0, _T("ID"), CFMT_NUMBER, 50);
	m_wndName.InsertColumn(1, _T("Description"), CFMT_TEXT, 250);
	//m_wndName.InsertColumn(2, _T("Qty"), CFMT_NUMBER, 50);
	m_wndName.InsertColumn(2, _T("Serv Price"), CFMT_MONEY, 80);
	m_wndName.InsertColumn(3, _T("Ins Price"), CFMT_MONEY, 80);
...
void CHMSTestProfileDialog::OnSetWindowEvents(){
	...
	m_wndGroupsearch.SetEvent(WE_CHECKVALUE, _OnGroupsearchCheckValueFnc);
	m_wndGroupsearch.SetEvent(WE_CHANGE, _OnGroupsearchChangeFnc);
	m_wndUsersearch.SetEvent(WE_CHECKVALUE, _OnUsersearchCheckValueFnc);
	m_wndUsersearch.SetEvent(WE_CHANGE, _OnUsersearchChangeFnc);
...
void CHMSTestProfileDialog::OnDoDataExchange(CDataExchange* pDX){
	...
	DDX_Text(pDX, m_wndGroupsearch.GetDlgCtrlID(), m_szGroupsearch);
	DDX_Text(pDX, m_wndUsersearch.GetDlgCtrlID(), m_szUsersearch);
...
int CHMSTestProfileDialog::SetMode(int nMode){
		...
		m_wndGroupsearch.EnableWindow(true);
		m_wndUsersearch.EnableWindow(true);
 		UpdateData(FALSE); 
 		return nOldMode; 
}
...
void CHMSTestProfileDialog::OnGroupsearchChange()
{
	UpdateData(true);
	OnProfileListLoadData();
}
void CHMSTestProfileDialog::OnUsersearchChange()
{
	UpdateData(true);
	OnProfileListLoadData();
}

int CHMSTestProfileDialog::OnGroupsearchCheckValue()
{
	return 0;
}

int CHMSTestProfileDialog::OnUsersearchCheckValue()
{
	return 0;
} 
void CHMSTestProfileDialog::OnSearchSelect()
{
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnProfileListLoadData();	
} 
...
long CHMSTestProfileDialog::OnProfileListLoadData()
{
	...
	if(!m_szGroupsearch.IsEmpty())
	{
		szWhere.Format(_T(" AND lower(ss_desc) like(chr(37)||lower('%s')||chr(37)) "), m_szGroupsearch);
	}

	if (!m_szUsersearch.IsEmpty())
	{
		szWhere.AppendFormat(_T(" AND lower(ss_default) like(chr(37)||lower('%s')||chr(37)) "), m_szUsersearch);
		//szWhere += _T(" AND lower(ss_default) like(chr(37)||lower('%s')||chr(37)) "), m_szUsersearch);
	}

	m_wndProfileList.BeginLoad(); 
	int nCount = 0;
	szSQL.Format(_T("SELECT ss_code as id, ss_desc as name, ss_default as userid ")\
		_T("FROM sys_sel WHERE ss_id='hms_testprofile' %s ORDER BY ss_code "), szWhere);
...
long CHMSTestProfileDialog::OnItemListLoadData()
{
	...
	szSQL.Format(_T(" SELECT hfl_groupid, hfl_feeid as testid,") \
				...
				_T(" WHERE htp_profileid='%s' ORDER BY hfl_groupid, hfl_line, hfl_name "), m_szProfileID);
...
long CHMSTestProfileDialog::OnGroupLoadData()
{
	...
	szSQL.Format(_T("SELECT hfg_id as id, hfg_name as name FROM hms_fee_group ") \
		_T("WHERE substr(hfg_id, 1, 2) IN('B1','B2','B3')  %s ORDER BY hfg_id "), szWhere);
...
long CHMSTestProfileDialog::OnNameLoadData()
{
	...
	szSQL.Format(_T("SELECT hfl_feeid as id, ")\
				 ...
				 _T("WHERE hfl_groupid='%s' and hfl_feeid not in(select htp_itemid from hms_testprofile where htp_profileid='%s') ")\
...
int CHMSTestProfileDialog::OnDeleteHMSTestProfileDialog()
{
	...
	szSQL.Format(_T("DELETE FROM hms_testprofile WHERE htp_itemid='%s' and htp_profileid='%s' and htp_profileid in(select ss_code from sys_sel where ss_id='hms_testprofile' and ss_default='%s')  "), szItemID, m_szProfileID, pMF->GetCurrentUser() ); 
 	int ret = pMF->ExecSQL(szSQL); 
 	if(ret >= 0){ 
 		OnItemListLoadData();
