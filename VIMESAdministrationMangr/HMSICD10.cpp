...
void CHMSICD10Setup::OnCreateComponents(){
	...
	m_wndName.Create(this,295, 560, 630, 585); 
	m_wndTreatDaysLabel.Create(this, _T("Treat Days"), 635, 560, 715, 585);
	m_wndTreatDays.Create(this,720, 560, 800, 585); 
...
//	m_wndGroup.SetCheckValue(true);
	...
	m_hms_icdTbl.SetTableName(_T("hms_icd"));
	m_hms_icdTbl.AddField(_T("hi_createdby"), dfText, 15); 
	m_hms_icdTbl.AddField(_T("hi_createddate"), dfDateTime); 
	m_hms_icdTbl.AddField(_T("hi_updatedby"), dfText, 15); 
	m_hms_icdTbl.AddField(_T("hi_updateddate"), dfDateTime); 
	m_hms_icdTbl.AddField(_T("hi_icd"), dfText, 13); 
	m_hms_icdTbl.AddField(_T("hi_groupid"), dfInteger); 
	m_hms_icdTbl.AddField(_T("hi_chapterid"), dfInteger); 
	m_hms_icdTbl.AddField(_T("hi_name"), dfText, 254); 
	m_hms_icdTbl.AddField(_T("hi_stdtreatdays"), dfInteger); 
...
void CHMSICD10Setup::GetDataToScreen(){
	...
	if(!szID.IsEmpty())
	{
		szSQL.Format(_T("SELECT * FROM hms_icd WHERE hi_icd='%s' "), szID);
		rs.ExecSQL(szSQL);
		if(rs.GetRecordCount()){
			...
			rs.GetValue(_T("hi_stdtreatdays"), m_nTreatDays);
			SetMode(VM_VIEW);
		}
	}
	UpdateData(false);
...
void CHMSICD10Setup::GetScreenToData(){
	CMainFrame *pMF = (CMainFrame *)AfxGetMainWnd(); 
	if(GetMode() == VM_ADD){
		m_hms_icdTbl.SetValue(_T("hi_createdby"), pMF->GetCurrentUser());
		m_hms_icdTbl.SetValue(_T("hi_createddate"), pMF->GetSysDateTime());
	}
	m_hms_icdTbl.SetValue(_T("hi_updatedby"), pMF->GetCurrentUser());
	m_hms_icdTbl.SetValue(_T("hi_updateddate"), pMF->GetSysDateTime());

	m_hms_icdTbl.SetValue(_T("hi_icd"), m_szID);
	m_hms_icdTbl.SetValue(_T("hi_groupid"), m_szGroupKey);
	m_hms_icdTbl.SetValue(_T("hi_chapterid"), m_szChapterKey);
	m_hms_icdTbl.SetValue(_T("hi_name"), m_szName);
	m_hms_icdTbl.SetValue(_T("hi_stdtreatdays"), m_nTreatDays);
...
int CHMSICD10Setup::OnListLoadData(){
	...
	_T(" FROM hms_icd %s ORDER BY hi_icd "), szWhere);
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndList.AddItems(
			rs.GetValue(_T("id")), 
...
int CHMSICD10Setup::OnIDCheckValue(){
	...
	if(m_szID.IsEmpty())
		return -1;
	szSQL.Format(_T("SELECT COUNT(*) FROM hms_icd WHERE hi_icd='%s' "), m_szID);
	rs.ExecSQL(szSQL);
	if(rs.GetIntValue() > 0)
	{
...
int CHMSICD10Setup::OnDeleteHMSICD10(){ 
	...
	if(rs.GetIntValue() > 0)
	{
 	...
 	szSQL.Format(_T("DELETE FROM  hms_icd WHERE  hi_icd='%s'  "), m_szID); 
...
int CHMSICD10Setup::OnSaveHMSICD10(){
 	...
 	CString szSQL; 
 	if(GetMode() == VM_ADD){ 
 		szSQL = m_hms_icdTbl.GetInsertSQL(); 
 	} 
 	else if(GetMode() == VM_EDIT){ 
 		CString szWhere; 
 		szWhere.Format(_T(" WHERE hi_icd='%s' "), m_szID); 
 		szSQL = m_hms_icdTbl.GetUpdateSQL(_T("")); 
 		szSQL += szWhere; 
 	} 
 //_fmsg(_T("%s"), szSQL); 
 	int ret = pMF->ExecSQL(szSQL); 
