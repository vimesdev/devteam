...
static int _OnSearchChangeFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnSearchChange();
} 
static int _OnSearchSetfocusFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnSearchKillfocus();
} 
static int _OnSearchKillfocusFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnSearchKillfocus();
} 
static int _OnSearchCheckValueFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnSearchCheckValue();
} 
static int _OnFindChangeFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnFindChange();
} 
static int _OnFindCheckValueFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnFindCheckValue();
}
static int _OnItemListLoadDataFnc(CWnd *pWnd){
	 return ((CSysSetupList*)pWnd)->OnItemListLoadData();
} 
static int _OnItemListDblClickFnc(CWnd *pWnd){
	return ((CSysSetupList*)pWnd)->OnItemListDblClick();
} 
static int _OnItemListSelChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	 return ((CSysSetupList*)pWnd)->OnItemListSelChange(nOldItem, nNewItem);
} 
static int _OnItemListDeleteItemFnc(CWnd *pWnd){
	 return ((CSysSetupList*)pWnd)->OnItemListDeleteItem();
} 
static int _OnItemListAddParaclinicFnc(CWnd *pWnd){
	return ((CSysSetupList*)pWnd)->OnItemListAddParaclinic();
} 
static int _OnIDChangeFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnIDChange();
} 
static int _OnIDSetfocusFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnIDKillfocus();
} 
static int _OnIDKillfocusFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnIDKillfocus();
} 
static int _OnIDCheckValueFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnIDCheckValue();
} 
static int _OnNameChangeFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnNameChange();
} 
static int _OnNameSetfocusFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnNameKillfocus();
} 
static int _OnNameKillfocusFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnNameKillfocus();
} 
static int _OnNameCheckValueFnc(CWnd *pWnd){
	 return ((CSysSetupList *)pWnd)->OnNameCheckValue();
} 
static int _OnAddSelFnc(CWnd *pWnd){
	CSysSetupList *pVw = (CSysSetupList *)pWnd;
	return pVw->OnAddSel();
} 
...
static int _OnListLoadDataFnc(CWnd *pWnd){
	 return ((CSysSetupList*)pWnd)->OnListLoadData();
} 
static int _OnListDblClickFnc(CWnd *pWnd){
	return ((CSysSetupList*)pWnd)->OnListDblClick();
} 
static int _OnListSelChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	 return ((CSysSetupList*)pWnd)->OnListSelChange(nOldItem, nNewItem);
} 
static int _OnListDeleteItemFnc(CWnd *pWnd){
	 return ((CSysSetupList*)pWnd)->OnListDeleteItem();
}

static int _OnListAddItemFnc(CWnd *pWnd){
	 return ((CSysSetupList*)pWnd)->OnListAddItem();
}

static int _OnListEditItemFnc(CWnd *pWnd){
	 return ((CSysSetupList*)pWnd)->OnListEditItem();
}

static int _OnSaveItemListFnc(CWnd *pWnd)
{
	 return ((CSysSetupList*)pWnd)->OnSaveItemList();
}

CSysSetupList::CSysSetupList(){
...
void CSysSetupList::OnCreateComponents()
{
	...
	m_wndDetails.Create(this, _T(""), 0, 0, 0, 0);
	m_wndDetails.ShowWindow(SW_HIDE);
	m_wndDetails.EnableWindow(FALSE);
	m_wndSearchID.Create(this, 0, 0, 0, 0);
	m_wndSearchID.ShowWindow(SW_HIDE);
	m_wndSearchID.EnableWindow(FALSE);
	m_wndClose.Create(this, _T(""), 0, 0, 0, 0);
	m_wndClose.ShowWindow(SW_HIDE);
	m_wndClose.EnableWindow(FALSE);

}
void CSysSetupList::OnInitializeComponents(){
	...
	m_wndItemList.InsertColumn(3, _T("Vndesc"), CFMT_TEXT, 200);
...
void CSysSetupList::OnSetWindowEvents(){
	...
	m_wndItemList.AddEvent(1, _T("Add paraclinic"), _OnItemListAddParaclinicFnc, 0, VK_DELETE, 0);
...
void CSysSetupList::OnDoDataExchange(CDataExchange* pDX){
...
void CSysSetupList::GetDataToScreen(){

}
void CSysSetupList::GetScreenToData(){
	UpdateData(TRUE);
	m_szID.Trim();
...
int CSysSetupList::OnSearchChange(){
	OnItemListLoadData();
	 return 0;
} 
int CSysSetupList::OnSearchSetfocus(){
	 return 0;
} 
int CSysSetupList::OnSearchKillfocus(){
	 return 0;
} 
int CSysSetupList::OnSearchCheckValue(){
	return 0;
}
int CSysSetupList::OnFindChange(){
	OnListLoadData();	
	 return 0;
} 
int CSysSetupList::OnFindCheckValue(){
	OnListLoadData();
	 return 0;
} 
int CSysSetupList::OnItemListDblClick(){
	 return 0;
} 
int CSysSetupList::OnItemListSelChange(int nOldItem, int nNewItem){
...
int CSysSetupList::OnItemListDeleteItem(){
	int nCurSel = m_wndItemList.GetCurSel();
	if(nCurSel < 0)
		return -1;
...
int CSysSetupList::OnSaveItemList()
{
	CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd();	
...
int CSysSetupList::OnItemListLoadData(){
...
int CSysSetupList::OnIDChange(){
	 return 0;
} 
int CSysSetupList::OnIDSetfocus(){
	 return 0;
} 
int CSysSetupList::OnIDKillfocus(){
	 return 0;
} 
int CSysSetupList::OnIDCheckValue(){
	 return 0;
} 
int CSysSetupList::OnNameChange(){
	 return 0;
} 
int CSysSetupList::OnNameSetfocus(){
	 return 0;
} 
int CSysSetupList::OnNameKillfocus(){
	 return 0;
} 
int CSysSetupList::OnNameCheckValue(){
	 return 0;
} 
int CSysSetupList::OnAddSel(){
...
int CSysSetupList::OnCancelSel(){
	EnableControls(FALSE);
	EnableButtons(TRUE, 0, -1);
	SetMode(GetMode()==VM_EDIT?VM_VIEW:VM_NONE);
	return 0;
} 
int CSysSetupList::OnCloseSel(){
	 return 0;
} 
int CSysSetupList::OnListDblClick(){
	 return 0;
} 
int CSysSetupList::OnListSelChange(int nOldItem, int nNewItem){
...
int CSysSetupList::OnListDeleteItem(){
...
int CSysSetupList::OnListAddItem(){
	CSysSetupListDlg dlg;
	if(dlg.DoModal() == IDOK)
	{
		CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd();
		CString szSQL;
		szSQL.Format(_T("SELECT COUNT(*) FROM sys_setuplist WHERE ssl_id='%s'"), dlg.m_szID);
		CRecord rs(&pMF->m_db);
		rs.ExecSQL(szSQL);
		...
		szSQL.Format(_T("INSERT INTO sys_setuplist VALUES('%s', '%s', %d)"), dlg.m_szID, dlg.m_szName, dlg.m_nType);
		if(pMF->m_db.ExecSQL(szSQL) > 0)
			OnListLoadData();
	}
	return 0;
...
int CSysSetupList::OnListEditItem(){
	CString szOldID;
	CSysSetupListDlg dlg;
	...
	if(dlg.DoModal() == IDOK)
	{
			...
			szSQL.Format(_T("UPDATE sys_setuplist \
							set ssl_id='%s', ssl_name='%s', ssl_type=%d \
							WHERE ssl_id='%s' "), dlg.m_szID, dlg.m_szName, dlg.m_nType, szOldID);
			if(pMF->m_db.ExecSQL(szSQL) > 0)
				OnListLoadData();
	}
	return 0;
...
int CSysSetupList::OnListLoadData(){
	...
	szWhere.Empty();
	if (!m_szFind.IsEmpty())
		szWhere.Format(_T("WHERE lower(ssl_name) LIKE (chr(37)||lower('%s')||chr(37)) "), m_szFind);
	szSQL.Format(_T("SELECT ssl_id as id, ssl_name as name, ssl_type as type \
					FROM sys_setuplist %s ORDER BY ssl_id"), szWhere);
	m_wndList.BeginLoad();
	int nItem = 1;
	int nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
...
int CSysSetupList::SetMode(int nMode){
...
int CSysSetupList::OnItemListAddParaclinic()
{
	if (m_szListID.IsEmpty()) 
		return -1;
	bool bQtyInput = true;
	if (m_szListID == _T("hms_test_main_index"))
	{
		bQtyInput = false;
	}
	CListParaclinic dlg(this, bQtyInput);
	dlg.m_szID = m_szListID;
	if (dlg.DoModal() == IDOK)
	{
		OnItemListLoadData();
