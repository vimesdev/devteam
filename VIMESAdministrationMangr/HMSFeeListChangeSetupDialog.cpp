...
static int _OnActiveHMSFeeListChangeSetupDialogFnc(CWnd *pWnd){
	return ((CHMSFeeListChangeSetupDialog*)pWnd)->OnActiveHMSFeeListChangeSetupDialog();
} 
...
void CHMSFeeListChangeSetupDialog::OnSetWindowEvents(){
	...
	m_wndList.AddEvent(1, _T("Delete"), _OnDeleteHMSFeeListChangeSetupDialogFnc, 0, VK_DELETE, 0);
	m_wndList.AddEvent(2, _T("Active"), _OnActiveHMSFeeListChangeSetupDialogFnc);
...
void CHMSFeeListChangeSetupDialog::GetDataToScreen(){
	...
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("SELECT hfc_code, hfc_description, hfc_applydate FROM hms_feelist_change WHERE hfc_code = '%s' ORDER BY hfc_applydate, hfc_code"), m_szCode);
	rs.ExecSQL(szSQL);
	if(rs.IsEOF())
		SetMode(VM_NONE);
	else
...
void CHMSFeeListChangeSetupDialog::GetScreenToData(){
	...
	m_hms_feelist_changeTbl.SetValue(_T("HFC_CREATEDBY"), pMF->GetCurrentUser());
	m_hms_feelist_changeTbl.SetValue(_T("HFC_CREATEDDATE"), pMF->GetSysDateTime());
...
long CHMSFeeListChangeSetupDialog::OnListLoadData(){
	...
	szSQL.Format(_T(" SELECT hfc_code AS code, hfc_description AS description, hfc_applydate AS applydate, hfc_active AS active FROM hms_feelist_change ORDER BY hfc_applydate "));
...
int CHMSFeeListChangeSetupDialog::OnDeleteHMSFeeListChangeSetupDialog(){
 	...
 	if(ShowMessage(1, MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO)
 		return -1;
 	szSQL.Format(_T("DELETE FROM hms_feelist_change WHERE hfc_code = '%s'"), m_szCode);
...
int CHMSFeeListChangeSetupDialog::OnSaveHMSFeeListChangeSetupDialog(){
 	...
 	if(GetMode() == VM_ADD){
		...
		szSQL.Format(_T("UPDATE hms_feelist_change SET hfc_active = 'N' "));
		pMF->ExecSQL(szSQL);
 		...
 		CString szWhere;
 		szWhere.Format(_T(" WHERE hfc_code = '%s' "), m_szCode);
 		szSQL = m_hms_feelist_changeTbl.GetUpdateSQL(_T("hfc_code, hfc_createdby, hfe_createddate"));
 		szSQL += szWhere;
 	}
 _fmsg(_T("%s"), szSQL);
 	int ret = pMF->ExecSQL(szSQL);
...
int CHMSFeeListChangeSetupDialog::OnActiveHMSFeeListChangeSetupDialog(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szSQL;
	szSQL.Format(_T("UPDATE hms_feelist_change SET hfc_active = 'N' WHERE hfc_code <> '%s' "), m_szCode);
	int ret = pMF->ExecSQL(szSQL);

	szSQL.Format(_T("UPDATE hms_feelist_change SET hfc_active = 'Y' WHERE hfc_code = '%s' "), m_szCode);
