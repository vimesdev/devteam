...
void CSysUserRolesDialog::GetDataToScreen(){
	...
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("SELECT * FROM sys_user_roles WHERE sys_user_roles_id = '%s'"), m_szUserRolesID);
	rs.ExecSQL(szSQL);
...
int CSysUserRolesDialog::OnDeleteSysUserRolesDialog(){
 	...
 	if(ShowMessage(1, MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO)
 		return -1;
 	szSQL.Format(_T("DELETE FROM sys_user_roles WHERE sys_user_roles_id = '%s'"), m_szUserRolesID );
...
int CSysUserRolesDialog::OnSaveSysUserRolesDialog(){
 	...
 	CString szSQL;
 	if(GetMode() == VM_ADD){
 		szSQL = m_sys_user_rolesTbl.GetInsertSQL(_T("sys_user_roles_id"));
 	}
 	else if(GetMode() == VM_EDIT){
 		CString szWhere;
 		szWhere.Format(_T(" WHERE sys_user_roles_id = '%s'"), m_szUserRolesID);
 		szSQL = m_sys_user_rolesTbl.GetUpdateSQL(_T("sys_user_roles_id"));
 		szSQL += szWhere;
 	}
 _fmsg(_T("%s"), szSQL);
 	int ret = pMF->ExecSQL(szSQL);
 	...
 	{
		ShowMessage(0, MB_ICONERROR, L"Tên nhóm quyền đã tồn tại. Vui lòng chọn tên khác!");
 	}
 	return ret;
 	return 0;
}
