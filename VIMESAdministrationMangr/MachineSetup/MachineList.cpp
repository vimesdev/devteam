...
void CMachineList::OnCreateComponents(){
	/*m_wndMachineListBox.Create(this, _T("Danh sách máy"), 5, 5, 805, 618);
	m_wndList.Create(this,10, 30, 800, 495); 
	m_wndGroupLabel.Create(this, _T("Group"), 10, 500, 110, 525);
	m_wndGroup.Create(this,115, 500, 800, 525); 
	m_wndMachineNameLabel.Create(this, _T("Tên máy"), 10, 530, 110, 555);
	m_wndMachineName.Create(this,115, 530, 800, 555); 
	m_wndShortNameLabel.Create(this, _T("S/N"), 10, 559, 110, 584);
	m_wndShortName.Create(this,115, 559, 635, 584); 
	m_wndQty.Create(this,731, 559, 800, 584); 
	m_wndQtyLabel.Create(this, _T("Số lượng"), 640, 559, 725, 585);
	m_wndMachineCodeLabel.Create(this, _T("Mã máy"), 10, 589, 110, 614);
	m_wndMachineCode.Create(this,115, 589, 479, 614); 
	m_wndSourceLabel.Create(this, _T("Source"), 485, 589, 573, 614);
	m_wndSource.Create(this,579, 589, 800, 614); 
	m_wndAdd.Create(this, _T("&Add"), 404, 619, 479, 644);
	m_wndEdit.Create(this, _T("&Edit"), 484, 619, 559, 644);
	m_wndDelete.Create(this, _T("&Delete"), 564, 619, 639, 644);
	m_wndSave.Create(this, _T("&Save"), 644, 619, 719, 644);
	m_wndCancel.Create(this, _T("&Cancel"), 724, 619, 799, 644);
	m_wndActive.Create(this, _T("Hoạt động"), 10, 619, 110, 644);*/
...
long CMachineList::OnListLoadData()
{
	...
	int nCount = 0, nIdx = 1;
	szSQL.Format(_T("SELECT * FROM hms_machine_list_for_ins ORDER BY MA_GROUP, MA_MACHINENAME"));
...
int CMachineList::OnDeleteMachineList()
{
	...
	if(nSel < 0) return 0;
 	...
 	if(ShowMessage(1, MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO)
 		return -1;
	m_szMachineCode = m_wndList.GetItemText(nSel, 4);
...
int CMachineList::OnSaveMachineList()
{
	...
	CString szEvent, szDesc, szWhere;
	CDbfMapParser parser;

	szWhere.Format(_T(" where ma_machinecode = '%s'"), m_szMachineCode);
	m_hms_machine_list_for_ins.Open(&pMF->m_db, szWhere);
	parser.PushRowData(&m_hms_machine_list_for_ins);

 	if(!IsValidateData()) 
 		return -1; 

	parser.PushRowData(&m_hms_machine_list_for_ins, true);
 ...
 //_fmsg(_T("%s"), szSQL); 
 	...
 	{ 
		if(GetMode() == VM_ADD )
		{
			OnListLoadData();
			SetMode(VM_ADD);
			szEvent = _T("hms_machine_list_for_ins_add");
		...
		else if (GetMode() == VM_EDIT)
		{
			OnListLoadData(); 
			SetMode(VM_VIEW); 
			szEvent = _T("hms_machine_list_for_ins_edit");
		}
		else
			SetMode(VM_VIEW); 
		szDesc.Format(_T("[ma_machinecode=%s] %s")
			, m_szMachineCode, parser.GetRowChange());
