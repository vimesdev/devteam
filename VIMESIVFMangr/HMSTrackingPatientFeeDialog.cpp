...
void CHMSTrackingPatientFeeDialog::OnInitializeComponents()
{
	...
	m_wndList.InsertColumn(11, _T("Bệnh nhân còn thiếu"), CFMT_MONEY, 110);
	m_wndList.InsertColumn(12, _T("Bệnh nhân còn thừa"), CFMT_MONEY, 110);
...
*/
	...
	szSQL.Format(_T(" SELECT hd_docno AS docno,") \
		...
		_T("   hms_doc.hfe_cost                                   AS amount,") \
		_T("   hfe_inspaid_amount                                 AS inspaid,") \
		_T("   hfe_deposit_amount                                 AS deposit,") \
		_T("   CASE WHEN hms_doc.hfe_cost-hfe_inspaid_amount-COALESCE(hfe_deposit_amount, 0) < 0 ") \
		_T("   THEN (hms_doc.hfe_cost-hfe_inspaid_amount-COALESCE(hfe_deposit_amount, 0))*-1 ELSE 0 END AS patpaidthua,") \
		_T("   CASE WHEN hms_doc.hfe_cost-hfe_inspaid_amount-COALESCE(hfe_deposit_amount, 0) > 0 ") \
		_T("   THEN hms_doc.hfe_cost-hfe_inspaid_amount-COALESCE(hfe_deposit_amount, 0) ELSE 0 END AS patpaidno,") \
		_T("   hms_getroomname(hb_deptid, hb_roomid) AS roomname,") \
		_T("   hb_bedid") \
		_T(" FROM hms_patient") \
		...
		_T(" ON(hb_docno = htr_docno") \
		_T(" AND hb_refidx = htr_idx)") \
		_T(" WHERE htr_status='I'") \
		_T(" AND hms_doc.hfe_cost> 0") \
		_T(" AND hb_status = 'O' %s") \
		_T(" ORDER BY hb_roomid"), szWhere);
	_fmsg(_T("%s"), szSQL);
	nCount = rs.ExecSQL(szSQL);
