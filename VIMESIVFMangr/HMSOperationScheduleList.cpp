...
int CHMSOperationScheduleList::OnListDeleteItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szSQL;
	szSQL.Format(_T("Update hms_operation set ho_matorder_id = 0, ") \
				_T(" ho_isordering = 'N', ho_schedule_date = NULL ") \
				_T(" where ho_docno = %ld and ho_idx = %ld"), m_nDocumentNo, m_nOrderID);
...
CString CHMSOperationScheduleList::GetQueryString(bool bPrintAll)
{
	...
	szSQL.Format(_T(" select ho_deptid as deptid,") \
				...
				_T("		(select hms_getroomname(hb_deptid, hb_roomid) from hms_bed where hb_docno = hd_docno and hb_status = 'O' AND hb_deptid = ho_deptid) room,") \
