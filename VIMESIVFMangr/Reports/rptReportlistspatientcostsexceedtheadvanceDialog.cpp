...
long CrptReportlistspatientcostsexceedtheadvanceDialog::OnListLoadData(){
	...
	_T(" 		(select sum(hfi_patpaid) from hms_fee_invoice where hfi_docno = docno and hfi_type ='A' and hfi_patpaid > 0 ) as ttucost, 	") \
	_T(" 		sum(cost) as cost 	     ") \
	...
	_T(" 	WHERE hcr_status ='I' and hfe_status ='O' and htr_deptid='%s'  ") \
	_T(" 	) as tbl  ") \
	...
	_T(" HAVING %s   ") \
	_T(" ORDER BY deptid,docno"),m_szDeptKey, szHaving);
...
void CrptReportlistspatientcostsexceedtheadvanceDialog::OnReportInpatientpatientlistreport()
{
	...
	for (int i =0; i <= 17; i++)
	{
		...
		while(!m_rs->IsEOF())
		{
			...
			m_rs->GetValue(_T("Patdebt"), tmpStr);
			rptDetail->SetValue(_T("6"), tmpStr);
			m_rs->GetValue(_T("discount"), tmpStr);
			rptDetail->SetValue(_T("7"), tmpStr);
			m_rs->GetValue(_T("TTucost"), tmpStr);
			rptDetail->SetValue(_T("8"), tmpStr);
			m_rs->GetValue(_T("Cost"), tmpStr);
			rptDetail->SetValue(_T("9"), tmpStr);
			m_rs->GetValue(_T("Thua"), tmpStr);
			rptDetail->SetValue(_T("10"), tmpStr);
			m_rs->GetValue(_T("Thieu"), tmpStr);
...
void CrptReportlistspatientcostsexceedtheadvanceDialog::OnExportInpatientpatientlistreport()
{
	...
	xls.SetCellText(0, 4, _T("BAO CAO DANH SACH BENH NHAN VUOT QUA MUC TAM UNG"),FMT_TEXT,true,16);
	...
	TranslateString(_T("Insurance Payment"), tmpStr);
	...
	TranslateString(_T("Total Advance"), tmpStr);
