...
static long _OnIndicationCheckValueFnc(CWnd *pWnd){
	return ((CHMSPatientDrugOrderEntry *)pWnd)->OnIndicationCheckValue();
}
...
static void _OnInDrugstoreSelectFnc(CWnd *pWnd){
	 ((CHMSPatientDrugOrderEntry*)pWnd)->OnInDrugstoreCheck();
}
...
CHMSPatientDrugOrderEntry::CHMSPatientDrugOrderEntry(CWnd *pParent, int nMode, int nType):
CGuiPopup(pParent)
{
...
//	m_szOrderDate = m_szCurrentOrderDate.Left(10);
	...
	//_fmsg(_T("%s"), szSQL);
		...
		if(!rs.IsEOF())
		{
			((CHMSDailyDrugOrderView*)m_pWndOrder)->OnDrugListLoadData();
		}
		else
		{
			
			((CHMSDailyDrugOrderView*) m_pWndOrder)->m_wndDrugList.DeleteAllItems();
...
_T(" 	product_saleprice2 as unitprice,") \
...
_T(" 	product_countryname as country, product_isprecision as precision, ") \
_T(" msl_storage_id as storage_id, ") \
_T(" get_storagecode(msl_storage_id) as storagecode ") \
...
_T(" 	and msl_qtyonhand-msl_qtyordered > 0 ") \
_T(" 	and (product_expdate= CAST_STRING2DATE('1752-09-14') or product_expdate > CAST_STRING2DATE('%s') ) %s ") \
_T(" GROUP BY msl_storage_id, product_id, product_name, product_uomname, product_saleprice1, product_saleprice2, product_classname, product_countryname, product_isprecision ") \
_T(" ORDER BY name, unit, unitprice "), m_szStorages, m_szOrderDate, szWhere );
	nCount = rs.ExecSQL(szSQL);
	...
	while(!rs.IsEOF()){ 
		...
		m_wndName.AddItems(
			...
			_T(""),
			rs.GetValue(_T("precision")),
			rs.GetValue(_T("storage_id")), 
			rs.GetValue(_T("saleprice")), 
			NULL);
		rs.MoveNext();
	}
	return nCount;
...
void CHMSPatientDrugOrderEntry::OnSaveSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	OnSaveHMSPatientDrugOrderEntry();
} 
void CHMSPatientDrugOrderEntry::OnCloseSelect(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	((CHMSDailyDrugOrderView*)m_pWndOrder)->GetDataToScreen();	
...
int CHMSPatientDrugOrderEntry::OnSaveHMSPatientDrugOrderEntry(){
	...
	if(nObjectID == 7)
		szObjectType = _T("S");
	long nTransactionID = ((CHMSDailyDrugOrderView*)m_pWndOrder)->GetTransactionID(m_nStorageID, szObjectType);
...
*/
	...
	if(bAddFlag){
		...
		if(nOrderID > 0)
		{
			...
			((CHMSDailyDrugOrderView*)m_pWndOrder)->m_szOrderDateEmtry = m_szOrderDate;
			((CHMSDailyDrugOrderView*)m_pWndOrder)->m_szPatientNameKey = m_szPatientNameKey;
			((CHMSDailyDrugOrderView*)m_pWndOrder)->m_nDays = m_nDays;
			((CHMSDailyDrugOrderView*)m_pWndOrder)->m_bMultipleDay = m_bMultipleDay;

			((CHMSDailyDrugOrderView*)m_pWndOrder)->Refresh(nTransactionID, nDocumentNo);
		}
		else
		{
...
BOOL CHMSPatientDrugOrderEntry::PreTranslateMessage(MSG* pMsg){
...
int CHMSPatientDrugOrderEntry::OnIndicationCheckValue(){
	UpdateData(true);
...
void CHMSPatientDrugOrderEntry::OnInDrugstoreCheck(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
