...
void CHMSSupplementExamineDialog::OnCreateComponents(){
	m_wndTreatmentInformation.Create(this, _T("Treatment Information"), 5, 5, 320, 82);
...
long CHMSSupplementExamineDialog::OnDepartmentLoadData(){
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT * FROM sys_dept WHERE sd_type in('DT') %s ORDER BY sd_id"), szWhere);
