...
void CHMSTreatMovementDialog::OnCreateComponents(){
	...
	m_wndNote.Create(this,140, 120, 501, 225, TRUE, FALSE, TRUE); 
...
//	m_wndSendDate.SetCheckValue(true);
	...
	m_wndPerformDepartment.InsertColumn(1, _T("Name"), CFMT_TEXT, 250);
...
void CHMSTreatMovementDialog::GetScreenToData(){
	...
	m_hms_treat_movementTbl.SetValue(_T("htm_send_deptid"), m_szSendDepartmentKey);
...
long CHMSTreatMovementDialog::OnPerformDepartmentLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szFilter;
	szFilter.Format(_T(" and sd_id <>'%s' and sd_type='DT' "), pMF->m_szDept);
...
void CHMSTreatMovementDialog::OnReturnDeptSelect(){
	...
	szSQL.Format(_T("UPDATE hms_treat_movement SET htm_status='T', htm_terminated_date=to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss') ") \
		_T("WHERE htm_docno=%ld and htm_refidx=%ld and htm_status='O' "),
		m_szTerminatedDate, m_nDocumentNo, pMF->m_nRefIndex);
