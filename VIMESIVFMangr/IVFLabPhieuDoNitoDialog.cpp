...
CIVFLabPhieuDoNitoDialog::CIVFLabPhieuDoNitoDialog(CWnd *pParent, CString szID, int nMode):
	CGuiDialog(pParent){
	SetMode(nMode);
	m_szID = szID;
	m_nDlgWidth = 605;
...
void CIVFLabPhieuDoNitoDialog::OnInitializeComponents(){
	...
	m_wndList.InsertColumn(1, _T("ID_THUNG"), CFMT_TEXT, 0);
	m_wndList.InsertColumn(2, _T("STT"), CFMT_NUMBER, 50);
	m_wndList.InsertColumn(3, _T("Thung"), CFMT_TEXT, 300);
...
void CIVFLabPhieuDoNitoDialog::UpdateJson(BOOL bSave){
	...
	{
		jArr.push_back((LPCTSTR)(m_wndList.GetItemText(i, 1)));
	}
...
void CIVFLabPhieuDoNitoDialog::GetDataToScreen(){
	...
	CRecord rs(&pMF->m_db);
	CString szSQL;
	szSQL.Format(_T("SELECT * FROM ivf_phieu_do_nito WHERE ivf_phieu_do_nito_id = '%s'"), m_szID);
	rs.ExecSQL(szSQL);
	if (rs.IsEOF())
	{
		SetMode(VM_ADD);
...
long CIVFLabPhieuDoNitoDialog::OnNguoidoLoadData(){
	...
	return pMF->LoadIVFLabUser(&m_wndNguoido, m_szNguoidoKey, szFilter);
/*
	CRecord rs(&pMF->m_db);
...
long CIVFLabPhieuDoNitoDialog::OnThungLoadData(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szFilter, szThungIds;
	szFilter.Empty();
	szThungIds.Empty();
	for (int i = 0; i < m_wndList.GetItemCount(); i++)
	{
		if (!szThungIds.IsEmpty())
		{
			szThungIds += _T(",");
		}
		szThungIds.AppendFormat(_T("'%s'"), m_wndList.GetItemText(i, 1));
	}
	if (!szThungIds.IsEmpty())
	{
		szFilter.AppendFormat(_T(" and ivf_storage_thung_id NOT IN (%s)"), szThungIds);
	}
	return pMF->LoadComboBoxList(&m_wndThung, m_szThungKey, _T("ivf_storage_thung"), szFilter);
/*
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
void CIVFLabPhieuDoNitoDialog::OnAddLineSelect(){
	...
	m_wndList.AddItems(0, m_szThungKey, int2str(m_wndList.GetItemCount() + 1), m_wndThung.GetCurrent(1), NULL);
	m_wndList.EndLoad();
	OnThungLoadData();
	m_wndThung.SetFocus();
	return;
...
long CIVFLabPhieuDoNitoDialog::OnListLoadData(){
	...
	m_wndList.BeginLoad(); 
	int nCount = 0, nIdx = 1;
	szSQL.Format(_T("SELECT ivf_phieu_do_nito_line_id AS line_id, id_thung,") \
	_T("       Ivf_get_setting_desc('ivf_storage_thung', id_thung) as thung") \
	_T(" FROM   ivf_phieu_do_nito_line ") \
	_T(" WHERE ID_PHIEU_DO_NITO = '%s'"), m_szID);

	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		m_wndList.AddItems(
			rs.GetValue(_T("line_id")),
			rs.GetValue(_T("id_thung")),
			int2str(nIdx++),
...
void CIVFLabPhieuDoNitoDialog::OnSaveAndAcceptSelect(){
	...
	{
		ShowMessageBox(_T("Chưa có thùng nào được chọn đổ"));
		return;
	}
	int ret = OnSaveIVFLabPhieuDoNitoDialog(TRUE);
	if(ret > 0)
	{
		CGuiDialog::OnOK();
	}
...
int CIVFLabPhieuDoNitoDialog::OnSaveIVFLabPhieuDoNitoDialog(BOOL bAccept){
_tprintf(_T("\r\nMode=%d"), GetMode());
 	if(GetMode() != VM_ADD && GetMode() != VM_EDIT)
 		return -1;
 	if(!IsValidateData())
	...
	szSQL.Format(_T("IVF_PHIEU_DO_NITO_CREATE('%s', '%s', '%s')"), m_szID, jData, bAccept?_T("Y"):_T("N"));
 _tprintf(_T("%s"), szSQL);
