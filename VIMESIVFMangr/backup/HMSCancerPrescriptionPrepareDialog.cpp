...
static int _OnListAddExtItemFnc(CWnd *pWnd){
	 return ((CHMSCancerPrescriptionPrepareDialog*)pWnd)->OnListAddExtItem();
}
...
static int _OnListCancelItemFnc(CWnd *pWnd){
	 return ((CHMSCancerPrescriptionPrepareDialog*)pWnd)->OnListCancelItem();
}

static int _OnListUnCancelItemFnc(CWnd *pWnd){
	 return ((CHMSCancerPrescriptionPrepareDialog*)pWnd)->OnListUnCancelItem();
}
...
void CHMSCancerPrescriptionPrepareDialog::OnCreateComponents(){
	...
	m_wndTreatDiagramLabel.Create(this, _T("Phác đồ điều trị"), 10, 30, 110, 55);
	m_wndTreatDiagram.Create(this,115, 30, 405, 55); 
	...
	m_wndDoctorLabel.Create(this, _T("Bác sĩ"), 615, 60, 715, 85);
	m_wndDoctor.Create(this,720, 60, 895, 85); 
...
void CHMSCancerPrescriptionPrepareDialog::OnInitializeComponents(){
	...
	m_wndList.InsertColumn(3, _T("LD/M2"), CFMT_TEXT|CFMT_CENTER, 70);
	m_wndList.InsertColumn(4, _T("LD/BN"), CFMT_TEXT|CFMT_CENTER, 70);
	m_wndList.InsertColumn(5, _T("Số lượng"), CFMT_TEXT|CFMT_RIGHT, 0);
	m_wndList.InsertColumn(6, _T("T/tích(ml)"), CFMT_TEXT|CFMT_CENTER, 70);
	...
	m_wndList.InsertColumn(14, _T(""), CFMT_TEXT, 0);//hcol_status
...
void CHMSCancerPrescriptionPrepareDialog::OnSetWindowEvents(){
	...
	m_wndList.AddEvent(0, _T("-"));
	m_wndList.AddEvent(3, _T("Hủy không sử dụng"), _OnListCancelItemFnc);	
	m_wndList.AddEvent(0, _T("-"));
	m_wndList.AddEvent(4, _T("Khôi phục lại hủy bỏ"), _OnListUnCancelItemFnc);	
...
void CHMSCancerPrescriptionPrepareDialog::GetScreenToData(){
	...
	m_hms_cancer_orderTbl.SetValue(_T("HCO_GFR"), m_nGFR);
...
int CHMSCancerPrescriptionPrepareDialog::SetMode(int nMode){
 		...
 		case VM_ADD: 
			...
			szSQL.Format(_T("SELECT * FROM hms_cancer_order_active ") \
			_T(" WHERE hco_docno=%ld"), m_nDocumentNo);
			m_nOrderId = 0;
			rs.ExecSQL(szSQL);
			...
			if(!rs.IsEOF())
			{
				...
				//m_wndSBACalcMethod.EnableWindow(FALSE);
			}
			m_wndTreatDiagram.SetFocus();
 			break;
 		case VM_EDIT: 
 			EnableControls(TRUE);
 			EnableButtons(TRUE, 3, 4, -1);
...
int CHMSCancerPrescriptionPrepareDialog::OnListCancelItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szSQL;
	...
	szSQL.Format(_T("HMS_CANCER_ORDER_CANCELLINE(%ld, %ld, %ld, '%s') "),
		m_nDocumentNo, m_nOrderId, m_nOrderline_ID, pMF->GetCurrentUser());
//_msg(_T("%s"), szSQL);
	int ret = str2int(pMF->ExecDML(szSQL));
//_msg(_T("%s"), szSQL);
_tprintf(_T("\r\n%d"), ret);
	...
	{
		ShowMessageBox(_T("Không hủy được đề mục. Kiểm tra lại trạng thái phiếu"));
	}
	 return 0;
}

int CHMSCancerPrescriptionPrepareDialog::OnListUnCancelItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szSQL;
	...
	szSQL.Format(_T("HMS_CANCER_ORDER_UNCANCELLINE(%ld, %ld, %ld, '%s') "),
		m_nDocumentNo, m_nOrderId, m_nOrderline_ID, pMF->GetCurrentUser());
//_msg(_T("%s"), szSQL);
	int ret = str2int(pMF->ExecDML(szSQL));
//_msg(_T("%s"), szSQL);
...
int CHMSCancerPrescriptionPrepareDialog::OnListAddExtItem(){
	CMainFrame *pMF = (CMainFrame*) AfxGetMainWnd();
	CString szSQL;
	if(m_szStatus != _T("O"))
	{
	...
	CHMSCancerPrescriptionPopupEx *newPopup = new CHMSCancerPrescriptionPopupEx(this, VM_ADD);
	newPopup->m_nTreatIdx = m_nTreatIdx;
...
//_msg(_T("%s"), szSQL);
	...
	while(!rs.IsEOF()){ 
		...
		rs.GetValue(_T("hcol_status"), szStatus);
		...
		if(szOldLine != szNewLine)
		{
			...
			if(szStatus == _T("C"))
			{
				m_wndList.SetItemBkColor(nItem, RGB(200, 200, 200));
			}
...
_T("   hcol_thetich,") \
_T("   hcol_thoigian_sd, hcol_dvthoigian_sd ") \
_T(" FROM hms_cancer_orderline") \
_T(" LEFT JOIN hms_cancer_orderitem") \
_T(" ON(hms_cancer_orderitem.hcoi_cancer_order_id      = hcol_cancer_order_id") \
_T(" AND hms_cancer_orderitem.hcoi_cancer_orderline_id = hcol_cancer_orderline_id)") \
...
_T(" LEFT JOIN m_product") \
_T(" ON(mp_product_id           = mpi_product_id)") \
_T(" WHERE hcol_cancer_order_id = %ld and hcol_status <> 'C' and nvl(mp_dungmoi,'N') <> 'Y' ") \
...
//_msg(_T("%s"), szSQL);
	...
	while(!rsl.IsEOF()){ 
		...
		else
		{
			...
			rsl.GetValue(_T("hcol_dvthoigian_sd"),tmpStr);
...
void CHMSCancerPrescriptionPrepareDialog::OnSendSelect(){
	...
	CString szSQL;
	int nCount =0;
	CString tmpStr, szIndex, szStatus;
	
	for(int i=0; i < m_wndList.GetItemCount(); i++){
		tmpStr = m_wndList.GetItemText(i, 9);
		szIndex = m_wndList.GetItemText(i, 0);
		szStatus = m_wndList.GetItemText(i, 14);
		if(!szIndex.IsEmpty() && tmpStr.IsEmpty() && szStatus != _T("C"))
		{
			nCount++;
		}
	}
