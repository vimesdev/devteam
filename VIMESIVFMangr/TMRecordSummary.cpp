...
void CTMRecordSummary::OnCreateComponents()
{
    ...
    m_wndTerminatedDateLabel.Create(this, _T("Terminated Date"), 10, 30, 150,
                                    55);
    ...
    m_wndParaclinicResultLabel.Create(this, _T("Paraclinic Result"), 10, 250,
                                      150, 280);
    ...
    m_wndRelatedDiseaseLabel.Create(this, _T("Related Disease"), 10, 435, 150,
                                    460);
    m_wndRelatedDisease.Create(this, 155, 435, 990, 485, 1, 0, 1);
    m_wndTreatmentMethodLabel.Create(this, _T("Treatment Method"), 10, 490, 150,
                                     515);
    ...
    m_wndParaclinicButton.Create(this, _T("\x43h\x1ECDn k\x1EBFt qu\x1EA3"), 47,
                                 285, 150, 310);
    m_wndLoadPreviousTreatmentInfo.Create(
        this, _T("Load previous treatment info"), 5, 660, 205, 685);
...
void CTMRecordSummary::OnSetWindowEvents()
{
    ...
    // m_wndTerminatedDate.SetEvent(WE_KILLFOCUS,
    // _OnTerminatedDateKillfocusFnc);
    ...
    // m_wndParaclinicResult.SetEvent(WE_SETFOCUS,
    // _OnParaclinicResultSetfocusFnc);
    // m_wndParaclinicResult.SetEvent(WE_KILLFOCUS,
    // _OnParaclinicResultKillfocusFnc);
    m_wndParaclinicResult.SetEvent(WE_CHECKVALUE,
                                   _OnParaclinicResultCheckValueFnc);
    ...
    // m_wndRelatedDisease.SetEvent(WE_KILLFOCUS,
    // _OnRelatedDiseaseKillfocusFnc);
    ...
    // m_wndTreatmentMethod.SetEvent(WE_SETFOCUS,
    // _OnTreatmentMethodSetfocusFnc);
    // m_wndTreatmentMethod.SetEvent(WE_KILLFOCUS,
    // _OnTreatmentMethodKillfocusFnc);
    m_wndTreatmentMethod.SetEvent(WE_CHECKVALUE,
                                  _OnTreatmentMethodCheckValueFnc);
    ...
    m_wndLoadPreviousTreatmentInfo.SetEvent(WE_CLICK,
                                            _OnLoadPreviousTreatmentInfoFnc);
    GetDataToScreen();
    GetScreenToData();
}
...
void CTMRecordSummary::GetDataToScreen()
{
    ...
    if (!rs.IsEOF())
    {
        ...
        if (tmpStr != _T("Y"))
        {
            ...
            szSQL.Format(
                _T("SELECT htr_admitdate, htr_sumtreat, htr_outpatient ")
                _T(" FROM hms_treatment_record ")
                _T(" WHERE htr_docno = %ld AND htr_idx = %ld"),
            ...
            if (nTreatTime > 0)
            {
                szSQL.Format(
                    _T("WITH tbl as (")
                    _T("	SELECT htr_admitdate as admit_date, ")
                    _T("htr_dischargedate as discharge_date, ")
                    _T("	last_value(htr_status) over (order by null) as ")
                    _T("status")
                    _T("	FROM hms_treatment_record")
                    ...
                    _T("trunc(current_timestamp) + interval '1' day + ")
                    _T("interval '6' hour end as discharge_date")
                    _T(" FROM tbl ")
                    _T(" GROUP BY status"),
                    m_nDocNo, nTreatTime);
                rs.ExecSQL(szSQL);
...
void CTMRecordSummary::GetScreenToData()
{
    ...
    m_hms_clinical_recordTbl.SetValue(_T("HCR_DISCHARGEDATE"),
                                      m_szTerminatedDate);
...
long CTMRecordSummary::OnResultLoadData()
{
    ...
    szSQL.Format(_T("SELECT ss_code as id, ss_desc as name FROM sys_sel WHERE ")
                 _T("1=1 AND ss_id = 'hms_result' ORDER BY id "),
                 szWhere);
    nCount = rs.ExecSQL(szSQL);
    while (!rs.IsEOF())
    {
        m_wndResult.AddItems(rs.GetValue(_T("id")), rs.GetValue(_T("name")),
                             NULL);
        rs.MoveNext();
    }
    return nCount;
...
int CTMRecordSummary::OnAdmitDiseaseCheckValue() { return 0; }
...
int CTMRecordSummary::OnParaclinicResultCheckValue() { return 0; }
...
int CTMRecordSummary::OnPathologyCheckValue() { return 0; }
...
void CTMRecordSummary::OnICD10Selendok()
{
    ...
    {
        m_szMainDisease.Format(_T("%s [%s]"), m_wndICD10.GetCurrent(1),
                               m_wndICD10.GetCurrent(0));
    }
    else
    {
...
int CTMRecordSummary::OnRelatedDiseaseCheckValue() { return 0; }
...
int CTMRecordSummary::OnTreatmentMethodCheckValue() { return 0; }
...
int CTMRecordSummary::OnSuggestionCheckValue() { return 0; }
...
void CTMRecordSummary::OnPrintSelect()
{
    ...
    szSQL.Format(
        _T(" SELECT   hcr_patientno as patientno,  ")
        _T("upper(Get_patientname(hcr_docno)) patient_name, ")
        _T("           Extract(YEAR FROM hp_birthdate) yob, hd_cardno, ")
        _T("hd_disrate,")
        _T("           hcr_recordno record_no, (select ho_desc from ")
        _T("hms_object where ho_id = hd_object) AS fobject,")
        _T("           to_char(hcr_admitdate, 'dd/mm/yyyy hh24:mi:ss') as ")
        _T("admit_date, ")
        _T("           to_char(hcr_dischargedate, 'dd/mm/yyyy hh24:mi:ss') as ")
        _T("discharged_date, ")
        ...
        _T("		   hcr_gsuggestion3 suggestion, hcr_cancer, ")
        _T("hcr_cancer_time, ")
    ...
    if (m_szOutpatient == _T("Y"))
    {
        szSQL.Format(
            ...
            _T("           hcr_recordno record_no, (select ho_desc from ")
            _T("hms_object where ho_id = hd_object) AS fobject,")
            _T("           to_char(htr_admitdate, 'dd/mm/yyyy hh24:mi:ss') as ")
            _T("admit_date, ")
            _T("           to_char(htr_dischargedate, 'dd/mm/yyyy ")
            _T("hh24:mi:ss') as discharged_date, ")
            _T("           hd_conclusion diagnostic, ")
            _T("           hcr_summarize disease_process, ")
            _T("           hcr_tests paraclinic_result, ")
            _T("           hcr_pathological pathology, ")
            ...
            _T(" WHERE     hcr_docno = %ld AND htr_idx = %ld "),
            m_nDocNo, pMF->m_nRefIndex);
    }
    ...
    if (nTreatTime > 0)
    {
        szSQL.Format(
            ...
            _T("hh24:mi:ss') from hms_treatment_record where htr_docno = ")
            _T("hcr_docno and htr_treattime = %d) as admit_date, ")
            _T("           (select to_char(max(htr_dischargedate), ")
            _T("'dd/mm/yyyy hh24:mi:ss') from hms_treatment_record where ")
            _T("htr_docno = hcr_docno and htr_treattime = %d) as ")
            _T("discharged_date, ")
            _T("           hd_conclusion diagnostic, ")
            _T("           hcr_summarize disease_process, ")
            _T("           hcr_tests paraclinic_result, ")
            ...
            _T(" LEFT JOIN hms_doc ON ( hcr_docno = hd_docno ) ")
            _T(" WHERE     hcr_docno = %ld "),
            nTreatTime, nTreatTime, m_nDocNo);
    }
    rs.ExecSQL(szSQL);
    if (rs.IsEOF())
        return;
    ...
    bool bRet = pMF->OnPostGenDocX(&rpt, szId, m_nDocNo, pMF->m_nRefIndex,
                                   szReportName, szItemID);
    if (bRet)
    {
        return;
    }
    ...
    if (rptHeader)
    {
        ...
        rptHeader->SetValue(_T("Dischargedate"),
                            rs.GetValue(_T("discharged_date")));
    }
    rptGrpHeader = rpt.AddDetail(rpt.GetGroupHeader(1));
    if (rptGrpHeader)
    {
        // rptGrpHeader->SetValue(_T("diagnostic"),
        // rs.GetValue(_T("diagnostic")));
        rptGrpHeader->SetValue(_T("admit_disease"),
                               rs.GetValue(_T("admit_disease")));
    }
    rptGrpHeader = rpt.AddDetail(rpt.GetGroupHeader(2));
    if (rptGrpHeader)
        rptGrpHeader->SetValue(_T("diseaseprocess"),
                               rs.GetValue(_T("disease_process")));
    rptGrpHeader = rpt.AddDetail(rpt.GetGroupHeader(3));
    if (rptGrpHeader)
        rptGrpHeader->SetValue(_T("paraclinicresult"),
                               rs.GetValue(_T("paraclinic_result")));
    ...
    if (rptGrpHeader)
    {
        rptGrpHeader->SetValue(_T("maindisease"),
                               rs.GetValue(_T("main_disease")));
        rptGrpHeader->SetValue(_T("relateddisease"),
                               rs.GetValue(_T("related_disease")));
    }
    rptGrpHeader = rpt.AddDetail(rpt.GetGroupHeader(6));
    if (rptGrpHeader)
        rptGrpHeader->SetValue(_T("treatmethod"),
                               rs.GetValue(_T("treatment_method")));
    ...
    tmpStr.Format(rpt.GetReportFooter()->GetValue(_T("PrintDate")),
                  szCurDate.Mid(8, 2), szCurDate.Mid(5, 2), szCurDate.Left(4));
    rpt.GetReportFooter()->SetValue(_T("PrintDate"), tmpStr);
    szCurDate = rs.GetValue(_T("discharged_date"));

    tmpStr.Format(rpt.GetReportFooter()->GetValue(_T("DischargeDate1")),
                  szCurDate.Left(2), szCurDate.Mid(3, 2), szCurDate.Mid(6, 4));
...
int CTMRecordSummary::OnEditTMRecordSummary()
{
    ...
    // Lấy ra khoa cuối cùng để check nếu ko phải khoa cuối thì không cho làm
    // phiếu tổng kết, #1003
    ...
    szSQL.Format(_T(" SELECT htr_deptid from HMS_TREATMENT_RECORD")
                 ...
                 _T(" HMS_TREATMENT_RECORD where htr_docno=%ld)"),
                 pMF->m_nDocumentNo, pMF->m_nDocumentNo);
    rs.ExecSQL(szSQL);
    //_msg(_T("%s"), szSQL);
    ...
    {
        ShowMessageBox(
            _T("Chỉ được phép làm phiếu tổng kết ở khoa cuối ra viện!"));
        return -1;
    }

    szSQL.Format(
        _T("SELECT htr_status, ")
        _T("		htrf_acceptedfee ")
    ...
    // == _T("Y")) && pMF->GetCurrentUser() != _T("admin"))
...
int CTMRecordSummary::OnSaveTMRecordSummary()
{
    ...
    if (ret > 0)
    {
        szSQL.Format(_T("UPDATE hms_treatment_record ")
                     _T(" SET htr_dischargedate = to_timestamp('%s', ")
...
int CTMRecordSummary::OnTMRecordSummaryListLoadData() { return 0; }
