...
long CHMSDailyOrderDate::OnOrderDateLoadData(){
	...
	int nCount = 0;
	szSQL.Format(_T("SELECT distinct trunc_date(mt_orderdate) as orderdate ") \
		_T(" FROM m_transaction ") \
		_T(" WHERE mt_orderno = '%s' ") \
		_T(" ORDER BY trunc_date(mt_orderdate)"), pMF->m_wndDailyDrugOrder.m_szOrderNo);
...
void CHMSDailyOrderDate::OnOkSelect(){
	...
	{		
		...
		szSQL.Format(_T(" SELECT distinct hpo_docno as docno, hpo_orderid as orderid, ") \
			...
			_T("	 trunc_date(htr_admitdate) as Admitdate,htr_maindisease as Diagnostic,htr_mainicd as icd, ") \
			...
			_T(" WHERE 	hpo_transaction_id=%ld and trunc_date(hpo_orderdate)= to_date('%s', 'yyyy/mm/dd hh24:mi:ss') ") \
		...
		if(!rpt.Init(_T("Reports/HMS/HT_BANGTONGHOPTHUOCTHEONGAY.RPT")) )
			...
			//Page Header
				...
				szSQL.Format(_T(" SELECT 	product_name as drugname, ") \
					...
					_T(" 	sum(hpol_issueqty) as issueqty") \
					_T(" FROM hms_ipharmaorder") \
					_T(" LEFT JOIN hms_ipharmaorder_line ON(hpo_orderid=hpol_orderid)") \
					_T(" WHERE hpo_transaction_id='%s'  and trunc_date(hpo_orderdate)= to_date('%s', 'yyyy/mm/dd hh24:mi:ss')") \
					_T(" GROUP BY drugname, unit ") \
