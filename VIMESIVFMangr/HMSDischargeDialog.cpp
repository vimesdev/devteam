...
void CHMSDischargeDialog::OnCreateComponents(){
	m_wndDischargeInformation.Create(this, _T("Discharge Information"), 5, 5, 1005, 675);
	...
	m_wndMainDisease.Create(this,135, 89, 1000, 164, true, false, true); 
	m_wndRelationDiseaseLabel.Create(this, _T("Relation Disease"), 10, 169, 130, 244);
	m_wndRelationDisease.Create(this,135, 169, 1000, 244,  true, false, true); 
	m_wndTreatMethodLabel.Create(this, _T("Treat Method"), 10, 249, 130, 349);
	m_wndTreatMethod.Create(this,135, 249, 1000, 349,  true, false, true); 
	m_wndAdmissionStatusLabel.Create(this, _T("Admission Status"), 10, 354, 130, 454);
	m_wndAdmissionStatus.Create(this,135, 354, 1000, 454, true, false, true); 
	m_wndYKienDenGhiLabel.Create(this, _T("Y kien den ghi"), 10, 459, 130, 559);
	m_wndYKienDenGhi.Create(this,135, 459, 1000, 559,  true, false, true); 
	m_wndConclusionLabel.Create(this, _T("Conclusion /Advice"), 10, 564, 130, 639);
	m_wndConclusion.Create(this,135, 564, 1000, 639,  true, false, true); 
	m_wndNoteLabel.Create(this, _T("Note"), 10, 645, 130, 670);
	m_wndNote.Create(this,135, 645, 1000, 670); 	
...
//	m_wndConclusion.SetCheckValue(true);
	...
	m_hms_treatmentTbl.AddField(_T("hcr_gsuggestion2"), dfText, 512);	//hcr_gsuggestion2
...
/*
	AddEvent(1, _T("Add	Ctrl+A"), _OnAddHMSDischargeDialogFnc, 0, 'A', VK_CONTROL);
	AddEvent(2, _T("Edit	Ctrl+E"), _OnEditHMSDischargeDialogFnc, 0, 'E', VK_CONTROL);
	AddEvent(3, _T("Delete	Ctrl+D"), _OnDeleteHMSDischargeDialogFnc, 0, 'D', VK_CONTROL);
	AddEvent(4, _T("Save	Ctrl+S"), _OnSaveHMSDischargeDialogFnc, 0, 'S', VK_CONTROL);
	AddEvent(0, _T("-"));
	AddEvent(5, _T("Cancel	Ctrl+T"), _OnCancelHMSDischargeDialogFnc, 0, 'T', VK_CONTROL);
*/
...
void CHMSDischargeDialog::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT htr_admitdate, htr_sumtreat, htr_outpatient FROM hms_treatment_record WHERE htr_docno = %ld AND htr_idx = %ld"),  pMF->m_nDocumentNo, pMF->m_nRefIndex);
	rs.ExecSQL(szSQL);
	if(!rs.IsEOF()){
	...
	{
		/*szSQL.Format(_T("SELECT min(htr_admitdate) AS admitdate  FROM hms_treatment_record where htr_docno=%ld and htr_status in('I','T') and htr_treattime=0 "), pMF->m_nDocumentNo);
		rs.ExecSQL(szSQL);
	...
	if (nTreatTime > 0)
	{
		szSQL.Format(_T("WITH tbl as (") \
				...
				_T("	last_value(htr_status) over (order by htr_idx asc) as status") \
				...
				_T(" extract(day from max(discharge_date) - min(admit_date)) + 1 as treatment_days ") \
				_T(" FROM tbl ") \
				_T(" GROUP BY status"), pMF->m_nDocumentNo, nTreatTime);
...
void CHMSDischargeDialog::GetScreenToData(){
	...
	m_hms_clinical_recordTbl.SetValue(_T("hcr_reldisease"), m_szRelationDisease);
...
int CHMSDischargeDialog::SetMode(int nMode){
 		...
 		case VM_EDIT: 
			...
			if(m_nTotalDay <= 0)
				m_nTotalDay = CompareDate(m_szEndDate.Left(10), m_szAdmitDate.Left(10));
...
int CHMSDischargeDialog::OnEditHMSDischargeDialog(){
 ...
 //		return -1; 
	...
	if((szAcceptedFee == _T("A") || szAcceptedFee == _T("P") || szAcceptedFee == _T("Y")) && (!pMF->CheckPermission(_T("IVF.TM.100.1"))))
	...
	if(nMaxIdx > pMF->m_nRefIndex && (!pMF->CheckPermission(_T("IVF.TM.100.1"))))
	{
...
int CHMSDischargeDialog::OnSaveHMSDischargeDialog(){
	...
	/*szSQL.Format(_T("UPDATE hms_clinical_record ") \
		_T(" SET hcr_dischargedate= to_timestamp('%s', 'yyyy/mm/dd hh24:mi:ss'), hcr_discharge='Y', ") \
