...
long CHMSMedicalCabinetDrugOrderEntry::OnDocumentNoLoadData(){
	...
	szSQL.Format(_T("SELECT distinct hd_docno as docno, ") \
		...
		_T(" HMS_GETSEX(hp_sex) as sex, hb_refidx as refidx, hd_object as objectid, HMS_GetObjectName(hd_object) as objectname ") \
		_T(" FROM hms_patient ") \
		...
		_T(" WHERE (htr_deptid='%s' or (htr_ctdeptid='%s' and htr_combinedtreat='Y') ) AND htr_status='I' and (htr_outpatient='Y' OR hb_status='O') ") \
		_T(" %s ORDER BY pname, docno "), pMF->m_szDept,  pMF->m_szDept, szWhere);
			//_T("	AND hd_object NOT IN(7) ") \
//_fmsg(_T("%s"), szSQL);
...
long CHMSMedicalCabinetDrugOrderEntry::OnNameLoadData(){
	...
	if (m_szObjectType == _T("S"))
	{
		...
		szWhere.AppendFormat(_T(" and l1.msl_storage_id in(-1,%s) "), pMF->m_szStorages);
