...
void CHMSSetupFood::OnInitializeComponents(){
	...
	m_wndType.InsertColumn(1, _T("Description"), CFMT_TEXT, 350);
	...
	m_wndList.InsertColumn(4, _T("Type"), CFMT_TEXT, 60);
...
void CHMSSetupFood::GetDataToScreen(){
	...
	szSQL.Format(_T("SELECT * FROM hms_feelist WHERE hfl_feeid='%s'"), m_szID);
	rs.ExecSQL(szSQL);
	if(rs.IsEOF())
		SetMode(VM_NONE);
	else
...
void CHMSSetupFood::GetScreenToData(){
	...
	if(GetMode() == VM_ADD)
	{
		CString szSQL;
		CRecord rs(&pMF->m_db);
		szSQL.Format(_T("SELECT coalesce(max(hfl_idx), 0)+1 FROM hms_feelist WHERE hfl_groupid='F0000' "));
		rs.ExecSQL(szSQL);
		...
		m_hms_feelistTbl.SetValue(_T("hfl_idx"), rs.GetIntValue());
	...
	m_hms_feelistTbl.SetValue(_T("hfl_groupid"), _T("F0000"));
	m_hms_feelistTbl.SetValue(_T("hfl_feeid"), m_szID);
...
long CHMSSetupFood::OnTypeLoadData(){
	...
	if(m_wndType.IsSearchKey() && !m_szTypeKey.IsEmpty()){
		szWhere.Format(_T(" AND cast(cs_code as integer) =%d "), ToLong(m_szTypeKey));
	};
	m_wndType.DeleteAllItems(); 
	int nCount = 0;
	szSQL.Format(_T(" SELECT ss_code as id, ss_desc as name FROM sys_sel WHERE ss_id='hms_foodtravelsupport' ORDER BY cast(ss_code as integer) %s"), szWhere);	
...
long CHMSSetupFood::OnListLoadData(){
	...
	szSQL.Format(_T("SELECT * FROM hms_feelist WHERE hfl_groupid='F0000' AND cast(hfl_typeid as integer ) > 0 ORDER BY hfl_feeid"));
	m_wndList.BeginLoad(); 
	int nCount = 0, nIdx=1;
	nCount = rs.ExecSQL(szSQL);
	while(!rs.IsEOF()){ 
		...
		m_wndList.AddItems(			
			...
			rs.GetValue(_T("hfl_typeid")), 
			rs.GetValue(_T("hfl_itype")),  NULL);
		rs.MoveNext();
	}
	m_wndList.EndLoad(); 
	return nCount;
...
int CHMSSetupFood::OnDeleteHMSSetupFood(){
	...
	szID = m_wndList.GetItemText(nSel, 1);
 	szSQL.Format(_T("DELETE FROM hms_feelist WHERE  hfl_feeid='%s' ") , szID); 	
