...
int CIVFStorageViewDialog::CreateItem(CGridListView* pList, CString szDesc, int nDungcu, int nControng, int nVitrichua, int nVitritrong)
{
	int col_width= 70;
	CString tmpStr;
...
_tprintf(_T("\npLine:%u\n"), pLine);
	...
	int nItem = pList->Add(pItem);
	CGridListItem* pItemReflect = pList->GetGridListItem(nItem);
	CGridCell* pCellReflect = pItemReflect->GetCell(0);
	_tprintf(_T("\nWidth: %d|Height:%d\n"), pCellReflect->GetWidth(), pCellReflect->GetHeight());
	for (int i = 0; i < pCellReflect->m_cellRows.GetSize(); i++)
	{
		_tprintf(_T("\nItem %d: %u|Inner Item: %d\n"), i+1, pCellReflect->GetRow(i), pCellReflect->GetRow(i)->GetItemCount());
	}
	//Structure 
	return 1;
}