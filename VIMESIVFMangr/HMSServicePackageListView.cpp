...
void CHMSServicePackageListView::OnInitializeComponents(){
	...
	m_wndList.InsertColumn(8, _T("Tên gói"), CFMT_TEXT, 125);
	...
	m_wndList.InsertColumn(12, _T("Ngày chốt"), CFMT_TEXT, 90);
	m_wndList.InsertColumn(13, _T("Trạng thái"), CFMT_TEXT|CFMT_CENTER, 80);
	m_wndList.InsertColumn(14, _T("Báo ăn"), CFMT_TEXT, 50);
...
void CHMSServicePackageListView::OnListDblClick(){
	...
	if(dlg.DoModal() == IDOK)
	{
		m_wndList.SetItemText(nSel, 13, _T("T"));
...
int CHMSServicePackageListView::OnListRollbackItem(){
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		szDate = m_wndList.GetItemText(nSel, 12);
...
_T(" AND hfelc_date       between to_date('%s','YYYY-MM-DD') and to_date('%s', 'YYYY-MM-DD') ") \
