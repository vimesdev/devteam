...
//	m_wndToDate.SetMax(CDate(pMF->GetSysDate()));
	...
	m_wndList.InsertColumn(10, _T("Ngày dự kiến"), CFMT_NUMBER, 60);
	m_wndList.InsertColumn(11, _T("Ngày sử dụng"), CFMT_NUMBER, 60);
	m_wndList.InsertColumn(12, _T("Chọn"), CFMT_TEXT|CFMT_CENTER, 50);
...
void CHMSServicePackageCloseDataDialog::OnListDblClick(){
	...
	if(nSel < 0)
		return;
	CString szMark = m_wndList.GetItemText(nSel, 12);
	if(szMark == _T("[X]"))
		szMark.Empty();
	else
		szMark = _T("[X]");
	m_wndList.SetItemText(nSel, 12, szMark);
...
_T("   (SELECT hfelc_docno FROM hms_fee_extraline_close WHERE hfelc_docno = hfe_docno and hfelc_extraline_id = hfel_fee_extraline_id and hfelc_date=to_date('%s', 'YYYY-MM-DD') ") \
_T("   )") \
_T(" %s ") \
...
void CHMSServicePackageCloseDataDialog::OnApplySelect(){
	...
	{
		CString szMark = m_wndList.GetItemText(i, 12);
		if(szMark == _T("[X]"))
		{
...
void CHMSServicePackageCloseDataDialog::OnMarkSelect(){
	...
	for (int i =0; i < m_wndList.GetItemCount(); i++)
	{
		m_wndList.SetItemText(i, 12, _T("[X]"));
	}
	m_wndList.RedrawWindow();
} 
