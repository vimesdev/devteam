...
_msg(_T("%s"), szSQL);
*/
	szSQL.Format(_T(" SELECT mt_storage_id AS storage_id,mt_createdby as createdby, mt_postedby as postedby, ") \
_T("   product_id,") \
...
_T("   product_unitprice"), m_szOrderNo);
	...
	while(!rs.IsEOF()){ 
		...
		if(nStorageID != nOldStorageID)
		{
			...
			szStorageName.Format(_T("%s [%d] Createdby: %s, Postedby: %s"), pMF->GetStorageName(nStorageID),nStorageID,rs.GetValue(_T("createdby")),rs.GetValue(_T("postedby"))  );
...
long CHMSDailyOrderView::OnDrugListLoadData(){
	...
	while(!rs.IsEOF()){ 
		...
		if(szStorageID != szOldStorageID)
		{
			...
			szStorageName.Format(_T("%s  -   Trang thai [%s]  -   Nguoi tao [%s] - Nguoi add: [%s]"), 
...
void CHMSDailyOrderView::OnResizeLayout()
{
	...
	AddBottom(&m_wndSend, 5, true);

	AddBottom(&m_wndPrint);
	AddBottom(&m_wndPrintDeliveryDrugs);
